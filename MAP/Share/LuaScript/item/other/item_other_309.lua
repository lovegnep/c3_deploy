...
x418309_var_FileId = 418309
x418309_var_QuestName = "金牌图腾礼包"
x418309_var_LevelMin = 70


function x418309_ProcEventEntry( varMap, varPlayer, varBagIdx )
...
function x418309_ProcIsSpellLikeScript( varMap, varPlayer)
	return 1; 
end
...
function x418309_ProcCancelImpacts( varMap, varPlayer )
	return 0; 
end
...
function x418309_ProcConditionCheck( varMap, varPlayer )
...
function x418309_ProcDeplete( varMap, varPlayer )
...
function x418309_ProcActivateOnce( varMap, varPlayer, varImpact )
	
	
	if GetLevel(varMap, varPlayer)< x418309_var_LevelMin then
	...
	if(DepletingUsedItem(varMap, varPlayer)) == 1 then
		...
		local varRan = random(1,1000)
			value = 2 
			if varRan <= 200 then
				exp = GetLevel(varMap, varPlayer) * 15000
...
--					Msg2Player( varMap, varPlayer, varMsg, 8, 3)
					...
					local guildvalue = GetLevel(varMap, varPlayer) * 5
			...
			elseif varRan <= 600 then
				local honor = GetLevel(varMap, varPlayer) * 5
...
--				ItemAppendBind( varMap, 11020002, 5)     -- 战车强化道具20%,后期添加
...
--				DeliverItemListSendToPlayer( varMap, varPlayer)
			elseif varRan <= 1000 then
				local bonusshengwang = GetLevel(varMap, varPlayer) * 10
...
function x418309_ProcActivateEachTick( varMap, varPlayer)
	return 1; 
end
