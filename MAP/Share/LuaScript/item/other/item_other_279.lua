...
x418279_var_FileId = 418279
x418279_var_QuestId = 10267
x418279_var_ItemId = 12030604
x418279_var_NoTimes = "您的剩余次数不足！"
x418279_var_StartWeek =61

x418279_var_NormalItemNum		=   16	



x418279_var_MainMenuTab	=	{
								...
								{	itemname = 	"无双无畏80螳螂"	,	describe = "1张#R极品赤炎螳螂降世符#W", 														          							varItem = 12050276, 																				  													varNum = 1 	,  probability =7  , times = 30		},  
								{	itemname = 	"大包裹"					,	describe = "1个#R镂刻兽革包（24格）#W或1个#R刺绣丝绸包（30格）#W", 													varItem1 = 19010009, varItem2 = 19010010, 									 														varNum = 1 	,  probability =30 , times = 30		},  
								...
								{	itemname = 	"节日时装"				,	describe = "1个#R14天节日时装礼包#W", 															 				  							varItem = 12030514, 																				  													varNum = 1 	,  probability =25 , times = 30		},  
								{	itemname = 	"星尘"						,	describe = "20个#R微瑕星辰#W或20个#R无瑕星辰#W或20个#R完美星辰#W", 													varItem1 = 11000550, varItem2 = 11000551, varItem3 = 11000552, 													varNum = 20	,  probability =50 , times = 30		},  
...
function x418279_SetLeaveTimes( varMap, varPlayer, MdIndex, MdData )
	if MdIndex == 1 then
	...
	elseif MdIndex == 2 then
		SetPlayerGameData(varMap, varPlayer, MD_CHONGCU_COUNT2[1], MD_CHONGCU_COUNT2[2],MD_CHONGCU_COUNT2[3], MdData ) 
...
function x418279_GetLeaveTimes( varMap, varPlayer, MdIndex )
	if MdIndex == 1 then
		return GetPlayerGameData(varMap, varPlayer, MD_CHONGCU_COUNT1[1], MD_CHONGCU_COUNT1[2],MD_CHONGCU_COUNT1[3]) 
	elseif MdIndex == 2 then
		return GetPlayerGameData(varMap, varPlayer, MD_CHONGCU_COUNT2[1], MD_CHONGCU_COUNT2[2],MD_CHONGCU_COUNT2[3]) 
	elseif MdIndex == 3 then
...
function x418279_ProcIsSpellLikeScript( varMap, varPlayer)
	return 1; 
end
function x418279_ProcCancelImpacts( varMap, varPlayer)
	return 0; 
end
function x418279_ProcConditionCheck( varMap, varPlayer)
...
function x418279_ProcDeplete( varMap, varPlayer)
...
function x418279_ProcActivateOnce( varMap, varPlayer, varImpact)
	LuaCallNoclosure( 418279, "ProcEnumEvent", varMap, varPlayer, varPlayer, 0)
end
...
function x418279_ProcActivateEachTick( varMap, varPlayer)
	return 1; 
end

function x418279_CheckDate( varMap, varPlayer)
	...
	SetPlayerGameData(varMap,varPlayer,MD_CHONGCU_INDEX[1],MD_CHONGCU_INDEX[2],MD_CHONGCU_INDEX[3],x418279_var_StartWeek)
end



function x418279_ProcEventEntry( varMap, varPlayer, varTalknpc, varScript, extid)
	local year,month,day = GetYearMonthDay()
	if year >= 2011 then
		return
	end	
		local Times = x418279_GetLeaveTimes(varMap, varPlayer, extid)
		Times = 30 - Times
		if Times <= 0 then
			Msg2Player( varMap, varPlayer, x418279_var_NoTimes, 8, 2)
			return
		end
		StartTalkTask( varMap)
		TalkAppendString( varMap, format( "\t您将进行#G%s#W的幸运抽奖，您有可能获得%s。如果您没有得到想要的道具，则会获得#R金卡奖励#W，祝您中奖！\n\t您确定要开始幸运抽奖了吗？\n注：\n\t1.金卡以道具形式给予，分为#R10两#W、#R20两#W、#R50两#W、#R100两#W、#R500两#W！\n\t2.所得道具均为#R绑定道具#W。", x418279_var_MainMenuTab[ extid].itemname,x418279_var_MainMenuTab[ extid].describe ,12030059 ) )
		if extid == 1 then		 
			AddQuestRadioItemBonus( varMap,12030605 , x418279_var_MainMenuTab[ extid].varNum)
			AddQuestRadioItemBonus( varMap,12030606	, x418279_var_MainMenuTab[ extid].varNum)
		elseif extid == 3 then
			AddQuestRadioItemBonus( varMap, x418279_var_MainMenuTab[ extid].varItem1, x418279_var_MainMenuTab[ extid].varNum)
			AddQuestRadioItemBonus( varMap, x418279_var_MainMenuTab[ extid].varItem2, x418279_var_MainMenuTab[ extid].varNum)			
		elseif extid == 4 or extid == 7 or extid == 8 then		 
			AddQuestRadioItemBonus( varMap, x418279_var_MainMenuTab[ extid].varItem1, x418279_var_MainMenuTab[ extid].varNum)
			AddQuestRadioItemBonus( varMap, x418279_var_MainMenuTab[ extid].varItem2, x418279_var_MainMenuTab[ extid].varNum)
			AddQuestRadioItemBonus( varMap, x418279_var_MainMenuTab[ extid].varItem3, x418279_var_MainMenuTab[ extid].varNum)
		else
		AddQuestRadioItemBonus( varMap, x418279_var_MainMenuTab[ extid].varItem, x418279_var_MainMenuTab[ extid].varNum)	
		end		

		SetPlayerRuntimeData( varMap, varPlayer, RD_VIP_BOX, extid)
		StopTalkTask()
		DeliverTalkInfoNM( varMap, varPlayer, varTalknpc,x418279_var_FileId,x418279_var_QuestId)

end

function x418279_ProcEnumEvent( varMap, varPlayer, varTalknpc, varQuest)
	local year,month,day = GetYearMonthDay()		
	if year > 2011 then
		...
		return 0
	elseif year == 2011	and month >7 then
		Msg2Player( varMap, varPlayer, "很抱歉，此道具已过使用期", 8, 2)
		DepletingUsedItem( varMap, varPlayer)
		GamePlayScriptLog( varMap, varPlayer, 1413)
		return 0
	...
	x418279_CheckDate( varMap, varPlayer)

	local Times = 0
	StartTalkTask(varMap)
	TalkAppendString( varMap, "\t《成吉思汗3 恶狼传说》中的珍品稀有道具应有尽有！" )

	for varI, s in x418279_var_MainMenuTab do
		Times = x418279_GetLeaveTimes(varMap, varPlayer, varI)
		Times = 30 - Times
		if(Times > 0) then
			TalkAppendButton(varMap, x418279_var_QuestId, "我要"..x418279_var_MainMenuTab[varI].itemname.."！(剩余"..Times.."次)",3,varI)
		else
			TalkAppendButton(varMap, x418279_var_QuestId, "#cb4b4b4我要"..x418279_var_MainMenuTab[varI].itemname.."(剩余0次)",3,varI)
		end
	end
...
function x418279_GetGift( varMap, varPlayer, varIndex)
	local varRand = random (1 , 100)
	if varIndex == 1 then
		if varRand <= x418279_var_MainMenuTab[varIndex].probability then
			varRand = random (1 , 10)
			if varRand <= 5 then
	...
	elseif varIndex == 3 then
		if varRand <= x418279_var_MainMenuTab[varIndex].probability then
			varRand = random (1 , 30)
			if varRand == 1 then
				return x418279_var_MainMenuTab[varIndex].varItem2 ,1
			else
				return x418279_var_MainMenuTab[varIndex].varItem1 ,0	
			end	
		else
			return 12030059,0
		end			
	elseif varIndex == 4 then
		if varRand <= x418279_var_MainMenuTab[varIndex].probability then
			...
			elseif varRand <= 8 then
				return x418279_var_MainMenuTab[varIndex].varItem2 ,0	
			else
				return x418279_var_MainMenuTab[varIndex].varItem3 ,1						
			end	
		else
			return 12030059,0
		end				
	...
	elseif varIndex == 8 then
		...
		if varRand <= x418279_var_MainMenuTab[varIndex].probability then
			return x418279_var_MainMenuTab[varIndex].varItem ,1
		else
			return 12030059,0
		end	
	end		
...
function x418279_ProcQuestAccept( varMap, varPlayer, varTalknpc, varQuest)
	local year,month,day = GetYearMonthDay()
	if year >= 2011 then
		return
	end	
	...
	local value,broadcast = x418279_GetGift( varMap, varPlayer, varIndex)
	local Times = x418279_GetLeaveTimes(varMap, varPlayer, varIndex)
	...
	local varMsg
		...
		if GetBagSpace(varMap, varPlayer) <= 0 then
			...
			if GetItemIDByIndexInBag( varMap, varPlayer, bagIndex) ~= x418279_var_ItemId then
				return
			end
		...
		if broadcast == 1 then
			...
			varMsg = format( "恭喜%s的%s打开大丰收黄金礼盒获得了%d个#{_ITEM%d}!", CountryName, Name, varNum, value )
			LuaAllScenceM2Wrold( varMap, varMsg, 5, 1)
			Msg2Player( varMap, varPlayer, format( "恭喜你在大丰收黄金礼盒抽奖中获得了%d个#{_ITEM%d}!", varNum, value), 8, 2)
			Msg2Player( varMap, varPlayer, format( "恭喜你在大丰收黄金礼盒抽奖中获得了%d个#{_ITEM%d}!", varNum, value), 8, 3)
		else
		if varNum ~= 0 then
				varMsg = format( "恭喜你在大丰收黄金礼盒抽奖中获得了%d个#{_ITEM%d}!", varNum, value)
			else
				varMsg = format( "恭喜你在大丰收黄金礼盒抽奖中获得了#{_ITEM%d}!", value)
			end
	...
	Times = Times -	1
	Times = 30 - Times
	x418279_SetLeaveTimes(varMap, varPlayer, varIndex, Times)
