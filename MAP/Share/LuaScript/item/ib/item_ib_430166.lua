-------- 神行符脚本
x430166_var_FileId = 430166 
x430166_var_NeedItemId = 12035279
x430166_var_CDTime = 60
x430166_var_Buf1 = 7721
x430166_var_MainTransScript = 430014
x430166_var_UICommandID = 111

function x430166_ProcEventEntry( varMap, varPlayer, varBagIdx )
	if GetItemIsOverTimeByBagPos(varMap ,varPlayer ,varBagIdx) == 1 then
		...
		LuaCallNoclosure(x430166_var_MainTransScript, "ProcEventEntry", varMap, varPlayer, varBagIdx, x430166_var_UICommandID)
end

function x430166_CheckCanTrans(varMap, varPlayer, varBagIdx, varIndex ,newSceneId )
	
	if GetItemIDByIndexInBag(varMap, varPlayer, varBagIdx) ~= x430166_var_NeedItemId then
		return 0
	end
	
	local ret = LuaCallNoclosure(x430166_var_MainTransScript, "CommonConditionCheck", varMap, varPlayer, varBagIdx, varIndex ,newSceneId)
	if (ret == 0) then
		return 0
	end
	
	local continuance = GetCurrentTime() - GetPlayerGameData(varMap, varPlayer, MD_TRANS_TIME[1], MD_TRANS_TIME[2], MD_TRANS_TIME[3])
	if continuance < x430166_var_CDTime then
		local varMsg = format("%d秒后才可使用下一次千里符", x430166_var_CDTime-continuance)
...
function x430166_SetTransMap(varMap, varPlayer, varIndex, varBagIdx, value)
	if varIndex == 5 then
...
function x430166_SetTransPosX(varMap, varPlayer, varIndex, varBagIdx, value)
	if varIndex == 5 then
...
function x430166_SetTransPosZ(varMap, varPlayer, varIndex, varBagIdx, value)
	if varIndex == 5 then
...
function x430166_ProcIsSpellLikeScript( varMap, varPlayer)
	return 0
end

function x430166_ProcCancelImpacts( varMap, varPlayer )
	return 0
end

function x430166_ProcConditionCheck( varMap, varPlayer )
	return 0
end

function x430166_ProcDeplete( varMap, varPlayer )

	return 0
end

function x430166_ProcActivateOnce( varMap, varPlayer )
end

function x430166_ProcActivateEachTick( varMap, varPlayer)
	return 1
end
