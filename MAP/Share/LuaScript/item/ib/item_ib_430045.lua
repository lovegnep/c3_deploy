--初级体力丹
x430045_var_FileId = 430045 
x430045_var_AddPhysicalForce =100		--增加体力值	
x430045_var_LimitCount =5			--限制使用个数





function x430045_ProcEventEntry( varMap, varPlayer, varBagIdx )
		return 0;
end
...
function x430045_ProcIsSpellLikeScript( varMap, varPlayer)
	return 1; 
end
...
function x430045_ProcCancelImpacts( varMap, varPlayer )
	return 0; 
end
...
function x430045_ProcConditionCheck( varMap, varPlayer )
	...
	if x430045_GetDayCount(varMap, varPlayer) >= x430045_var_LimitCount then
			local varText = format("每天只能使用#G%d#cffcf00个体力丹！",x430045_var_LimitCount)
			StartTalkTask(varMap)
			TalkAppendString(varMap, varText)
			DeliverTalkTips(varMap, varPlayer)
			StopTalkTask(varMap)
...
function x430045_ProcDeplete( varMap, varPlayer )
...
function x430045_ProcActivateEachTick( varMap, varPlayer)
	return 1; 
end

function x430045_GetDayCount(varMap, varPlayer)
...
function x430045_SetDayCount(varMap, varPlayer)
	local day = GetDayOfYear()
	local rday = GetPlayerGameData(varMap, varPlayer, MD_WUYILINGJIANG_DATE[1], MD_WUYILINGJIANG_DATE[2], MD_WUYILINGJIANG_DATE[3])
