-------- 神行符脚本
x430207_var_FileId = 430207 
x430207_var_NeedItemId = 12036626
x430207_var_CDTime = 60
x430207_var_Buf1 = 7721
x430207_var_MainTransScript = 430014
x430207_var_UICommandID = 111

function x430207_ProcEventEntry( varMap, varPlayer, varBagIdx )
	if GetItemIsOverTimeByBagPos(varMap ,varPlayer ,varBagIdx) == 1 then
		...
		LuaCallNoclosure(x430207_var_MainTransScript, "ProcEventEntry", varMap, varPlayer, varBagIdx, x430207_var_UICommandID)
end

function x430207_CheckCanTrans(varMap, varPlayer, varBagIdx, varIndex ,newSceneId )
	
	if GetItemIDByIndexInBag(varMap, varPlayer, varBagIdx) ~= x430207_var_NeedItemId then
		return 0
	end
	
	local ret = LuaCallNoclosure(x430207_var_MainTransScript, "CommonConditionCheck", varMap, varPlayer, varBagIdx, varIndex ,newSceneId)
	if (ret == 0) then
		return 0
	end
	
	local continuance = GetCurrentTime() - GetPlayerGameData(varMap, varPlayer, MD_TRANS_TIME[1], MD_TRANS_TIME[2], MD_TRANS_TIME[3])
	if continuance < x430207_var_CDTime then
		local varMsg = format("%d秒后才可使用下一次千里符", x430207_var_CDTime-continuance)
...
function x430207_SetTransMap(varMap, varPlayer, varIndex, varBagIdx, value)
	if varIndex == 5 then
...
function x430207_SetTransPosX(varMap, varPlayer, varIndex, varBagIdx, value)
	if varIndex == 5 then
...
function x430207_SetTransPosZ(varMap, varPlayer, varIndex, varBagIdx, value)
	if varIndex == 5 then
...
function x430207_ProcIsSpellLikeScript( varMap, varPlayer)
	return 0
end

function x430207_ProcCancelImpacts( varMap, varPlayer )
	return 0
end

function x430207_ProcConditionCheck( varMap, varPlayer )
	return 0
end

function x430207_ProcDeplete( varMap, varPlayer )

	return 0
end

function x430207_ProcActivateOnce( varMap, varPlayer )
end

function x430207_ProcActivateEachTick( varMap, varPlayer)
	return 1
end
