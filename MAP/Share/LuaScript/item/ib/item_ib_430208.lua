-------- 神行符脚本
x430208_var_FileId = 430208 
x430208_var_NeedItemId = 12036627
x430208_var_CDTime = 60
x430208_var_Buf1 = 7721
x430208_var_MainTransScript = 430014
x430208_var_UICommandID = 111

function x430208_ProcEventEntry( varMap, varPlayer, varBagIdx )
	if GetItemIsOverTimeByBagPos(varMap ,varPlayer ,varBagIdx) == 1 then
		...
		LuaCallNoclosure(x430208_var_MainTransScript, "ProcEventEntry", varMap, varPlayer, varBagIdx, x430208_var_UICommandID)
end

function x430208_CheckCanTrans(varMap, varPlayer, varBagIdx, varIndex ,newSceneId )
	
	if GetItemIDByIndexInBag(varMap, varPlayer, varBagIdx) ~= x430208_var_NeedItemId then
		return 0
	end
	
	local ret = LuaCallNoclosure(x430208_var_MainTransScript, "CommonConditionCheck", varMap, varPlayer, varBagIdx, varIndex ,newSceneId)
	if (ret == 0) then
		return 0
	end
	
	local continuance = GetCurrentTime() - GetPlayerGameData(varMap, varPlayer, MD_TRANS_TIME[1], MD_TRANS_TIME[2], MD_TRANS_TIME[3])
	if continuance < x430208_var_CDTime then
		local varMsg = format("%d秒后才可使用下一次千里符", x430208_var_CDTime-continuance)
...
function x430208_SetTransMap(varMap, varPlayer, varIndex, varBagIdx, value)
	if varIndex == 5 then
...
function x430208_SetTransPosX(varMap, varPlayer, varIndex, varBagIdx, value)
	if varIndex == 5 then
...
function x430208_SetTransPosZ(varMap, varPlayer, varIndex, varBagIdx, value)
	if varIndex == 5 then
...
function x430208_ProcIsSpellLikeScript( varMap, varPlayer)
	return 0
end

function x430208_ProcCancelImpacts( varMap, varPlayer )
	return 0
end

function x430208_ProcConditionCheck( varMap, varPlayer )
	return 0
end

function x430208_ProcDeplete( varMap, varPlayer )

	return 0
end

function x430208_ProcActivateOnce( varMap, varPlayer )
end

function x430208_ProcActivateEachTick( varMap, varPlayer)
	return 1
end
