...
x550703_var_FileId = 550703
x550703_var_TransPosloulan  ="minju3biasnai41"
x550703_var_TransPoslaiyin  ="minju3biasnai42"
x550703_var_TransPoskunlun  ="minju3biasnai43"
x550703_var_TransPosdunhuang="minju3biasnai44"
x550703_var_Notice = "您的等级还不足#R20级#Y，无法前往边塞民居！"


function x550703_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x550703_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x550703_TransToPosition(varMap, varObj)
			end
	...
	x550703_TransToPosition(varMap, varPlayer)
end

function x550703_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if varMap == 51 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550703_var_TransPosloulan)
		
		elseif varMap == 151 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550703_var_TransPoslaiyin)
		
		elseif varMap == 251 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550703_var_TransPoskunlun)
		
		elseif varMap == 351 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550703_var_TransPosdunhuang)
	
	end
end


function x550703_ProcMapTimerTick( varMap, monsterobjid )

end


function x550703_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x550703_ProcEventEntry( varMap, monsterobjid )
