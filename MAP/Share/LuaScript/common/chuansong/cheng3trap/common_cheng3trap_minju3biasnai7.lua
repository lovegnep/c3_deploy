...
x550706_var_FileId = 550706
x550706_var_TransPosloulan  ="minju3biasnai71"
x550706_var_TransPoslaiyin  ="minju3biasnai72"
x550706_var_TransPoskunlun  ="minju3biasnai73"
x550706_var_TransPosdunhuang="minju3biasnai74"
x550706_var_Notice = "您的等级还不足#R20级#Y，无法前往边塞！"


function x550706_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x550706_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x550706_TransToPosition(varMap, varObj)
			end
	...
	x550706_TransToPosition(varMap, varPlayer)
end

function x550706_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if varMap == 60 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550706_var_TransPosloulan)
		
		elseif varMap == 160 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550706_var_TransPoslaiyin)
		
		elseif varMap == 260 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550706_var_TransPoskunlun)
		
		elseif varMap == 360 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550706_var_TransPosdunhuang)
	
	end
end


function x550706_ProcMapTimerTick( varMap, monsterobjid )

end


function x550706_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x550706_ProcEventEntry( varMap, monsterobjid )
