...
x550704_var_FileId = 550704
x550704_var_TransPosloulan  ="minju3biasnai51"
x550704_var_TransPoslaiyin  ="minju3biasnai52"
x550704_var_TransPoskunlun  ="minju3biasnai53"
x550704_var_TransPosdunhuang="minju3biasnai54"
x550704_var_Notice = "您的等级还不足#R20级#Y，无法前往边塞！"


function x550704_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x550704_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x550704_TransToPosition(varMap, varObj)
			end
	...
	x550704_TransToPosition(varMap, varPlayer)
end

function x550704_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if varMap == 58 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550704_var_TransPosloulan)
		
		elseif varMap == 158 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550704_var_TransPoslaiyin)
		
		elseif varMap == 258 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550704_var_TransPoskunlun)
		
		elseif varMap == 358 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550704_var_TransPosdunhuang)
	
	end
end


function x550704_ProcMapTimerTick( varMap, monsterobjid )

end


function x550704_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x550704_ProcEventEntry( varMap, monsterobjid )
