...
x550707_var_FileId = 550707
x550707_var_TransPosloulan  ="minju3biasnai81"
x550707_var_TransPoslaiyin  ="minju3biasnai82"
x550707_var_TransPoskunlun  ="minju3biasnai83"
x550707_var_TransPosdunhuang="minju3biasnai84"
x550707_var_Notice = "您的等级还不足#R20级#Y，无法前往边塞！"


function x550707_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x550707_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x550707_TransToPosition(varMap, varObj)
			end
	...
	x550707_TransToPosition(varMap, varPlayer)
end

function x550707_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if varMap == 61 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550707_var_TransPosloulan)
		
		elseif varMap == 161 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550707_var_TransPoslaiyin)
		
		elseif varMap == 261 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550707_var_TransPoskunlun)
		
		elseif varMap == 361 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550707_var_TransPosdunhuang)
	
	end
end


function x550707_ProcMapTimerTick( varMap, monsterobjid )

end


function x550707_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x550707_ProcEventEntry( varMap, monsterobjid )
