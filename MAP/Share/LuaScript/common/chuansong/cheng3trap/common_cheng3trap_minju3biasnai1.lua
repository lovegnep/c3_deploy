...
x550700_var_FileId = 550700
x550700_var_TransPosloulan  ="minju3biasnai11"
x550700_var_TransPoslaiyin  ="minju3biasnai12"
x550700_var_TransPoskunlun  ="minju3biasnai13"
x550700_var_TransPosdunhuang="minju3biasnai14"
x550700_var_Notice = "您的等级还不足#R20级#Y，无法前往边塞密屋！"


function x550700_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x550700_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x550700_TransToPosition(varMap, varObj)
			end
	...
	x550700_TransToPosition(varMap, varPlayer)
end

function x550700_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if varMap == 51 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550700_var_TransPosloulan)
		
		elseif varMap == 151 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550700_var_TransPoslaiyin)
		
		elseif varMap == 251 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550700_var_TransPoskunlun)
		
		elseif varMap == 351 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550700_var_TransPosdunhuang)
	
	end
end


function x550700_ProcMapTimerTick( varMap, monsterobjid )

end


function x550700_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x550700_ProcEventEntry( varMap, monsterobjid )
