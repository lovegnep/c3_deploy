...
x300301_var_FileId = 300301
x300301_var_TransPosloulan  ="dunhuangbiansaidongkou1"
x300301_var_TransPoslaiyin  ="dunhuangbiansaidongkou1"
x300301_var_TransPoskunlun  ="dunhuangbiansaidongkou1"
x300301_var_TransPosdunhuang="dunhuangbiansaidongkou1"
x300301_var_Notice = "您的等级还不足#R135级#Y，无法前往藏经阁！"


function x300301_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 135 then 
					Msg2Player(varMap,varPlayer,"#Y"..x300301_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300301_TransToPosition(varMap, varObj)
			end
	...
	x300301_TransToPosition(varMap, varPlayer)
end

function x300301_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300301_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300301_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300301_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300301_var_TransPosdunhuang)
	
	end
end


function x300301_ProcMapTimerTick( varMap, monsterobjid )

end


function x300301_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300301_ProcEventEntry( varMap, monsterobjid )
