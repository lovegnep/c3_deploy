...
x490007_var_FileId = 490007
x490007_var_TransPosloulan  ="kunlunwangchengdongkou1"
x490007_var_TransPoslaiyin  ="kunlunwangchengdongkou1"
x490007_var_TransPoskunlun  ="kunlunwangchengdongkou1"
x490007_var_TransPosdunhuang="kunlunwangchengdongkou1"
x490007_var_Notice = "您的等级还不足，无法前往通天之门！"


function x490007_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 999 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490007_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490007_TransToPosition(varMap, varObj)
			end
	...
	x490007_TransToPosition(varMap, varPlayer)
end

function x490007_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490007_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490007_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490007_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490007_var_TransPosdunhuang)
	
	end
end


function x490007_ProcMapTimerTick( varMap, monsterobjid )

end


function x490007_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490007_ProcEventEntry( varMap, monsterobjid )
