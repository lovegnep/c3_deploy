...
x490450_var_FileId = 490450
x490450_var_TransPosloulan  ="shabaketominju1"
x490450_var_TransPoslaiyin  ="shabaketominju1"
x490450_var_TransPoskunlun  ="shabaketominju1"
x490450_var_TransPosdunhuang="shabaketominju1"


function x490450_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490450_TransToPosition(varMap, varObj)
			end
	...
	x490450_TransToPosition(varMap, varPlayer)
end

function x490450_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490450_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490450_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490450_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490450_var_TransPosdunhuang)
	
	end
end


function x490450_ProcMapTimerTick( varMap, monsterobjid )

end


function x490450_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490450_ProcEventEntry( varMap, monsterobjid )
