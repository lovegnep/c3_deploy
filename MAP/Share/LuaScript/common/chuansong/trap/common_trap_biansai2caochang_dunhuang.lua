...
x300229_var_FileId = 300229
x300229_var_TransPosloulan  ="bianjing2caochangdunhuang"
x300229_var_TransPoslaiyin  ="bianjing2caochangdunhuang"
x300229_var_TransPoskunlun  ="bianjing2caochangdunhuang"
x300229_var_TransPosdunhuang="bianjing2caochangdunhuang"



function x300229_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300229_TransToPosition(varMap, varObj)
			end
	...
	x300229_TransToPosition(varMap, varPlayer)
end

function x300229_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300229_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300229_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300229_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300229_var_TransPosdunhuang)
	
	end
end


function x300229_ProcMapTimerTick( varMap, monsterobjid )

end


function x300229_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300229_ProcEventEntry( varMap, monsterobjid )
