...
x490003_var_FileId = 490003
x490003_var_TransPosloulan  ="loulanbiansaidongkou1"
x490003_var_TransPoslaiyin  ="loulanbiansaidongkou1"
x490003_var_TransPoskunlun  ="loulanbiansaidongkou1"
x490003_var_TransPosdunhuang="loulanbiansaidongkou1"
x490003_var_Notice = "您的等级还不足#R135级#Y，无法前往神迹峡谷！"


function x490003_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 135 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490003_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490003_TransToPosition(varMap, varObj)
			end
	...
	x490003_TransToPosition(varMap, varPlayer)
end

function x490003_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490003_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490003_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490003_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490003_var_TransPosdunhuang)
	
	end
end


function x490003_ProcMapTimerTick( varMap, monsterobjid )

end


function x490003_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490003_ProcEventEntry( varMap, monsterobjid )
