...
x300604_var_FileId = 300604
x300604_var_TransPosloulan  ="tataer2qiyan0"
x300604_var_TransPoslaiyin  ="tataer2qiyan1"
x300604_var_TransPoskunlun  ="tataer2qiyan2"
x300604_var_TransPosdunhuang="tataer2qiyan3"



function x300604_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300604_TransToPosition(varMap, varObj)
			end
	...
	x300604_TransToPosition(varMap, varPlayer)
end

function x300604_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300604_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300604_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300604_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300604_var_TransPosdunhuang)
	
	end
end


function x300604_ProcMapTimerTick( varMap, monsterobjid )

end


function x300604_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300604_ProcEventEntry( varMap, monsterobjid )
