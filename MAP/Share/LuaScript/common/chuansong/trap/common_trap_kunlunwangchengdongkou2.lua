...
x490008_var_FileId = 490008
x490008_var_TransPosloulan  ="kunlunwangchengdongkou2"
x490008_var_TransPoslaiyin  ="kunlunwangchengdongkou2"
x490008_var_TransPoskunlun  ="kunlunwangchengdongkou2"
x490008_var_TransPosdunhuang="kunlunwangchengdongkou2"
x490008_var_Notice = "您的等级还不足#R110级#Y，无法前往无极深渊！"


function x490008_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 110 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490008_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490008_TransToPosition(varMap, varObj)
			end
	...
	x490008_TransToPosition(varMap, varPlayer)
end

function x490008_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490008_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490008_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490008_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490008_var_TransPosdunhuang)
	
	end
end


function x490008_ProcMapTimerTick( varMap, monsterobjid )

end


function x490008_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490008_ProcEventEntry( varMap, monsterobjid )
