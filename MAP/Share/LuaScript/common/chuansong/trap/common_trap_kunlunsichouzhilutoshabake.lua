...
x490442_var_FileId = 490442
x490442_var_TransPosloulan  ="kunlunsichouzhilutoshabake"
x490442_var_TransPoslaiyin  ="kunlunsichouzhilutoshabake"
x490442_var_TransPoskunlun  ="kunlunsichouzhilutoshabake"
x490442_var_TransPosdunhuang="kunlunsichouzhilutoshabake"

function x490442_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490442_TransToPosition(varMap, varObj)
			end
	...
	x490442_TransToPosition(varMap, varPlayer)
end

function x490442_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490442_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490442_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490442_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490442_var_TransPosdunhuang)
	
	end
end


function x490442_ProcMapTimerTick( varMap, monsterobjid )

end


function x490442_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490442_ProcEventEntry( varMap, monsterobjid )
