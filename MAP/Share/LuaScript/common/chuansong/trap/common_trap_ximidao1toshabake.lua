...
x490481_var_FileId = 490481
x490481_var_TransPosloulan  ="ximidao1toshabake"
x490481_var_TransPoslaiyin  ="ximidao1toshabake"
x490481_var_TransPoskunlun  ="ximidao1toshabake"
x490481_var_TransPosdunhuang="ximidao1toshabake"


function x490481_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490481_TransToPosition(varMap, varObj)
			end
	...
	x490481_TransToPosition(varMap, varPlayer)
end

function x490481_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490481_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490481_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490481_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490481_var_TransPosdunhuang)
	
	end
end


function x490481_ProcMapTimerTick( varMap, monsterobjid )

end


function x490481_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490481_ProcEventEntry( varMap, monsterobjid )
