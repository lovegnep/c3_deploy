...
x300296_var_FileId = 300296
x300296_var_TransPosloulan  ="minju2biasnai4"
x300296_var_TransPoslaiyin  ="minju2biasnai4"
x300296_var_TransPoskunlun  ="minju2biasnai4"
x300296_var_TransPosdunhuang="minju2biasnai4"
x300296_var_Notice = "您的等级还不足#R20级#Y，无法前往边塞！"


function x300296_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x300296_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300296_TransToPosition(varMap, varObj)
			end
	...
	x300296_TransToPosition(varMap, varPlayer)
end

function x300296_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300296_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300296_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300296_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300296_var_TransPosdunhuang)
	
	end
end


function x300296_ProcMapTimerTick( varMap, monsterobjid )

end


function x300296_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300296_ProcEventEntry( varMap, monsterobjid )
