...
x490040_var_FileId = 490040
x490040_var_TransPosloulan  ="hougong2wangcheng0"
x490040_var_TransPoslaiyin  ="hougong2wangcheng1"
x490040_var_TransPoskunlun  ="hougong2wangcheng2"
x490040_var_TransPosdunhuang="hougong2wangcheng3"
x490040_var_Notice = "您的等级还不足#R10级#Y，无法前往王宫！"


function x490040_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 10 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490040_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490040_TransToPosition(varMap, varObj)
			end
	...
	x490040_TransToPosition(varMap, varPlayer)
end

function x490040_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if varMap == 70 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490040_var_TransPosloulan)
		
		elseif varMap == 170 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490040_var_TransPoslaiyin)
		
		elseif varMap == 270 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490040_var_TransPoskunlun)
		
		elseif varMap == 370 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490040_var_TransPosdunhuang)
	
	end
end


function x490040_ProcMapTimerTick( varMap, monsterobjid )

end


function x490040_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490040_ProcEventEntry( varMap, monsterobjid )
