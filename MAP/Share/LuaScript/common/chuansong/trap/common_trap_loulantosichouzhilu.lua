...
x490430_var_FileId = 490430
x490430_var_TransPosloulan  ="loulantosichouzhilu"
x490430_var_TransPoslaiyin  ="loulantosichouzhilu"
x490430_var_TransPoskunlun  ="loulantosichouzhilu"
x490430_var_TransPosdunhuang="loulantosichouzhilu"
x490430_var_Notice = "您的等级还不足#R40级#Y，无法前往楼兰丝绸之路！"

function x490430_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 40 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490430_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490430_TransToPosition(varMap, varObj)
			end
	...
	x490430_TransToPosition(varMap, varPlayer)
end

function x490430_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490430_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490430_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490430_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490430_var_TransPosdunhuang)
	
	end
end


function x490430_ProcMapTimerTick( varMap, monsterobjid )

end


function x490430_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490430_ProcEventEntry( varMap, monsterobjid )
