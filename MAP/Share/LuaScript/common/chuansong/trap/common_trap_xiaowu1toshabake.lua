...
x490451_var_FileId = 490451
x490451_var_TransPosloulan  ="minju1toshabake"
x490451_var_TransPoslaiyin  ="minju1toshabake"
x490451_var_TransPoskunlun  ="minju1toshabake"
x490451_var_TransPosdunhuang="minju1toshabake"


function x490451_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490451_TransToPosition(varMap, varObj)
			end
	...
	x490451_TransToPosition(varMap, varPlayer)
end

function x490451_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490451_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490451_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490451_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490451_var_TransPosdunhuang)
	
	end
end


function x490451_ProcMapTimerTick( varMap, monsterobjid )

end


function x490451_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490451_ProcEventEntry( varMap, monsterobjid )
