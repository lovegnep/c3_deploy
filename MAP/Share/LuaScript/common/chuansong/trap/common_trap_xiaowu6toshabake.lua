...
x490461_var_FileId = 490461
x490461_var_TransPosloulan  ="minju6toshabake"
x490461_var_TransPoslaiyin  ="minju6toshabake"
x490461_var_TransPoskunlun  ="minju6toshabake"
x490461_var_TransPosdunhuang="minju6toshabake"


function x490461_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490461_TransToPosition(varMap, varObj)
			end
	...
	x490461_TransToPosition(varMap, varPlayer)
end

function x490461_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490461_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490461_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490461_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490461_var_TransPosdunhuang)
	
	end
end


function x490461_ProcMapTimerTick( varMap, monsterobjid )

end


function x490461_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490461_ProcEventEntry( varMap, monsterobjid )
