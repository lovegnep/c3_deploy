...
x300431_var_FileId = 300431
x300431_var_TransPosloulan  ="caochang2muchangloulan"
x300431_var_TransPoslaiyin  ="caochang2muchangtianshan"
x300431_var_TransPoskunlun  ="caochang2muchangkunlun"
x300431_var_TransPosdunhuang="caochang2muchangdunhuang"
x300431_var_MapName ={
											[50]="楼兰",
											[150]="天山",
											[250]="昆仑",
...
x300431_var_Notice = "您的等级还不足#R20级#Y，无法前往%s边塞！"


function x300431_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 20 then  
					Msg2Player(varMap,varPlayer,"#Y"..format(x300431_var_Notice,x300431_var_MapName[varMap]).."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300431_TransToPosition(varMap, varObj)
			end
	...
	x300431_TransToPosition(varMap, varPlayer)
end

function x300431_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if varMap == 50 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300431_var_TransPosloulan)
		
		elseif varMap == 150 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300431_var_TransPoslaiyin)
		
		elseif varMap == 250 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300431_var_TransPoskunlun)
		
		elseif varMap == 350 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300431_var_TransPosdunhuang)
	
	end
end


function x300431_ProcMapTimerTick( varMap, monsterobjid )

end


function x300431_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300431_ProcEventEntry( varMap, monsterobjid )
