...
x490458_var_FileId = 490458
x490458_var_TransPosloulan  ="shabaketominju5"
x490458_var_TransPoslaiyin  ="shabaketominju5"
x490458_var_TransPoskunlun  ="shabaketominju5"
x490458_var_TransPosdunhuang="shabaketominju5"


function x490458_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490458_TransToPosition(varMap, varObj)
			end
	...
	x490458_TransToPosition(varMap, varPlayer)
end

function x490458_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490458_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490458_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490458_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490458_var_TransPosdunhuang)
	
	end
end


function x490458_ProcMapTimerTick( varMap, monsterobjid )

end


function x490458_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490458_ProcEventEntry( varMap, monsterobjid )
