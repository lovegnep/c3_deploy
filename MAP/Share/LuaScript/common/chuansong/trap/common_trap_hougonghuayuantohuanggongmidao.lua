...
x490404_var_FileId = 490404
x490404_var_TransPosloulan  ="hougonghuayuantohuanggongmidao"
x490404_var_TransPoslaiyin  ="hougonghuayuantohuanggongmidao"
x490404_var_TransPoskunlun  ="hougonghuayuantohuanggongmidao"
x490404_var_TransPosdunhuang="hougonghuayuantohuanggongmidao"
x490404_var_Notice = "对不起，您的等级不足70级，无法前往混沌之路"


function x490404_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	local minOfDay = GetMinOfDay()
	local weekIdx = GetWeek()
	
	if weekIdx == 6 and minOfDay >= 890 and minOfDay <= 900 then
			Msg2Player(varMap,varPlayer,"决战后花园期间，无法前往混沌之路",8 ,2)
			return
	end		
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 70 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490404_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490404_TransToPosition(varMap, varObj)
			end
	...
	x490404_TransToPosition(varMap, varPlayer)
end

function x490404_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490404_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490404_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490404_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490404_var_TransPosdunhuang)
	
	end
end


function x490404_ProcMapTimerTick( varMap, monsterobjid )

end


function x490404_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490404_ProcEventEntry( varMap, monsterobjid )
