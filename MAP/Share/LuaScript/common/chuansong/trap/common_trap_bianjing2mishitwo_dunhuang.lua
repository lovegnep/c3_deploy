...
x270157_var_FileId = 270157
x270157_var_TransPosloulan  ="bianjing2mishitwo3"
x270157_var_TransPoslaiyin  ="bianjing2mishitwo3"
x270157_var_TransPoskunlun  ="bianjing2mishitwo3"
x270157_var_TransPosdunhuang="bianjing2mishitwo3"
x270157_var_Notice = "您的等级还不足#R20级#Y，无法前往边塞密室！"


function x270157_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x270157_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x270157_TransToPosition(varMap, varObj)
			end
	...
	x270157_TransToPosition(varMap, varPlayer)
end

function x270157_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x270157_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x270157_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x270157_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x270157_var_TransPosdunhuang)
	
	end
end


function x270157_ProcMapTimerTick( varMap, monsterobjid )

end


function x270157_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x270157_ProcEventEntry( varMap, monsterobjid )
