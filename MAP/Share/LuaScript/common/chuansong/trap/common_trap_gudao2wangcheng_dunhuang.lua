...
x300220_var_FileId = 300220
x300220_var_TransPosloulan  ="gudao2wangchengdunhuang"
x300220_var_TransPoslaiyin  ="gudao2wangchengdunhuang"
x300220_var_TransPoskunlun  ="gudao2wangchengdunhuang"
x300220_var_TransPosdunhuang="gudao2wangchengdunhuang"



function x300220_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300220_TransToPosition(varMap, varObj)
			end
	...
	x300220_TransToPosition(varMap, varPlayer)
end

function x300220_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300220_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300220_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300220_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300220_var_TransPosdunhuang)
	
	end
end


function x300220_ProcMapTimerTick( varMap, monsterobjid )

end


function x300220_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300220_ProcEventEntry( varMap, monsterobjid )
