...
x490023_var_FileId = 490023
x490023_var_TransPosloulan  ="daduhuayuantohuanggong"
x490023_var_TransPoslaiyin  ="daduhuayuantohuanggong"
x490023_var_TransPoskunlun  ="daduhuayuantohuanggong"
x490023_var_TransPosdunhuang="daduhuayuantohuanggong"
x490023_var_Notice = "您的等级还不足#R70级#Y，无法前往大都·皇城！"


function x490023_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 70 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490023_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490023_TransToPosition(varMap, varObj)
			end
	...
	x490023_TransToPosition(varMap, varPlayer)
end

function x490023_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490023_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490023_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490023_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490023_var_TransPosdunhuang)
	
	end
end


function x490023_ProcMapTimerTick( varMap, monsterobjid )

end


function x490023_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490023_ProcEventEntry( varMap, monsterobjid )
