...
x300300_var_FileId = 300300
x300300_var_TransPosloulan  ="dadumingjutwo2dadu1"
x300300_var_TransPoslaiyin  ="dadumingjutwo2dadu2"
x300300_var_TransPoskunlun  ="dadumingjutwo2dadu3"
x300300_var_TransPosdunhuang="dadumingjutwo2dadu4"
x300300_var_Notice = "您的等级还不足#R20级#Y，无法前往大都！"


function x300300_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x300300_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300300_TransToPosition(varMap, varObj)
			end
	...
	x300300_TransToPosition(varMap, varPlayer)
end

function x300300_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300300_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300300_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300300_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300300_var_TransPosdunhuang)
	
	end
end


function x300300_ProcMapTimerTick( varMap, monsterobjid )

end


function x300300_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300300_ProcEventEntry( varMap, monsterobjid )
