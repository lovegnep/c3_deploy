...
x300482_var_FileId = 300482
x300482_var_TransPosLou="loulanbianjingexit2xiaoshuidaowest"
x300482_var_TransPosLai="laiyinbianjingexit2xiaoshuidaowest"
x300482_var_TransPosKun="kunlunbianjingexit2xiaoshuidaowest"
x300482_var_TransPosDun="dunhuangbianjingexit2xiaoshuidaowest"
x300482_var_Notice = "您的等级还不足#R20级#Y，无法前往王国边塞！"


function x300482_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
			...
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x300482_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300482_TransToPosition(varMap, varObj)
			end
	...
	x300482_TransToPosition(varMap, varPlayer)
end

function x300482_TransToPosition(varMap, varPlayer)
	if varMap == 76 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300482_var_TransPosLou)
	elseif varMap == 176 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300482_var_TransPosLai)
	elseif varMap == 276 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300482_var_TransPosKun)
	elseif varMap == 376 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300482_var_TransPosDun)
	end
end


function x300482_ProcMapTimerTick( varMap, monsterobjid )

end


function x300482_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300482_ProcEventEntry( varMap, monsterobjid )
