...
x490472_var_FileId = 490472
x490472_var_TransPosloulan  ="shabaketominju12"
x490472_var_TransPoslaiyin  ="shabaketominju12"
x490472_var_TransPoskunlun  ="shabaketominju12"
x490472_var_TransPosdunhuang="shabaketominju12"


function x490472_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490472_TransToPosition(varMap, varObj)
			end
	...
	x490472_TransToPosition(varMap, varPlayer)
end

function x490472_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490472_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490472_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490472_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490472_var_TransPosdunhuang)
	
	end
end


function x490472_ProcMapTimerTick( varMap, monsterobjid )

end


function x490472_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490472_ProcEventEntry( varMap, monsterobjid )
