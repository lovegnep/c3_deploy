...
x490508_var_FileId = 490508
x490508_var_TransPosloulan  ="xianrengu5toxianrengu4"
x490508_var_TransPoslaiyin  ="xianrengu5toxianrengu4"
x490508_var_TransPoskunlun  ="xianrengu5toxianrengu4"
x490508_var_TransPosdunhuang="xianrengu5toxianrengu4"
x490508_var_Notice = "对不起，您的等级不足90级，无法前往仙人谷"


function x490508_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 90 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490508_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490508_TransToPosition(varMap, varObj)
			end
	...
	x490508_TransToPosition(varMap, varPlayer)
end

function x490508_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490508_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490508_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490508_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490508_var_TransPosdunhuang)
	
	end
end


function x490508_ProcMapTimerTick( varMap, monsterobjid )

end


function x490508_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490508_ProcEventEntry( varMap, monsterobjid )
