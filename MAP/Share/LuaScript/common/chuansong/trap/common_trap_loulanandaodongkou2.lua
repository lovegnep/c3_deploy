...
x490020_var_FileId = 490020
x490020_var_TransPosloulan  ="loulanandaodongkou2"
x490020_var_TransPoslaiyin  ="loulanandaodongkou2"
x490020_var_TransPoskunlun  ="loulanandaodongkou2"
x490020_var_TransPosdunhuang="loulanandaodongkou2"
x490020_var_Notice = "您的等级还不足#R125级#Y，无法前往葬金暗室！"


function x490020_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 125 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490020_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490020_TransToPosition(varMap, varObj)
			end
	...
	x490020_TransToPosition(varMap, varPlayer)
end

function x490020_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490020_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490020_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490020_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490020_var_TransPosdunhuang)
	
	end
end


function x490020_ProcMapTimerTick( varMap, monsterobjid )

end


function x490020_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490020_ProcEventEntry( varMap, monsterobjid )
