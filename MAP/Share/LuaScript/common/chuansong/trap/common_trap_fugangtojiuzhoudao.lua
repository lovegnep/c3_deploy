...
x490205_var_FileId = 490205
x490205_var_TransPosloulan  ="fugangtojiuzhoudao"
x490205_var_TransPoslaiyin  ="fugangtojiuzhoudao"
x490205_var_TransPoskunlun  ="fugangtojiuzhoudao"
x490205_var_TransPosdunhuang="fugangtojiuzhoudao"
x490205_var_Notice = "你的等级不足50级！"


function x490205_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 50 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490205_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490205_TransToPosition(varMap, varObj)
			end
	...
	x490205_TransToPosition(varMap, varPlayer)
end

function x490205_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490205_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490205_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490205_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490205_var_TransPosdunhuang)
	
	end
end


function x490205_ProcMapTimerTick( varMap, monsterobjid )

end


function x490205_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490205_ProcEventEntry( varMap, monsterobjid )
