...
x490485_var_FileId = 490485
x490485_var_TransPosloulan  ="ximidao3toshabake"
x490485_var_TransPoslaiyin  ="ximidao3toshabake"
x490485_var_TransPoskunlun  ="ximidao3toshabake"
x490485_var_TransPosdunhuang="ximidao3toshabake"


function x490485_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490485_TransToPosition(varMap, varObj)
			end
	...
	x490485_TransToPosition(varMap, varPlayer)
end

function x490485_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490485_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490485_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490485_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490485_var_TransPosdunhuang)
	
	end
end


function x490485_ProcMapTimerTick( varMap, monsterobjid )

end


function x490485_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490485_ProcEventEntry( varMap, monsterobjid )
