...
x300283_var_FileId = 300283
x300283_var_TransPos0  ="wangcheng2migongone1"
x300283_var_TransPos1  ="wangcheng2migongone2"
x300283_var_TransPos2  ="wangcheng2migongone3"
x300283_var_TransPos3  ="wangcheng2migongone4"
x300283_var_Notice = "您的等级还不足#R25级#Y，无法前往王国迷宫！"


function x300283_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 25 then 
					Msg2Player(varMap,varPlayer,"#Y"..x300283_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300283_TransToPosition(varMap, varObj)
			end
	...
	x300283_TransToPosition(varMap, varPlayer)
end


function x300283_TransToPosition(varMap, varPlayer)

	local varCountry = GetCurCountry(varMap,varPlayer)

	if varMap == 50 and varCountry == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300283_var_TransPos0)
	elseif varMap == 150 and varCountry == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300283_var_TransPos1)
	elseif varMap == 250 and varCountry == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300283_var_TransPos2)
	elseif varMap == 350 and varCountry == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300283_var_TransPos3)
	else
		StartTalkTask(varMap)
...
function x300283_ProcMapTimerTick( varMap, monsterobjid )

end


function x300283_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300283_ProcEventEntry( varMap, monsterobjid )
