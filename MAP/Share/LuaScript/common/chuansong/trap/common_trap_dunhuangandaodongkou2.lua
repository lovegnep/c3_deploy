...
x490014_var_FileId = 490014
x490014_var_TransPosloulan  ="dunhuangandaodongkou2"
x490014_var_TransPoslaiyin  ="dunhuangandaodongkou2"
x490014_var_TransPoskunlun  ="dunhuangandaodongkou2"
x490014_var_TransPosdunhuang="dunhuangandaodongkou2"
x490014_var_Notice = "您的等级还不足#R125级#Y，无法前往藏宝暗室！"


function x490014_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 125 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490014_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490014_TransToPosition(varMap, varObj)
			end
	...
	x490014_TransToPosition(varMap, varPlayer)
end

function x490014_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490014_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490014_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490014_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490014_var_TransPosdunhuang)
	
	end
end


function x490014_ProcMapTimerTick( varMap, monsterobjid )

end


function x490014_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490014_ProcEventEntry( varMap, monsterobjid )
