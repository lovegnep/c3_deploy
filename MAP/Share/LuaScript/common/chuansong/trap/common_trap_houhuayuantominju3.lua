...
x490419_var_FileId = 490419
x490419_var_TransPosloulan  ="houhuayuantominju3"
x490419_var_TransPoslaiyin  ="houhuayuantominju3"
x490419_var_TransPoskunlun  ="houhuayuantominju3"
x490419_var_TransPosdunhuang="houhuayuantominju3"
x490419_var_Notice = "对不起，您的等级不足10级，无法前往皇宫秘道"


function x490419_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	local minOfDay = GetMinOfDay()
	local weekIdx = GetWeek()
	
	if weekIdx == 6 and minOfDay >= 890 and minOfDay <= 900 then
		Msg2Player(varMap,varPlayer,"决战后花园期间，无法进入民居",8 ,2)
		return
	end		
	...
	local nLevel = GetLevel(varMap, varPlayer)
		if nLevel < 10 then 
			Msg2Player(varMap,varPlayer,"#Y"..x490419_var_Notice.."#W",4,3)
			return
		end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490419_TransToPosition(varMap, varObj)
			end
	...
	x490419_TransToPosition(varMap, varPlayer)
end

function x490419_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490419_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490419_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490419_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490419_var_TransPosdunhuang)
	
	end
end


function x490419_ProcMapTimerTick( varMap, monsterobjid )

end


function x490419_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490419_ProcEventEntry( varMap, monsterobjid )
