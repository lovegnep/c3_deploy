...
x490460_var_FileId = 490460
x490460_var_TransPosloulan  ="shabaketominju6"
x490460_var_TransPoslaiyin  ="shabaketominju6"
x490460_var_TransPoskunlun  ="shabaketominju6"
x490460_var_TransPosdunhuang="shabaketominju6"


function x490460_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490460_TransToPosition(varMap, varObj)
			end
	...
	x490460_TransToPosition(varMap, varPlayer)
end

function x490460_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490460_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490460_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490460_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490460_var_TransPosdunhuang)
	
	end
end


function x490460_ProcMapTimerTick( varMap, monsterobjid )

end


function x490460_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490460_ProcEventEntry( varMap, monsterobjid )
