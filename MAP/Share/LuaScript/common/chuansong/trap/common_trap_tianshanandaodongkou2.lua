...
x490018_var_FileId = 490018
x490018_var_TransPosloulan  ="tianshanandaodongkou2"
x490018_var_TransPoslaiyin  ="tianshanandaodongkou2"
x490018_var_TransPoskunlun  ="tianshanandaodongkou2"
x490018_var_TransPosdunhuang="tianshanandaodongkou2"
x490018_var_Notice = "您的等级还不足#R125级#Y，无法前往葬宝暗室！"


function x490018_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 125 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490018_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490018_TransToPosition(varMap, varObj)
			end
	...
	x490018_TransToPosition(varMap, varPlayer)
end

function x490018_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490018_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490018_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490018_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490018_var_TransPosdunhuang)
	
	end
end


function x490018_ProcMapTimerTick( varMap, monsterobjid )

end


function x490018_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490018_ProcEventEntry( varMap, monsterobjid )
