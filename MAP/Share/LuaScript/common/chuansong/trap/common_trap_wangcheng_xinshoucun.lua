...
x550503_var_FileId = 550503
x550503_var_TransPos0	=	"loulanwangcheng2xinshoucun"
x550503_var_TransPos1	=	"laiyinwangcheng2xinshoucun"
x550503_var_TransPos2	=	"kunlunwangcheng2xinshoucun"
x550503_var_TransPos3	=	"dunhuangwangcheng2xinshoucun"



function x550503_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x550503_TransToPosition(varMap, varObj)
			end
	...
	x550503_TransToPosition(varMap, varPlayer)

end

function x550503_TransToPosition(varMap, varPlayer)

	local varCountry = GetCurCountry(varMap,varPlayer)

	if varMap == 50 and varCountry == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550503_var_TransPos0)
	elseif varMap == 150 and varCountry == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550503_var_TransPos1)
	elseif varMap == 250 and varCountry == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550503_var_TransPos2)
	elseif varMap == 350 and varCountry == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550503_var_TransPos3)
	else
		StartTalkTask(varMap)
...
function x550503_ProcMapTimerTick( varMap, monsterobjid )

end


function x550503_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x550503_ProcEventEntry( varMap, monsterobjid )
