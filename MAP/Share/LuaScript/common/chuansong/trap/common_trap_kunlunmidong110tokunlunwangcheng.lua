...
x490102_var_FileId = 490102
x490102_var_TransPosloulan  ="kunlunmidong110tokunlunwangcheng"
x490102_var_TransPoslaiyin  ="kunlunmidong110tokunlunwangcheng"
x490102_var_TransPoskunlun  ="kunlunmidong110tokunlunwangcheng"
x490102_var_TransPosdunhuang="kunlunmidong110tokunlunwangcheng"
x490102_var_Notice = "你的等级还不足110级！"


function x490102_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 110 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490102_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	i = 0, memberCount - 1 do
				...
				x490102_TransToPosition(varMap, varObj)
			end
	...
	x490102_TransToPosition(varMap, varPlayer)
end

function x490102_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490102_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490102_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490102_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490102_var_TransPosdunhuang)
	
	end
end


function x490102_ProcMapTimerTick( varMap, monsterobjid )

end


function x490102_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490102_ProcEventEntry( varMap, monsterobjid )
