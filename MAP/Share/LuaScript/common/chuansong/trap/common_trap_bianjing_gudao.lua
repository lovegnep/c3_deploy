...
x300424_var_FileId = 300424
x300424_var_TransPosloulan  ="bianjing2gudaoloulan"
x300424_var_TransPoslaiyin  ="bianjing2gudaoloulan"
x300424_var_TransPoskunlun  ="bianjing2gudaoloulan"
x300424_var_TransPosdunhuang="bianjing2gudaoloulan"



function x300424_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300424_TransToPosition(varMap, varObj)
			end
	...
	x300424_TransToPosition(varMap, varPlayer)
end

function x300424_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300424_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300424_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300424_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300424_var_TransPosdunhuang)
	
	end
end


function x300424_ProcMapTimerTick( varMap, monsterobjid )

end


function x300424_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300424_ProcEventEntry( varMap, monsterobjid )
