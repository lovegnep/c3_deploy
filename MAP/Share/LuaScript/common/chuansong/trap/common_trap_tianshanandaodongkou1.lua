...
x490017_var_FileId = 490017
x490017_var_TransPosloulan  ="tianshanandaodongkou1"
x490017_var_TransPoslaiyin  ="tianshanandaodongkou1"
x490017_var_TransPoskunlun  ="tianshanandaodongkou1"
x490017_var_TransPosdunhuang="tianshanandaodongkou1"
x490017_var_Notice = "您的等级还不足#R125级#Y，无法前往葬宝暗室！"


function x490017_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 125 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490017_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490017_TransToPosition(varMap, varObj)
			end
	...
	x490017_TransToPosition(varMap, varPlayer)
end

function x490017_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490017_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490017_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490017_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490017_var_TransPosdunhuang)
	
	end
end


function x490017_ProcMapTimerTick( varMap, monsterobjid )

end


function x490017_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490017_ProcEventEntry( varMap, monsterobjid )
