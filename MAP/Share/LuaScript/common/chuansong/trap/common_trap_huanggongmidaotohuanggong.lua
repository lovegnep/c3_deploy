...
x490402_var_FileId = 490402
x490402_var_TransPosloulan  ="huanggongmidaotohuanggong"
x490402_var_TransPoslaiyin  ="huanggongmidaotohuanggong"
x490402_var_TransPoskunlun  ="huanggongmidaotohuanggong"
x490402_var_TransPosdunhuang="huanggongmidaotohuanggong"
x490402_var_Notice = "对不起，您的等级不足70级，无法前往皇宫"


function x490402_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 70 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490402_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490402_TransToPosition(varMap, varObj)
			end
	...
	x490402_TransToPosition(varMap, varPlayer)
end

function x490402_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490402_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490402_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490402_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490402_var_TransPosdunhuang)
	
	end
end


function x490402_ProcMapTimerTick( varMap, monsterobjid )

end


function x490402_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490402_ProcEventEntry( varMap, monsterobjid )
