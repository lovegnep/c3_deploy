...
x490401_var_FileId = 490401
x490401_var_TransPosloulan  ="huanggongtohuanggongmidao"
x490401_var_TransPoslaiyin  ="huanggongtohuanggongmidao"
x490401_var_TransPoskunlun  ="huanggongtohuanggongmidao"
x490401_var_TransPosdunhuang="huanggongtohuanggongmidao"
x490401_var_Notice = "对不起，您的等级不足70级，无法前往混沌之路"


function x490401_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
...
--		Msg2Player(varMap,varPlayer,"很抱歉，服务器等级排行榜最后一名达到70级才可前往皇宫秘道",8,2)
--		return
--	end
	...
	if weekIdx == 6 and minOfDay >= 870 and minOfDay <= 890 then
		...
		if Level < 40 then
			...
			local Level = GetLevel(varMap,varPlayer)
			if Level < 70 then 
			Msg2Player(varMap,varPlayer,"#Y"..x490401_var_Notice.."#W",4,3)
			return
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490401_TransToPosition(varMap, varObj)
			end
	...
	x490401_TransToPosition(varMap, varPlayer)
end

function x490401_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490401_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490401_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490401_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490401_var_TransPosdunhuang)
	
	end
end


function x490401_ProcMapTimerTick( varMap, monsterobjid )

end


function x490401_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490401_ProcEventEntry( varMap, monsterobjid )
