...
x490002_var_FileId = 490002
x490002_var_TransPosloulan  ="tianshanbiansaidongkou2"
x490002_var_TransPoslaiyin  ="tianshanbiansaidongkou2"
x490002_var_TransPoskunlun  ="tianshanbiansaidongkou2"
x490002_var_TransPosdunhuang="tianshanbiansaidongkou2"
x490002_var_Notice = "您的等级还不足#R115级#Y，无法前往黄泉秘洞！"


function x490002_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 115 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490002_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490002_TransToPosition(varMap, varObj)
			end
	...
	x490002_TransToPosition(varMap, varPlayer)
end

function x490002_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490002_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490002_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490002_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490002_var_TransPosdunhuang)
	
	end
end


function x490002_ProcMapTimerTick( varMap, monsterobjid )

end


function x490002_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490002_ProcEventEntry( varMap, monsterobjid )
