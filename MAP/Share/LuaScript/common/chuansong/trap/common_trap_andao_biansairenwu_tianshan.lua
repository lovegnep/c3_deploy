...
x300254_var_FileId = 300254
x300254_var_TransPosloulan  ="andao2bianjingrenwu0"
x300254_var_TransPoslaiyin  ="andao2bianjingrenwu1"
x300254_var_TransPoskunlun  ="andao2bianjingrenwu2"
x300254_var_TransPosdunhuang="andao2bianjingrenwu3"
x300254_var_Notice = "您的等级还不足#R20级#Y，无法前往边塞！"


function x300254_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x300254_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300254_TransToPosition(varMap, varObj)
			end
	...
	x300254_TransToPosition(varMap, varPlayer)
end

function x300254_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if varMap == 76 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300254_var_TransPosloulan)
		
		elseif varMap == 176 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300254_var_TransPoslaiyin)
		
		elseif varMap == 276 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300254_var_TransPoskunlun)
		
		elseif varMap == 376 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300254_var_TransPosdunhuang)
	
	end
end


function x300254_ProcMapTimerTick( varMap, monsterobjid )

end


function x300254_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300254_ProcEventEntry( varMap, monsterobjid )
