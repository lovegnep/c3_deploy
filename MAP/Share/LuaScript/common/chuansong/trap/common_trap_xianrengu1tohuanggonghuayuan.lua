...
x490406_var_FileId = 490406
x490406_var_TransPosloulan  ="xianrengu1tohuanggonghuayuan"
x490406_var_TransPoslaiyin  ="xianrengu1tohuanggonghuayuan"
x490406_var_TransPoskunlun  ="xianrengu1tohuanggonghuayuan"
x490406_var_TransPosdunhuang="xianrengu1tohuanggonghuayuan"
x490406_var_Notice = "对不起，您的等级不足90级，无法前往后花园"


function x490406_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	--	Msg2Player(varMap,varPlayer,"很抱歉，服务器等级排行榜最后一名达到70级才可前往后花园",8,2)
	--	return
	--end
	...
	if weekIdx == 6 and minOfDay >= 870 and minOfDay <= 890 then
		...
		local Level = GetLevel(varMap,varPlayer)
		if Level < 90 then 
			Msg2Player(varMap,varPlayer,"#Y"..x490406_var_Notice.."#W",4,3)
			return
		end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490406_TransToPosition(varMap, varObj)
			end
	...
	x490406_TransToPosition(varMap, varPlayer)
end

function x490406_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490406_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490406_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490406_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490406_var_TransPosdunhuang)
	
	end
end


function x490406_ProcMapTimerTick( varMap, monsterobjid )

end


function x490406_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490406_ProcEventEntry( varMap, monsterobjid )
