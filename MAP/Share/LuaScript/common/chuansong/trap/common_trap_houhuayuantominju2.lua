...
x490417_var_FileId = 490417
x490417_var_TransPosloulan  ="houhuayuantominju2"
x490417_var_TransPoslaiyin  ="houhuayuantominju2"
x490417_var_TransPoskunlun  ="houhuayuantominju2"
x490417_var_TransPosdunhuang="houhuayuantominju2"
x490417_var_Notice = "对不起，您的等级不足10级，无法前往皇宫秘道"


function x490417_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	local minOfDay = GetMinOfDay()
	local weekIdx = GetWeek()
	
	if weekIdx == 6 and minOfDay >= 890 and minOfDay <= 900 then
		Msg2Player(varMap,varPlayer,"决战后花园期间，无法进入民宅",8 ,2)
		return
	end		
	...
	local nLevel = GetLevel(varMap, varPlayer)
		if nLevel < 10 then 
			Msg2Player(varMap,varPlayer,"#Y"..x490417_var_Notice.."#W",4,3)
			return
		end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490417_TransToPosition(varMap, varObj)
			end
	...
	x490417_TransToPosition(varMap, varPlayer)
end

function x490417_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490417_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490417_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490417_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490417_var_TransPosdunhuang)
	
	end
end


function x490417_ProcMapTimerTick( varMap, monsterobjid )

end


function x490417_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490417_ProcEventEntry( varMap, monsterobjid )
