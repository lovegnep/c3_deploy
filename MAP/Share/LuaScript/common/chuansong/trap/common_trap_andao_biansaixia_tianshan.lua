...
x300257_var_FileId = 300257
x300257_var_TransPosloulan  ="andao2bianjingxai0"
x300257_var_TransPoslaiyin  ="andao2bianjingxai1"
x300257_var_TransPoskunlun  ="andao2bianjingxai2"
x300257_var_TransPosdunhuang="andao2bianjingxai3"
x300257_var_Notice = "您的等级还不足#R20级#Y，无法前往边塞！"


function x300257_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x300257_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300257_TransToPosition(varMap, varObj)
			end
	...
	x300257_TransToPosition(varMap, varPlayer)
end

function x300257_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if varMap == 76 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300257_var_TransPosloulan)
		
		elseif varMap == 176 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300257_var_TransPoslaiyin)
		
		elseif varMap == 276 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300257_var_TransPoskunlun)
		
		elseif varMap == 376 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300257_var_TransPosdunhuang)
	
	end
end


function x300257_ProcMapTimerTick( varMap, monsterobjid )

end


function x300257_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300257_ProcEventEntry( varMap, monsterobjid )
