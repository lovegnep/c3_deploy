...
x490464_var_FileId = 490464
x490464_var_TransPosloulan  ="shabaketominju8"
x490464_var_TransPoslaiyin  ="shabaketominju8"
x490464_var_TransPoskunlun  ="shabaketominju8"
x490464_var_TransPosdunhuang="shabaketominju8"


function x490464_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490464_TransToPosition(varMap, varObj)
			end
	...
	x490464_TransToPosition(varMap, varPlayer)
end

function x490464_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490464_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490464_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490464_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490464_var_TransPosdunhuang)
	
	end
end


function x490464_ProcMapTimerTick( varMap, monsterobjid )

end


function x490464_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490464_ProcEventEntry( varMap, monsterobjid )
