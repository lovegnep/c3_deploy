...
x490101_var_FileId = 490101
x490101_var_TransPosloulan  ="dunhuangmidong110todunhuangwangcheng"
x490101_var_TransPoslaiyin  ="dunhuangmidong110todunhuangwangcheng"
x490101_var_TransPoskunlun  ="dunhuangmidong110todunhuangwangcheng"
x490101_var_TransPosdunhuang="dunhuangmidong110todunhuangwangcheng"
x490101_var_Notice = "您的等级不足110级，无法进入无底山坳。"


function x490101_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 110 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490101_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	i = 0, memberCount - 1 do
				...
				x490101_TransToPosition(varMap, varObj)
			end
	...
	x490101_TransToPosition(varMap, varPlayer)
end

function x490101_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490101_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490101_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490101_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490101_var_TransPosdunhuang)
	
	end
end


function x490101_ProcMapTimerTick( varMap, monsterobjid )

end


function x490101_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490101_ProcEventEntry( varMap, monsterobjid )
