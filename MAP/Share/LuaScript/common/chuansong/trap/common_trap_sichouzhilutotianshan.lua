...
x490433_var_FileId = 490433
x490433_var_TransPosloulan  ="sichouzhilutotianshan"
x490433_var_TransPoslaiyin  ="sichouzhilutotianshan"
x490433_var_TransPoskunlun  ="sichouzhilutotianshan"
x490433_var_TransPosdunhuang="sichouzhilutotianshan"
x490433_var_TuchengzhanQuestID = 7559

function x490433_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
local minute = GetMinOfDay()	
local weekIdx = GetWeek()

if minute >= 900 and minute < 1020 then
	if 	x490433_IsTianShanRuoGuo(varMap, varPlayer) > 0 then
		if x490433_IsLianMeng(varMap, varPlayer) ~= 1 then
			if x490433_IsBenGuo(varMap, varPlayer) == 0 then
				if weekIdx == 6 then
					if IsHaveQuestNM(varMap, varPlayer, x490433_var_TuchengzhanQuestID ) <= 0 then
	...
	x490433_TransToPosition(varMap, varPlayer)
end

function x490433_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490433_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490433_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490433_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490433_var_TransPosdunhuang)
	
	end
end


function x490433_ProcMapTimerTick( varMap, monsterobjid )

end


function x490433_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490433_ProcEventEntry( varMap, monsterobjid )

end

function x490433_IsTianShanRuoGuo(varMap, varPlayer)
	...
	if maxlevel >= 70 then
			if tianshanscore <= allscore*0.2 then	
				if x490433_IsMinGuo(varMap, varPlayer) == 1 then
	 	 			return 1
	 	 		end
			end
	end
...
function x490433_IsLianMeng(varMap, varPlayer)
...
function x490433_IsBenGuo(varMap, varPlayer)
...
function x490433_IsMinGuo(varMap, varPlayer)
	local loulanscore   = GetCountryParam(varMap, 0, CD_LASTWEEK_STRONG_WEAK_JIFEN)
	local tianshanscore = GetCountryParam(varMap, 1, CD_LASTWEEK_STRONG_WEAK_JIFEN)
	local kunlunscore   = GetCountryParam(varMap, 2, CD_LASTWEEK_STRONG_WEAK_JIFEN)
	local dunhuangscore = GetCountryParam(varMap, 3, CD_LASTWEEK_STRONG_WEAK_JIFEN)
