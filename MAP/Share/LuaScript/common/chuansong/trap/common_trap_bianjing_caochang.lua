...
x300428_var_FileId = 300428
x300428_var_TransPosloulan  ="bianjing2caochangloulan"
x300428_var_TransPoslaiyin  ="bianjing2caochangtianshan"
x300428_var_TransPoskunlun  ="bianjing2caochangkunlun"
x300428_var_TransPosdunhuang="bianjing2caochangdunhuang"



function x300428_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300428_TransToPosition(varMap, varObj)
			end
	...
	x300428_TransToPosition(varMap, varPlayer)
end

function x300428_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if varMap == 51 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300428_var_TransPosloulan)
		
		elseif varMap == 151 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300428_var_TransPoslaiyin)
		
		elseif varMap == 251 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300428_var_TransPoskunlun)
		
		elseif varMap == 351 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300428_var_TransPosdunhuang)
	
	end
end


function x300428_ProcMapTimerTick( varMap, monsterobjid )

end


function x300428_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300428_ProcEventEntry( varMap, monsterobjid )
