...
x490438_var_FileId = 490438
x490438_var_TransPosloulan  ="loulansichouzhilutoshabake"
x490438_var_TransPoslaiyin  ="loulansichouzhilutoshabake"
x490438_var_TransPoskunlun  ="loulansichouzhilutoshabake"
x490438_var_TransPosdunhuang="loulansichouzhilutoshabake"

function x490438_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490438_TransToPosition(varMap, varObj)
			end
	...
	x490438_TransToPosition(varMap, varPlayer)
end

function x490438_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490438_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490438_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490438_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490438_var_TransPosdunhuang)
	
	end
end


function x490438_ProcMapTimerTick( varMap, monsterobjid )

end


function x490438_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490438_ProcEventEntry( varMap, monsterobjid )
