...
x490467_var_FileId = 490467
x490467_var_TransPosloulan  ="minju9toshabake"
x490467_var_TransPoslaiyin  ="minju9toshabake"
x490467_var_TransPoskunlun  ="minju9toshabake"
x490467_var_TransPosdunhuang="minju9toshabake"


function x490467_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490467_TransToPosition(varMap, varObj)
			end
	...
	x490467_TransToPosition(varMap, varPlayer)
end

function x490467_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490467_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490467_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490467_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490467_var_TransPosdunhuang)
	
	end
end


function x490467_ProcMapTimerTick( varMap, monsterobjid )

end


function x490467_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490467_ProcEventEntry( varMap, monsterobjid )
