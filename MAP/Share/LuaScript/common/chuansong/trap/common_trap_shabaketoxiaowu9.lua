...
x490466_var_FileId = 490466
x490466_var_TransPosloulan  ="shabaketominju9"
x490466_var_TransPoslaiyin  ="shabaketominju9"
x490466_var_TransPoskunlun  ="shabaketominju9"
x490466_var_TransPosdunhuang="shabaketominju9"


function x490466_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490466_TransToPosition(varMap, varObj)
			end
	...
	x490466_TransToPosition(varMap, varPlayer)
end

function x490466_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490466_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490466_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490466_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490466_var_TransPosdunhuang)
	
	end
end


function x490466_ProcMapTimerTick( varMap, monsterobjid )

end


function x490466_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490466_ProcEventEntry( varMap, monsterobjid )
