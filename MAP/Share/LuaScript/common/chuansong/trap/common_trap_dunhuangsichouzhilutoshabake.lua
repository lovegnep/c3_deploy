...
x490444_var_FileId = 490444
x490444_var_TransPosloulan  ="dunhuangsichouzhilutoshabake"
x490444_var_TransPoslaiyin  ="dunhuangsichouzhilutoshabake"
x490444_var_TransPoskunlun  ="dunhuangsichouzhilutoshabake"
x490444_var_TransPosdunhuang="dunhuangsichouzhilutoshabake"

function x490444_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	elseif selfType == 1 then	
		...
		if busId ~= -1 then
			...
			ReadyBusChangeScene(varMap, 472, busId)
			for	varI = 0, memberCount - 1 do
	...
	x490444_TransToPosition(varMap, varPlayer)
end

function x490444_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490444_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490444_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490444_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490444_var_TransPosdunhuang)
	
	end
end


function x490444_ProcMapTimerTick( varMap, monsterobjid )

end


function x490444_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490444_ProcEventEntry( varMap, monsterobjid )
