...
x490476_var_FileId = 490476
x490476_var_TransPosloulan  ="shabaketominju14"
x490476_var_TransPoslaiyin  ="shabaketominju14"
x490476_var_TransPoskunlun  ="shabaketominju14"
x490476_var_TransPosdunhuang="shabaketominju14"


function x490476_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490476_TransToPosition(varMap, varObj)
			end
	...
	x490476_TransToPosition(varMap, varPlayer)
end

function x490476_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490476_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490476_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490476_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490476_var_TransPosdunhuang)
	
	end
end


function x490476_ProcMapTimerTick( varMap, monsterobjid )

end


function x490476_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490476_ProcEventEntry( varMap, monsterobjid )
