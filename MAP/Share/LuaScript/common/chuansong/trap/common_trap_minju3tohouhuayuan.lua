...
x490420_var_FileId = 490420
x490420_var_TransPosloulan  ="minju3tohouhuayuan"
x490420_var_TransPoslaiyin  ="minju3tohouhuayuan"
x490420_var_TransPoskunlun  ="minju3tohouhuayuan"
x490420_var_TransPosdunhuang="minju3tohouhuayuan"
x490420_var_Notice = "对不起，您的等级不足10级，无法前往皇宫秘道"


function x490420_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	local nLevel = GetLevel(varMap, varPlayer)
		if nLevel < 10 then 
			Msg2Player(varMap,varPlayer,"#Y"..x490420_var_Notice.."#W",4,3)
			return
		end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490420_TransToPosition(varMap, varObj)
			end
	...
	x490420_TransToPosition(varMap, varPlayer)
end

function x490420_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490420_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490420_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490420_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490420_var_TransPosdunhuang)
	
	end
end


function x490420_ProcMapTimerTick( varMap, monsterobjid )

end


function x490420_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490420_ProcEventEntry( varMap, monsterobjid )
