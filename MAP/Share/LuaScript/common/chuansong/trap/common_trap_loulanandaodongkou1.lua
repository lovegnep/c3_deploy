...
x490019_var_FileId = 490019
x490019_var_TransPosloulan  ="loulanandaodongkou1"
x490019_var_TransPoslaiyin  ="loulanandaodongkou1"
x490019_var_TransPoskunlun  ="loulanandaodongkou1"
x490019_var_TransPosdunhuang="loulanandaodongkou1"
x490019_var_Notice = "您的等级还不足#R125级#Y，无法前往葬金暗室！"


function x490019_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 125 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490019_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490019_TransToPosition(varMap, varObj)
			end
	...
	x490019_TransToPosition(varMap, varPlayer)
end

function x490019_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490019_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490019_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490019_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490019_var_TransPosdunhuang)
	
	end
end


function x490019_ProcMapTimerTick( varMap, monsterobjid )

end


function x490019_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490019_ProcEventEntry( varMap, monsterobjid )
