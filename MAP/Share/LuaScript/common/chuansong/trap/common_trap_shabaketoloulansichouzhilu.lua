...
x490439_var_FileId = 490439
x490439_var_TransPosloulan  ="shabaketoloulansichouzhilu"
x490439_var_TransPoslaiyin  ="shabaketoloulansichouzhilu"
x490439_var_TransPoskunlun  ="shabaketoloulansichouzhilu"
x490439_var_TransPosdunhuang="shabaketoloulansichouzhilu"
x490439_var_TuchengzhanQuestID = 7559


function x490439_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
local minute = GetMinOfDay()	
local weekIdx = GetWeek()
         
if minute >= 900 and minute < 1020 then
	if 	x490439_IsLouLanRuoGuo(varMap, varPlayer) > 0 then
		if x490439_IsLianMeng(varMap, varPlayer) ~= 1 then
			if x490439_IsBenGuo(varMap, varPlayer) == 0 then
				if weekIdx == 6 then
					if IsHaveQuestNM(varMap, varPlayer, x490439_var_TuchengzhanQuestID ) <= 0 then
	...
	elseif selfType == 1 then	
		...
		if busId ~= -1 then
			...
			ReadyBusChangeScene(varMap, 67, busId)
			for	varI = 0, memberCount - 1 do
	...
	x490439_TransToPosition(varMap, varPlayer)
end

function x490439_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490439_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490439_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490439_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490439_var_TransPosdunhuang)
	
	end
end


function x490439_ProcMapTimerTick( varMap, monsterobjid )

end


function x490439_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490439_ProcEventEntry( varMap, monsterobjid )

end

function x490439_IsLouLanRuoGuo(varMap, varPlayer)
  local maxlevel = GetTopListInfo_MaxLevel(GetWorldID(varMap, varPlayer))
	...
	if maxlevel >= 70 then
			if loulanscore <= allscore*0.2 then
				if x490439_IsMinGuo(varMap, varPlayer) == 1 then
	 	 			return 1
	 	 		end
			end
	end
...
function x490439_IsLianMeng(varMap, varPlayer)
...
function x490439_IsBenGuo(varMap, varPlayer)
...
function x490439_IsMinGuo(varMap, varPlayer)
	local loulanscore   = GetCountryParam(varMap, 0, CD_LASTWEEK_STRONG_WEAK_JIFEN)
	local tianshanscore = GetCountryParam(varMap, 1, CD_LASTWEEK_STRONG_WEAK_JIFEN)
	local kunlunscore   = GetCountryParam(varMap, 2, CD_LASTWEEK_STRONG_WEAK_JIFEN)
	local dunhuangscore = GetCountryParam(varMap, 3, CD_LASTWEEK_STRONG_WEAK_JIFEN)
