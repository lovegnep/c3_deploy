...
x300527_var_FileId = 300527
x300527_var_TransPosloulan  ="gubaoshree2gubaotwoloulan"
x300527_var_TransPoslaiyin  ="gubaoshree2gubaotwotianshan"
x300527_var_TransPoskunlun  ="gubaoshree2gubaotwokunlun"
x300527_var_TransPosdunhuang="gubaoshree2gubaotwodunhuang"



function x300527_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300527_TransToPosition(varMap, varObj)
			end
	...
	x300527_TransToPosition(varMap, varPlayer)
	
end

function x300527_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300527_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300527_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300527_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300527_var_TransPosdunhuang)
	
	end
end


function x300527_ProcMapTimerTick( varMap, monsterobjid )

end


function x300527_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300527_ProcEventEntry( varMap, monsterobjid )
