...
x270154_var_FileId = 270154
x270154_var_TransPosloulan  ="bianjing2mishitwo0"
x270154_var_TransPoslaiyin  ="bianjing2mishitwo0"
x270154_var_TransPoskunlun  ="bianjing2mishitwo0"
x270154_var_TransPosdunhuang="bianjing2mishitwo0"
x270154_var_Notice = "您的等级还不足#R20级#Y，无法前往后宫！"


function x270154_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x270154_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x270154_TransToPosition(varMap, varObj)
			end
	...
	x270154_TransToPosition(varMap, varPlayer)
end

function x270154_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x270154_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x270154_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x270154_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x270154_var_TransPosdunhuang)
	
	end
end


function x270154_ProcMapTimerTick( varMap, monsterobjid )

end


function x270154_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x270154_ProcEventEntry( varMap, monsterobjid )
