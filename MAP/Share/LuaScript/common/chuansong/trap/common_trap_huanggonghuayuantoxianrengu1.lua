...
x490405_var_FileId = 490405
x490405_var_TransPosloulan  ="huanggonghuayuantoxianrengu1loulan"
x490405_var_TransPoslaiyin  ="huanggonghuayuantoxianrengu1tianshan"
x490405_var_TransPoskunlun  ="huanggonghuayuantoxianrengu1kunlun"
x490405_var_TransPosdunhuang="huanggonghuayuantoxianrengu1dunhuang"
x490405_var_Notice = "对不起，您的等级不足90级，无法前往仙人谷"


function x490405_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	local minOfDay = GetMinOfDay()
	local weekIdx = GetWeek()
	
	if weekIdx == 6 and minOfDay >= 890 and minOfDay <= 900 then
			Msg2Player(varMap,varPlayer,"决战后花园期间，无法传送至仙人谷",8 ,2)
			return
	end		
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 90 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490405_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490405_TransToPosition(varMap, varObj)
			end
	...
	x490405_TransToPosition(varMap, varPlayer)
end

function x490405_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490405_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490405_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490405_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490405_var_TransPosdunhuang)
	
	end
end


function x490405_ProcMapTimerTick( varMap, monsterobjid )

end


function x490405_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490405_ProcEventEntry( varMap, monsterobjid )
