...
x490479_var_FileId = 490479
x490479_var_TransPosloulan  ="huanggongtoshabake"
x490479_var_TransPoslaiyin  ="huanggongtoshabake"
x490479_var_TransPoskunlun  ="huanggongtoshabake"
x490479_var_TransPosdunhuang="huanggongtoshabake"


function x490479_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490479_TransToPosition(varMap, varObj)
			end
	...
	x490479_TransToPosition(varMap, varPlayer)
end

function x490479_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490479_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490479_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490479_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490479_var_TransPosdunhuang)
	
	end
end


function x490479_ProcMapTimerTick( varMap, monsterobjid )

end


function x490479_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490479_ProcEventEntry( varMap, monsterobjid )
