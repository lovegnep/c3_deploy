...
x270151_var_FileId = 270151
x270151_var_TransPosloulan  ="bianjing2mishione1"
x270151_var_TransPoslaiyin  ="bianjing2mishione1"
x270151_var_TransPoskunlun  ="bianjing2mishione1"
x270151_var_TransPosdunhuang="bianjing2mishione1"
x270151_var_Notice = "您的等级还不足#R20级#Y，无法前往边塞密室！"


function x270151_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x270151_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x270151_TransToPosition(varMap, varObj)
			end
	...
	x270151_TransToPosition(varMap, varPlayer)
end

function x270151_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x270151_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x270151_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x270151_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x270151_var_TransPosdunhuang)
	
	end
end


function x270151_ProcMapTimerTick( varMap, monsterobjid )

end


function x270151_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x270151_ProcEventEntry( varMap, monsterobjid )
