...
x300233_var_FileId = 300233
x300233_var_TransPosloulan  ="bianjingshang2andao0"
x300233_var_TransPoslaiyin  ="bianjingshang2andao1"
x300233_var_TransPoskunlun  ="bianjingshang2andao2"
x300233_var_TransPosdunhuang="bianjingshang2andao3"
x300233_var_Notice = "您的等级还不足#R20级#Y，无法前往边塞暗道！"


function x300233_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x300233_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300233_TransToPosition(varMap, varObj)
			end
	...
	x300233_TransToPosition(varMap, varPlayer)
end

function x300233_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if (51 == varMap) then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300233_var_TransPosloulan)
	elseif (151 == varMap) then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300233_var_TransPoslaiyin)
	elseif (251 == varMap) then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300233_var_TransPoskunlun)
	elseif (351 == varMap) then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300233_var_TransPosdunhuang)
	end
	
end


function x300233_ProcMapTimerTick( varMap, monsterobjid )

end


function x300233_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300233_ProcEventEntry( varMap, monsterobjid )
