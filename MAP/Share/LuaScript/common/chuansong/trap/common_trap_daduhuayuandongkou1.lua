...
x490022_var_FileId = 490022
x490022_var_TransPosloulan  ="daduhuayuandongkou1"
x490022_var_TransPoslaiyin  ="daduhuayuandongkou1"
x490022_var_TransPoskunlun  ="daduhuayuandongkou1"
x490022_var_TransPosdunhuang="daduhuayuandongkou1"
x490022_var_Notice = "您的等级还不足#R130级#Y，无法前往后宫仙洞！"


function x490022_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 130 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490022_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490022_TransToPosition(varMap, varObj)
			end
	...
	x490022_TransToPosition(varMap, varPlayer)
end

function x490022_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490022_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490022_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490022_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490022_var_TransPosdunhuang)
	
	end
end


function x490022_ProcMapTimerTick( varMap, monsterobjid )

end


function x490022_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490022_ProcEventEntry( varMap, monsterobjid )
