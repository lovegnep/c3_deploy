...
x300285_var_FileId = 300285
x300285_var_TransPos0  ="wangcheng2xiaowuone1"
x300285_var_TransPos1  ="wangcheng2xiaowuone2"
x300285_var_TransPos2  ="wangcheng2xiaowuone3"
x300285_var_TransPos3  ="wangcheng2xiaowuone4"



function x300285_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300285_TransToPosition(varMap, varObj)
			end
	...
	x300285_TransToPosition(varMap, varPlayer)

end

function x300285_TransToPosition(varMap, varPlayer)

	local varCountry = GetCurCountry(varMap,varPlayer)

	if varMap == 50 and varCountry == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300285_var_TransPos0)
	elseif varMap == 150 and varCountry == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300285_var_TransPos1)
	elseif varMap == 250 and varCountry == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300285_var_TransPos2)
	elseif varMap == 350 and varCountry == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300285_var_TransPos3)
	else
		StartTalkTask(varMap)
...
function x300285_ProcMapTimerTick( varMap, monsterobjid )

end


function x300285_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300285_ProcEventEntry( varMap, monsterobjid )
