...
x490480_var_FileId = 490480
x490480_var_TransPosloulan  ="shabaketoximidao1"
x490480_var_TransPoslaiyin  ="shabaketoximidao1"
x490480_var_TransPoskunlun  ="shabaketoximidao1"
x490480_var_TransPosdunhuang="shabaketoximidao1"


function x490480_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490480_TransToPosition(varMap, varObj)
			end
	...
	x490480_TransToPosition(varMap, varPlayer)
end

function x490480_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490480_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490480_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490480_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490480_var_TransPosdunhuang)
	
	end
end


function x490480_ProcMapTimerTick( varMap, monsterobjid )

end


function x490480_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490480_ProcEventEntry( varMap, monsterobjid )
