...
x490462_var_FileId = 490462
x490462_var_TransPosloulan  ="shabaketominju7"
x490462_var_TransPoslaiyin  ="shabaketominju7"
x490462_var_TransPoskunlun  ="shabaketominju7"
x490462_var_TransPosdunhuang="shabaketominju7"


function x490462_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490462_TransToPosition(varMap, varObj)
			end
	...
	x490462_TransToPosition(varMap, varPlayer)
end

function x490462_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490462_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490462_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490462_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490462_var_TransPosdunhuang)
	
	end
end


function x490462_ProcMapTimerTick( varMap, monsterobjid )

end


function x490462_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490462_ProcEventEntry( varMap, monsterobjid )
