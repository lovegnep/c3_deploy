...
x300259_var_FileId = 300259
x300259_var_TransPosloulan  ="saimahouhuayuan2kadizigan"
x300259_var_TransPoslaiyin  ="saimahouhuayuan2kadizigan"
x300259_var_TransPoskunlun  ="saimahouhuayuan2kadizigan"
x300259_var_TransPosdunhuang="saimahouhuayuan2kadizigan"
x300259_var_Notice = "您的等级还不足#R40级！"


function x300259_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	
	if IsHaveSpecificImpact( varMap, varPlayer, 32584) < 1 then return end
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 40 then 
					Msg2Player(varMap,varPlayer,"#Y"..x300259_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300259_TransToPosition(varMap, varObj)
			end
	...
	x300259_TransToPosition(varMap, varPlayer)
end

function x300259_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300259_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300259_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300259_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300259_var_TransPosdunhuang)
	
	end
end


function x300259_ProcMapTimerTick( varMap, monsterobjid )

end


function x300259_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300259_ProcEventEntry( varMap, monsterobjid )
