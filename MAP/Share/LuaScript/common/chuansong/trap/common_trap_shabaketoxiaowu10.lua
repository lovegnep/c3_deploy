...
x490468_var_FileId = 490468
x490468_var_TransPosloulan  ="shabaketominju10"
x490468_var_TransPoslaiyin  ="shabaketominju10"
x490468_var_TransPoskunlun  ="shabaketominju10"
x490468_var_TransPosdunhuang="shabaketominju10"


function x490468_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490468_TransToPosition(varMap, varObj)
			end
	...
	x490468_TransToPosition(varMap, varPlayer)
end

function x490468_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490468_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490468_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490468_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490468_var_TransPosdunhuang)
	
	end
end


function x490468_ProcMapTimerTick( varMap, monsterobjid )

end


function x490468_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490468_ProcEventEntry( varMap, monsterobjid )
