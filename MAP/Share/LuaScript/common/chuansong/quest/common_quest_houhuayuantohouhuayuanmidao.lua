...
x490411_var_FileId = 490411
x490411_var_TransPosloulan  ="houhuayuantohouhuayuanmidao"
x490411_var_TransPoslaiyin  ="houhuayuantohouhuayuanmidao"
x490411_var_TransPoskunlun  ="houhuayuantohouhuayuanmidao"
x490411_var_TransPosdunhuang="houhuayuantohouhuayuanmidao"
x490411_var_Notice = "您的等级还不足#R70级#Y，无法前往后花园密道！"


function x490411_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if weekIdx == 6 and minOfDay >= 890 and minOfDay <= 900 then
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 70 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490411_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490411_TransToPosition(varMap, varObj)
			end
	...
	x490411_TransToPosition(varMap, varPlayer)
end

function x490411_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490411_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490411_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490411_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490411_var_TransPosdunhuang)
	
	end
end


function x490411_ProcMapTimerTick( varMap, monsterobjid )

end


function x490411_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490411_ProcEventEntry( varMap, monsterobjid )
