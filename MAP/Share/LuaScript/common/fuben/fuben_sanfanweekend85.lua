...
x701031_var_FileId 		= 701031
x701031_var_ScriptName	= "提交三番令"

x701031_var_SubmitItem	= 11990011

x701031_var_TitleInfo		= "\t父亲临走之前吩咐过我要收集#G三番令#W，只要你交给我一个#G三番令#W我就能给予你完成三番副本对应的经验和天赋。当然这么做会#Y占用一次三番副本次数#W。\n\t怎么样，你愿意交给我一个#G三番令#W，获得等同于完成三番副本的经验和天赋吗？\n \n#G小提示：\n\t一天只能提交一次三番令。\n\t有江湖请柬或朝廷请柬效果时提交，经验奖励会翻倍。"

						
function x701031_ProcEventEntry(varMap, varPlayer, varTalknpc, varQuest,idExt)	
	...
	StartTalkTask(varMap)
		...
		TalkAppendString(varMap,"#Y"..x701031_var_ScriptName.."#W")
		TalkAppendString(varMap,x701031_var_TitleInfo)
		
	StopTalkTask()
	DeliverTalkInfo(varMap, varPlayer, varTalknpc, x701031_var_FileId, x701031_var_FileId)
end


function x701031_ProcEnumEvent(varMap, varPlayer, varTalknpc, varQuest)
	local week = GetWeek()
	if week == 0 or week == 6 then
		TalkAppendButton(varMap,x701031_var_FileId,x701031_var_ScriptName,3,0)
	end
end

function x701031_ProcAcceptCheck( varMap, varPlayer, varTalknpc )
    return 1
end


function x701031_ProcAccept( varMap, varPlayer )
	local week = GetWeek()
	if week == 1 or week == 2 or week == 3 or week == 4 or week == 5 then
				Msg2Player(varMap, varPlayer,format("提交#{_ITEM%d}失败，请于周六或周日再来", x701031_var_SubmitItem), 0, 2)
				Msg2Player(varMap, varPlayer,format("提交#{_ITEM%d}失败，请于周六或周日再来", x701031_var_SubmitItem), 0, 3)
				return
	end			
	...
	if HaveItemInBag(varMap, varPlayer, x701031_var_SubmitItem) <= 0 then
		local varMsg = format("很抱歉，你没有#{_ITEM%d}，提交失败", x701031_var_SubmitItem)
	...
	if DelItem(varMap, varPlayer, x701031_var_SubmitItem, 1) ~= 1 then return 0 end
