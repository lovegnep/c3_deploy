...
x700009_var_RandomBossRate		=	0.1*6
...
function x700009_CreateRandomMonster(varMap)
	...
	local nStdRate = LuaCallNoclosure(930206,"Showzhanshen", varMap,nFubenType,nFubenMode )/100
	...
	if nMonsterCount1 > 0 then
		...
		for i = 1, nMonsterCount1 do
			...
			elseif (nMonsterId == varPlayer and nMonsterId > 0) then
				...
				WriteLog(1, format("x700009_ProcSubMonsterDie --- 1炳岿?F┣buff buffid %d", x700009_var_Boss_kill_monster_wudi_buff));
				return 1
			end
		end
		...
		if (nIsKillKeyBoss == 1 and nMainBossId > 0) then
			...
			WriteLog(1, format("x700009_ProcSubMonsterDie --- 1炳癸?F┣矗蔼端?`buff buffid %d", x700009_var_Boss_hurt_up_buffid));
			return 1
		end
	end
