...
function x302526_ProcEventEntry( varMap, varPlayer, varBagIdx )
...
function x302526_ProcIsSpellLikeScript( varMap, varPlayer)
	return 1; 
end
...
function x302526_ProcCancelImpacts( varMap, varPlayer )
	return 0; 
end
...
function x302526_ProcConditionCheck( varMap, varPlayer )
...
function x302526_ProcDeplete( varMap, varPlayer )
	return 1;
end
...
function x302526_ProcActivateOnce( varMap, varPlayer, varImpact )
	...
	if(DepletingUsedItem(varMap, varPlayer)) == 1 then
		...
		StopTalkTask();
		DeliverTalkTips(varMap, varPlayer);
		Msg2Player(varMap,varPlayer,"端午节快乐！",CHAT_TYPE_SELF,CHAT_RIGHTDOWN)
		
		local nMoney = 500
		AddMoney(varMap, varPlayer, 3, nMoney)
		
		GamePlayScriptLog( varMap, varPlayer, 3800)
		...
		if temp <= 3 then
			itemid = 12050314
			GamePlayScriptLog( varMap, varPlayer, 3801)
			temp2 = 1
		elseif temp <= 503 then
			itemid = 12030059
			GamePlayScriptLog( varMap, varPlayer, 3802)
		elseif temp <=570  then
			itemid = 12030775
			GamePlayScriptLog( varMap, varPlayer, 3803)
			temp2 = 1
		elseif temp <=577  then
			itemid = 10306087
			GamePlayScriptLog( varMap, varPlayer, 3804)
			temp2 = 1
		elseif temp <=585  then
			local itemidzuhe = {11011485,11011486,11011487}
			local tempitem = random(3)
			itemid = itemidzuhe[tempitem]
			GamePlayScriptLog( varMap, varPlayer, 3805)
			temp2 = 1
		elseif temp <=595  then
			itemid =11000906
			GamePlayScriptLog( varMap, varPlayer, 3806)
			temp2 = 1
		end
		if itemid ~=-1 then		
			...
			if varRet > 0 then
					StartTalkTask(varMap)
					TalkAppendString(varMap,"使用烟花（端午节版）意外获得了@item_"..itemid)
					StopTalkTask(varMap)
					DeliverTalkTips(varMap,varPlayer)
					...
					varMsg = format( "恭喜%s的%s使用烟花（端午节版）获得了%d个#{_ITEM%d}!", CountryName, Name, 1, itemid )
					if temp2 ==1 then
...
function x302526_ProcActivateEachTick( varMap, varPlayer)
	return 1; 
end
...
function x302526_ProcTopListYanghua(varMap,varPlayer)
