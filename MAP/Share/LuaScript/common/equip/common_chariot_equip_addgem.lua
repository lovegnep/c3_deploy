...
x800134_var_FileId 									= 800134

x800134_var_QuestName								="战车宝石镶嵌"


-- 返回值常量定义
x800134_var_Success = 0
x800134_var_Failed = 1
x800134_var_MaterialError = 2	-- 材料错误
x800134_var_MoneyNotEnough = 3		-- 金钱不足
x800134_var_HoleFull = 4			-- 满孔
x800134_var_HoleIsNotEnough = 5		-- 没有孔


x800134_var_InvalidPos = -1
x800134_var_InvalidItemID = 0


function x800134_ProcEnumEvent(varMap, varPlayer, varTalknpc, varQuest)
 	TalkAppendButton(varMap, x800134_var_FileId, x800134_var_QuestName, 3)
end


function x800134_ProcEventEntry(varMap, varPlayer, varTalknpc, idScript, idExt)	
	return 0
end

function x800134_ProcItemWanfa( varMap, varPlayer, varWanfaType )
	...
	if dataCount < 2+2*1 or dataCount > 2*MAX_ITEM_GEM+2 then
		WriteLog( 1, format("x800134_ProcItemWanfa param count invalid! varMap=%d,PlayerGUID=%X,paramCount=%d",varMap,GetGUID(varMap,varPlayer),dataCount))
		return
	end
	...
	local GemInfo = {}
	for i=1, gemCnt do
		--WriteLog( 1, format("x800134_ProcItemWanfa gemPos = %d, HolePos=%d", wanfaDataTable[2+2*(i-1)+1], wanfaDataTable[2+2*(i-1)+2]))
	...
	if IsItemValid( varMap, varPlayer, bagPos ) == 0 then
		WriteLog( 1, format("x800134_ProcItemWanfa item invalid! varMap=%d,PlayerGUID=%X, bagPos=%d",varMap,GetGUID(varMap,varPlayer),bagPos));			
		return
	end
	...
	if varHole < gemCnt then
		NotifyItemWanfa( varMap, varPlayer, varWanfaType, x800134_var_HoleIsNotEnough )
		return
	end
	...
	for i=1, gemCnt do
		...
		if varGem ~= x800134_var_InvalidItemID then
			return
		end
		
		-- 宝石是否合法
		if IsGemCanUseToEquip(varMap, varPlayer, bagPos, GemInfo[i].GemPos) == -1 then
			NotifyItemWanfa( varMap, varPlayer, varWanfaType, x800134_var_MaterialError )
			return
		end
	...
	if IsEnoughMoney( varMap, varPlayer, varNeedMoney ) == 0 then
		NotifyItemWanfa( varMap, varPlayer, varWanfaType, x800134_var_MoneyNotEnough )
		return
	end
	...
	for i=1, gemCnt do
		if ChariotEquipAddGem( varMap, varPlayer, bagPos, GemInfo[i].GemPos, GemInfo[i].HolePos ) == -1 then
			NotifyItemWanfa( varMap, varPlayer, varWanfaType, x800134_var_Failed )
			WriteLog(1, format("x800134_ProcItemWanfa ChariotEquipAddGem failed(1)! varMap=%d,PlayerGUID=%X",varMap,GetGUID(varMap,varPlayer) ))
			return
		end
	end
	...
	NotifyItemWanfa( varMap, varPlayer, varWanfaType, x800134_var_Success )
	--日志
	WriteEquipLog( varMap, varPlayer, bagPos, eEquipAct_ChariotEquipAddGem )
