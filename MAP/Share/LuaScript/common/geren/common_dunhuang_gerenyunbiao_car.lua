...
x300852_var_FileId = 300852
x300852_var_QuestName = "创建镖车"


x300852_var_GodIndex = 1210

x300852_var_Carinfo = {
						{ CarType = 7,BaseAI = 3,AIScript = 3,Script = 300852 },
						{ CarType = 8,BaseAI = 3,AIScript = 3,Script = 300852 },
						{ CarType = 9,BaseAI = 3,AIScript = 3,Script = 300852 },
						{ CarType = 38,BaseAI = 3,AIScript = 3,Script = 300852 },
						{ CarType = 39,BaseAI = 3,AIScript = 3,Script = 300852 },
						{ CarType = 40,BaseAI = 3,AIScript = 3,Script = 300852 },
...
x300852_var_QuestId = {
						{ varId = 7608,varCostMoney = 1000, GameId = 51 },
						{ varId = 7609,varCostMoney = 2000, GameId = 61 },
						{ varId = 7610,varCostMoney = 3000,GameId = 71 },
						{ varId = 7611,varCostMoney = 5000,GameId = 81 },
						{ varId = 7612,varCostMoney = 6000,GameId = 741 },
						{ varId = 7613,varCostMoney = 7500,GameId = 751 },
					  }
...
function x300852_ProcEventEntry(varMap, varPlayer, varTalknpc)	
...
function x300852_ProcEnumEvent(varMap, varPlayer, varTalknpc)
	
end


function x300852_ProcCreateBus( varMap, varPlayer,varQuest ,varBusID)

    local PosX = 180.6
    local PosZ = 81.4
	local varObj = CreateBus(varMap, x300852_var_Carinfo[varBusID].CarType, PosX, PosZ, x300852_var_Carinfo[varBusID].BaseAI, x300852_var_Carinfo[varBusID].AIScript, x300852_var_Carinfo[varBusID].Script, varPlayer, x300852_var_GodIndex,varQuest,-1)
end

function x300852_ProcCreateBusOK(varMap, varPlayer, busId,varTalknpc,varQuest)	
...
function x300852_DeleteBus(varMap, varObj)
...
function x300852_ProcIntoBusEvent(varMap, varPlayer,varTalknpc)
...
function x300852_ProcIntoBusAfter(varMap, varPlayer,varBusID)
	...
	for varI,itm in x300852_var_QuestId do
...
function x300852_ProcLeaveBusEvent(varMap, varPlayer,BusId)
		StartTalkTask(varMap)
...
function x300852_ProcDie(varMap,BusId,idKiller)
	...
	if idKiller == -1 then
		...
		if OwnerID ~= -1 then
			x300852_Abandon( varMap, OwnerID  )
...
function x300852_ProcTime(varMap,BusId)

end

function x300852_ProcLifeTimeOut(varMap,BusId)
    local varPlayer = GetBusMemberObjId(varMap,BusId,0)
    if(IsPlayerStateNormal(varMap, varPlayer) ~= 0) then
		x300852_Abandon( varMap, varPlayer  )
	end
	DeleteBus(varMap, BusId,1)
end

function x300852_ProcWaitTimeOut(varMap,BusId)
	local varPlayer = GetBusMemberObjId(varMap,BusId,0)
	if(IsPlayerStateNormal(varMap, varPlayer) ~= 0) then
		x300852_Abandon( varMap, varPlayer  )
...
function x300852_ProcBusStopWhenOwnerFarAway(varMap, ownerId)
end
...
function x300852_CleaupQuest( varMap, varPlayer )
...
function x300852_SetDayCount(varMap, varPlayer)
