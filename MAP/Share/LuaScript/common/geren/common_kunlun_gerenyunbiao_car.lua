...
x300853_var_FileId = 300853
x300853_var_QuestName = "创建镖车"


x300853_var_GodIndex = 1210
...
x300853_var_QuestId = {
						{ varId = 7614,varCostMoney = 1000, GameId = 51 },
						{ varId = 7615,varCostMoney = 2000, GameId = 61 },
						{ varId = 7616,varCostMoney = 3000,GameId = 71 },
						{ varId = 7617,varCostMoney = 5000,GameId = 81 },
						{ varId = 7618,varCostMoney = 6000,GameId = 741 },
						{ varId = 7619,varCostMoney = 7500,GameId = 751 },
					  }
...
function x300853_ProcEventEntry(varMap, varPlayer, varTalknpc)	
...
function x300853_ProcEnumEvent(varMap, varPlayer, varTalknpc)
	
end


function x300853_ProcCreateBus( varMap, varPlayer,varQuest ,varBusID)

    local PosX = 186.5
    local PosZ = 83.4
	local varObj = CreateBus(varMap, x300853_var_Carinfo[varBusID].CarType, PosX, PosZ, x300853_var_Carinfo[varBusID].BaseAI, x300853_var_Carinfo[varBusID].AIScript, x300853_var_Carinfo[varBusID].Script, varPlayer, x300853_var_GodIndex,varQuest,-1)
end


function x300853_ProcCreateBusOK(varMap, varPlayer, busId,varTalknpc,varQuest)		
	if busId ~= -1 then
		...
		TalkAppendString(varMap,"镖车创建失败，请稍后重试");
		StopTalkTask()
...
function x300853_DeleteBus(varMap, varObj)
...
function x300853_ProcIntoBusEvent(varMap, varPlayer,varTalknpc)
...
function x300853_ProcIntoBusAfter(varMap, varPlayer,varBusID)
	...
	for varI,itm in x300853_var_QuestId do
...
function x300853_ProcLeaveBusEvent(varMap, varPlayer,BusId)
		StartTalkTask(varMap)
...
function x300853_ProcDie(varMap,BusId,idKiller)
	...
	if idKiller == -1 then
		...
		if OwnerID ~= -1 then
			x300853_Abandon( varMap, OwnerID  )
...
function x300853_ProcTime(varMap,BusId)

end

function x300853_ProcLifeTimeOut(varMap,BusId)
    local varPlayer = GetBusMemberObjId(varMap,BusId,0)
    if(IsPlayerStateNormal(varMap, varPlayer) ~= 0) then
		x300853_Abandon( varMap, varPlayer  )
	end
	DeleteBus(varMap, BusId,1)
end

function x300853_ProcWaitTimeOut(varMap,BusId)
	local varPlayer = GetBusMemberObjId(varMap,BusId,0)
	if(IsPlayerStateNormal(varMap, varPlayer) ~= 0) then
		x300853_Abandon( varMap, varPlayer  )
...
function x300853_ProcBusStopWhenOwnerFarAway(varMap, ownerId)
end
...
function x300853_CleaupQuest( varMap, varPlayer )
...
function x300853_SetDayCount(varMap, varPlayer)
