...
x300763_var_FileId = 300763
x300763_var_QuestName = "创建镖车"
x300763_var_Mis_Count   = 3

x300763_var_GodIndex = 1210

x300763_var_Carinfo = {
						{ CarType = 42,BaseAI = 3,AIScript = 3,Script = 300763 },
						{ CarType = 43,BaseAI = 3,AIScript = 3,Script = 300763 },
						{ CarType = 44,BaseAI = 3,AIScript = 3,Script = 300763 }
					}

x300763_var_QuestId = 7031
x300763_var_GameId = 71
...
function x300763_ProcEventEntry(varMap, varPlayer, varTalknpc)	
...
function x300763_ProcEnumEvent(varMap, varPlayer, varTalknpc)
	
end


function x300763_ProcCreateBus( varMap, varPlayer,varQuest ,varBusID)
  
  local PosX = 207	
	local PosZ = 192
	local varObj = CreateBus(varMap, x300763_var_Carinfo[varBusID].CarType, PosX, PosZ, x300763_var_Carinfo[varBusID].BaseAI, x300763_var_Carinfo[varBusID].AIScript, x300763_var_Carinfo[varBusID].Script, varPlayer, x300763_var_GodIndex,varQuest,-1)
end

function x300763_ProcCreateBusOK(varMap,varPlayer,busId,varTalknpc,varQuest)
...
function x300763_DeleteBus(varMap, varObj)
...
function x300763_ProcIntoBusEvent(varMap, varPlayer,varTalknpc)
...
function x300763_ProcIntoBusAfter(varMap, varPlayer,varBusID)
	...
	local varCostMoney = x300763_CalcCostMoney(varMap, varPlayer)
...
function x300763_ProcLeaveBusEvent(varMap, varPlayer,BusId)
		StartTalkTask(varMap)
...
function x300763_ProcDie(varMap,BusId,idKiller)
	...
	if idKiller == -1 then
		...
		if OwnerID ~= -1 then
			x300763_Abandon( varMap, OwnerID  )
	...
	if ObjType == 1 then     
		...
		local varCostMoney = x300763_CalcAddMoney(varMap, BusId)
...
function x300763_ProcTime(varMap,BusId)

end

function x300763_ProcLifeTimeOut(varMap,BusId)
    local varPlayer = GetBusMemberObjId(varMap,BusId,0)
    if(IsPlayerStateNormal(varMap, varPlayer) ~= 0) then
		x300763_Abandon( varMap, varPlayer  )
	end
	DeleteBus(varMap, BusId,1)
end

function x300763_ProcWaitTimeOut(varMap,BusId)
	local varPlayer = GetBusMemberObjId(varMap,BusId,0)
	if(IsPlayerStateNormal(varMap, varPlayer) ~= 0) then
		x300763_Abandon( varMap, varPlayer  )
...
function x300763_ProcBusStopWhenOwnerFarAway(varMap, ownerId)
end
...
function x300763_Abandon( varMap, varPlayer )

	if IsHaveQuestNM( varMap, varPlayer, x300763_var_QuestId) ~= 0 then	
		DelQuestNM( varMap, varPlayer, x300763_var_QuestId)
		x300763_SetDayCount(varMap, varPlayer)
		return
	end

end

function x300763_CleaupQuest( varMap, varPlayer )
...
function x300763_GetDayCount(varMap, varPlayer)

	if x300763_var_Mis_Count > 0 then
...
function x300763_SetDayCount(varMap, varPlayer)
	local varToday = GetDayOfYear()
...
function x300763_CalcCostMoney(varMap, varPlayer)
	local varCount = x300763_GetDayCount(varMap, varPlayer)
	if varCount == 0 then
		return 75000
...
function x300763_CalcAddMoney(varMap, BusId)
	local nBusIndex = GetBusIndex(varMap,BusId)
	print ("x300763_CalcAddMoney,nBusIndex:",nBusIndex)
	if nBusIndex == x300763_var_Carinfo[1].CarType then
			return 52500
	elseif nBusIndex == x300763_var_Carinfo[2].CarType then
			return 105000
	elseif nBusIndex == x300763_var_Carinfo[3].CarType then
			return 157500
	end
	return 0
end
