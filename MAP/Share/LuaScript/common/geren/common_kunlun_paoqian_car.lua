...
x300865_var_FileId = 300865
x300865_var_QuestName = "创建飞行器"


x300865_var_GodIndex = 1210
x300865_var_Carinfo = {
						{ CarType = 3,BaseAI = 3,AIScript = 3,Script = 300865 },
						{ CarType = 5,BaseAI = 3,AIScript = 3,Script = 300865 },
						{ CarType = 6,BaseAI = 3,AIScript = 3,Script = 300865 },
						{ CarType = 35,BaseAI = 3,AIScript = 3,Script = 300865 },
						{ CarType = 36,BaseAI = 3,AIScript = 3,Script = 300865 },
						{ CarType = 37,BaseAI = 3,AIScript = 3,Script = 300865 },
					}

x300865_var_QuestId = {
						{ varId = 7632,varCostMoney =5000 ,AcceptId = 281 },
						{ varId = 7633,varCostMoney = 10000, AcceptId = 291 },
						{ varId = 7634, varCostMoney = 15000, AcceptId = 301 },
						{ varId = 7635, varCostMoney = 30000, AcceptId = 761 },
						{ varId = 7636, varCostMoney = 40000, AcceptId = 771 },
						{ varId = 7637, varCostMoney = 50000, AcceptId = 781 },
					  }

x300865_var_MainScriptId = 300862
...
function x300865_ProcEventEntry(varMap, varPlayer, varTalknpc)	
...
function x300865_ProcEnumEvent(varMap, varPlayer, varTalknpc, varQuest)
	
end


function x300865_ProcCreateBus( varMap, varPlayer,varQuest,varBusID )

  local PosX,PosZ = GetWorldPos(varMap, varPlayer)

	local varObj = CreateBus(varMap, x300865_var_Carinfo[varBusID].CarType, PosX, PosZ, x300865_var_Carinfo[varBusID].BaseAI, x300865_var_Carinfo[varBusID].AIScript, x300865_var_Carinfo[varBusID].Script, varPlayer, x300865_var_GodIndex,varQuest,-1)
end


function x300865_ProcCreateBusOK(varMap, varPlayer, busId,varTalknpc,varQuest)			
...
function x300865_DeleteBus(varMap, varObj)
...
function x300865_ProcIntoBusEvent(varMap, varPlayer,varTalknpc)
...
function x300865_ProcIntoBusAfter(varMap, varPlayer,varBusID)
	...
	for varI,itm in x300865_var_QuestId do

		if varQuest == itm.varId then
			local bRet = LuaCallNoclosure( x300865_var_MainScriptId, "CostMoney", varMap, varPlayer, varQuest)
			if bRet == 1 then

				if AddQuest(varMap, varPlayer, varQuest,x300865_var_MainScriptId,0,0,0,1) == 1 then
					...
					LuaCallNoclosure( x300865_var_MainScriptId, "QuestLogRefresh", varMap, varPlayer, varQuest)	
					local CarName = GetName( varMap, varPlayer ).."的飞行器"
...
function x300865_ProcLeaveBusEvent(varMap, varPlayer,BusId)
		StartTalkTask(varMap)
...
function x300865_ProcDie(varMap,BusId,idKiller)
	...
	if idKiller == -1 then
		...
		if OwnerID ~= -1 then
			x300865_Abandon( varMap, OwnerID  )
...
function x300865_ProcTime(varMap,BusId)

end

function x300865_ProcLifeTimeOut(varMap,BusId)
    local varPlayer = GetBusMemberObjId(varMap,BusId,0)
    if(IsPlayerStateNormal(varMap, varPlayer) ~= 0) then
		x300865_Abandon( varMap, varPlayer  )
	end
	DeleteBus(varMap, BusId,1)
end

function x300865_ProcWaitTimeOut(varMap,BusId)
...
function x300865_ProcBusStopWhenOwnerFarAway(varMap, ownerId)
...
function x300865_Abandon( varMap, varPlayer )
	...
	for varI,itm in x300865_var_QuestId do
		...
		if IsHaveQuestNM( varMap, varPlayer, itm.varId) ~= 0 then	
			...
			x300865_SetDayCount(varMap, varPlayer)
			return
		end
	end
...
function x300865_CleaupQuest( varMap, varPlayer )
...
function x300865_SetDayCount(varMap, varPlayer)
