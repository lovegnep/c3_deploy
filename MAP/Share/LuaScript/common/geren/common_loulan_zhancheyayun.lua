...
x300761_var_FileId 	  = 300761
x300761_var_QuestKind 				= 	1                       
x300761_var_Mis_Count   = 3
x300761_var_QuestId = 7031


x300761_var_ExtTarget					=	{ {type=20,n=1,target="战车"} }

x300761_var_ContinueInfo				= 	"\t如果不清除这些阿兰人，送往中国的贡品就不会按时完成。"
x300761_var_QuestCompleted			= 	"\t您已经将战车安全护送到此！请收下您应得的奖励！"

x300761_var_QuestName = "【个人】战车护送"
x300761_var_LevelLess = 70

x300761_var_BusScriptId = 300763

x300761_var_QuestTarget = "  将战车交给@npc_123829"
x300761_var_QuestInfo = "\t你知道#G屠城战#W吗？我们正为它的到来而#G备战#W，我急需一批勇士将战车护送到#G楼兰王城#W，交给#G阿鲁图#W，在你将战车顺利护送给他之后，会获得#G高额#W的#G经验#W奖励。\n\t每人每天只可护送#G三次#W战车，随着护送#G次数#W的#G增加#W，获得的#G经验#W也会越来越多，当然，#G花费#W的现银也会#G更多#W。本次护送，你需花费#R%d两#W现银。"
x300761_var_QuestHelp = "\t#G护送途中如果你离开战车过远，战车会因无人引导而停止前进！\n\t#G如果战车受到袭击被摧毁，任务失败！#G如果你离开战车超过5分钟，战车将丢失，任务失败！#G领取战车后，所花费的现银不会返还，#R国运期间#G会增加国家实力榜积分，#R同时获得#G双倍奖励！"
x300761_var_SubmitNPCGUID = 123829
x300761_var_NPCName = "@npc_123829"
x300761_var_Questruse="将战车送到楼兰王城的@npc_123829处！"

x300761_ChangleToggleWorld = {} --哪几个World开放经验奖励双倍

--15:28 2013/7/24 新加参数
x300761_var_nExpRate = 1			-- 经验比率
x300761_var_nMoneyRate = 0.048			-- 现银比率
x300761_var_nGoldsRate = 0.012			-- 金卡比率
--DECLARE_QUEST_INFO_STOP--
							

function x300761_ProcEnumEvent( varMap, varPlayer, varTalknpc, varQuest )

	local varLevel = GetLevel(varMap,varPlayer)
	
	if varLevel < x300761_var_LevelLess then
		return
	end

	local varHaveQuest = IsHaveQuestNM(varMap, varPlayer, x300761_var_QuestId )
	local varTalkNpcGUID = GetMonsterGUID(varMap,varTalknpc)
	if varHaveQuest <= 0 then
		if x300761_var_SubmitNPCGUID ~= varTalkNpcGUID then
			local varState = GetQuestStateNM(varMap,varPlayer,varTalknpc,x300761_var_QuestId)
			TalkAppendButton(varMap,x300761_var_QuestId,x300761_var_QuestName,varState,0);
		end
	end
...
function x300761_ProcEventEntry( varMap, varPlayer, varTalknpc, varQuest,varExtIdx )
	...
	if x300761_var_SubmitNPCGUID ~= varTalkNpcGUID then
		...
		if(varHaveQuest > 0) then
			local varDone = x300761_CheckSubmit(varMap, varPlayer, varTalknpc);
			if(varDone > 0) then
				if varExtIdx ~= -1 then
					x300761_DispatchCompletedInfo( varMap, varPlayer, varTalknpc,0) 
				end
			else
				if varExtIdx ~= -1 then
					x300761_DispatchContinueInfo( varMap, varPlayer, varTalknpc,0 ) 
				end
			end
		else
			if x300761_var_SubmitNPCGUID ~= varTalkNpcGUID then
				x300761_DispatchQuestInfo( varMap, varPlayer, varTalknpc,0 )	
			else
...
function x300761_DispatchQuestInfo( varMap, varPlayer, varTalknpc,varIndex )
	StartTalkTask( varMap);
		
		TalkAppendString(varMap, "#Y"..x300761_var_QuestName);
		TalkAppendString(varMap, format(x300761_var_QuestInfo,x300761_CalcCostMoney(varMap, varPlayer)/1000) )
		
		TalkAppendString( varMap, "#Y任务目标：")
		TalkAppendString( varMap, x300761_var_QuestTarget)
		TalkAppendString( varMap, " ")
		
		if x300761_var_QuestHelp ~= "" then
			TalkAppendString(varMap, "#Y任务提示：")
			TalkAppendString(varMap, x300761_var_QuestHelp )
		end
		
		
		
		local varExperience = x300761_CalcExperience(varMap, varPlayer)
		...
		if (IsHaveSpecificImpact( varMap, varPlayer, 9011) == 1 or IsHaveSpecificImpact( varMap, varPlayer, 9013) == 1) then
			local Money = x300761_CalcMoney(varMap, varPlayer)
			if Money > 0 then
				AddQuestMoneyBonus2( varMap, Money)
			end
		elseif (IsHaveSpecificImpact( varMap, varPlayer, 9012) == 1 ) then
			local Golsds = x300761_CalcGolds(varMap, varPlayer)
			if Golsds > 0 then
	...
	DeliverTalkInfoNM(varMap, varPlayer, varTalknpc, x300761_var_FileId,  x300761_var_QuestId);
end
...
function x300761_DispatchContinueInfo( varMap, varPlayer, varTalknpc,varIndex )
	
	StartTalkTask(varMap)

		
		TalkAppendString(varMap,"#Y"..x300761_var_QuestName)
		local info = format(x300761_var_QuestInfo,x300761_CalcCostMoney(varMap, varPlayer)/1000)
	...
	StopTalkTask()
	DeliverTalkRequire(varMap, varPlayer, varTalknpc, x300761_var_FileId, x300761_var_QuestId,0);
...
function x300761_DispatchCompletedInfo( varMap, varPlayer, varTalknpc,varIndex )
	
	StartTalkTask(varMap)

		
		TalkAppendString(varMap,"#Y"..x300761_var_QuestName)
		TalkAppendString(varMap,x300761_var_QuestCompleted)
		TalkAppendString(varMap," ")
	...
	StopTalkTask(varMap)
	DeliverTalkContinueNM(varMap, varPlayer, varTalknpc, x300761_var_FileId , x300761_var_QuestId);
...
function x300761_ProcQuestAccept( varMap, varPlayer, varTalknpc, varQuest )
	
	--2011-4-13 by rj 增加对押运交付任务人通过外挂进行的接受任务操作
	local varTalkNpcGUID = GetMonsterGUID(varMap, varTalknpc)
	if varTalkNpcGUID == x300761_var_SubmitNPCGUID then
		return
	end
	...
	if GetMoney(varMap, varPlayer,0) < x300761_CalcCostMoney(varMap, varPlayer) then
		StartTalkTask(varMap)
	...
	if x300761_GetDayCount(varMap, varPlayer) >= x300761_var_Mis_Count then
	...
	local varCount = x300761_GetDayCount(varMap, varPlayer)
	LuaCallNoclosure( x300761_var_BusScriptId, "ProcCreateBus", varMap, varPlayer ,x300761_var_QuestId,varCount+1)
...
function x300761_CheckSubmit(varMap, varPlayer, varTalknpc)
...
function x300761_ProcAccept( varMap, varPlayer )
	
end



function x300761_ProcQuestSubmit( varMap, varPlayer, varTalknpc, varButtonClick, varQuest )
	...
	if varRet == 1 then 
		...
		local varReadme = "您完成了任务："..x300761_var_QuestName
		
		StartTalkTask(varMap)
		TalkAppendString(varMap, "您完成了任务："..x300761_var_QuestName);
		StopTalkTask(varMap)
		DeliverTalkTips(varMap,varPlayer)
		...
		if ret == 1  then   -- 国运令期间期间			
			...
			if (IsHaveSpecificImpact( varMap, varPlayer, 9011) == 1 or IsHaveSpecificImpact( varMap, varPlayer, 9013) == 1) then
				local varMoney = x300761_CalcMoney(varMap, varPlayer)
				if varMoney > 0 then
			...
			elseif (IsHaveSpecificImpact( varMap, varPlayer, 9012) == 1 ) then
				local varGolds = x300761_CalcGolds(varMap, varPlayer)
				if varGolds > 0 then
		...
		x300761_SetDayCount(varMap, varPlayer)
		...
		x300761_GuoYun(varMap, varPlayer, varQuest)
	end
...
function x300761_ProcQuestAbandon( varMap, varPlayer,varQuest )
	...
	if varHaveQuest > 0 then
		...
		if IsBusMember(varMap, varPlayer) == 1 then
			...
			if varBusID ~= -1 and IsObjValid (varMap,varBusID) == 1 then
				...
				if GetBusQuestID(varMap, varBusID) == x300761_var_QuestId then
					DeleteBus(varMap, varBusID,1)
				end
			else
		...
		local varQuestIdx = GetQuestIndexByID(varMap, varPlayer, x300761_var_QuestId); 
		DelQuest(varMap, varPlayer, x300761_var_QuestId);
...
function x300761_ProcQuestAttach( varMap, varPlayer, varTalknpc, varTalkNpcGUID,varQuestIdx, varQuest )
	...
	if varHaveQuest > 0 then
		if varTalkNpcGUID == x300761_var_SubmitNPCGUID then
			local varState = GetQuestStateNM(varMap,varPlayer,varTalknpc,x300761_var_QuestId)
			TalkAppendButton(varMap, x300761_var_QuestId, x300761_var_QuestName,7,0);
		end
	end
...
function x300761_QuestLogRefresh( varMap, varPlayer, varQuest)
	...
	if varHaveQuest > 0 then
		...
		AddQuestLogCustomText( varMap,
								"",						
								x300761_var_QuestName,        
								x300761_var_QuestTarget,		
								x300761_var_NPCName,			
								x300761_var_Questruse,               
								format(x300761_var_QuestInfo,x300761_CalcCostMoney(varMap, varPlayer)/1000),	
								""					
								)
		StopTalkTask(varMap)
		DeliverTalkRefreshQuest(varMap, varPlayer, x300761_var_QuestId);
	end

end

function x300761_ProcQuestLogRefresh( varMap, varPlayer, varQuest)
	x300761_QuestLogRefresh( varMap, varPlayer, varQuest );
end




function x300761_GetDayCount(varMap, varPlayer)

	if x300761_var_Mis_Count > 0 then
...
function x300761_SetDayCount(varMap, varPlayer)
	local varToday = GetDayOfYear()
...
function x300761_CalcCostMoney(varMap, varPlayer)
	local varCount = x300761_GetDayCount(varMap, varPlayer)
	if varCount == 0 then
		return 75000
...
function x300761_CalcExperience(varMap, varPlayer)
	local varCount = x300761_GetDayCount(varMap, varPlayer)
	local varLevel = GetLevel(varMap,varPlayer)
	local Toggle = x300761_CheckMissonToggle( varMap, varPlayer )
	local zhancheexp = 0
	if varCount == 0 then
		if varLevel < 80 then
	...
	return zhancheexp * x300761_var_nExpRate
end
function x300761_CalcMoney(varMap, varPlayer)
	local varCount = x300761_GetDayCount(varMap, varPlayer)
	local varLevel = GetLevel(varMap,varPlayer)
	local Toggle = x300761_CheckMissonToggle( varMap, varPlayer )
	...
	return zhancheMoney * x300761_var_nMoneyRate
end
function x300761_CalcGolds(varMap, varPlayer)
	local varCount = x300761_GetDayCount(varMap, varPlayer)
	local varLevel = GetLevel(varMap,varPlayer)
	local Toggle = x300761_CheckMissonToggle( varMap, varPlayer )
	...
	return zhancheGolds * x300761_var_nGoldsRate
end

function x300761_GuoYun(varMap, varPlayer, varQuest)
...
-- 检测当前服务器是否开放
--------------------------------------------------------------
function x300761_CheckMissonToggle( varMap, varPlayer )
	local nToggle = 0		
		for i, item in x300761_ChangleToggleWorld do 
