...
x300917_var_FileId = 300917
x300917_var_QuestName = "创建镖车"
x300917_var_Mis_Count   = 3

x300917_var_GodIndex = 1210
...
x300917_var_QuestId = 7034
x300917_var_GameId = 71
...
function x300917_ProcEventEntry(varMap, varPlayer, varTalknpc)	
...
function x300917_ProcEnumEvent(varMap, varPlayer, varTalknpc)
	
end


function x300917_ProcCreateBus( varMap, varPlayer,varQuest ,varBusID)
  
    local PosX = 207
	local PosZ = 192
	local varObj = CreateBus(varMap, x300917_var_Carinfo[varBusID].CarType, PosX, PosZ, x300917_var_Carinfo[varBusID].BaseAI, x300917_var_Carinfo[varBusID].AIScript, x300917_var_Carinfo[varBusID].Script, varPlayer, x300917_var_GodIndex,varQuest,-1)
	
end

function x300917_ProcCreateBusOK(varMap, varPlayer, busId,varTalknpc,varQuest)
...
function x300917_DeleteBus(varMap, varObj)
...
function x300917_ProcIntoBusEvent(varMap, varPlayer,varTalknpc)
...
function x300917_ProcIntoBusAfter(varMap, varPlayer,varBusID)
	...
	local varCostMoney = x300917_CalcCostMoney(varMap, varPlayer)
...
function x300917_ProcLeaveBusEvent(varMap, varPlayer,BusId)
		StartTalkTask(varMap)
...
function x300917_ProcDie(varMap,BusId,idKiller)
	...
	if idKiller == -1 then
		...
		if OwnerID ~= -1 then
			x300917_Abandon( varMap, OwnerID  )
	...
	if ObjType == 1 then     
		...
		local varCostMoney = x300917_CalcAddMoney(varMap, BusId)
...
function x300917_ProcTime(varMap,BusId)

end

function x300917_ProcLifeTimeOut(varMap,BusId)
    local varPlayer = GetBusMemberObjId(varMap,BusId,0)
    if(IsPlayerStateNormal(varMap, varPlayer) ~= 0) then
		x300917_Abandon( varMap, varPlayer  )
	end
	DeleteBus(varMap, BusId,1)
end

function x300917_ProcWaitTimeOut(varMap,BusId)
	local varPlayer = GetBusMemberObjId(varMap,BusId,0)
	if(IsPlayerStateNormal(varMap, varPlayer) ~= 0) then
		x300917_Abandon( varMap, varPlayer  )
...
function x300917_ProcBusStopWhenOwnerFarAway(varMap, ownerId)
end
...
function x300917_Abandon( varMap, varPlayer )

	if IsHaveQuestNM( varMap, varPlayer, x300917_var_QuestId) ~= 0 then	
		DelQuestNM( varMap, varPlayer, x300917_var_QuestId)
		x300917_SetDayCount(varMap, varPlayer)
		return
	end

end

function x300917_CleaupQuest( varMap, varPlayer )
...
function x300917_GetDayCount(varMap, varPlayer)

	if x300917_var_Mis_Count > 0 then
...
function x300917_SetDayCount(varMap, varPlayer)
	local varToday = GetDayOfYear()
...
function x300917_CalcCostMoney(varMap, varPlayer)
	local varCount = x300917_GetDayCount(varMap, varPlayer)
	if varCount == 0 then
		return 75000
...
function x300917_CalcAddMoney(varMap, BusId)
	local nBusIndex = GetBusIndex(varMap,BusId)
	if nBusIndex == x300917_var_Carinfo[1].CarType then
			return 52500
	elseif nBusIndex == x300917_var_Carinfo[2].CarType then
			return 105000
	elseif nBusIndex == x300917_var_Carinfo[3].CarType then
			return 157500
	end
	return 0
end
