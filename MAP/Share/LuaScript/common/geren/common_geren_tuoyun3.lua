...
x302620_var_ScriptId 	  = 302620
x302620_var_MissionName = "托运官府马车"

--DECLARE_QUEST_INFO_STOP--
x302620_CostGold		= 3000


x302620_var_SubMissionID		=	{
...
function x302620_ProcEnumEvent( varMap, varPlayer, targetId, MissionId )
	
	local level = GetLevel( varMap, varPlayer )
	if ( level >= 30 ) then
		TalkAppendButton( varMap, x302620_var_ScriptId, x302620_var_MissionName, 3, -1 )
	end
...
--点击（按钮）点击该任务后执行此脚本
function x302620_ProcEventEntry( varMap, varPlayer, targetId, MissionId, nExtIdx )
		...
		StopTalkTask(varMap)
		DeliverTalkInfo(varMap, varPlayer, targetId, x302620_var_ScriptId, -1);

end

--检查是否能接受此任务
--返回值：-2--现金不够  -1--没有相关任务 0--镖车不在有效距离	1--可以接受任务
function x302620_IsCanAccept( varMap, varPlayer )
	
	local bHaveMission = 0
	for i,itm in x302620_var_SubMissionID do
		...
		if bHaveMission > 0 then
			...
			if GetGoldMode( varMap, varPlayer ) == 1 then
				-- 现金模式
				if GetMoney(varMap, varPlayer,2) < x302620_CostGold then
					StartTalkTask(varMap)
				...
				-- 金卡模式
				if GetMoney(varMap, varPlayer,3) < x302620_CostGold then
					StartTalkTask(varMap)
...
--接受
--**********************************
function x302620_ProcAcceptCheck( varMap, varPlayer, targetId, MissionId )
		
	local bRet = x302620_IsCanAccept( varMap, varPlayer )
	...
	if GetGoldMode( varMap, varPlayer ) == 1 then
		--现金模式
		if CostMoney( varMap, varPlayer, 2, x302620_CostGold, 301 ) == -1 then
			StartTalkTask(varMap)
			TalkAppendString(varMap, "很抱歉，您身上携带的现金不够支付本次托运费用。");
			StopTalkTask(varMap)
			DeliverTalkTips(varMap,varPlayer)
		...
		--金卡模式
		if CostMoney( varMap, varPlayer, 3, x302620_CostGold, 301 ) == -1 then
			StartTalkTask(varMap)
			TalkAppendString(varMap, "很抱歉，您身上携带的金卡不够支付本次托运费用。");
			StopTalkTask(varMap)
			DeliverTalkTips(varMap,varPlayer)
		...
		if bHaveMission > 0 then
				if BusId ~= -1 then
					if varMap == itm.mapID then
						...
						NewWorld(varMap, varPlayer, itm.mapID, itm.DestPosX, itm.DestPosZ, x302620_var_ScriptId)	
...
--接受
--**********************************
function x302620_ProcAccept( varMap, varPlayer )	
end

--完成任务
--返回1代表成功，0代表交任务失败
function x302620_ProcQuestSubmit( varMap, varPlayer, targetId, selectId, MissionId )
end
...
--放弃
--********************************************************************
function x302620_ProcQuestAbandon( varMap, varPlayer,MissionId )  	
end


function x302620_ProcQuestAttach( varMap, varPlayer, npcId, npcGuid,misIndex, MissionId )
end

---------------------------------------------------------------------------------------------------
--任务日志刷新
---------------------------------------------------------------------------------------------------
function x302620_QuestLogRefresh( varMap, varPlayer, MissionId)
end

function x302620_ProcQuestLogRefresh( varMap, varPlayer, MissionId)
