...
function x302612_GetQuestGolds(varMap, varPlayer, MissionId)
	...
	if ( missionIndex ~= 0 and missionNum ~= 0 ) then
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel>= 70  then
				nGolds = floor(x302612_var_QuestExpBase[missionIndex][missionNum] * ceil(nLevel*900))
			else 
				nGolds = floor(x302612_var_QuestExpBase[missionIndex][missionNum] * ceil(nLevel*100*2.5*2))
			end
	end
