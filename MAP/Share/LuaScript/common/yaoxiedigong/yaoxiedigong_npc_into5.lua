...
x310038_var_FileId = 310038


x310038_var_QuestName="圣山地宫传入1"



x310038_var_MaxHuman = 100
x310038_var_MaxHumanOther = 200 

x310038_var_ItemId = 12030027

function x310038_IsOpenedTime()
...
function x310038_HaveAccepted(varMap, varPlayer)
...
function x310038_ProcEnumEvent( varMap, varPlayer, varTalknpc, varQuest )
	if GetLevel(varMap, varPlayer) < 80 then
		return
	end
    TalkAppendButton(varMap,x310038_var_FileId,"圣山地宫(80级)",14,-1)
     
end


function x310038_ProcEventEntry( varMap, varPlayer, varTalknpc,idScript,idExt )
		...
		StartTalkTask(varMap)
		  ...
		  local varReadme = Readme_1..Readme_2..Readme_3
			...
			StopTalkTask(varMap)	
		DeliverTalkInfo(varMap,varPlayer,varTalknpc,x310038_var_FileId,-1)
	return 
...
function x310038_ProcAcceptCheck( varMap, varPlayer, varTalknpc )
		return 1
end


function x310038_IsGameTime()
	local minute = GetMinOfDay();
...
function x310038_ProcAccept( varMap, varPlayer )
	...
	--换天清零	
	if varLastday ~= GetDayOfYear() then
		x310038_SetMD( varMap, varPlayer, MD_YAOXIE_DIGONG_DAYCOUNT, 0)	
	end

	--换周清零
	if varLastweek ~= GetWeekIndex() then
		x310038_SetMD( varMap, varPlayer, MD_YAOXIE_DIGONG_WEEKCOUNT, 0)	
	end

	--获取每日和每周完成次数
	local daycount = x310038_GetMD( varMap, varPlayer, MD_YAOXIE_DIGONG_DAYCOUNT)
	local weekcount = x310038_GetMD( varMap, varPlayer, MD_YAOXIE_DIGONG_WEEKCOUNT)	
	...
	if bHave == 0 then
		if x310038_IsOpenedTime() < 0 then
...
--	if x310038_HaveAccepted(varMap, varPlayer) > 0 then
--		
...
--	end
	...
	if bHave == 0 then
		if GetItemCount(varMap, varPlayer, x310038_var_ItemId)  == 0 then
	...
	local curNum = 0
	if countryId == 0 then
		curNum = YX_LOULAN_HUMAN_NUM5 
	elseif countryId == 1 then
		curNum = YX_LAIYIN_HUMAN_NUM5 
	elseif countryId == 2 then
		curNum = YX_KUNLUN_HUMAN_NUM5 
	elseif countryId == 3 then
		curNum = YX_DUNHUANG_HUMAN_NUM5
	end
	local sign = 0;
	...
	if daycount < 3 and weekcount < 7 then
		x310038_SetMD( varMap, varPlayer, MD_YAOXIE_DIGONG_DAYCOUNT, daycount + 1)	
		x310038_SetMD( varMap, varPlayer, MD_YAOXIE_DIGONG_WEEKCOUNT, weekcount + 1)
	end
	
	if sign == 0 then
		return
	elseif sign == 1 then
		if DelItem(varMap, varPlayer, x310038_var_ItemId, 1) ~= 1 then return 0 end
	elseif sign == 2 then
	...
	if countryId == 0 then
		NewSceneId = YX_LOULAN_SCENE_ID5
		YX_LOULAN_HUMAN_NUM5 = YX_LOULAN_HUMAN_NUM5 + 1
	elseif countryId == 1 then
		NewSceneId = YX_LAIYIN_SCENE_ID5
		YX_LAIYIN_HUMAN_NUM5 = YX_LAIYIN_HUMAN_NUM5 + 1
	elseif countryId == 2 then
		NewSceneId = YX_KUNLUN_SCENE_ID5
		YX_KUNLUN_HUMAN_NUM5 = YX_KUNLUN_HUMAN_NUM5 + 1
	elseif countryId == 3 then
		NewSceneId = YX_DUNHUANG_SCENE_ID5
		YX_DUNHUANG_HUMAN_NUM5 = YX_DUNHUANG_HUMAN_NUM5 + 1
	end
	local PosX = 67
	local PosZ = 63
	
	NewWorld(varMap, varPlayer, NewSceneId,PosX ,PosZ, 310038 )
end
...
function x310038_GetMD( varMap, varPlayer, mdName)
    return GetPlayerGameData( varMap, varPlayer, mdName[ 1], mdName[ 2], mdName[ 3] )
end

function x310038_SetMD( varMap, varPlayer, mdName, value)
