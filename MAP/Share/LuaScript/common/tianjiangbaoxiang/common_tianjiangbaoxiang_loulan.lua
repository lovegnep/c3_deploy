...
x310010_var_FileId = 310010

x310010_var_QuestList = { 8022,8023,8024,8025,8026,8027 }
x310010_var_QuestItem = { {quest=8022,item1=12030051, item2=12030052},
						  {quest=8023,item1=12030051, item2=12030053},
						  {quest=8024,item1=12030052, item2=12030053},
						  {quest=8025,item1=12030051, item2=12030052},
						  {quest=8026,item1=12030051, item2=12030053},
						  {quest=8027,item1=12030052, item2=12030053}
						}

x310010_var_AcceptMessage		= "您接受了任务：%s"
x310010_var_AbandonMessage	= "您放弃了任务：%s"
x310010_var_CompleteMessage	= "您完成了任务：%s"

x310010_var_QuestCommentMessage	=	"不在接任务时间，请关注活动公告"
x310010_var_QuestAcceptFailedMessage_Accepted	=	"你在本日已经做过本任务了，请明日再来"
x310010_var_QuestSubmitFailedMessage_OverTime	=	"你已经错过了交任务的时间，请放弃任务"
x310010_var_QuestAcceptTime1 = { 450, 480 }	
x310010_var_QuestAcceptTime2 = { 750, 780 }	
x310010_var_QuestAcceptTime3 = { 1170, 1200 }	
x310010_var_AwardItem = 12030028
x310010_var_VirtualQuestId = 1362

x310010_var_RandItem = {11010100, 11010101, 11010102}

function x310010_GetQuest()
	local varIndex = random(1, 6);
	return x310010_var_QuestList[varIndex]
end

function x310010_GetMDAccepted(varMap, varPlayer, varIndex)
	if varIndex == 1 then
...
function x310010_SetMDAccepted(varMap, varPlayer, varIndex, value)
	if varIndex == 1 then
...
function x310010_GetMDDate(varMap, varPlayer)
...
function x310010_SetMDDate(varMap, varPlayer, day)
...
function x310010_NextDayReset(varMap, varPlayer)
	local varToday = GetDayOfYear()
	local varLastday = x310010_GetMDDate(varMap, varPlayer)

	if varToday ~= varLastday then
		x310010_SetMDDate(varMap, varPlayer, 0)
...
function x310010_IsNextDay(varMap, varPlayer)
...
function x310010_HaveAccepted( varMap, varPlayer )
	...
	if x310010_GetMDDate(varMap, varPlayer) > 0 then
		return 1
	end
...
function x310010_ProcAcceptCheckTime(  )
	...
	if  minspan >= x310010_var_QuestAcceptTime2[1] and minspan <= x310010_var_QuestAcceptTime2[2] then
		return 2
	elseif  minspan >= x310010_var_QuestAcceptTime3[1] and minspan <= x310010_var_QuestAcceptTime3[2] then
		return 3
	end
...
function x310010_ProcEnumEvent( varMap, varPlayer, varTalknpc, varQuest )
	varQuest = x310010_GetQuest();

	for varI, item in x310010_var_QuestList do
...
function x310010_ProcEventEntry( varMap, varPlayer, varTalknpc, varQuest )
	if IsHaveQuestNM( varMap, varPlayer, varQuest ) == 0 then	
		if x310010_IsNextDay(varMap, varPlayer) > 0 then
			x310010_NextDayReset(varMap, varPlayer) 
		end
		...
		local varRet = QuestCheckAcceptNM( varMap, varPlayer, varTalknpc, varQuest )
			...
			if varRet > 0 then	
				...
				AddQuestItemBonus(varMap, x310010_var_AwardItem, 1)
				
				StopTalkTask(varMap);
				DeliverTalkInfoNM(varMap, varPlayer, varTalknpc, x310010_var_FileId, varQuest);
			else
...
function x310010_ProcQuestAccept( varMap, varPlayer, varTalknpc, varQuest )
	if x310010_ProcAcceptCheckTime() < 0 then
		local varQuestName = GetQuestNameNM(varMap, varPlayer, varQuest);
			StartTalkTask(varMap);
			TalkAppendString(varMap, x310010_var_QuestCommentMessage);
			StopTalkTask(varMap);
			DeliverTalkTips(varMap,varPlayer);
			Msg2Player(varMap,varPlayer,x310010_var_QuestCommentMessage,8,2)
		return 
	end

	if x310010_HaveAccepted( varMap, varPlayer ) > 0 then
		StartTalkTask(varMap);
		TalkAppendString(varMap, x310010_var_QuestAcceptFailedMessage_Accepted);
		StopTalkTask(varMap);
		DeliverTalkTips(varMap,varPlayer);
		Msg2Player(varMap,varPlayer,x310010_var_QuestAcceptFailedMessage_Accepted,8,2)
		return
	end
	...
	if varRet > 0 then 
		...
		if varRet > 0 then
			QuestCom( varMap, varPlayer, x310010_var_VirtualQuestId)
			...
			x310010_SetMDDate(varMap, varPlayer, day)
			...
			local varQuestName = GetQuestNameNM( varMap, varPlayer, varQuest )
			local message = format(x310010_var_AcceptMessage, varQuestName);
			StartTalkTask(varMap)
			...
			for varI, item in x310010_var_QuestItem do
...
function x310010_ProcQuestAbandon( varMap, varPlayer, varQuest )
	local varRet = 0
	...
	if varRet == 1 then
		...
		local varQuestName = GetQuestNameNM( varMap, varPlayer, varQuest )
		local message = format(x310010_var_AbandonMessage, varQuestName);
		StartTalkTask(varMap);
		TalkAppendString(varMap, message);
		StopTalkTask();
		DeliverTalkTips(varMap, varPlayer);
		...
		QuestUnCom( varMap, varPlayer, x310010_var_VirtualQuestId)

	end
end


function x310010_ProcQuestAttach( varMap, varPlayer, varTalknpc, varTalkNpcGUID, varQuestIdx, varQuest )
...
function x310010_ProcQuestSubmit( varMap, varPlayer, varTalknpc, varButtonClick, varQuest )
	if IsHaveQuestNM( varMap, varPlayer, varQuest ) == 0 then	
		return
	end
	...
	LuaCallNoclosure( 203782, "Finishbaoxiang", varMap, varPlayer)
	QuestUnCom( varMap, varPlayer, x310010_var_VirtualQuestId)
	...
	x310010_SetMDDate(varMap, varPlayer, day)
		...
		local varQuestName = GetQuestNameNM( varMap, varPlayer, varQuest )
		local message = format(x310010_var_CompleteMessage, varQuestName);
		StartTalkTask(varMap);
			TalkAppendString(varMap, message);
		StopTalkTask();
		DeliverTalkTips(varMap, varPlayer);
		...
		ItemAppend( varMap, x310010_var_AwardItem, 1 )
		...
		if GetLevel(varMap, varPlayer) >= 80 and random(1, 100) <= 22 then
			...
			rndItemid = x310010_var_RandItem[varIndex]
		...
		if varRet > 0 then
			...
			message = format("#Y获得物品#R#{_ITEM%d}#Y的奖励", x310010_var_AwardItem )
			StartTalkTask(varMap);
			TalkAppendString(varMap, message);
			StopTalkTask();
			DeliverTalkTips(varMap, varPlayer);
...
function x310010_ProcQuestItemChanged( varMap, varPlayer, varItemData, varQuest )
...
function x310010_TianJiangBaoXiangDisconnect( varMap, varPlayer )

end

function x310010_ProcGpOpenItemBox( varMap, varPlayer, varTalknpc, varGpType, varNeedItem )
	return LuaCallNoclosure( QUEST_SCRIPT, "ProcGpOpenItemBox", varMap, varPlayer, varTalknpc, varGpType, varNeedItem )
end

function x310010_ProcGpRecycle( varMap, varPlayer, varTalknpc, varGpType, varNeedItem )
	return LuaCallNoclosure( QUEST_SCRIPT, "ProcGpRecycle", varMap, varPlayer, varTalknpc, varGpType, varNeedItem )
end

function x310010_ProcQuestLogRefresh( varMap, varPlayer, varQuest)
...
function x310010_ProcGpProcOver( varMap, varPlayer, varTalknpc )
end


function x310010_OpenCheck( varMap, varPlayer, varTalknpc )
end
