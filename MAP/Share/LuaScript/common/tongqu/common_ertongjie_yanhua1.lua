...
function x300327_ProcEventEntry( varMap, varPlayer, varBagIdx )
...
function x300327_ProcIsSpellLikeScript( varMap, varPlayer)
	return 1; 
end
...
function x300327_ProcCancelImpacts( varMap, varPlayer )
	return 0; 
end
...
function x300327_ProcConditionCheck( varMap, varPlayer )
...
function x300327_ProcDeplete( varMap, varPlayer )
	return 1;
end
...
function x300327_ProcActivateOnce( varMap, varPlayer, varImpact )
	...
	if(DepletingUsedItem(varMap, varPlayer)) == 1 then
		local yanhua = {8324,8325,8326}
		local temp = random(3)
		SendSpecificImpactToUnit(varMap, varPlayer, varPlayer, varPlayer,yanhua[temp], 0);

		
		StartTalkTask(varMap);
		TalkAppendString(varMap, "3周年快乐！");
		StopTalkTask();
		DeliverTalkTips(varMap, varPlayer);
		Msg2Player(varMap,varPlayer,"3周年快乐！",CHAT_TYPE_SELF,CHAT_RIGHTDOWN)
		
		local nMoney = 500
		AddMoney(varMap, varPlayer, 3, nMoney)
		
		GamePlayScriptLog( varMap, varPlayer, 3741)
		...
		if temp <= 3 then
			itemid = 12050314
			GamePlayScriptLog( varMap, varPlayer, 3742)
			temp2 = 1
		elseif temp <= 503 then
			itemid = 12030059
			GamePlayScriptLog( varMap, varPlayer, 3743)
		elseif temp <=570  then
			itemid = 12030775
			GamePlayScriptLog( varMap, varPlayer, 3744)
			temp2 = 1
		elseif temp <=577  then
			itemid = 10306087
			GamePlayScriptLog( varMap, varPlayer, 3745)
			temp2 = 1
		end
		if itemid ~=-1 then		
			...
			if varRet > 0 then
					StartTalkTask(varMap)
					TalkAppendString(varMap,"使用烟花（3周年纪念版）意外获得了@item_"..itemid)
					StopTalkTask(varMap)
					DeliverTalkTips(varMap,varPlayer)
					...
					varMsg = format( "恭喜%s的%s使用烟花（3周年纪念版）获得了%d个#{_ITEM%d}!", CountryName, Name, 1, itemid )
					if temp2 ==1 then
...
function x300327_ProcActivateEachTick( varMap, varPlayer)
	return 1; 
end
...
function x300327_ProcTopListYanghua(varMap,varPlayer)
