...
x300899_var_FileId = 300899
x300899_var_QuestName = "【国家】敌国刺杀"

x300899_var_DayCountLimited = 1


x300899_var_SceneIdList = {51,151,251,351}

function x300899_ProcEnumEvent( varMap, varPlayer, varTalknpc, varIndex )
	
	local varCountry = GetCurCountry( varMap, varPlayer )
	local Level = GetLevel(varMap, varPlayer)
	if Level >= 40 then
		if varCountry<=3 and varCountry>=0 then
			if varMap == x300899_var_SceneIdList[varCountry+1] then
		
				TalkAppendButton(varMap,x300899_var_FileId, x300899_var_QuestName, 8)
			end
		end
	end	
end

function x300899_ProcEventEntry( varMap , varPlayer , varTalknpc , varState , varIndex )
	local varCountry = GetCurCountry( varMap, varPlayer )
		...
		StartTalkTask(varMap)
			...
			local varScript = 300887

			local varQuestId1 = 7674
			local varQuestId2 = 7677
			local varQuestId3 = 7680
			local varQuestId4 = 7683
			...
			local questNewId1 	= 7854
			local questNewId2 	= 7855
			local questNewId3 	= 7856
			local questNewId4 	= 7857
			...
			local nDayCount1 = x300899_GetDayCount(varMap, varPlayer, DQ_SHAGUAI_LOULAN_DATE,DQ_SHAGUAI_LOULAN_DAYCOUNT)
			local nDayCount2 = x300899_GetDayCount(varMap, varPlayer, DQ_SHAGUAI_LAIYIN_DATE,DQ_SHAGUAI_LAIYIN_DAYCOUNT)
			local nDayCount3 = x300899_GetDayCount(varMap, varPlayer, DQ_SHAGUAI_KUNLUN_DATE,DQ_SHAGUAI_KUNLUN_DAYCOUNT)
			local nDayCount4 = x300899_GetDayCount(varMap, varPlayer, DQ_SHAGUAI_DUNHUANG_DATE,DQ_SHAGUAI_DUNHUANG_DAYCOUNT)
			
			if varCountry == 0 then
				if nDayCount2>=x300899_var_DayCountLimited and nDayCount3>=x300899_var_DayCountLimited and nDayCount4>=x300899_var_DayCountLimited then
					StartTalkTask(varMap);
			...
			elseif varCountry == 1 then
				if nDayCount1>=x300899_var_DayCountLimited and nDayCount3>=x300899_var_DayCountLimited and nDayCount4>=x300899_var_DayCountLimited then
					StartTalkTask(varMap);
						TalkAppendString(varMap, "很抱歉，您已经接取了三个国家的任务，请明日再来！");
					StopTalkTask();
					DeliverTalkTips(varMap, varPlayer);
			...
			elseif varCountry == 2 then
				if nDayCount1>=x300899_var_DayCountLimited and nDayCount2>=x300899_var_DayCountLimited and nDayCount4>=x300899_var_DayCountLimited then
					StartTalkTask(varMap);
						TalkAppendString(varMap, "很抱歉，您已经接取了三个国家的任务，请明日再来！");
					StopTalkTask();
					DeliverTalkTips(varMap, varPlayer);
			...
			elseif varCountry == 3 then
				if nDayCount1>=x300899_var_DayCountLimited and nDayCount2>=x300899_var_DayCountLimited and nDayCount3>=x300899_var_DayCountLimited then
					StartTalkTask(varMap);
						TalkAppendString(varMap, "很抱歉，您已经接取了三个国家的任务，请明日再来！");
					StopTalkTask();
					DeliverTalkTips(varMap, varPlayer);
			...
			if varCountry == 0 then
				if nDayCount2<x300899_var_DayCountLimited then
				...
				if nDayCount3<x300899_var_DayCountLimited then				
				...
				if nDayCount4<x300899_var_DayCountLimited then
			...
			elseif varCountry == 1 then
				if nDayCount1<x300899_var_DayCountLimited then				
...
function x300899_GetDayCount(varMap, varPlayer, nDayIndex, nDayCountIndex)
