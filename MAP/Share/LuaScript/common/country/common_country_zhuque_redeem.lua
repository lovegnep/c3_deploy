x300659_var_FileId = 300659
x300659_var_QuestName = "【国家】领取朱雀守护津贴"
x300659_var_GameId =1093
x300659_var_GongValud = 32
x300659_var_Multiple = 2*5
x300659_var_Empireextra = 10000*5

function x300659_ProcEnumEvent( varMap, varPlayer, varTalknpc, varIndex )
	if GetGameOpenById(x300659_var_GameId) > 0 then
		
		TalkAppendButton(varMap,x300659_var_FileId, x300659_var_QuestName, 3)
		
	end
	
end

function x300659_ProcEventEntry( varMap , varPlayer , varTalknpc , varState , varIndex )
	GetCountryQuestData(varMap,varPlayer,CD_INDEX_CHUGUO_COUNT, x300659_var_FileId,-1,"ProcReturn1")
	
end

function x300659_ProcReturn1(varMap, varPlayer,QuestData,varQuest)
	local varCount = QuestData + 1
	local maxcount = x300659_GetMaxChuGuo(varMap, varPlayer) + 1
	...
	local toggle = 0
		...
		if IsQinglongZhuque(varMap, varPlayer, 1) > 0 then
			awardJinKa =floor(100*varCount*0.5/10) * x300659_var_Multiple
				...
				if GetCurCountry(varMap, varPlayer) == GetEmpireCountry()  then
					awardJinKa = awardJinKa + x300659_var_Empireextra 
				end
		...
		elseif IsInCountrySpecialGuild(varMap, varPlayer,1) > 0 then
			awardJinKa1 =floor(100*varCount*0.5/10/4) * x300659_var_Multiple
				...
				if GetCurCountry(varMap, varPlayer) == GetEmpireCountry()  then
					awardJinKa1 = awardJinKa1 + x300659_var_Empireextra 
				end
	...
	if x300659_RuoGuoMulti(varMap, varPlayer) > 0 then
				if GetCurCountry(varMap, varPlayer) == GetEmpireCountry()  then
					awardJinKa = awardJinKa - x300659_var_Empireextra
					awardJinKa1 = awardJinKa1 - x300659_var_Empireextra
				end
		 RawardExp = floor(awardExp*x300659_RuoGuoMulti(varMap, varPlayer))
		 RawardJinKa   = floor(awardJinKa * x300659_RuoGuoMulti(varMap, varPlayer))
		 RawardShengWang = floor(awardShengWang * x300659_RuoGuoMulti(varMap, varPlayer))
		 RawardRongyu = floor(awardRongyu * x300659_RuoGuoMulti(varMap, varPlayer))
		 RawardExp1 = floor(awardExp1 * x300659_RuoGuoMulti(varMap, varPlayer))
		 RawardJinKa1 = floor(awardJinKa1 * x300659_RuoGuoMulti(varMap, varPlayer))
 		 RawardShengWang1 = floor(awardShengWang1 * x300659_RuoGuoMulti(varMap, varPlayer))
		 RawardRongyu1 = floor(awardRongyu1 * x300659_RuoGuoMulti(varMap, varPlayer))
		...
		local varReadme = "#Y【国家】领取朱雀守护津贴#r";
		local Readme2 = "\t#W朱雀守护津贴，是每一个为守护国家做出贡献的，达到#R60级#W的#G朱雀守护帮会成员#W及#G朱雀之主#W都可以领取的奖励。#r"
		...
		StopTalkTask(varMap)	
		DeliverTalkInfo(varMap,varPlayer,-1,x300659_var_FileId,-1)
		
		return 	
end


function x300659_ProcAcceptCheck( varMap, varPlayer, varTalknpc )
		return 1
end

function x300659_ProcAccept( varMap, varPlayer)
	local hour,minute,second = GetHourMinSec()
		...
		if	GetLevel(varMap, varPlayer) < 60 then
			StartTalkTask(varMap)
			TalkAppendString(varMap, "很抱歉，达到60级才可以领取朱雀守护津贴。")
			DeliverTalkTips(varMap, varPlayer)
			StopTalkTask(varMap)
			Msg2Player(varMap,varPlayer,"很抱歉，达到60级才可以领取朱雀守护津贴。",8,2)
			return
		end		
		if IsInCountrySpecialGuild(varMap, varPlayer,1)~=1 then
				StartTalkTask(varMap)
				TalkAppendString(varMap, "很抱歉，朱雀守护帮会成员才可以领取朱雀守护津贴。")
				DeliverTalkTips(varMap, varPlayer)
				StopTalkTask(varMap)
				Msg2Player(varMap,varPlayer,"很抱歉，朱雀守护帮会成员才可以领取朱雀守护津贴。",8,2)		
			return
		end
		
		if GetExploit(varMap, varPlayer) < x300659_var_GongValud then			
			local varMsg = format("很抱歉，您的功勋值不足%d点，不能领取。",x300659_var_GongValud)
			StartTalkTask(varMap)
			TalkAppendString(varMap, varMsg)
			DeliverTalkTips(varMap, varPlayer)
			StopTalkTask(varMap)
		...
		if varLastday ~= 0 and GetDayOfYear() == varLastday then
			StartTalkTask(varMap)
			TalkAppendString(varMap, "很抱歉，已经领过朱雀守护津贴。")
			DeliverTalkTips(varMap, varPlayer)
			StopTalkTask(varMap)
			Msg2Player(varMap,varPlayer,"很抱歉，已经领过朱雀守护津贴。",8,2)
			return
		end
		...
		GetJoinGuildTime(varMap,varPlayer,x300659_var_FileId)
end

function x300659_ProcMemberJoinTimeReturn( varMap, varPlayer, param )
	
	if param == -1 or GetCurrentTime()-param<24*3600 then
		StartTalkTask(varMap)
		TalkAppendString(varMap, "很抱歉，您加入朱雀守护帮的时间低于24小时，不能领取津贴。")
		DeliverTalkTips(varMap, varPlayer)
		StopTalkTask(varMap)
		Msg2Player(varMap,varPlayer,"很抱歉，您加入朱雀守护帮的时间低于24小时，不能领取津贴。",8,2)
		return		
	end
	
	GetCountryQuestData(varMap,varPlayer,CD_INDEX_CREATE_ZHUQUE_DATE, x300659_var_FileId,-1,"Newqinglong")
end

function x300659_Newqinglong(varMap, varPlayer,QuestData,varQuest)
	local curDay = GetDayTime()
		if curDay == QuestData then
			StartTalkTask(varMap);
			TalkAppendString(varMap, "很抱歉，当天上台的朱雀守护成员不能领取工资");
			StopTalkTask();
			DeliverTalkTips(varMap, varPlayer);
			Msg2Player(varMap,varPlayer,"很抱歉，当天上台的朱雀守护成员不能领取工资",4,2)
			return
		end
	GetCountryQuestData(varMap,varPlayer,CD_INDEX_CHUGUO_COUNT, x300659_var_FileId,-1,"ProcReturn2")
end


function x300659_ProcReturn2(varMap, varPlayer,QuestData,varQuest)
	local selfGuildId = GetGuildID(varMap, varPlayer)
	...
	if IsQinglongZhuque(varMap, varPlayer, 1) > 0 then
		...
		TalkAppendString(varMap, varStr);
			if  x300659_RuoGuoMulti(varMap, varPlayer) > 0 then
				local honor = floor(varCount*3/10*x300659_RuoGuoMulti(varMap, varPlayer))
		...
		TalkAppendString(varMap, varStr);
			if x300659_RuoGuoMulti(varMap, varPlayer) > 0 then
				local exp = floor(varLevel*70*varCount*x300659_RuoGuoMulti(varMap, varPlayer))
		...
		local awardMoney =  50*varCount * x300659_var_Multiple
				if GetCurCountry(varMap, varPlayer) == GetEmpireCountry()  then
					awardMoney = awardMoney + x300659_var_Empireextra*10 
				end			
		...
		TalkAppendString(varMap, varStr);
			if  x300659_RuoGuoMulti(varMap, varPlayer) > 0 then
				...
				local money = floor(awardMoney/10 * x300659_RuoGuoMulti(varMap, varPlayer))
					if money >= floor(50*maxcount * x300659_var_Multiple/10) - floor(awardMoney/10) then
					 money = floor(50*maxcount * x300659_var_Multiple/10) - floor(awardMoney/10)
					end
		...
		TalkAppendString(varMap, varStr);
				if  x300659_RuoGuoMulti(varMap, varPlayer) > 0 then
					local varShengWang = GetShengWang( varMap, varPlayer )
					local nShengWang2 = floor(3*varCount*x300659_RuoGuoMulti(varMap, varPlayer))
		...
		SetPlayerGameData(varMap, varPlayer, MD_SHOUHU_DATE[1], MD_SHOUHU_DATE[2], MD_SHOUHU_DATE[3], GetDayOfYear())
			...
			GamePlayScriptLog( varMap, varPlayer, 1442)
	...
	elseif IsInCountrySpecialGuild(varMap, varPlayer,1) > 0 then 
		...
		TalkAppendString(varMap, varStr);
			if  x300659_RuoGuoMulti(varMap, varPlayer) > 0 then
				local honor = floor(varCount*3/10/4*x300659_RuoGuoMulti(varMap, varPlayer))
		...
		TalkAppendString(varMap, varStr);
			if x300659_RuoGuoMulti(varMap, varPlayer) > 0 then
				local exp = floor(varLevel*70*varCount/4*x300659_RuoGuoMulti(varMap, varPlayer))
		...
		TalkAppendString(varMap, varStr);
				if  x300659_RuoGuoMulti(varMap, varPlayer) > 0 then
					local varShengWang = GetShengWang( varMap, varPlayer )
					local nShengWang2 = floor(varCount*3/4*x300659_RuoGuoMulti(varMap, varPlayer))
		...
		local awardMoney = floor(100*varCount*0.5/4) * x300659_var_Multiple
				if GetCurCountry(varMap, varPlayer) == GetEmpireCountry()  then
					awardMoney = awardMoney + x300659_var_Empireextra*10 
				end			
		--AddMoney( varMap, varPlayer,3, floor(awardMoney/10) )
		...
		TalkAppendString(varMap, varStr);
			if  x300659_RuoGuoMulti(varMap, varPlayer) > 0 then
				...
				local money = floor(awardMoney/10 * x300659_RuoGuoMulti(varMap, varPlayer))
					if money >= floor(100*maxcount*0.5/4 * x300659_var_Multiple/10) - floor(awardMoney/10) then
						money = floor(100*maxcount*0.5/4 * x300659_var_Multiple/10) - floor(awardMoney/10)
					end
				--AddMoney( varMap, varPlayer,3, money)
				--更现金奖励
				AddGold(varMap, varPlayer, money )	
...
function x300659_RuoGuoMulti(varMap, varPlayer)
	...
	local allscore		= loulanscore + tianshanscore + kunlunscore + dunhuangscore
		if maxlevel >= 70 then
			if selfscore <= allscore * 0.2 and selfscore > 0 and allscore > 0 then
				if x300659_IsMinGuo(varMap, varPlayer) == 1 then
...
function x300659_IsMinGuo(varMap, varPlayer)
	local loulanscore   = GetCountryParam(varMap, 0, CD_LASTWEEK_STRONG_WEAK_JIFEN)
	local tianshanscore = GetCountryParam(varMap, 1, CD_LASTWEEK_STRONG_WEAK_JIFEN)
	local kunlunscore   = GetCountryParam(varMap, 2, CD_LASTWEEK_STRONG_WEAK_JIFEN)
	local dunhuangscore = GetCountryParam(varMap, 3, CD_LASTWEEK_STRONG_WEAK_JIFEN)
...
function x300659_GetMaxChuGuo(varMap, varPlayer)
