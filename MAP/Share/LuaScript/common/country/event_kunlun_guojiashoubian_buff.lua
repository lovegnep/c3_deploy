...
x300856_var_TimerIndex				=	6
x300856_var_Shoubian_FileId			=	300859
x300856_var_Quest_Id				=	7575





function x300856_GetQuestId( varMap, varPlayer )
	
	return x300856_var_Quest_Id
end


function x300856_ProcIntervalOverEvent( varMap, varPlayer )
	local varCountry = GetCurCountry( varMap, varPlayer )

	local nMyQuestId = x300856_GetQuestId( varMap, varPlayer )
	...
	if varCountry==0 then
		if varMap==51 then
			if x300856_CheckPlayerInArea(varMap, varPlayer)>0 then
				x300856_AddShoubianBonus(varMap, varPlayer)
			end
		end
	end
...
function x300856_ProcActiveEvent( varMap, varPlayer )
	local nMyQuestId = x300856_GetQuestId( varMap, varPlayer )
...
function x300856_ProcFadeOutEvent( varMap, varPlayer )

	local varMyQuest = x300856_GetQuestId( varMap, varPlayer )
	...
	LuaCallNoclosure(x300856_var_Shoubian_FileId,"QuestLogRefresh",varMap, varPlayer, varMyQuest)
end
...
function x300856_CheckPlayerInArea(varMap, varPlayer)
	local varX,z =GetWorldPos(varMap,varPlayer)

	if varX>x300856_var_x1 and varX<x300856_var_x2 and z>x300856_var_z1 and z<x300856_var_z2 then
		return 1;
	end
...
function x300856_AddShoubianBonus(varMap, varPlayer)
	local varLevel = GetLevel(varMap, varPlayer)
	local varDayCount = LuaCallNoclosure(x300856_var_Shoubian_FileId,"GetDayCount",varMap, varPlayer)
...
function x300856_ProcessQuestFail(varMap, varPlayer, varQuest)
end




function x300856_GetDayCount(varMap, varPlayer)
