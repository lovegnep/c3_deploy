...
x310221_var_LevelMax					= 40

x310221_var_GameId					= 1023
x310221_var_NeedMoney					= 10000

--DECLARE_QUEST_INFO_START--

x310221_var_FileId 					= 	310221                  

x310221_var_LevelLess					= 	25                      


x310221_var_QuestName				= 	"新兵速成"

x310221_var_MaxKillNum				= 20

x310221_var_MaxCount					= 5


--DECLARE_QUEST_INFO_STOP--


function x310221_ProcEnumEvent( varMap, varPlayer, varTalknpc, varQuest )
...
function x310221_ProcEventEntry( varMap, varPlayer, varTalknpc, varQuest )

	if GetGameOpenById(x310221_var_GameId) ~= 1 then
	...
	if IsHaveQuestNM( varMap, varPlayer, varQuest ) == 0 then	
		...
		x310221_DispatchQuestInfo( varMap, varPlayer, varTalknpc, varQuest )
		
	
	else 
		local varQuestIdx = GetQuestIndexByID( varMap, varPlayer, varQuest )
		local killcount = GetQuestParam(varMap, varPlayer, varQuestIdx, 0)
		if killcount == x310221_var_MaxKillNum then
			x310221_DispatchCompletedInfo( varMap, varPlayer, varTalknpc,varQuest )
			
		else
			x310221_DispatchContinueInfo( varMap, varPlayer, varTalknpc,varQuest )
...
function x310221_ProcQuestAccept( varMap, varPlayer, varTalknpc, varQuest )
	...
	if GetLevel(varMap, varPlayer) < x310221_var_LevelLess then
		StartTalkTask(varMap);
	...
	local varDaycount = x310221_GetDayCount(varMap, varPlayer)
	if varDaycount >= x310221_var_MaxCount then
		StartTalkTask(varMap);
	...
	if varDaycount > 0 then
		if IsEnoughGold(varMap, varPlayer,x310221_var_NeedMoney) < 1 then
			StartTalkTask(varMap);
	...
	if varRet > 0 then 
		...
		if varRet > 0 then
			x310221_ProcQuestLogRefresh( varMap, varPlayer, varQuest)
			if varDaycount > 0 then
				...
				if goldMode == 1 then
					...
					if cash >= x310221_var_NeedMoney then
						CostMoney(varMap, varPlayer, 2, x310221_var_NeedMoney,320)
					end
				else
					local cash = GetMoney(varMap, varPlayer,2)
					local bind = GetMoney(varMap, varPlayer,3)
					if bind >= x310221_var_NeedMoney then
						CostMoney(varMap, varPlayer, 3, x310221_var_NeedMoney)
					elseif cash >= x310221_var_NeedMoney then
						CostMoney(varMap, varPlayer, 2, x310221_var_NeedMoney,320)
					end
				end
			end
...
function x310221_ProcQuestSubmit( varMap, varPlayer, varTalknpc, varButtonClick, varQuest )
	...
	if varRet == 1 then 
		x310221_SetDayCount(varMap, varPlayer)
		...
		x310221_GiveReward(varMap,varPlayer)
		
		x310221_SetFirst(varMap, varPlayer)
...
function x310221_GiveReward(varMap,varPlayer)

	local varExpBonus = x310221_GetExp( varMap, varPlayer )
...
function x310221_ProcQuestAttach( varMap, varPlayer, varTalknpc, varTalkNpcGUID,varQuestIdx, varQuest )
	if IsHaveQuestNM( varMap, varPlayer, varQuest) == 0 then	
		return
	end
...
function x310221_ProcQuestAbandon( varMap, varPlayer,varQuest )
	if IsHaveQuestNM( varMap, varPlayer, varQuest ) <= 0 then
		return -1;
	end
...
function x310221_ProcQuestObjectKilled( varMap, varPlayer, varObjData, varObj, varQuest )
		...
		for varI = 0, HumenNum-1 do
				...
				if humanObjId ~= -1 then
				    ...
				    if IsHaveQuestNM( varMap, humanObjId, varQuest ) > 0 then	
					    ...
					    if KilledNum == x310221_var_MaxKillNum-1 then
						    SetQuestByIndex( varMap, humanObjId, varQuestIdx, 0, KilledNum+1 )
											SetQuestByIndex( varMap, humanObjId, varQuestIdx, 7, 1 )
											StartTalkTask(varMap)
											if ObjName == "" then
												varText = format( "#cffcf00【个人】新兵速成：您击毙了怪物(%d/%d)", KilledNum+1, x310221_var_MaxKillNum )
											else
												varText = format( "#cffcf00【个人】新兵速成：您击毙了%s(%d/%d)", ObjName, KilledNum+1, x310221_var_MaxKillNum )
											end
											TalkAppendString( varMap, varText )
											StopTalkTask( varMap )
											DeliverTalkTips( varMap, humanObjId )
											x310221_ProcQuestLogRefresh( varMap, humanObjId, varQuest)
									elseif KilledNum < x310221_var_MaxKillNum-1 then
											SetQuestByIndex( varMap, humanObjId, varQuestIdx, 0, KilledNum+1 )
											StartTalkTask(varMap)
											if ObjName == "" then
												varText = format( "#cffcf00【个人】新兵速成：您击毙了怪物(%d/%d)", KilledNum+1, x310221_var_MaxKillNum )
...
function x310221_ProcQuestItemChanged( varMap, varPlayer, varItemData, varQuest )
	return 0
end



function x310221_ProcAcceptCheck(varMap, varPlayer, varTalknpc)
	return 1
end

function x310221_GetCost(varMap, varPlayer)
	local varDaycount =x310221_GetDayCount(varMap,varPlayer)
	if varDaycount > 0 then
		return 10000
	end
...
function x310221_DispatchQuestInfo( varMap, varPlayer, varTalknpc, varQuest)
	local awardexp = x310221_GetExp( varMap, varPlayer )
	...
	TalkAppendString( varMap,"   消灭等级不比你低5级以上的20只怪物，你就可以完成新兵的训练。")
	 TalkAppendString( varMap, "\n#Y任务花费：" )
        local cost = x310221_GetCost(varMap, varPlayer)
	...
	StopTalkTask()
	DeliverTalkInfoNM(varMap, varPlayer, varTalknpc, x310221_var_FileId, varQuest,0);
end




function x310221_DispatchContinueInfo( varMap, varPlayer, varTalknpc,varQuest )
	...
	StopTalkTask()
	DeliverTalkRequire(varMap, varPlayer, varTalknpc, x310221_var_FileId, varQuest,0);
end




function x310221_DispatchCompletedInfo( varMap, varPlayer, varTalknpc,varQuest )
	local awardexp = x310221_GetExp( varMap, varPlayer )
	...
	StopTalkTask()
	DeliverTalkContinueNM(varMap, varPlayer, varTalknpc, x310221_var_FileId, varQuest);

end

function x310221_ProcQuestLogRefresh( varMap, varPlayer, varQuest)
	...
	local varStr = "  消灭20只等级适当的怪物("..killcount.."/"..tostring(x310221_var_MaxKillNum)..")";	
	StartTalkTask(varMap)	
		local awardexp = x310221_GetExp( varMap, varPlayer )

		AddQuestExpBonus(varMap,  awardexp);
			
	local count =	x310221_GetDayCount(varMap, varPlayer)
...
function x310221_GetDayCount(varMap, varPlayer)
	if x310221_var_MaxCount > 0 then
...
function x310221_SetDayCount(varMap, varPlayer)
	local varToday = GetDayOfYear()
	local varLastday = GetPlayerGameData(varMap, varPlayer, MD_XINBINGSUCHENG_DATE[1], MD_XINBINGSUCHENG_DATE[2], MD_XINBINGSUCHENG_DATE[3])
	if varLastday ~= varToday then
