...
x300609_var_FileId = 300609
x300609_var_QuestName = "创建镖车"


x300609_var_GodIndex = 1210
x300609_var_Carinfo = {CarType = 4,BaseAI = 3,AIScript = 3,Script = 300609 }

x300609_var_CostMoney = 10000
...
function x300609_ProcEventEntry(varMap, varPlayer, varTalknpc)	
...
function x300609_ProcEnumEvent(varMap, varPlayer, varTalknpc, varQuest)
	
end


function x300609_ProcCreateBus( varMap, varPlayer,varQuest )

  local PosX,PosZ = GetWorldPos(varMap, varPlayer)
	local varObj = CreateBus(varMap, x300609_var_Carinfo.CarType, PosX, PosZ, x300609_var_Carinfo.BaseAI, x300609_var_Carinfo.AIScript, x300609_var_Carinfo.Script, varPlayer, x300609_var_GodIndex,varQuest,-1)
end

function x300609_ProcCreateBusOK(varMap,varPlayer,busId,varTalknpc,varQuest)	
...
function x300609_DeleteBus(varMap, varObj)
...
function x300609_ProcIntoBusEvent(varMap, varPlayer,varTalknpc)
...
function x300609_ProcIntoBusAfter(varMap, varPlayer,varBusID)
...
function x300609_ProcLeaveBusEvent(varMap, varPlayer,BusId)
		StartTalkTask(varMap)
...
function x300609_ProcTime(varMap,BusId)

end

function x300609_ProcLifeTimeOut(varMap,BusId)
...
function x300609_ProcBusStopWhenOwnerFarAway(varMap, ownerId)
end

function x300609_ProcWaitTimeOut(varMap,BusId)
...
function x300609_ProcDie(varMap,BusId,idKiller)
	...
	if ObjType == 1 then     
		...
		if OwnerCountry == KillerCountry then
	        ...
	        AddMoney(varMap,idKiller,0,x300609_var_CostMoney)
		  	StartTalkTask(varMap);TalkAppendString(varMap, "您击杀了敌国的神兽，并获得了奖励！");StopTalkTask(varMap);DeliverTalkTips(varMap,idKiller)
			Msg2Player(varMap,idKiller,format("您击杀了敌国的神兽，并获得了现银#{_MONEY%d}。", x300609_var_CostMoney),8,2)	
		end 
		...
		if OwnerID ~= -1 then		
			...
			varMsg1 = "楼兰王国"..GuildTitle1.."的神兽被"..varCountry..GuildTitle.."的"..killerName.."击杀"
			if IsHideName(varMap,idKiller) > 0 then
				varMsg1 = "楼兰王国"..GuildTitle1.."的神兽被"..HIDE_NAME.."击杀"
			end
