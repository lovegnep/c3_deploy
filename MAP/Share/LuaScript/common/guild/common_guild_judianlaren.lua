x300920_var_FileId = 300920
x300920_var_QuestName="【国家】进入领主战场"

x300920_var_Laird_Signup                  	= 300     
x300920_var_Laird_WaitBattle              	= 301     
x300920_var_Laird_BeginBattle             	= 302     
x300920_var_Laird_EndBattle               	= 303     

function x300920_ProcEnumEvent( varMap, varPlayer, varTalknpc, varQuest )
    ...
    if nBattleStatus ~= x300920_var_Laird_BeginBattle then
    	return 1
    end

    if LuaCallNoclosure(300918,"GetBattleEnterSceneFlag",varMap) ~= 1 then
       return 1
    end

	TalkAppendButton(varMap,x300920_var_FileId,x300920_var_QuestName,3);
end


function x300920_ProcEventEntry( varMap, varPlayer, varTalknpc )
    ...
    StartTalkTask( varMap )
    TalkAppendString( varMap, "确认要进入领主战场吗？" )
    StopTalkTask( varMap )
    DeliverTalkInfo( varMap, varPlayer, varTalknpc, x300920_var_FileId,-1 )
...
function x300920_ProcAcceptCheck( varMap, varPlayer, varTalknpc )
    return 1
end
...
function x300920_ProcAccept( varMap, varPlayer )
    ...
    GameBattleRequestEnterScene(varMap, varPlayer,2) 
end

function x300920_ProcLairdBattleChangeScene(varMap,varPlayer,nRetCode,newSceneID,nBattleCamp)
	...
	if nRetCode == 0 then
	    ...
	    if varMap == newSceneID then
	        ...
	        if Flag == 0 then
			    SetPos( varMap , varPlayer , 43 ,90 )
	        else
	            SetPos( varMap , varPlayer , 205,147 )
	        end
		else
			...
			NewWorld( varMap , varPlayer , newSceneID , PosX , PosZ,300920 )
		end
		
	else
		x300920_ProcLairdBattleTransError(varMap,varPlayer,nRetCode )
	end
end

function x300920_ProcLairdBattleTransError(varMap,varPlayer,nRetCode )
...
function x300920_ProcQuestAbandon( varMap, varPlayer, varQuest )
end




function x300920_ProcContinue( varMap, varPlayer, varTalknpc )
end




function x300920_CheckSubmit( varMap, varPlayer )
end




function x300920_ProcQuestSubmit( varMap, varPlayer, varTalknpc,varRadioSelected, varQuest )
end
