...
x300867_var_FileId 					= 	300867                  

x300867_var_QuestKind 				= 	1                       
x300867_var_LevelLess					= 	20                      
x300867_var_QuestName				= 	"【帮会】帮会神兽"           

x300867_var_QuestTarget				= 	"  护送神兽经过@npc_132154中转后到达@npc_134516处"          
x300867_var_QuestInfo				= 	"\t帮主带领帮会成员，每天可在王城参加一次【帮会】帮会神兽任务。\n\t帮主需要在帮会任务发布人处缴纳#G10两现银#W才能接受此任务。帮会成员只需此时在帮主周围，便可自动接到任务，进入护送神兽状态。\n\t在护送神兽过程中，神兽只跟随帮主移动，如帮主离神兽过远，神兽会#G停止前进#W。如果神兽受到袭击并死亡，帮主的#G押金#W将被对方抢走，全体成员#G任务失败#W。\n\t帮主离开神兽一段时间或掉线，神兽消失，所有人#G任务失败#W。\n\t由于路途遥远，神兽中途需要休息，帮主在@npc_132154处点击#R神兽驿站#W按扭，当前在驿站周围的帮众都会获得经验奖励。并且在最后完成护兽任务时都会获得经验加成的奖励。\n\t顺利护送神兽到达指定地点，帮主可得到返还的押金和#G巨额经验#W，参与护送的帮会成员得到#G巨额经验#W。\n\t帮主到达指定地点提交任务，此时只有帮主周围并且已经参与护送的帮会成员才能完成任务并获得#G巨额奖励#W，同时帮会增加100点帮会经验#W。当帮主发布#G护兽令#W时2小时内回复任务可获得翻倍奖励（经验，帮贡）和2点使命点！"            
x300867_var_QuestCompleted			= 	"恭喜您顺利完成任务！这是您应得的奖励，请收下！"  
x300867_var_QuestHelp				=	"\t接受任务后，会在帮主萤幕正下方出现辅助图示：#R上\下车#W和护送#G成员列表。帮主通过列表踢除表内成员，解除其护送神兽状态。当帮主提交任务时，周围参与护送的帮众越多，您所获得的奖励也会更多。随着您所在帮会等级的提高，您获得的奖励也会相应增加。"          
x300867_var_ExtTarget		=	{ {type=20,n=1,target="护送神兽"}} 
 
--DECLARE_QUEST_INFO_STOP--



x300867_var_BonusMoney8               =   150  
x300867_var_GuildExpBonus				=	1

x300867_SubmitNPCGUID				= 134516 
x300867_var_BusScriptId = 300868
x300867_var_Leader_Index = 5		
x300867_var_QuestId  = 7021
x300867_var_CostMoney = 10000
x300867_var_Exp = 100*3*25*1*1*0.4            
x300867_var_RingQuestIndex = 1	

function x300867_ProcEnumEvent( varMap, varPlayer, varTalknpc, varQuest )
	
	
	SetPlayerRuntimeData(varMap,varPlayer,RD_GUILDCONTEND_KILL_GOLD,1)	
	...
	if IsHaveQuestNM( varMap, varPlayer, x300867_var_QuestId ) == 0 then	
		local varState = GetQuestStateNM(varMap,varPlayer,varTalknpc,x300867_var_QuestId)
		TalkAppendButton(varMap,x300867_var_QuestId,x300867_var_QuestName,8);
		return 1
	end
	return 0
end
...
function x300867_ProcEventEntry( varMap, varPlayer, varTalknpc, varQuest,varExtIdx )
	...
	if(varHaveQuest > 0) then
		...
		if x300867_SubmitNPCGUID == varTalkNpcGUID then
			x300867_DispatchCompletedInfo( varMap, varPlayer, varTalknpc) 
		end

	else
		if varExtIdx ~= x300867_var_FileId then
			x300867_DispatchQuestInfo( varMap, varPlayer, varTalknpc )	
			
			SetPlayerRuntimeData(varMap,varPlayer,RD_GUILDCONTEND_KILL_GOLD,2)
...
function x300867_ProcQuestAccept( varMap, varPlayer, varTalknpc, varQuest )
	...
	local pos = GetGuildOfficial(varMap, varPlayer)
  	if pos ~= x300867_var_Leader_Index then			
  		StartTalkTask(varMap)
	...
	if varLevel < x300867_var_LevelLess then
		StartTalkTask(varMap)
		...
		if GetPlayerRuntimeData(varMap,varPlayer,RD_GUILDCONTEND_KILL_GOLD) == 3 then
	 		GetGuildQuestData(varMap, varPlayer, GD_GUILD_INDEX_YUNBIAO_ACCEPT_TIME, x300867_var_FileId,varQuest,"ProcReturn")
	 	end

	
		if GetPlayerRuntimeData(varMap,varPlayer,RD_GUILDCONTEND_KILL_GOLD) == 2 then
			StartTalkTask(varMap)
					
					SetPlayerRuntimeData(varMap,varPlayer,RD_GUILDCONTEND_KILL_GOLD,3)
					TalkAppendString(varMap,"#Y【帮会】帮会神兽#W\n  \n\t当您领取神兽时，帮会成员需在您的身边才可接收到接受任务的提示；在您完成任务时，只有在您身边的帮会成员才能完成任务获得奖励。\n \n#G您确定要领取帮会神兽吗?")
			StopTalkTask()
			DeliverTalkInfoNM(varMap, varPlayer, varTalknpc, x300867_var_FileId, x300867_var_QuestId)
		end
...
function x300867_ProcReturn1(varMap, varPlayer,QuestData,varQuest,varTalknpc)
 ...
 local curTime = GetCurrentTime()
	...
	x300867_GetBonus( varMap,varPlayer, isMultiple, varMemberCount,raingMisFinishing )
end


function x300867_GetBonus( varMap, varPlayer, isMultiple, varMemberCount, raingMisFinishing)	
	...
	local varLevel = GetLevel(varMap, varPlayer)
	local varExpBonus = x300867_var_Exp * varLevel* exExp
    ...
    if varExpBonus > 0 then
		...
		if isMultiple == 1 then
			local ExpBonus1 = ceil (x300867_var_Exp * varLevel*3)* exExp
			local Exp3 = ceil (x300867_var_Exp * varLevel*3 )* exExp
			local ExpBonus2 = ceil (x300867_var_Exp * varLevel*2)* exExp
			local pos = GetGuildOfficial(varMap, varPlayer)
		...
		if 	GuildLevel == 2 then
			local varLevel = GetLevel(varMap, varPlayer)
			local varExp = ceil (x300867_var_Exp* varLevel *0.25 )
			if varMemberCount >= 1  then
		...
		if 	GuildLevel == 3 then
			local varLevel = GetLevel(varMap, varPlayer)
			local varExp = ceil (x300867_var_Exp* varLevel *0.5 )
			if varMemberCount >= 1  then
		...
		if 	GuildLevel == 4 then
			local varLevel = GetLevel(varMap, varPlayer)
			local varExp = ceil (x300867_var_Exp* varLevel *0.75 )
			if varMemberCount >= 1  then
		...
		if 	GuildLevel == 5 then
			local varLevel = GetLevel(varMap, varPlayer)
			local varExp = ceil (x300867_var_Exp * varLevel*1 )
			if varMemberCount >= 1  then
		...
		if 	GuildLevel == 6 then
			local varLevel = GetLevel(varMap, varPlayer)
			local varExp = ceil (x300867_var_Exp * varLevel *1.25 )
			if varMemberCount >= 1  then
					if varMemberCount>=70 then
						varMemberCount=70
					end
		...
		if 	GuildLevel == 7 then
			local varLevel = GetLevel(varMap, varPlayer)
			local varExp = ceil (x300867_var_Exp * varLevel *1.5 )
			if varMemberCount >= 1  then
					if varMemberCount>=70 then
						varMemberCount=70
					end
    ...
    if x300867_var_BonusMoney8 > 0 then
		AddGuildUserPoint(varMap,varPlayer,x300867_var_BonusMoney8)	
		local BonusMoney8 = x300867_var_BonusMoney8
		StartTalkTask(varMap)
		TalkAppendString(varMap, "获得#R帮贡"..BonusMoney8.."点#o的奖励" );
		StopTalkTask(varMap)
		DeliverTalkTips(varMap,varPlayer)
		Msg2Player(varMap,varPlayer,format("获得#R帮贡"..BonusMoney8.."点#o的奖励", x300867_var_BonusMoney8),8,2)
		if isMultiple == 1 then
			local pos = GetGuildOfficial(varMap, varPlayer)
				if pos == 5 then
	...
	--老玩家额外经验
	if IsHaveSpecificImpact( varMap, varPlayer, 8864) == 1 then
		x300867_var_GuildExpBonus = x300867_var_GuildExpBonus*2
	end
	local bangExp = x300867_var_GuildExpBonus
	if bangExp > 0 then
		local pos = GetGuildOfficial(varMap, varPlayer)
		if pos == x300867_var_Leader_Index then
			if maxlevel >= 70 and ruoguoopen == 1 then
	...
	local pos = GetGuildOfficial(varMap, varPlayer)
  	if pos == x300867_var_Leader_Index then			
		if x300867_var_CostMoney > 0 then
			AddMoney( varMap , varPlayer ,0, x300867_var_CostMoney )
...
function x300867_ProcQuestSubmit( varMap, varPlayer, varTalknpc, varButtonClick, varQuest )
...
function x300867_ProcQuestAttach( varMap, varPlayer, varTalknpc, varTalkNpcGUID, varQuestIdx, varQuest )

	if IsHaveQuestNM( varMap, varPlayer, x300867_var_QuestId ) == 0 then	
		return
	end

	if varTalkNpcGUID == x300867_SubmitNPCGUID then	
		local varState = GetQuestStateNM(varMap,varPlayer,varTalknpc,x300867_var_QuestId)
		TalkAppendButton( varMap, x300867_var_QuestId,x300867_var_QuestName, 7,x300867_var_FileId ) 
	end
end
...
function x300867_ProcGuildConvoyClearMemberQuest( varMap, varPlayer,varState )
	...
	local varRet = DelQuest( varMap, varPlayer, x300867_var_QuestId  )
	if varRet == 1 then
...
function x300867_ProcGuildConvoySubmitQuest( varMap, varPlayer )

	local varQuestIdx = GetQuestIndexByID(varMap, varPlayer, x300867_var_QuestId); 
	
	
	local raingMisFinishing = GetQuestParam( varMap, varPlayer,varQuestIdx,x300867_var_RingQuestIndex )

	
	
	if DelQuest( varMap, varPlayer, x300867_var_QuestId  ) < 1 then
		return 
	end
	...
	GetGuildQuestData(varMap, varPlayer, GD_GUILD_INDEX_MULTI_MISSION_HUSHOU, x300867_var_FileId,varMemberCount,"ProcReturn1",raingMisFinishing)
end
...
function x300867_ProcQuestAbandon( varMap, varPlayer, varQuest )
...
function x300867_ClearBusQuest( varMap, varPlayer )
	AbandonGuildConvoyQuest(varMap, varPlayer, 18)
end

function x300867_ProcReturn(varMap, varPlayer,QuestData,varQuest,varTalknpc)
	...
	if QuestData == varCurDayTime then					
		...
		if CurrMoney < x300867_var_CostMoney then
		    StartTalkTask(varMap)
	    ...
	    local varBusID = LuaCallNoclosure( x300867_var_BusScriptId, "ProcCreateBus", varMap, varPlayer,varQuest )
...
function x300867_ProcAccept( varMap, varPlayer )
	...
	local pos = GetGuildOfficial(varMap, varPlayer)
  	if pos == x300867_var_Leader_Index then			
		...
		local varNum = CostMoney( varMap , varPlayer ,0, x300867_var_CostMoney,301 )
		if varNum == -1 then
			StartTalkTask(varMap)
	...
	local varCurDayTime = GetDayTime()						
	local varRet = AddQuest( varMap, varPlayer,x300867_var_QuestId , x300867_var_FileId,0,0,0,1 ) 
	...
	if varRet == 1 then
		...
		x300867_QuestLogRefresh( varMap, varPlayer, x300867_var_QuestId)
		...
		SetQuestByIndex(varMap, varPlayer, varQuestIdx, x300867_var_RingQuestIndex, 0);		
...
function x300867_DispatchQuestInfo( varMap, varPlayer, varTalknpc,varIndex )

	StartTalkTask(varMap)

		
		TalkAppendString(varMap,"#Y"..x300867_var_QuestName)
		TalkAppendString(varMap,format("%s", x300867_var_QuestInfo))
		TalkAppendString(varMap," ")

		
		TalkAppendString( varMap,"#Y任务目标：")
		TalkAppendString( varMap,format("%s", x300867_var_QuestTarget))
		TalkAppendString( varMap," ")

		
		if x300867_var_QuestHelp ~= "" then

			TalkAppendString(varMap,"#Y任务提示：")
			TalkAppendString(varMap,format("%s", x300867_var_QuestHelp))
			TalkAppendString(varMap," ")
		end
		...
		local varExpBonus = x300867_var_Exp * varLevel
		varExpBonus =floor(varExpBonus+0.5)
	    if varExpBonus > 0 then
		...
		if x300867_var_GuildExpBonus>0 then
...
function x300867_DispatchContinueInfo( varMap, varPlayer, varTalknpc )
	...
	DeliverTalkRequire(varMap, varPlayer, varTalknpc, x300867_var_FileId, x300867_var_QuestId,0);
...
function x300867_DispatchCompletedInfo( varMap, varPlayer, varTalknpc )
	
	StartTalkTask(varMap)

		
		TalkAppendString(varMap,"#Y"..x300867_var_QuestName)
		TalkAppendString(varMap,x300867_var_QuestCompleted)
		TalkAppendString(varMap," ")
	...
	StopTalkTask(varMap)
	DeliverTalkContinueNM(varMap, varPlayer, varTalknpc, x300867_var_FileId, x300867_var_QuestId);
  LuaCallNoclosure( 256267, "Finishhss", varMap, varPlayer)
end
...
function x300867_QuestLogRefresh( varMap, varPlayer, varQuest)
 StartTalkTask(varMap)
	...
	if varHaveQuest > 0 then
		...
		if x300867_var_BonusMoney8 > 0 then
			AddQuestMoneyBonus8(varMap, x300867_var_BonusMoney8 )
		end	
		
		if x300867_var_GuildExpBonus>0 then
			AddQuestGuildExpBonus(varMap,x300867_var_GuildExpBonus)
		end
		if varLevel >= 80 then
			local refixInhExp = RefixInherenceExp(varMap, varPlayer, 400)
		...
		AddQuestLogCustomText( varMap,
								"",						
								x300867_var_QuestName,        
								x300867_var_QuestTarget,		
								"",					
								"将神兽安全护送到边境交给@npc_134516",                 
								x300867_var_QuestInfo,	
								""					
								)
	end
...
function x300867_ProcQuestLogRefresh( varMap, varPlayer, varQuest)
	x300867_QuestLogRefresh( varMap, varPlayer, x300867_var_QuestId)
end


function x300867_ProcQuestObjectKilled( varMap, varPlayer, varObjData, varObj, varQuest )
end



function x300867_ProcMeberOffline(varMap,varPlayer)
