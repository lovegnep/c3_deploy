...
x300990_var_FileId = 300990
x300990_var_QuestName = "三环任务-第二环的创建镖车脚本"
--DECLARE_QUEST_INFO_STOP--


x300990_var_GodIndex = 1210

x300990_var_Carinfo = {

						{ CarType = 46,BaseAI = 3,AIScript = 3,Script = 300990 }
...
x300990_var_QuestId = {
						{ varId = 9099,varCostMoney = 0, GameId = 51 },
					  }
...
function x300990_ProcEventEntry(varMap, varPlayer, varTalknpc)	
...
function x300990_ProcEnumEvent(varMap, varPlayer, varTalknpc)
	
end


function x300990_ProcCreateBus( varMap, varPlayer,varQuest ,varBusID)

    local PosX = 65
    local PosZ = 179.5
    PosX , PosZ = GetWorldPos( varMap, varPlayer)
	local varObj = CreateBus(varMap, x300990_var_Carinfo[1].CarType, PosX, PosZ, x300990_var_Carinfo[1].BaseAI, x300990_var_Carinfo[1].AIScript, x300990_var_Carinfo[1].Script, varPlayer, x300990_var_GodIndex,varQuest,-1)
end

function x300990_ProcCreateBusOK(varMap,varPlayer,busId,varTalknpc,varQuest)	
...
function x300990_DeleteBus(varMap, varObj)
...
function x300990_ProcIntoBusEvent(varMap, varPlayer,varTalknpc)
	local InBus = IsBusMember(varMap, varPlayer)
	if InBus == 1 then
		StartTalkTask(varMap)
		  	TalkAppendString(varMap,"你已经领取战车了");
		StopTalkTask()
		DeliverTalkTips(varMap,varPlayer)
		local varReadme = "你已经领取战车了"
...
function x300990_ProcIntoBusAfter(varMap, varPlayer,varBusID)
...
function x300990_ProcLeaveBusEvent(varMap, varPlayer,BusId)
		StartTalkTask(varMap)
...
function x300990_ProcDie(varMap,BusId,idKiller)
	...
	if idKiller == -1 then
		...
		if OwnerID ~= -1 then
			x300990_Abandon( varMap, OwnerID  )
			local varReadme = "您的战车被摧毁"
			StartTalkTask(varMap);
			TalkAppendString(varMap, "战车被摧毁！");
			StopTalkTask(varMap);
	...
	if ObjType == 1 then     
		...
		for varI,itm in x300990_var_QuestId do
		...
		if OwnerID ~= -1 then
			...
			local varReadme = "您的战车被"..varCountry..GuildTitle.."的玩家"..killerName.."摧毁，任务失败！"
			if IsHideName(varMap,idKiller) > 0 then
				varReadme = "您的战车被"..HIDE_NAME.."摧毁，任务失败！"
			end
			StartTalkTask(varMap);TalkAppendString(varMap, "战车被摧毁！");StopTalkTask(varMap);DeliverTalkTips(varMap,OwnerID);Msg2Player(varMap,OwnerID,varReadme,8,2)
		...
		if OwnerID ~= -1 then
			x300990_Abandon( varMap, OwnerID  )
			local varReadme = "您的战车被怪物摧毁，任务失败！"
...
function x300990_ProcTime(varMap,BusId)

end

function x300990_ProcLifeTimeOut(varMap,BusId)
    local varPlayer = GetBusMemberObjId(varMap,BusId,0)
    if(IsPlayerStateNormal(varMap, varPlayer) ~= 0) then
		x300990_Abandon( varMap, varPlayer  )
	end
	DeleteBus(varMap, BusId,1)
end

function x300990_ProcWaitTimeOut(varMap,BusId)
	local varPlayer = GetBusMemberObjId(varMap,BusId,0)
	if(IsPlayerStateNormal(varMap, varPlayer) ~= 0) then
		x300990_Abandon( varMap, varPlayer  )
...
function x300990_ProcBusStopWhenOwnerFarAway(varMap, ownerId)
end
...
function x300990_Abandon( varMap, varPlayer )
...
function x300990_CleaupQuest( varMap, varPlayer )
