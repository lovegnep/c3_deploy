x305036_var_FileId = 305036
x305036_var_Mis_Count = 10 

function x305036_ProcEventEntry( varMap, varPlayer, varBagIdx )

	if x305036_GetDayCount(varMap, varPlayer) >= x305036_var_Mis_Count then
	...
	if varMap == 466 then
		...
		CreateMonster( varMap, 56613, PlayerPosX, PlayerPosZ, 0, -1, x305036_var_FileId, 160038, 21)
		...
		x305036_SetDayCount(varMap, varPlayer)
		
	else
		local varText = format("这里毫无灵气，无法使用金色元素符文，请在仙人谷二层使用！" )
		StartTalkTask(varMap)
...
function x305036_ProcIsSpellLikeScript( varMap, varPlayer)
	return 0; 
	
	
end

function x305036_ProcCancelImpacts( varMap, varPlayer )
	return 0; 
end

function x305036_ProcConditionCheck( varMap, varPlayer )
	return 0
end

function x305036_ProcDeplete( varMap, varPlayer )

	return 0
end

function x305036_ProcActivateOnce( varMap, varPlayer )
end

function x305036_ProcActivateEachTick( varMap, varPlayer)
	return 1; 
end

function x305036_ProcDie(varMap, varPlayer, varKiller)
	GamePlayScriptLog(varMap, varKiller, 551)
end

function x305036_GetDayCount(varMap, varPlayer)
	if x305036_var_Mis_Count > 0 then
...
function x305036_SetDayCount(varMap, varPlayer)
	local varToday = GetDayOfYear()
	local varLastday = GetPlayerGameData(varMap, varPlayer, MD_HOUHUAYUAN_ZHAOHUAN_DATE[1], MD_HOUHUAYUAN_ZHAOHUAN_DATE[2], MD_HOUHUAYUAN_ZHAOHUAN_DATE[3])
	if varLastday ~= varToday then
