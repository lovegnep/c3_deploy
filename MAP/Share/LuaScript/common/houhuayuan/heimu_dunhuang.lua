...
x320003_g_ScriptId		=	320003
x320003_g_MissionName	=	"【帮会】收集黑木"
x320003_g_MissionId 	= 	8503
x320003_g_MissionTarget	=	"　前往@npc_69200收集#G黑木#W"
x320003_g_MissionInfo	=	"\t黑色的树，发出黑的的光芒，古人传说黑色的木头可以炼制最锋利的武器……\t在地下粮仓的三层，四层和五层，生长着神秘的黑木。那里充斥着重重危险，您愿意去那里考验自己的能力，将五个黑木带回来吗？"
x320003_g_MissionHelp	=	"\t#G黑木#W只生长在地下粮仓的三层，四层和五层。并且数量稀少。随着你所在帮会等级的提高，你获得的奖励也会相应增加。\n\t此任务每天只能接取一次，放弃任务后只能在次日再次领取。"
x320003_g_MissionRuse	=	"\t当你到达@npc_69200，进行收集#G黑木#W时，如果受到攻击，将无法收集完成。你可以进入角落，将周围的怪物杀死后，再进行收集。五层的怪物比较密集，您最好好组队进行收集！"
x320003_g_MissionCompleted = "\t我需要的黑木，你已经找到了？太感谢了，辛苦了，这是给你的酬谢。"
x320003_g_SubmitNPCGUID	=	132020
x320003_g_GroupPoint	=	{ {type=10,id =901,target = "黑木"}}

x320003_g_NPCName		= "@npc_132020"

x320003_g_LevelLess		= 	70 

x320003_g_BonusMoney8	=   50
x320003_g_GuildExpBonus =	1

x320003_g_Mis_Count		= 1
x320003_g_Leader_Index	= 5
--DECLARE_QUEST_INFO_STOP--         
function x320003_ProcEnumEvent( sceneId, selfId, targetId, MissionId )
	...
	if level < 70 or level > 95 then
  	...
  	if IsHaveQuestNM( sceneId, selfId, x320003_g_MissionId ) == 0 then	-- 如果没有这个任务
		--local state = GetQuestStateNM( sceneId, selfId, targetId, MissionId);
		TalkAppendButton( sceneId, x320003_g_ScriptId, x320003_g_MissionName , 8, -1)	
		--GetGuildQuestData(sceneId, selfId, GD_GUILD_INDEX_GATHER_ISSUE_TIME, x320003_g_ScriptId,MissionId,"OnCallBackEnumerate",targetId)
		return 
	end
...
--选中任务
function x320003_ProcEventEntry( sceneId, selfId, targetId, MissionId )
...
--local GuildLevel = GetGuildSimpleData(GetGuildID( sceneId, selfId ))--得到帮会简单数据(等级) 
	--if 	GuildLevel == 2 then		     	
		if IsHaveQuestNM( sceneId, selfId, MissionId ) > 0 then	-- 如果有这个任务
			local bDone = x320003_CheckSubmit(sceneId, selfId,targetId, MissionId);
			if(bDone > 0) then
				x320003_DispatchCompletedInfo( sceneId, selfId, targetId,MissionId) --完成任务显示
			else
				x320003_DispatchContinueInfo( sceneId, selfId, targetId,MissionId ) --未完成任务显示
			end
		else
			x320003_DispatchMissionInfo( sceneId, selfId, targetId,MissionId )		--任务信息显示
		end
	--end
end
...
----检查任务是否可提交
-----------------------------------------------------------------------------------------------------
function x320003_CheckSubmit(sceneId, selfId,targetId,MissionId)

	local misIndex = GetQuestIndexByID(sceneId, selfId, MissionId);
	local state = GetQuestStateNM(sceneId,selfId,targetId,MissionId)
	TalkAppendButton( sceneId, MissionId,x320003_g_MissionName, 7,state ) 
...
----向客户端发送任务完成信息
-----------------------------------------------------------------------------------------------------
function x320003_DispatchCompletedInfo( sceneId, selfId, targetId,MissionId )
	
	StartTalkTask(sceneId)

		--任务完成信息
		TalkAppendString(sceneId,"#Y"..x320003_g_MissionName)
		TalkAppendString(sceneId,x320003_g_MissionCompleted)
		...
		--2 帮供
		if x320003_g_BonusMoney8>0 then
			AddQuestMoneyBonus8(sceneId,x320003_g_BonusMoney8)
		end
			AddQuestItemBonus(sceneId, 12250015, 1)
		--3 帮会经验
		-- if x320003_g_GuildExpBonus>0 then
			-- AddQuestGuildExpBonus(sceneId,x320003_g_GuildExpBonus)
		-- end
	StopTalkTask(sceneId)
	DeliverTalkContinueNM(sceneId, selfId, targetId, x320003_g_ScriptId, MissionId);
...
----向客户端发送任务未完成
-----------------------------------------------------------------------------------------------------
function x320003_DispatchContinueInfo( sceneId, selfId, targetId,MissionId )
	
	StartTalkTask(sceneId)

		--任务继续信息
		TalkAppendString(sceneId,"#Y"..x320003_g_MissionName)
		TalkAppendString(sceneId,format("%s", x320003_g_MissionInfo))
		TalkAppendString(sceneId," ")
	...
	--任务提示信息
	StopTalkTask(sceneId)
	DeliverTalkRequire(sceneId, selfId, targetId, x320003_g_ScriptId, MissionId,0);
...
----向客户端发送任务信息
-----------------------------------------------------------------------------------------------------
function x320003_DispatchMissionInfo( sceneId, selfId, NPCId,MissionId )
	...
	StartTalkTask(sceneId)
		...
		TalkAppendString( sceneId,format("%s", x320003_g_MissionTarget))
		TalkAppendString( sceneId," ")

		--提示信息
		if x320003_g_MissionHelp ~= "" then

			TalkAppendString(sceneId,"#Y任务提示：")
			TalkAppendString(sceneId,format("%s", x320003_g_MissionHelp))
			TalkAppendString(sceneId," ")
		end
		...
		-- end
	StopTalkTask(sceneId)
	DeliverTalkInfoNM(sceneId, selfId, NPCId, x320003_g_ScriptId, MissionId);
end
--
--
-----------------------------------------------------------------------------------------------------
----获得奖励
-----------------------------------------------------------------------------------------------------
function x320003_OnReturn1(sceneId, selfId,MissionData,MissionId,targetId)
	...
	x320003_GetBonus( sceneId,selfId, isMultiple )
end

function x320003_GetBonus( sceneId, selfId, isMultiple )
...
--奖励帮贡
    if x320003_g_BonusMoney8 > 0 then
		    AddGuildUserPoint(sceneId,selfId,x320003_g_BonusMoney8)	--增加帮贡
			local BonusMoney8 = x320003_g_BonusMoney8
			StartTalkTask(sceneId)
			TalkAppendString(sceneId, "获得#R帮贡"..BonusMoney8.."点#o的奖励");
			StopTalkTask(sceneId)
			DeliverTalkTips(sceneId,selfId)	
			Msg2Player(sceneId,selfId,format("获得#R帮贡"..BonusMoney8.."点#o的奖励", x320003_g_BonusMoney8),8,2)
			if isMultiple == 1 then
			local pos = GetGuildOfficial(sceneId, selfId)
				if pos == 5 then
					local guildid =GetGuildID(sceneId, selfId)
				...
				elseif pos == 4 then
					...
					StopTalkTask(sceneId)
					DeliverTalkTips(sceneId,selfId)	
					Msg2Player(sceneId,selfId,format("帮会多倍任务额外获得#R帮贡"..BonusMoney8.."点#o的奖励", x320003_g_BonusMoney8),8,2)
				end
			end
	end
	-- 奖励帮会经验
	-- local bangExp = x320003_g_BangExp 
	-- if bangExp > 0 then
...
----接受
----**********************************
function x320003_ProcAcceptCheck( sceneId, selfId, targetId, MissionId )
	...
	return 
 	...
 	local level = GetLevel(sceneId,selfId)
	if level < x320003_g_LevelLess then
		StartTalkTask(sceneId)
	...
	if x320003_GetDayCount(sceneId, selfId) >= x320003_g_Mis_Count then
		StartTalkTask(sceneId)
   ...
   --GetGuildQuestData(sceneId, selfId, GD_GUILD_INDEX_GATHER_ISSUE_TIME, x320003_g_ScriptId,MissionId,"OnReturn")
   
   --检查前置任务
	local FrontMissiontId1, FrontMissiontId2, FrontMissiontId3 = GetFrontQuestIdNM( sceneId, selfId, x320003_g_MissionId )
	...
	if AddQuestNM( sceneId, selfId,	x320003_g_MissionId) == 1 then
		...
		Msg2Player(sceneId,selfId,Readme,8,2)
		local misIndex = GetQuestIndexByID(sceneId, selfId, x320003_g_MissionId);
...
----完成任务
----返回1代表成功，0代表交任务失败
function x320003_ProcQuestSubmit( sceneId, selfId, targetId, selectId, MissionId )
	...
	-- 粉丝先锋团
	local ret = LuaCallNoclosure( 302510, "CheckMission", sceneId, selfId, 320003)
	if ret == 0 then
		return 0
	elseif ret == 2 then
		LuaCallNoclosure( 302510, "FinishMission", sceneId, selfId, 320003)
	end
...
--	DeliverTalkTips(sceneId,selfId)
	...
	x320003_SetDayCount(sceneId, selfId)
	GetGuildQuestData(sceneId, selfId, GD_GUILD_INDEX_MULTI_MISSION_HEIMU, x320003_g_ScriptId,MissionId,"OnReturn1",targetId)
...
function x320003_ProcQuestAttach( sceneId, selfId, npcId, npcGuid,misIndex, MissionId )
	
	if IsHaveQuestNM( sceneId, selfId, MissionId) == 0 then	-- 如果没有这个任务
		return
	end

	if npcGuid == x320003_g_SubmitNPCGUID then
		local itemNum = GetItemCount( sceneId, selfId, 13011005 )
		local state = GetQuestStateNM(sceneId,selfId,npcId,MissionId)
		if itemNum < 5 then
		TalkAppendButton(sceneId, MissionId, x320003_g_MissionName,state);
		else
		TalkAppendButton(sceneId, MissionId, x320003_g_MissionName,state);
		end
	end
...
----放弃
----********************************************************************
function x320003_ProcQuestAbandon( sceneId, selfId,MissionId )
...
function x320003_OnReturn(sceneId, selfId,MissionData,MissionId,targetId)
...
----接受
----********************************************************************
function x320003_ProcAccept( sceneId, selfId )

end

--物品改变
function x320003_ProcQuestItemChanged( sceneId, selfId, itemdataId, MissionId, nChangeType )
	...
	if nChangeType ==1 then
		...
		if itemNum < NeedNum then
			...
			x320003_QuestLogRefresh( sceneId, selfId, MissionId );
		end
...
----任务日志刷新
-----------------------------------------------------------------------------------------------------
function x320003_QuestLogRefresh( sceneId, selfId, MissionId)
	...
	if bHaveMission > 0 then
		...
		if x320003_g_BonusMoney8 > 0 then
			AddQuestMoneyBonus8(sceneId, x320003_g_BonusMoney8 )	--增加帮贡
		end
			AddQuestItemBonus(sceneId, 12250015, 1)
		--3 帮会经验
		-- if x320003_g_GuildExpBonus>0 then
		...
		AddQuestLogCustomText( sceneId,
								"",								-- 标题
								x320003_g_MissionName,			-- 任务名字
								x320003_g_MissionTarget..text,	--任务目标
								x320003_g_NPCName,				--任务NPC
								x320003_g_MissionRuse,			--任务攻略
								x320003_g_MissionInfo,			--任务描述
								""								--任务小提示
								)
		StopTalkTask(sceneId)
...
function x320003_ProcQuestLogRefresh( sceneId, selfId, MissionId)
...
----取得此任务当天当前已完成次数
-----------------------------------------------------------------------------------------------------
function x320003_GetDayCount(sceneId, selfId)

	if x320003_g_Mis_Count > 0 then
...
----更新当天接任务次数
-----------------------------------------------------------------------------------------------------
function x320003_SetDayCount(sceneId, selfId)
	local today = GetDayOfYear()
