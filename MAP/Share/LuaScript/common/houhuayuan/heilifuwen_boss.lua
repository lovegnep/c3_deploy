x305033_var_FileId = 305033
x305033_var_Mis_Count = 10

function x305033_ProcEventEntry( varMap, varPlayer, varBagIdx )
	
	if x305033_GetDayCount(varMap, varPlayer) >= x305033_var_Mis_Count then
	...
	if varMap == 46 then
		...
		if varRand <= 33 then
			CreateMonster( varMap, 56610, PlayerPosX, PlayerPosZ, 0, -1, x305033_var_FileId, 160038, 21)
			local message = "召唤出了一个黑戾残暴者"
			Msg2Player( varMap, varPlayer, message, 8, 2)
			Msg2Player( varMap, varPlayer, message, 8, 3)
		elseif varRand<=66 then
			CreateMonster( varMap, 56610, PlayerPosX, PlayerPosZ, 0, -1, x305033_var_FileId, 160038, 21)
			CreateMonster( varMap, 56610, PlayerPosX, PlayerPosZ, 0, -1, x305033_var_FileId, 160038, 21)
			local message = "召唤出了两个黑戾残暴者"
			Msg2Player( varMap, varPlayer, message, 8, 2)
			Msg2Player( varMap, varPlayer, message, 8, 3)
		else
			CreateMonster( varMap, 56611, PlayerPosX, PlayerPosZ, 0, -1, x305033_var_FileId, 160038, 21)
		...
		x305033_SetDayCount(varMap, varPlayer)
		
	else
		local varText = format("这里毫无灵气，无法使用黑戾符文，请在地下粮仓二层使用！" )
		StartTalkTask(varMap)
...
function x305033_ProcIsSpellLikeScript( varMap, varPlayer)
	return 0; 
	
	
end

function x305033_ProcCancelImpacts( varMap, varPlayer )
	return 0; 
end

function x305033_ProcConditionCheck( varMap, varPlayer )
	return 0
end

function x305033_ProcDeplete( varMap, varPlayer )

	return 0
end

function x305033_ProcActivateOnce( varMap, varPlayer )
end

function x305033_ProcActivateEachTick( varMap, varPlayer)
	return 1; 
end

function x305033_ProcDie(varMap, varPlayer, varKiller)
end



function x305033_GetDayCount(varMap, varPlayer)
	if x305033_var_Mis_Count > 0 then
...
function x305033_SetDayCount(varMap, varPlayer)
	local varToday = GetDayOfYear()
	local varLastday = GetPlayerGameData(varMap, varPlayer, MD_HOUHUAYUAN_ZHAOHUAN_DATE[1], MD_HOUHUAYUAN_ZHAOHUAN_DATE[2], MD_HOUHUAYUAN_ZHAOHUAN_DATE[3])
	if varLastday ~= varToday then
