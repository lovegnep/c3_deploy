...
x300336_var_FileId 					= 	300336                  
x300336_var_QuestId 				= 	8051                    

x300336_var_QuestKind 				= 	1                       
x300336_var_LevelLess					= 	40                      

x300336_var_QuestName				= 	"【个人】藏宝图的秘密"                                        
x300336_var_QuestTarget				= 	"  去#aB#h00CCFF{goto_%d,%d,%d}%s(%d,%d)#aE处使用铁铲挖掘宝藏"      
x300336_var_QuestInfo				= 	"通过阅读这张神龙藏宝图，您发现了宝藏的所在。去#aB#h00CCFF{goto_%d,%d,%d}%s(%d,%d)#aE处去挖掘宝藏吧。\n\t注意：当你有#G江湖请柬#W状态时，有可能会挖到现银奖励，否则，只能挖到银卡奖励。"            
x300336_var_ContinueInfo				= 	""  
x300336_var_QuestCompleted			= 	"\t看来您已经得到@item_12030107,并初步了解藏宝图的秘密了吧？现在我给您一条线索，传说按照这条线索追查下去有可能得到天龙的宝藏！天龙的魂魄有可能栖息在地下粮仓，粮仓里深处的五个妖孽趁天龙魂魄虚弱的时候得到了它。打败那些妖孽就有可能得到@item_12030108。"  
x300336_var_QuestHelp				=	"去杂货商人处买一个铁铲，然后去宝藏所在地使用"          

x300336_var_strQuestAbandon         = "您放弃了任务：【个人】藏宝图的秘密"


x300336_var_ExtTarget					=	{ {type=20,n=1,target="去杂货商人处买一把铁铲，然后去宝藏所在地使用。"} }



x300336_var_ExpBonus					= 	0                    	
x300336_var_BonusItem					=	{}	

x300336_var_BonusMoney1               = 	0  
x300336_var_BonusMoney2               =   0  
x300336_var_BonusMoney3               =   0  
x300336_var_BonusMoney4               =   0  
x300336_var_BonusMoney5               =   0  
x300336_var_BonusMoney6               =   0  

x300336_var_BonusChoiceItem           =   {}


--DECLARE_QUEST_INFO_STOP--


x300336_MP_TARGET1					= 1
x300336_MP_TARGET2					= 2
x300336_MP_TARGET3					= 3
x300336_MP_TARGET4					= 4 
x300336_MP_ISCOMPLETE				= 7	

x300336_PREVIOUS_QUESTID			= -1		
x300336_NEXT_QUESTID				= 7552		
x300336_NEXT_QUESTSCRIPTID			= 300754	
x300336_QUEST_HUANSHU				= 0			


x300336_var_QuestTargetTable0        = {
...
x300336_var_CallerId      = 0
x300336_var_CountryID     = 0
x300336_var_CallerName    = ""

x300336_var_DigRange      = 2

x300336_var_MoneyBuff     = 7046

x300336_var_GemBuff       = 7047

x300336_var_MedicineBuff  = 7048

x300336_var_MonsterBuff   = 7049	




function x300336_ProcEnumEvent( varMap, varPlayer, varTalknpc, varQuest )
...
function x300336_ProcEventEntry(varMap, varPlayer, varTalknpc, varQuest,varExtIdx)	

	local varHaveQuest	= IsHaveQuestNM(varMap, varPlayer, x300336_var_QuestId);
	if(varHaveQuest > 0) then
		local varDone = x300336_CheckSubmit(varMap, varPlayer, varTalknpc);
		if(varDone > 0) then
			if varExtIdx ~= -1 then
				x300336_DispatchCompletedInfo( varMap, varPlayer, varTalknpc ) 
			end
		else
			if varExtIdx ~= -1 then
				x300336_DispatchContinueInfo( varMap, varPlayer, varTalknpc ) 
			end
		end
	else
		x300336_DeliverTalkInfo( varMap, varPlayer, varTalknpc )	
	end
end
...
function x300336_ProcAcceptCheck( varMap, varPlayer, varTalknpc)
	local varHaveQuest	= IsHaveQuestNM(varMap, varPlayer, x300336_var_QuestId);
	if( varHaveQuest > 0) then
		return 0;
	else
...
function x300336_ProcQuestAccept( varMap, varPlayer, varTalknpc, varQuest)
	return 0
end
...
function x300336_ProcQuestAbandon(varMap, varPlayer, varQuest)
	local varHaveQuest	= IsHaveQuestNM(varMap, varPlayer, x300336_var_QuestId);
	if(varHaveQuest > 0) then
		local varQuestIdx = GetQuestIndexByID(varMap, varPlayer, x300336_var_QuestId); 
		DelQuest(varMap, varPlayer, x300336_var_QuestId)

		StartTalkTask( varMap)
		TalkAppendString( varMap, x300336_var_strQuestAbandon)
		StopTalkTask( varMap)
		DeliverTalkTips( varMap, varPlayer)
		Msg2Player( varMap, varPlayer, x300336_var_strQuestAbandon, 8, 2)
	end
end
...
function x300336_CheckSubmit(varMap, varPlayer, varTalknpc)

	local varQuestIdx = GetQuestIndexByID(varMap, varPlayer, x300336_var_QuestId);
	return GetQuestParam(varMap, varPlayer, varQuestIdx, x300336_MP_ISCOMPLETE);
...
function x300336_GetBonus( varMap, varPlayer,varTalknpc, varButtonClick )

	
	if x300336_var_BonusMoney1 > 0 then
	    AddMoney( varMap, varPlayer, 1, x300336_var_BonusMoney1 )
	    Msg2Player(varMap,varPlayer,format("获得银卡#{_MONEY%d}的奖励。", x300336_var_BonusMoney1),4,2)
	end

	
	if x300336_var_BonusMoney2 > 0 then
	    AddMoney( varMap, varPlayer, 0, x300336_var_BonusMoney2 )
	    Msg2Player(varMap,varPlayer,format("获得银币#{_MONEY%d}的奖励。", x300336_var_BonusMoney2),4,2)
	end

	
	if x300336_var_BonusMoney3 > 0 then
	    AddMoney( varMap, varPlayer, 3, x300336_var_BonusMoney3 )
	    Msg2Player(varMap,varPlayer,format("获得金卡#{_MONEY%d}的奖励。", x300336_var_BonusMoney3),4,2)
	end

	
	if x300336_var_BonusMoney4 > 0 then
		local varRongYu = GetRongYu( varMap, varPlayer )
		varRongYu = varRongYu + x300336_var_BonusMoney4
		SetRongYu( varMap, varPlayer, varRongYu )
		Msg2Player(varMap,varPlayer,format("获得江湖声望%d的奖励。", x300336_var_BonusMoney4),4,2)
	end

	
	if x300336_var_BonusMoney5 > 0 then
		local varShengWang = GetShengWang( varMap, varPlayer )
		varShengWang = varShengWang + x300336_var_BonusMoney5
		SetShengWang( varMap, varPlayer, varShengWang )
		Msg2Player(varMap,varPlayer,format("获得朝廷声望%d的奖励。", x300336_var_BonusMoney5),4,2)
	end

	
	if x300336_var_BonusMoney6 > 0 then
		AddHonor(varMap,varPlayer,x300336_var_BonusMoney6)
		Msg2Player(varMap,varPlayer,format("获得荣誉值%d的奖励。", x300336_var_BonusMoney6),4,2)
	end

 	
	for varI, item in x300336_var_BonusItem do
	...
	for varI, item in x300336_var_BonusChoiceItem do
...
function x300336_ProcQuestSubmit(varMap, varPlayer, varTalknpc, varRadioSelected, varQuest)
	...
	if varHaveQuest > 0 then
		...
		local varQuestIdx = GetQuestIndexByID(varMap, varPlayer, x300336_var_QuestId)
		local completed = GetQuestParam(varMap, varPlayer, varQuestIdx, x300336_MP_ISCOMPLETE)

		if completed == 1 then
			local result = x300336_CheckPlayerBagFull(varMap,varPlayer)
			if result == 1 then
		    	x300336_GetBonus( varMap,varPlayer,varTalknpc,varRadioSelected )
		    	DelQuest(varMap, varPlayer, x300336_var_QuestId)
...
function x300336_CheckPlayerBagFull( varMap ,varPlayer )
	...
	StartItemTask(varMap)
	for j, item in x300336_var_BonusItem do
		ItemAppendBind( varMap, item.item, item.n )
    end
	...
	if bAdd == 0 then
		...
		for j, item in x300336_var_BonusChoiceItem do
	        StartItemTask(varMap)
			ItemAppendBind( varMap, item.item, item.n )
			local varRet = StopItemTask(varMap,varPlayer)
			if varRet <= 0 then
...
function x300336_DeliverTalkInfo( varMap, varPlayer, varTalknpc )

	StartTalkTask(varMap)

		
		TalkAppendString(varMap,"#Y"..x300336_var_QuestName)
		TalkAppendString(varMap,x300336_var_QuestInfo)
		TalkAppendString(varMap," ")

		
		TalkAppendString( varMap,"#Y任务目标：")
		TalkAppendString( varMap,x300336_var_QuestTarget)
		TalkAppendString( varMap," ")

		
		if x300336_var_QuestHelp ~= "" then

			TalkAppendString(varMap,"#Y任务提示：")
			TalkAppendString(varMap,x300336_var_QuestHelp )
			TalkAppendString(varMap," ")
		end
		...
		if x300336_var_ExpBonus> 0 then
			AddQuestExpBonus(varMap, x300336_var_ExpBonus )
		end

		
		if x300336_var_BonusMoney1 > 0 then
			AddQuestMoneyBonus1(varMap, x300336_var_BonusMoney1 )
		end

		
		if x300336_var_BonusMoney2 > 0 then
			AddQuestMoneyBonus2(varMap, x300336_var_BonusMoney2 )
		end
		
		if x300336_var_BonusMoney3 > 0 then
			AddQuestMoneyBonus3(varMap, x300336_var_BonusMoney3 )
		end

		
		if x300336_var_BonusMoney4 > 0 then
			AddQuestMoneyBonus4(varMap, x300336_var_BonusMoney4 )
		end

		
		if x300336_var_BonusMoney5 > 0 then
			AddQuestMoneyBonus5(varMap, x300336_var_BonusMoney5 )
		end

		
		if x300336_var_BonusMoney6 > 0 then
			AddQuestMoneyBonus6(varMap, x300336_var_BonusMoney6 )
		end
	...
	StopTalkTask()
	DeliverTalkInfoNM(varMap, varPlayer, varTalknpc, x300336_var_FileId, x300336_var_QuestId);
end




function x300336_ProcDigbyShovel( varMap, varPlayer)
    
    if IsHaveQuestNM( varMap, varPlayer, x300336_var_QuestId) == 0 then       
        return 0
    else
        local varQuestIdx = GetQuestIndexByID( varMap, varPlayer, x300336_var_QuestId)
        local ts = GetQuestParam( varMap, varPlayer, varQuestIdx, x300336_MP_TARGET4)
        if ts ~= varMap then
    		StartTalkTask( varMap)
        ...
        local varX = GetQuestParam( varMap, varPlayer, varQuestIdx, x300336_MP_TARGET2)
        local z = GetQuestParam( varMap, varPlayer, varQuestIdx, x300336_MP_TARGET3)
        local curX, curZ = GetWorldPos( varMap, varPlayer)
        curX = abs( varX - curX)
        curZ = abs( z - curZ)
        if curX > x300336_var_DigRange or curZ > x300336_var_DigRange then
    		StartTalkTask( varMap)
        ...
        while control == 1 do 
            ...
            elseif varRand > 30 and varRand <= 33 then 
							local money = 0
        			if level >= 40 and level <= 59 then
        				money = 10000
        			elseif level >= 60 and level <= 79 then
	            ...
	            Msg2Player( varMap, varPlayer, "在藏宝图的指引下，您得到了#R银卡"..money2.."两#cffcf00，恭喜恭喜。", 8, 2)
	            AddMoney( varMap, varPlayer, 1, money)    
           -- elseif varRand > 23 and varRand <= 33 then 
	          --      StartTalkTask( varMap)
	        	--	TalkAppendString( varMap, "在藏宝图的指引下，您得到了#R银卡3两#cffcf00，恭喜恭喜。" )
	        	--	StopTalkTask( varMap)
	        	--	DeliverTalkTips( varMap, varPlayer)
	          --      Msg2Player( varMap, varPlayer, "在藏宝图的指引下，您得到了#R银卡3两#cffcf00，恭喜恭喜。", 8, 2)
	           --     AddMoney( varMap, varPlayer, 1, 3000)
    
            elseif varRand > 33 and varRand <= 53 then 
            		local itemid = 0
	        			if level >= 40 and level <= 59 then
           ...
           -- elseif varRand > 43 and varRand <= 48 then 
            --    StartItemTask( varMap)
           --     ItemAppendBind( varMap, 11000201, 1)
         ...
         --       Msg2Player( varMap, varPlayer, "在藏宝图的指引下，您得到了#R一个#{_ITEM11000201}#cffcf00，恭喜恭喜。", 8, 2)
            ...
            elseif varRand > 53 and varRand <= 68 then 
                ...
                StartItemTask( varMap)
                ItemAppendBind( varMap, itemid, 3)               
                local result = StopItemTask( varMap, varPlayer)
                if result == 0 then
	            		StartTalkTask( varMap)
	            		TalkAppendString( varMap, "背包已满，无法放入挖到的物品！" )
            ...
            elseif varRand > 68 and varRand <= 73 then 
            		local itemid = 0
	        			if level >= 40 and level <= 59 then
	        				itemid = 11000202														-- 神传水晶
	        			elseif level >= 60 and level <= 79 then
            ...
            elseif varRand > 73 and varRand <= 74 then 
            ...
            elseif varRand > 74 and varRand <= 95 then 
                if x300336_CallBoss( varMap, varPlayer) == 0 then
                    brk = 1
                else

                end
           -- elseif varRand > 90 and varRand <= 95 then 
           --     StartItemTask( varMap)
           --    	ItemAppendBind( varMap, 11000201, 1)
         ...
         --       Msg2Player( varMap, varPlayer, "在藏宝图的指引下，您得到了#R一个#{_ITEM11000201}#cffcf00，恭喜恭喜。", 8, 2)
            ...
            elseif varRand > 95 and varRand <= 100 then 
        ...
        DelQuest( varMap, varPlayer, x300336_var_QuestId)
        -- 提示任务完成
        Msg2Player( varMap, varPlayer, "您完成了："..x300336_var_QuestName, 8, 2)
        Msg2Player( varMap, varPlayer, "您完成了："..x300336_var_QuestName, 8, 3)
        -- 写日志
...
function x300336_CallBoss( varMap, varPlayer)
    local monsters = {}
    local varCount = 1
    ...
    for varI, item in x300336_var_MonsterTable do
    ...
    for varI, item in x300336_var_MonsterLive do
    ...
    local mapName = ""
    CreateMonster( varMap, monsters[ varI].bossId, varX + random( 1, 3), z + random( 1, 3), 1, -1, x300336_var_FileId, -1, 21, 30*60*1000)
    
    for varI, item in x300336_var_MonsterLive do
        if item.varMap == varMap then
            x300336_var_MonsterLive[ varI].live = 1
...
function x300336_ProcDie( varMap, varPlayer, varKiller)
    ...
    for varI, item in x300336_var_MonsterLive do
        if item.varMap == varMap then
            x300336_var_MonsterLive[ varI].live = 0
        end
    end
...
function x300336_DispatchContinueInfo( varMap, varPlayer, varTalknpc )
end




function x300336_DispatchCompletedInfo( varMap, varPlayer, varTalknpc )
end




function x300336_ProcQuestObjectKilled(varMap, varPlayer, varObjData ,varObj, varQuest )
end




function x300336_ProcQuestItemChanged( varMap, varPlayer, varItemData, varQuest )
end




function x300336_ProcQuestAttach( varMap, varPlayer, varTalknpc, varTalkNpcGUID, varQuestIdx, varQuest )
end




function x300336_ProcQuestLogRefresh( varMap, varPlayer, varQuest)
    local varQuestIdx = GetQuestIndexByID( varMap, varPlayer, x300336_var_QuestId)
    local mapID = GetQuestParam( varMap, varPlayer, varQuestIdx, x300336_MP_TARGET1)
    local varTalknpc = GetQuestParam( varMap, varPlayer, varQuestIdx, x300336_MP_TARGET4)
    local varX = GetQuestParam( varMap, varPlayer, varQuestIdx, x300336_MP_TARGET2)
    local z = GetQuestParam( varMap, varPlayer, varQuestIdx, x300336_MP_TARGET3)
    local strMapName = ""
	...
	StartTalkTask(varMap)
        AddQuestLogCustomText( varMap,
                                "",                             
                                x300336_var_QuestName,             
                                format( x300336_var_QuestTarget, varTalknpc, varX, z,strMapName,varX,z),
                                "无",                             
                                x300336_var_QuestHelp,
                                format( x300336_var_QuestInfo, varTalknpc, varX, z,strMapName,varX,z),
                                "" )
...
function x300336_ProcForceAddQuest( varMap, varPlayer, varTalknpc, varQuest)
	if( varQuest ~= x300336_var_QuestId) then
		return 0;
	end

	
	if(x300336_ProcAcceptCheck( varMap, varPlayer, -1 ) ~= 1) then
		return 0;
	end
	local varRet = AddQuest( varMap, varPlayer, x300336_var_QuestId, x300336_var_FileId, 0, 0, 0, 0)
	if(varRet == 0) then
		StartTalkTask(varMap)
	    ...
	    local size = getn( x300336_var_QuestTargetTable0)
	    ...
	    local varQuestIdx = GetQuestIndexByID(varMap, varPlayer, x300336_var_QuestId)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x300336_MP_TARGET1, x300336_var_QuestTargetTable0[ varI].varMap)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x300336_MP_TARGET2, x300336_var_QuestTargetTable0[ varI].posX)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x300336_MP_TARGET3, x300336_var_QuestTargetTable0[ varI].posZ)
	    local mapid = GetCurCountry( varMap, varPlayer) * 100 + x300336_var_QuestTargetTable0[ varI].varMap + 50
	    if x300336_var_QuestTargetTable0[ varI].varMap == 2 then
	        mapid = 0
	    elseif x300336_var_QuestTargetTable0[ varI].varMap == 3 then
	        mapid = 15
	    elseif x300336_var_QuestTargetTable0[ varI].varMap == 5 then
	        mapid = 5	        
	    elseif x300336_var_QuestTargetTable0[ varI].varMap == 8 then
	        mapid = 8		      
	    elseif x300336_var_QuestTargetTable0[ varI].varMap == 11 then
	        mapid = 11	
	    elseif x300336_var_QuestTargetTable0[ varI].varMap == 17 then
	        mapid = 17	     
	    elseif x300336_var_QuestTargetTable0[ varI].varMap == 20 then
	        mapid = 20	        
	    elseif x300336_var_QuestTargetTable0[ varI].varMap == 23 then
	        mapid = 23		     
	    end
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x300336_MP_TARGET4, mapid)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x300336_MP_ISCOMPLETE, 0)
	    ...
	    local size = getn( x300336_var_QuestTargetTable1)
	    local varI = random( 1, size)
	    ...
	    local varQuestIdx = GetQuestIndexByID(varMap, varPlayer, x300336_var_QuestId)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x300336_MP_TARGET1, x300336_var_QuestTargetTable1[ varI].varMap)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x300336_MP_TARGET2, x300336_var_QuestTargetTable1[ varI].posX)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x300336_MP_TARGET3, x300336_var_QuestTargetTable1[ varI].posZ)
	    local mapid= GetCurCountry( varMap, varPlayer) * 100 + x300336_var_QuestTargetTable1[ varI].varMap + 50
	    if x300336_var_QuestTargetTable1[ varI].varMap == 2 then
	        mapid = 0
	    elseif x300336_var_QuestTargetTable1[ varI].varMap == 3 then
	        mapid = 15
	    elseif x300336_var_QuestTargetTable1[ varI].varMap == 5 then
	        mapid = 5	        
	    elseif x300336_var_QuestTargetTable1[ varI].varMap == 8 then
	        mapid = 8		      
	    elseif x300336_var_QuestTargetTable1[ varI].varMap == 11 then
	        mapid = 11	
	    elseif x300336_var_QuestTargetTable1[ varI].varMap == 17 then
	        mapid = 17	     
	    elseif x300336_var_QuestTargetTable1[ varI].varMap == 20 then
	        mapid = 20	        
	    elseif x300336_var_QuestTargetTable1[ varI].varMap == 23 then
	        mapid = 23		        
	    end
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x300336_MP_TARGET4, mapid)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x300336_MP_ISCOMPLETE, 0)
	    ...
	    local size = getn( x300336_var_QuestTargetTable2)
	    local varI = random( 1, size)
	    ...
	    local varQuestIdx = GetQuestIndexByID(varMap, varPlayer, x300336_var_QuestId)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x300336_MP_TARGET1, x300336_var_QuestTargetTable2[ varI].varMap)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x300336_MP_TARGET2, x300336_var_QuestTargetTable2[ varI].posX)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x300336_MP_TARGET3, x300336_var_QuestTargetTable2[ varI].posZ)
	    local mapid = GetCurCountry( varMap, varPlayer) * 100 + x300336_var_QuestTargetTable2[ varI].varMap + 50
	    if x300336_var_QuestTargetTable2[ varI].varMap == 2 then
	        mapid = 0
	    elseif x300336_var_QuestTargetTable2[ varI].varMap == 3 then
	        mapid = 15
	    elseif x300336_var_QuestTargetTable2[ varI].varMap == 5 then
	        mapid = 5	        
	    elseif x300336_var_QuestTargetTable2[ varI].varMap == 8 then
	        mapid = 8		      
	    elseif x300336_var_QuestTargetTable2[ varI].varMap == 11 then
	        mapid = 11	
	    elseif x300336_var_QuestTargetTable2[ varI].varMap == 17 then
	        mapid = 17	     
	    elseif x300336_var_QuestTargetTable2[ varI].varMap == 20 then
	        mapid = 20	        
	    elseif x300336_var_QuestTargetTable2[ varI].varMap == 23 then
	        mapid = 23		
	    end
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x300336_MP_TARGET4, mapid)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x300336_MP_ISCOMPLETE, 0)
	    ...
	    local size = getn( x300336_var_QuestTargetTable3)
	    local varI = random( 1, size)
	    ...
	    local varQuestIdx = GetQuestIndexByID(varMap, varPlayer, x300336_var_QuestId)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x300336_MP_TARGET1, x300336_var_QuestTargetTable3[ varI].varMap)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x300336_MP_TARGET2, x300336_var_QuestTargetTable3[ varI].posX)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x300336_MP_TARGET3, x300336_var_QuestTargetTable3[ varI].posZ)
	    local mapid = GetCurCountry( varMap, varPlayer) * 100 + x300336_var_QuestTargetTable3[ varI].varMap + 50
	    if x300336_var_QuestTargetTable3[ varI].varMap == 2 then
	        mapid = 0
	    elseif x300336_var_QuestTargetTable3[ varI].varMap == 3 then
	        mapid = 15
	    elseif x300336_var_QuestTargetTable3[ varI].varMap == 5 then
	        mapid = 5	        
	    elseif x300336_var_QuestTargetTable3[ varI].varMap == 8 then
	        mapid = 8		      
	    elseif x300336_var_QuestTargetTable3[ varI].varMap == 11 then
	        mapid = 11	
	    elseif x300336_var_QuestTargetTable3[ varI].varMap == 17 then
	        mapid = 17	     
	    elseif x300336_var_QuestTargetTable3[ varI].varMap == 20 then
	        mapid = 20	        
	    elseif x300336_var_QuestTargetTable3[ varI].varMap == 23 then
	        mapid = 23		        
	    end
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x300336_MP_TARGET4, mapid)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x300336_MP_ISCOMPLETE, 0)
...
function x300336_ProcCreaterGetBonus(varMap,guid,questdata)
