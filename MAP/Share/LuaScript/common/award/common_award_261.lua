...
x580261_g_ScriptId = 580261
x580261_g_MissionName = "瑞兔贺岁红包"

--一次最多显示的奖品数
x580261_g_MaxOnceAwardNum = 1
x580261_g_ItemBonus =	{	{zhiye=-1,id=12030541,num=1}	--瑞兔贺岁红包"
						}


--点击（NPC）
function x580261_ProcEnumEvent( sceneId, selfId, targetId, MissionId )
	return 0
...
--点击（按钮）点击该任务后执行此脚本
function x580261_ProcEventEntry( sceneId, selfId, targetId,state,index )	
	
	--未判断背包是否有足够空间

	x580261_DispatchMissionInfo( sceneId, selfId, targetId,index)

end

function x580261_DispatchMissionInfo( sceneId, selfId, targetId,index)
	...
	DeliverTalkInfo(sceneId, selfId, targetId, x580261_g_ScriptId, -1);

end

--领奖
function x580261_AddAward( sceneId, selfId,itemNum,index, dbIndex )
	...
	local IsHaveItem = 0
	for i,itm in x580261_g_ItemBonus do
		if itm.zhiye == -1 then
			IsHaveItem = 1
...
--接受
--**********************************
function x580261_ProcAccept( sceneId, selfId )
end

function x580261_ProcAcceptCheck( sceneId, selfId, NPCId )
	return 1
end
--点击（确定）
function x580261_ProcQuestAccept( sceneId, selfId, NPCId )
end

--**********************************
--放弃
--**********************************
function x580261_ProcQuestAbandon( sceneId, selfId, MissionId )
end
--**********************************
--继续
--**********************************
function x580261_OnContinue( sceneId, selfId, targetId )
end
--**********************************
--检测是否可以提交
--**********************************
function x580261_CheckSubmit( sceneId, selfId )
end
--**********************************
--提交
--**********************************
function x580261_ProcQuestSubmit( sceneId, selfId, targetId,selectRadioId, MissionId )
end
--**********************************
--杀死怪物或玩家
--**********************************
function x580261_ProcQuestObjectKilled( sceneId, selfId, objdataId, objId, MissionId )
end
--**********************************
--进入区域事件
