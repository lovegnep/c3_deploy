...
x270010_var_QuestIdPre =3580
x270010_var_QuestId = 3581
x270010_var_LevelLess	= 	78 
x270010_var_QuestIdNext = 3582
x270010_var_Name	={}
x270010_var_noDemandItem ={{varId=13020314,varNum=1}}
x270010_var_ExtTarget={{type=20,n=1,target="到@npc_141010放下酒坛询问红娘的爷爷"}}
x270010_var_QuestName="【主线】爷爷的美酒"
x270010_var_QuestInfo="\t我看那张生也是一表人才，莺莺美貌贤淑，二人若能喜结良缘也不失为是一件美事，不过我还有些事情要向我爷爷问清楚。\n\t我这里有一坛美酒，把它放在城边的大树下，请那里一个手拿红线和书的老头喝酒，等他喝到高兴时，再问他我说的这件事，他一定会告诉你。"  
...
function x270010_useitem(varMap,varPlayer)
		...
		local distanceRet = 100 - (134 - varX) * (134 - varX) - (401 - z) * (401 - z)
		local varQuestIdx = GetQuestIndexByID(varMap,varPlayer,x270010_var_QuestId)
		if varMap == 0 then
		 			...
		 			if x270010_var_doing==0 then
							...
							SetQuestByIndex(varMap,varPlayer,varQuestIdx,7,1)
				 			...
				 			if x270010_var_PROTECTINFO.ScneneTimerIndex >= 0 then
									x270010_var_yuelao=CreateMonster(varMap, 445,134,400,3,1,-1,-1,20,15000,0,"老人")
        					x270010_var_doing =1
