...
x301462_var_GatherPointTypeId = 134 
x301462_var_ItemId = 13011607 
x301462_var_BufIndex = 7600
x301462_var_GroupPoint	=	{ {type=10,varId =134,target = "丹木"}}
--DECLARE_QUEST_INFO_STOP--

x301462_var_Growpointlevel = {
...
x301462_var_GrowpointPos = {
							 {posx1=176, posz1=216,posx2=38, posz2=131,posx3=205, posz3=175,
							 },							
		}		


function 	x301462_ProcGpCreate(varMap,varGpType,varX,varY)
	
	local varBoxId = ItemBoxEnterScene(varX, varY, x301462_var_GatherPointTypeId, varMap, 0, x301462_var_ItemId)
	SetGrowPointObjID(varMap,x301462_var_GatherPointTypeId,varX, varY,varBoxId)
end




function	 x301462_ProcGpOpen(varMap,varPlayer,varTalknpc)
	return x301462_ProcGpOpenItemBox(varMap, varPlayer, varTalknpc, x301462_var_GatherPointTypeId, x301462_var_ItemId)
...
function	 x301462_ProcGpRecycle(varMap,varPlayer,varTalknpc)
	return   x301462_ProcGuildRecycle( varMap, varPlayer, varTalknpc, x301462_var_GatherPointTypeId, x301462_var_ItemId )
...
function	x301462_ProcGpProcOver(varMap,varPlayer,varTalknpc)
...
function x301462_OpenCheck(varMap,varPlayer,AbilityId,AblityLevel)
...
function x301462_GetQuestDesc(varMap, varPlayer)
	for varI,itm in x301462_var_Growpointlevel do
...
function x301462_ProcGpOpenItemBox( varMap, varPlayer, varTalknpc, varGpType, varNeedItem )
	local varQuestDesc = x301462_GetQuestDesc(varMap, varPlayer)
	...
	if varMap == 23 and varLevel > 105 then
		StartTalkTask(varMap)
	...
	if varMap == 24 and varLevel > 110 then
		StartTalkTask(varMap)
		TalkAppendString(varMap,"您的等级过高")
		StopTalkTask(varMap)
		DeliverTalkTips(varMap,varPlayer)
	...
	if varMap == 25 and varLevel > 115 then
		StartTalkTask(varMap)
		TalkAppendString(varMap,"您的等级过高")
		StopTalkTask(varMap)
		DeliverTalkTips(varMap,varPlayer)
...
function x301462_ProcGuildRecycle( varMap, varPlayer, varTalknpc, varGpType, varNeedItem )
	local varQuestDesc = x301462_GetQuestDesc(varMap, varPlayer)
	if varQuestDesc == nil then
		return 1
	end
	...
	local varNeedItem = varQuestDesc.itmID
	local varLevel = GetLevel(varMap,varPlayer)
	if varLevel <=  115 then
		if varMap == 24 then
			 local varRet = random(1,80000)
			 if varRet <= 10 then 
					local varName = GetName(varMap,varPlayer)
					local varMsg1 = "由于"..varName.."在收集图腾时触发了天变，耶路撒冷附近出现天龙之魂"
					...
					CreateMonster(varMap,3059,x301462_var_GrowpointPos[1].posx1, x301462_var_GrowpointPos[1].posz1, 16,-1, -1,13079463,21,1000 * 60 *15 )
					CreateMonster(varMap,3059,x301462_var_GrowpointPos[1].posx2, x301462_var_GrowpointPos[1].posz2, 16,-1, -1,13079463,21,1000 * 60 *15 )
					CreateMonster(varMap,3059,x301462_var_GrowpointPos[1].posx3, x301462_var_GrowpointPos[1].posz3, 16,-1, -1,13079463,21,1000 * 60 *15 )
