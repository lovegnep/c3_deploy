...
x301459_var_GatherPointTypeId = 130 
x301459_var_ItemId = 13011604 
x301459_var_BufIndex = 7600
x301459_var_GroupPoint	=	{ {type=10,varId =130,target = "丹木"}}
--DECLARE_QUEST_INFO_STOP--

x301459_var_Growpointlevel = {
...
x301459_var_GrowpointPos = {
							{posx1=131, posz1=55,posx2=88, posz2=150,posx3=142, posz3=74,
							},							
		}		


function 	x301459_ProcGpCreate(varMap,varGpType,varX,varY)
	
	local varBoxId = ItemBoxEnterScene(varX, varY, x301459_var_GatherPointTypeId, varMap, 0, x301459_var_ItemId)
	SetGrowPointObjID(varMap,x301459_var_GatherPointTypeId,varX, varY,varBoxId)
end




function	 x301459_ProcGpOpen(varMap,varPlayer,varTalknpc)
	return x301459_ProcGpOpenItemBox(varMap, varPlayer, varTalknpc, x301459_var_GatherPointTypeId, x301459_var_ItemId)
...
function	 x301459_ProcGpRecycle(varMap,varPlayer,varTalknpc)
	return   x301459_ProcGuildRecycle( varMap, varPlayer, varTalknpc, x301459_var_GatherPointTypeId, x301459_var_ItemId )
...
function	x301459_ProcGpProcOver(varMap,varPlayer,varTalknpc)
...
function x301459_OpenCheck(varMap,varPlayer,AbilityId,AblityLevel)
...
function x301459_GetQuestDesc(varMap, varPlayer)
	for varI,itm in x301459_var_Growpointlevel do
...
function x301459_ProcGpOpenItemBox( varMap, varPlayer, varTalknpc, varGpType, varNeedItem )
	
	local varQuestDesc = x301459_GetQuestDesc(varMap, varPlayer)
	...
	if varMap == 11 and varLevel >75 then
		StartTalkTask(varMap)
	...
	if varMap == 13 and varLevel >80 then
		StartTalkTask(varMap)
		TalkAppendString(varMap,"您的等级过高")
		StopTalkTask(varMap)
		DeliverTalkTips(varMap,varPlayer)
	...
	if varMap == 16 and varLevel >85 then
		StartTalkTask(varMap)
		TalkAppendString(varMap,"您的等级过高")
		StopTalkTask(varMap)
		DeliverTalkTips(varMap,varPlayer)
...
function x301459_ProcGuildRecycle( varMap, varPlayer, varTalknpc, varGpType, varNeedItem )
	local varQuestDesc = x301459_GetQuestDesc(varMap, varPlayer)
	if varQuestDesc == nil then
		return 1
	end
	...
	if varLevel <=  85 then
		if varMap == 13 then
			...
			if varRet <= 10 then 
					...
					local varMsg1 = "由于"..varName.."在收集图腾时触发了天变，巴格达附近出现天龙之魂"
					...
					CreateMonster(varMap,3059,x301459_var_GrowpointPos[1].posx1, x301459_var_GrowpointPos[1].posz1, 16,-1, -1,13079463,21,1000 * 60 *15 )
					CreateMonster(varMap,3059,x301459_var_GrowpointPos[1].posx2, x301459_var_GrowpointPos[1].posz2, 16,-1, -1,13079463,21,1000 * 60 *15 )
					CreateMonster(varMap,3059,x301459_var_GrowpointPos[1].posx3, x301459_var_GrowpointPos[1].posz3, 16,-1, -1,13079463,21,1000 * 60 *15 )
