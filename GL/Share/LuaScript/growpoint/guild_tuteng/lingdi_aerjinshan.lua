...
x301458_var_GatherPointTypeId = 124 
x301458_var_ItemId = 13011604 
x301458_var_BufIndex = 7600
x301458_var_GroupPoint	=	{ {type=10,varId =124,target = "丹木"}}
--DECLARE_QUEST_INFO_STOP--

x301458_var_Growpointlevel = {
...
x301458_var_GrowpointPos = {
							 {posx1=176, posz1=220,posx2=43, posz2=57,posx3=86, posz3=60,
							 },							
		}		


function 	x301458_ProcGpCreate(varMap,varGpType,varX,varY)
	
	local varBoxId = ItemBoxEnterScene(varX, varY, x301458_var_GatherPointTypeId, varMap, 0, x301458_var_ItemId)
	SetGrowPointObjID(varMap,x301458_var_GatherPointTypeId,varX, varY,varBoxId)
end




function	 x301458_ProcGpOpen(varMap,varPlayer,varTalknpc)
	return x301458_ProcGpOpenItemBox(varMap, varPlayer, varTalknpc, x301458_var_GatherPointTypeId, x301458_var_ItemId)
...
function	 x301458_ProcGpRecycle(varMap,varPlayer,varTalknpc)
	return   x301458_ProcGuildRecycle( varMap, varPlayer, varTalknpc, x301458_var_GatherPointTypeId, x301458_var_ItemId )
...
function	x301458_ProcGpProcOver(varMap,varPlayer,varTalknpc)
...
function x301458_OpenCheck(varMap,varPlayer,AbilityId,AblityLevel)
...
function x301458_GetQuestDesc(varMap, varPlayer)
	for varI,itm in x301458_var_Growpointlevel do
...
function x301458_ProcGpOpenItemBox( varMap, varPlayer, varTalknpc, varGpType, varNeedItem )
	local varQuestDesc = x301458_GetQuestDesc(varMap, varPlayer)
	...
	if varMap == 8 and varLevel > 65 then
		StartTalkTask(varMap)
	...
	if varMap == 9 and varLevel > 70 then
		StartTalkTask(varMap)
		TalkAppendString(varMap,"您的等级过高")
		StopTalkTask(varMap)
		DeliverTalkTips(varMap,varPlayer)
	...
	if varMap ==10 and varLevel > 75 then
		StartTalkTask(varMap)
		TalkAppendString(varMap,"您的等级过高")
		StopTalkTask(varMap)
		DeliverTalkTips(varMap,varPlayer)
...
function x301458_ProcGuildRecycle( varMap, varPlayer, varTalknpc, varGpType, varNeedItem )
	local varQuestDesc = x301458_GetQuestDesc(varMap, varPlayer)
	if varQuestDesc == nil then
		return 1
	end
	...
	if varLevel <=  75 then
		if varMap == 9 then
			 ...
			 if varRet <= 10 then 
					...
					local varMsg1 = "由于"..varName.."在收集图腾时触发了天变，碎叶城附近出现天龙之魂"
					...
					CreateMonster(varMap,3059,x301458_var_GrowpointPos[1].posx1, x301458_var_GrowpointPos[1].posz1, 16,-1, -1,13079463,21,1000 * 60 *15 )
					CreateMonster(varMap,3059,x301458_var_GrowpointPos[1].posx2, x301458_var_GrowpointPos[1].posz2, 16,-1, -1,13079463,21,1000 * 60 *15 )
					CreateMonster(varMap,3059,x301458_var_GrowpointPos[1].posx3, x301458_var_GrowpointPos[1].posz3, 16,-1, -1,13079463,21,1000 * 60 *15 )
