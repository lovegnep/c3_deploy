...
x301461_var_GatherPointTypeId = 133 
x301461_var_ItemId = 13011606 
x301461_var_BufIndex = 7600
x301461_var_GroupPoint	=	{ {type=10,varId =133,target = "丹木"}}
--DECLARE_QUEST_INFO_STOP--

x301461_var_Growpointlevel = {
...
x301461_var_GrowpointPos = {
							 {posx1=150, posz1=216,posx2=29, posz2=122,posx3=137, posz3=46,
							 },							
		}		


function 	x301461_ProcGpCreate(varMap,varGpType,varX,varY)
	
	local varBoxId = ItemBoxEnterScene(varX, varY, x301461_var_GatherPointTypeId, varMap, 0, x301461_var_ItemId)
	SetGrowPointObjID(varMap,x301461_var_GatherPointTypeId,varX, varY,varBoxId)
end




function	 x301461_ProcGpOpen(varMap,varPlayer,varTalknpc)
	return x301461_ProcGpOpenItemBox(varMap, varPlayer, varTalknpc, x301461_var_GatherPointTypeId, x301461_var_ItemId)
...
function	 x301461_ProcGpRecycle(varMap,varPlayer,varTalknpc)
	return   x301461_ProcGuildRecycle( varMap, varPlayer, varTalknpc, x301461_var_GatherPointTypeId, x301461_var_ItemId )
...
function	x301461_ProcGpProcOver(varMap,varPlayer,varTalknpc)
...
function x301461_OpenCheck(varMap,varPlayer,AbilityId,AblityLevel)
...
function x301461_GetQuestDesc(varMap, varPlayer)
	for varI,itm in x301461_var_Growpointlevel do
...
function x301461_ProcGpOpenItemBox( varMap, varPlayer, varTalknpc, varGpType, varNeedItem )
	local varQuestDesc = x301461_GetQuestDesc(varMap, varPlayer)
	...
	if varMap == 20 and varLevel > 95 then
		StartTalkTask(varMap)
	...
	if varMap == 21 and varLevel > 100 then
		StartTalkTask(varMap)
		TalkAppendString(varMap,"您的等级过高")
		StopTalkTask(varMap)
		DeliverTalkTips(varMap,varPlayer)
	...
	if varMap == 22 and varLevel > 105 then
		StartTalkTask(varMap)
		TalkAppendString(varMap,"您的等级过高")
		StopTalkTask(varMap)
		DeliverTalkTips(varMap,varPlayer)
...
function x301461_ProcGuildRecycle( varMap, varPlayer, varTalknpc, varGpType, varNeedItem )
	local varQuestDesc = x301461_GetQuestDesc(varMap, varPlayer)
	if varQuestDesc == nil then
		return 1
	end
	...
	local varNeedItem = varQuestDesc.itmID
	local varLevel = GetLevel(varMap,varPlayer)
	if varLevel <=  105 then
		if varMap == 21 then
			 local varRet = random(1,80000)
			 if varRet <= 10 then 
					local varName = GetName(varMap,varPlayer)
					local varMsg1 = "由于"..varName.."在收集图腾时触发了天变，神圣罗马帝国附近出现天龙之魂"
					...
					CreateMonster(varMap,3059,x301461_var_GrowpointPos[1].posx1, x301461_var_GrowpointPos[1].posz1, 16,-1, -1,13079463,21,1000 * 60 *15 )
					CreateMonster(varMap,3059,x301461_var_GrowpointPos[1].posx2, x301461_var_GrowpointPos[1].posz2, 16,-1, -1,13079463,21,1000 * 60 *15 )
					CreateMonster(varMap,3059,x301461_var_GrowpointPos[1].posx3, x301461_var_GrowpointPos[1].posz3, 16,-1, -1,13079463,21,1000 * 60 *15 )
