...
x301460_var_GatherPointTypeId = 132 
x301460_var_ItemId = 13011605 
x301460_var_BufIndex = 7600
x301460_var_GroupPoint	=	{ {type=10,varId =132,target = "丹木"}}
--DECLARE_QUEST_INFO_STOP--

x301460_var_Growpointlevel = {
...
x301460_var_GrowpointPos = {
							{posx1=75, posz1=214,posx2=147, posz2=99,posx3=35, posz3=76,
							},							
		}		


function 	x301460_ProcGpCreate(varMap,varGpType,varX,varY)
	
	local varBoxId = ItemBoxEnterScene(varX, varY, x301460_var_GatherPointTypeId, varMap, 0, x301460_var_ItemId)
	SetGrowPointObjID(varMap,x301460_var_GatherPointTypeId,varX, varY,varBoxId)
end




function	 x301460_ProcGpOpen(varMap,varPlayer,varTalknpc)
	return x301460_ProcGpOpenItemBox(varMap, varPlayer, varTalknpc, x301460_var_GatherPointTypeId, x301460_var_ItemId)
...
function	 x301460_ProcGpRecycle(varMap,varPlayer,varTalknpc)
	return   x301460_ProcGuildRecycle( varMap, varPlayer, varTalknpc, x301460_var_GatherPointTypeId, x301460_var_ItemId )
...
function	x301460_ProcGpProcOver(varMap,varPlayer,varTalknpc)
...
function x301460_OpenCheck(varMap,varPlayer,AbilityId,AblityLevel)
...
function x301460_GetQuestDesc(varMap, varPlayer)
	for varI,itm in x301460_var_Growpointlevel do
...
function x301460_ProcGpOpenItemBox( varMap, varPlayer, varTalknpc, varGpType, varNeedItem )
	
	local varQuestDesc = x301460_GetQuestDesc(varMap, varPlayer)
	...
	if varMap == 17 and varLevel >85 then
		StartTalkTask(varMap)
	...
	if varMap == 18 and varLevel >90 then
		StartTalkTask(varMap)
		TalkAppendString(varMap,"您的等级过高")
		StopTalkTask(varMap)
		DeliverTalkTips(varMap,varPlayer)
	...
	if varMap == 19 and varLevel >95 then
		StartTalkTask(varMap)
		TalkAppendString(varMap,"您的等级过高")
		StopTalkTask(varMap)
		DeliverTalkTips(varMap,varPlayer)
...
function x301460_ProcGuildRecycle( varMap, varPlayer, varTalknpc, varGpType, varNeedItem )
	local varQuestDesc = x301460_GetQuestDesc(varMap, varPlayer)
	if varQuestDesc == nil then
		return 1
	end
	...
	if varLevel <=  95 then
		if varMap == 18 then
			...
			if varRet <= 10 then 
					...
					local varMsg1 = "由于"..varName.."在收集图腾时触发了天变，基辅公国附近出现天龙之魂"
					...
					CreateMonster(varMap,3059,x301460_var_GrowpointPos[1].posx1, x301460_var_GrowpointPos[1].posz1, 16,-1, -1,13079463,21,1000 * 60 *15 )
					CreateMonster(varMap,3059,x301460_var_GrowpointPos[1].posx2, x301460_var_GrowpointPos[1].posz2, 16,-1, -1,13079463,21,1000 * 60 *15 )
					CreateMonster(varMap,3059,x301460_var_GrowpointPos[1].posx3, x301460_var_GrowpointPos[1].posz3, 16,-1, -1,13079463,21,1000 * 60 *15 )
