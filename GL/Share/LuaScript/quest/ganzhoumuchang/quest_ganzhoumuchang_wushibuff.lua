...
x203736_var_QuestId = 3769
x203736_var_LevelLess	= 	1 
x203736_var_QuestIdNext = 3781
x203736_var_Name	={}
x203736_var_ExtTarget={{type=20,n=1,target="找到@npc_132043学习并使用2级勇冠三军"}}
x203736_var_QuestName="【新手】职业的进阶"
x203736_var_QuestInfo="\t你已经熟悉了生存的必要条件和王城周围的环境，现在你需要加强的就是你的自身实力了，王城的技能大师可以帮你快速地提升自身的能力。\n\t现在，你可以去找他了。"  
x203736_var_QuestTarget=""		
x203736_var_QuestCompleted="\t我就是这里的技能大师，如果你愿意，我将是你终生的导师。\n\t活到老学到老，等你觉得自己杀敌变得缓慢的时候记得来找我学习新的技能。"					
x203736_var_ContinueInfo="\t不清楚如何提升技能等级？在我这里就可以学习、提升武士的技能。\n\t点击#G武士技能学习#W按钮就能进入技能学习介面，里面#G明亮的技能#W表示你现在能掌握的技能，点击相应图示即可学习技能，当然你要有一定的经验和银子！\n\t最后记得学习完技能后，要将自己#G技能栏内技能的图示#W拖到下方#G快捷栏内#W，才能在需要的时候，方便快捷的使用。"
x203736_var_QuestHelp =	"#G您可以按下键盘上的#R2#G键，或者左键点击您下方技能快捷栏中的#R勇冠三军#G技能。#W"
x203736_var_DemandItem ={}
x203736_var_ExpBonus = 855
x203736_var_BonusMoney1 =137
...
x203736_var_NpcGUID ={{ guid = 132043, varName = "武士技能大师"} }
--DECLARE_QUEST_INFO_STOP--


function x203736_ProcEnumEvent(varMap, varPlayer, varTalknpc, varQuest)
	if IsQuestHaveDone(varMap, varPlayer, x203736_var_QuestId) > 0 then
		return 
	end
	if IsQuestHaveDone(varMap, varPlayer, x203736_var_QuestIdPre)> 0 then
		if IsHaveQuest(varMap,varPlayer, x203736_var_QuestId) <= 0 then
			if GetZhiye( varMap, varPlayer)==0 then
				local varState = GetQuestStateNM(varMap,varPlayer,varTalknpc,x203736_var_QuestId)
				TalkAppendButtonNM( varMap, varPlayer, varTalknpc, x203736_var_QuestId, varState, -1 )
...
function x203736_DispatchQuestInfo( varMap, varPlayer, varTalknpc )
		...
		for varI, item in x203736_var_BonusItem do
...
function x203736_ProcEventEntry(varMap, varPlayer, varTalknpc, varQuest)	
	...
	if IsHaveQuest(varMap,varPlayer, x203736_var_QuestId) > 0 then
			if x203736_CheckSubmit(varMap, varPlayer, varTalknpc) <= 0 then
						...
						StopTalkTask()
						DeliverTalkMenu(varMap, varPlayer, varTalknpc, x203736_var_FileId, x203736_var_QuestId);
			else
						StartTalkTask(varMap)
						TalkAppendString(varMap,"#Y"..x203736_var_QuestName)
						...
						StopTalkTask()
						DeliverTalkContinueNM(varMap, varPlayer, varTalknpc, x203736_var_FileId, x203736_var_QuestId);
			end
  elseif x203736_ProcAcceptCheck(varMap, varPlayer, varTalknpc) > 0 then
	  	...
	  	TalkAppendString(varMap,"#Y"..x203736_var_QuestName)
			...
			for varI, item in x203736_var_ExtTarget do
			...
			x203736_DispatchQuestInfo( varMap, varPlayer, varTalknpc )
	  	StopTalkTask()
			DeliverTalkInfoNM(varMap, varPlayer, varTalknpc, x203736_var_FileId, x203736_var_QuestId);
  end
...
function x203736_CheckSubmit( varMap, varPlayer, varTalknpc)
	local varQuestIdx = GetQuestIndexByID( varMap, varPlayer, x203736_var_QuestId )
...
function x203736_ProcQuestAccept( varMap, varPlayer, varTalknpc, varQuest )
		...
		local varFrontQuest1, varFrontQuest2, varFrontQuest3 = GetFrontQuestIdNM( varMap, varPlayer,x203736_var_QuestId  )
		...
		if x203736_ProcAcceptCheck(varMap, varPlayer, varTalknpc) > 0 then
					local retmiss = AddQuest( varMap, varPlayer, x203736_var_QuestId, x203736_var_FileId, 0, 0, 0, 1)
					if retmiss ==0 then
...
function x203736_ProcQuestAbandon(varMap, varPlayer, varQuest)
		...
		DelQuest(varMap, varPlayer, x203736_var_QuestId)
...
function x203736_CheckPlayerBagFull( varMap ,varPlayer,varRadioSelected )
	...
	for j, item in x203736_var_BonusItem do
...
function x203736_ProcQuestSubmit(varMap, varPlayer, varTalknpc, varRadioSelected, varQuest)
	...
	elseif x203736_CheckPlayerBagFull( varMap ,varPlayer,varRadioSelected ) <= 0 then
				...
				DeliverTalkTips(varMap,varPlayer)
				return 0
	elseif DelQuest(varMap, varPlayer, x203736_var_QuestId) > 0 then
				QuestCom(varMap, varPlayer, x203736_var_QuestId)
   ...
   LuaCallNoclosure( QUEST_SCRIPT, "ProcEventEntry", varMap, varPlayer, varTalknpc, x203736_var_QuestIdNext )	
end
...
local bFind  = 0
    for varI ,item in x203736_var_NpcGUID do
