...
x300721_var_FileId = 300721                 
x300721_var_LevelLess					= 	30

--DECLARE_QUEST_INFO_STOP--



x300721_var_QuestId = 1200

x300721_var_SubQuest = {
[0] = 6546,
[1] = 7588,
[2] = 7589,
[3] = 7590,
...
x300721_var_addFriendPoint = 150
x300721_var_QuestName ="【好友】意气相投"
x300721_var_ParamIndexBegin = 2
x300721_var_ParamIndexEnd = 6
function x300721_CalcQuestEXP(varMap, varPlayer)
	
	local haveSubQuestId = x300721_HaveWhichSubQuest(varMap, varPlayer)
	...
	for varI=x300721_var_ParamIndexBegin, x300721_var_ParamIndexEnd do
...
function x300721_SetCompleteTimes(varMap, varPlayer, times)
...
function x300721_SetCompleteDay(varMap, varPlayer)
		...
		x300721_SetCompleteTimes(varMap, varPlayer, 1)
end

function x300721_GetCompleteDay(varMap, varPlayer)
...
function x300721_SetKillMonsterNum(varMap, varPlayer, varNum)
		...
		x300721_KillMonsterRefresh(varMap, varPlayer)
end

function x300721_GetKillMonsterNum(varMap, varPlayer)
...
function x300721_AddKillMonsterNum(varMap, varPlayer, varNum)
	varNum = varNum + x300721_GetKillMonsterNum(varMap, varPlayer)
	x300721_SetKillMonsterNum(varMap, varPlayer, varNum)
end
	
function x300721_GetSubQuestId(humanLevel)
	local questLevel = 0
	...
	return x300721_var_SubQuest[questLevel]
end

function x300721_DeleteAllSubQuest( varMap, varPlayer )
	for varI, quest in x300721_var_SubQuest do
		DelQuestNM( varMap, varPlayer, quest )
	end
end

function x300721_IsSubQuest( varMap, varPlayer, varQuest )
	for varI, quest in x300721_var_SubQuest do
...
function x300721_HaveWhichSubQuest(varMap, varPlayer)
	for varI, quest in x300721_var_SubQuest do
...
function x300721_IsHaveSubQuest(varMap, varPlayer)
	for varI, quest in x300721_var_SubQuest do
		if IsHaveQuestNM( varMap, varPlayer, quest ) == 1 then
			return 1
		end
...
function x300721_HaveDoneWhichSubQuest(varMap, varPlayer, varTalknpc)
...
function x300721_ProcEventEntry( varMap, varPlayer, varTalknpc, varQuest, varIndex )
	local haveSubQuestId = x300721_HaveWhichSubQuest(varMap, varPlayer)
	local haveDoneQuestId = -1
	if haveSubQuestId > 0 then
		haveDoneQuestId = x300721_HaveDoneWhichSubQuest(varMap, varPlayer, varTalknpc)
	end
	
	local varExp = x300721_CalcQuestEXP(varMap, varPlayer)
	...
	if haveDoneQuestId > 0 then
		...
		TalkAppendString(varMap, "\t好友度:"..x300721_var_addFriendPoint.."点")
		TalkAppendString(varMap, "\t经验值:"..varExp.."点")
		StopTalkTask()
	  DeliverTalkContinueNM(varMap, varPlayer, varTalknpc, x300721_var_FileId, varQuest);
	  return
	end
	...
	if haveSubQuestId > 0 then
		...
		TalkAppendString(varMap, "\t经验值:"..varExp.."点")
		StopTalkTask()
		DeliverTalkRequire(varMap, varPlayer, varTalknpc, x300721_var_FileId, varQuest,0);
		return
	end
	...
	DeliverTalkInfo(varMap, varPlayer, varTalknpc, x300721_var_QuestId, -1)
...
function x300721_ProcEnumEvent(varMap, varPlayer, varTalknpc, varQuest)
	...
	TalkAppendButton(varMap, x300721_var_QuestId, x300721_var_QuestName, varState, -1)
...
function x300721_ProcAcceptCheck(varMap, varPlayer, varTalknpc)
...
function x300721_CheckPushList(varMap, varPlayer, varTalknpc)
	return 1
end

function x300721_DoAccept( varMap, varPlayer )
	...
	local completeDay = x300721_GetCompleteDay(varMap, varPlayer)
	...
	for varI = 0, varTeamSize - 1 do
		local varTalknpc = GetNearTeamMember(varMap, varPlayer, varI)
		local completeDay = x300721_GetCompleteDay(varMap, varTalknpc)
		if( completeDay == varToday) then
	...
	local varQuest = x300721_GetSubQuestId(averageLevel)
	
	local varHaveQuest = x300721_IsHaveSubQuest(varMap, varPlayer)
	...
	for varI = 0, varTeamSize - 1 do
		local varTalknpc = GetNearTeamMember(varMap, varPlayer, varI)
		local varHaveQuest = x300721_IsHaveSubQuest(varMap, varTalknpc)
		if varHaveQuest == 1 then
	...
	for varI = 0, varTeamSize - 1 do
		...
		local varStr = format("您领取了任务:%s", x300721_var_QuestName)
		...
		QuestCom(varMap,aId,x300721_var_QuestId)
		x300721_SetCompleteDay(varMap, aId, varToday)
		x300721_SetKillMonsterNum(varMap, aId, 0)
	...
	for varI = 0, varTeamSize - 1 do
		...
		local paramIndex = x300721_var_ParamIndexBegin
...
function x300721_ProcQuestAccept( varMap, varPlayer, varTalknpc, varQuest )
	local varStr = x300721_DoAccept( varMap, varPlayer )
	if varStr ~= nil then
...
function x300721_ProcQuestAbandon(varMap, varPlayer, varQuest)

	x300721_DeleteAllSubQuest( varMap, varPlayer )
	QuestUnCom(varMap,varPlayer,x300721_var_QuestId)
	local varToday = GetDayOfYear()
	x300721_SetCompleteDay(varMap, varPlayer, varToday)
...
function x300721_CheckSubmit( varMap, varPlayer, varQuest )
	local haveSubQuestId = x300721_HaveWhichSubQuest(varMap, varPlayer)
	local haveDoneQuestId = -1
	if haveSubQuestId > 0 then
		haveDoneQuestId = x300721_HaveDoneWhichSubQuest(varMap, varPlayer, 1)
	end
...
function x300721_ProcQuestSubmit( varMap, varPlayer, varTalknpc, varButtonClick, varQuest )
  ...
  if IsHaveQuestNM( varMap,varPlayer,haveDoneQuestId )<= 0 then
	...
	for varI=x300721_var_ParamIndexBegin, x300721_var_ParamIndexEnd do
			...
			if guid > 0 and GetFriendName(varMap, varPlayer, guid) ~= "" then
				local varStr = format("你与%s增加了%d点好友度", GetFriendName(varMap, varPlayer, guid), x300721_var_addFriendPoint)
	...
	if varRet > 0 then 
		...
		for varI=x300721_var_ParamIndexBegin, x300721_var_ParamIndexEnd do
			if nil ~= guidTable[varI] then
				...
				local point = GetFriendPointByGuid(varMap, varPlayer, guid)
				SetFriendPointByGuid(varMap, varPlayer, guid, point + x300721_var_addFriendPoint)
			end
		end
...
function x300721_ProcQuestObjectKilled(varMap, varPlayer, varObjData, varObj, varQuest)

	if x300721_IsSubQuest( varMap, varPlayer, varQuest ) < 0 then
		return -1;
	end
	...
	if NeedKilledNum > 0 then 
		...
		for varI = 0, HumenNum-1 do
        	...
        	if humanObjId ~= -1 and IsPlayerStateNormal(varMap, humanObjId) > 0 and GetHp(varMap, humanObjId) > 0 and IsHaveQuestNM( varMap, humanObjId, varQuest ) > 0 then	
          		...
          		local KilledNum = x300721_GetKillMonsterNum(varMap, humanObjId)

                if KilledNum < NeedKilledNum then

                	x300721_AddKillMonsterNum(varMap, humanObjId, 1)
                  	...
                  	if KilledNum == NeedKilledNum - 1 then
						...
						x300721_ProcQuestLogRefresh(varMap, humanObjId, -1)
...
function x300721_ProcAreaEntered(varMap, varPlayer, varArea, varQuest)
	

end

function x300721_ProcMapTimerTick(varMap, varPlayer )
	 
		
end

function x300721_ProcAreaLeaved(varMap, varPlayer, varScript, varQuest )
...
function x300721_ProcQuestItemChanged(varMap, varPlayer, varItemData, varQuest)
	
end
function x300721_ProcQuestAttach( varMap, varPlayer, varTalknpc, varTalkNpcGUID, varQuestIdx, varQuest )
...
function x300721_PositionUseItem( varMap, varPlayer, varBagIdx, varImpact )
	return  LuaCallNoclosure( QUEST_SCRIPT, "PositionUseItem",varMap, varPlayer, varBagIdx, varImpact)
end

function x300721_ProcGpOpenItemBox( varMap, varPlayer, varTalknpc, varGpType, varNeedItem )
	return LuaCallNoclosure( QUEST_SCRIPT, "ProcGpOpenItemBox",varMap, varPlayer, varTalknpc, varGpType, varNeedItem)
end

function x300721_ProcGpRecycle( varMap, varPlayer, varTalknpc, varGpType, varNeedItem )
	return LuaCallNoclosure( QUEST_SCRIPT, "ProcGpRecycle",varMap, varPlayer, varTalknpc, varGpType, varNeedItem)
end

function x300721_ProcGpProcOver( varMap, varPlayer, varTalknpc )
	LuaCallNoclosure( QUEST_SCRIPT, "ProcGpProcOver",varMap, varPlayer, varTalknpc)
end

function x300721_OpenCheck( varMap, varPlayer, varTalknpc )
	LuaCallNoclosure( QUEST_SCRIPT, "OpenCheck", varMap, varPlayer, varTalknpc )
end

function x300721_KillMonsterRefresh(varMap, varPlayer)
	local subQuest = x300721_HaveWhichSubQuest(varMap, varPlayer)
  local szQuestTarget = x300721_GetQuestTarget(varMap, varPlayer)
	StartTalkTask(varMap)

	AddQuestLogCustomText( varMap,
							"",						
							x300721_var_QuestName,        
							szQuestTarget,		
							"",			
							"",               
							"",	
...
function x300721_GetQuestHelp(varMap, varPlayer)
	local subQuest = x300721_HaveWhichSubQuest(varMap, varPlayer)
	local szQuestHelp = ""
	if tonumber(6546) == tonumber(subQuest) then
		szQuestHelp = "到王国迷宫一层消灭200只@npc_90051后向@npc_123759覆命。"
	elseif tonumber(7588) == tonumber(subQuest) then
		szQuestHelp = "到王国迷宫二层消灭200只@npc_90053后向@npc_123759覆命。"
	elseif tonumber(7589) == tonumber(subQuest) then
		szQuestHelp = "到王国迷宫三层消灭200只@npc_90055后向@npc_123759覆命。"
	elseif tonumber(7590) == tonumber(subQuest) then
		szQuestHelp = "到王国迷宫四层消灭200只@npc_90057后向@npc_123759覆命。"
	elseif tonumber(7591) == tonumber(subQuest) then
		szQuestHelp = "到王国迷宫五层消灭200只@npc_90059后向@npc_123759覆命。"
	elseif tonumber(7592) == tonumber(subQuest) then
		szQuestHelp = "到王国迷宫六层消灭200只@npc_90061后向@npc_123759覆命。"
	elseif tonumber(7593) == tonumber(subQuest) then
		szQuestHelp = "到王国迷宫七层消灭200只@npc_90063后向@npc_123759覆命。"
	elseif tonumber(7594) == tonumber(subQuest) then
		szQuestHelp = "到王国迷宫八层消灭200只@npc_14004后向@npc_123759覆命。"
	elseif tonumber(7595) == tonumber(subQuest) then
		szQuestHelp = "到王国迷宫九层消灭200只@npc_14004后向@npc_123759覆命。"
	end
	return szQuestHelp
end

function x300721_GetQuestTarget(varMap, varPlayer)
	local subQuest = x300721_HaveWhichSubQuest(varMap, varPlayer)
	local curKillMonsterNum = x300721_GetKillMonsterNum(varMap, varPlayer)
	local szMonsterNum = "(" .. curKillMonsterNum .. "/200)"
	local szQuestTarget = ""
	if tonumber(6546) == tonumber(subQuest) then
		szQuestTarget = "  除掉200只@npc_90051" .. szMonsterNum
	elseif tonumber(7588) == tonumber(subQuest) then
		szQuestTarget = "  除掉200只@npc_90053" .. szMonsterNum
	elseif tonumber(7589) == tonumber(subQuest) then
		szQuestTarget = "  除掉200只@npc_90055" .. szMonsterNum
	elseif tonumber(7590) == tonumber(subQuest) then
		szQuestTarget = "  除掉200只@npc_90057" .. szMonsterNum
	elseif tonumber(7591) == tonumber(subQuest) then
		szQuestTarget = "  除掉200只@npc_90059" .. szMonsterNum
	elseif tonumber(7592) == tonumber(subQuest) then
		szQuestTarget = "  除掉200只@npc_90061" .. szMonsterNum
	elseif tonumber(7593) == tonumber(subQuest) then
		szQuestTarget = "  除掉200只@npc_90063" .. szMonsterNum
	elseif tonumber(7594) == tonumber(subQuest) then
...
function x300721_QuestLogRefresh( varMap, varPlayer, varQuest)
	local subQuest = x300721_HaveWhichSubQuest(varMap, varPlayer)
	local szQuestDesc = "\t为了让大家更多的结识朋友，为了让大家更快的增加好友度，"
	.."您可以组队来找我领取意气相投。哪怕队中有陌生人，只要接取意气相投也会把你们自动加为好友。"
	.."完成本任务，会增加组队间好友的好友度。"
	
	local szQuestHelp = x300721_GetQuestHelp(varMap, varPlayer)
	
	local szQuestTarget = x300721_GetQuestTarget(varMap, varPlayer)
	local szQuestNPC = "@npc_123759"
	local varExp = x300721_CalcQuestEXP(varMap, varPlayer)
	StartTalkTask(varMap)
		local szQuestBonus = "\n#Y\n奖励内容：\n#W好友度:"..x300721_var_addFriendPoint.."点";
		local szQuestExp = "\n#W经验值:"..varExp.."点";
	AddQuestLogCustomText( varMap,
							"标题",						
							x300721_var_QuestName,        
...
function x300721_ProcQuestLogRefresh( varMap, varPlayer, varQuest)
	x300721_QuestLogRefresh( varMap, varPlayer, varQuest );
end
