...
x300807_var_QuestId 				= 	9418                    
x300807_var_FileId 					= 	300807                  
x300807_var_QuestKind 				= 	1                       
x300807_var_LevelLess					= 	75                      


x300807_var_QuestName				= 	"【个人】军需武备"
x300807_var_QuestTarget				= 	""          
x300807_var_QuestInfo				= 	"\t时下多事之秋，王国中有很多棘手的事情需要得力的人去做，我希望你能助上一臂之力。\n\t你是否要接受任务？"         
x300807_var_ContinueInfo				= 	"\t怎么？你连这些简单的任务都无法完成？看来我们要另外物色人选了。"  
x300807_var_QuestCompleted			= 	"\t你的表现让我很欣慰，希望你以后能够继续为我们的王国多做贡献！"  
x300807_var_QuestHelp				=	"\t军需武备任务每轮为#G30环#W子任务，环数越高，#G经验奖励#W也越高！\n\t军需武备中收集的物品，很多可以通过生活技能制作，或和其他玩家交易得来。"          


x300807_var_ExtTarget					=	{ {type=20,n=1,target="【个人】军需武备"}}



x300807_var_ExpBonus					= 	0;
x300807_var_BonusItem					=	{}	

x300807_var_BonusMoney1               = 	0
x300807_var_BonusMoney2               =   0
x300807_var_BonusMoney3               =   0

x300807_var_BonusChoiceItem           =   {}


--DECLARE_QUEST_INFO_STOP--

x300807_var_GameId					=	1030
x300807_var_DayCountLimited			=	1


x300807_var_EnterTime					=	{                       
											{min=0*60+0,  max=23*60+59},
										}

x300807_var_MaxStep					=	30
x300807_var_ReplyNpcId				=	132153
x300807_var_ReplyNpcObjId				=	3


x300807_var_BaseBonusRate = {50000,52000,54000,56000,58000,60000,62000,64000,66000,68000}  
x300807_var_ExtraBonusRate = {1,1.6,2.9}  




x300807_var_QuestInfo2				= 	"\t现在世道纷乱，百姓处在水深火热之中，王国中的有识之士准备组织起来保境安民，但目前国中物资匮乏，急需补充。"
x300807_var_ContinueInfo2				= 	"\t这件物品着这么难以找到吗？"
x300807_var_QuestCompleted2			= 	"\t谢谢你的慷慨解囊。"
x300807_var_QuestHelp2					=	"\t军需武备任务每轮为#G30环#W子任务，环数越高，#G经验奖励#W也越高！\n\t军需武备中收集的物品，很多可以通过生活技能制作，或和其他玩家交易得来。"            

x300807_var_QuestTable2 =	{ 
...
x300807_ChangleToggleWorld = {} --哪几个World开放经验奖励双倍

function x300807_GetQuestId( varMap, varPlayer )
	return x300807_var_QuestId
end

function x300807_GetQuestStep(varMap, varPlayer)
	...
	if nLastFlag>=x300807_GetMaxStep(varMap,varPlayer) then
		nLastFlag = 0
	end
	return nLastFlag+1
...
function x300807_SaveQuestStep(varMap, varPlayer)

	local nLastFlag = GetPlayerGameData(varMap, varPlayer, MD_GUILD_CIRCLE_MISSION_CURRENT_ID[1], MD_GUILD_CIRCLE_MISSION_CURRENT_ID[2], MD_GUILD_CIRCLE_MISSION_CURRENT_ID[3])



	if nLastFlag<(x300807_GetMaxStep(varMap,varPlayer)-1) then
		...
		local varMyQuest = x300807_GetQuestId( varMap, varPlayer )	
		x300807_DispatchQuestInfo( varMap, varPlayer, x300807_var_ReplyNpcObjId, varMyQuest, 0 )
	else
		...
		x300807_SetDayCount(varMap, varPlayer)
...
function x300807_ResetQuestStep(varMap, varPlayer)
	SetPlayerGameData(varMap, varPlayer, MD_GUILD_CIRCLE_MISSION_CURRENT_ID[1], MD_GUILD_CIRCLE_MISSION_CURRENT_ID[2], MD_GUILD_CIRCLE_MISSION_CURRENT_ID[3],0)
end

function x300807_GetQuestTypeById(varStep)
	
	local varIndex =0
	for varI,item in x300807_var_QuestTable2 do
		varIndex = varIndex+1;
...
function x300807_GetMaxStep(varMap,varPlayer)
	return x300807_var_MaxStep
end

function x300807_GetData(varMap, varPlayer)
	local varStep = x300807_GetQuestStep(varMap, varPlayer)
	local varIndex = x300807_GetQuestTypeById(varStep)
	local n1 = mod(varStep,10)
	local Toggle = x300807_CheckMissonToggle( varMap, varPlayer )
	...
	local nBaseBonusRate = x300807_var_BaseBonusRate[n1]  
	local nExtraBonusRate = x300807_var_ExtraBonusRate[n2] 
...
function x300807_ProcEnumEvent( varMap, varPlayer, varTalknpc, varQuest )
	if GetGameOpenById(x300807_var_GameId)<=0 then
		return
	end
    ...
    if varLevel < x300807_var_LevelLess then
       return
    end
	...
	if IsHaveQuestNM( varMap, varPlayer, varMyQuest ) == 0 then
		...
		TalkAppendButton(varMap, varMyQuest, x300807_var_QuestName,8,1);
		
	else
		local varState = GetQuestStateNM(varMap, varPlayer, varTalknpc, varMyQuest);

		TalkAppendButton(varMap, varMyQuest, x300807_var_QuestName,varState,varState);
...
function x300807_ProcEventEntry( varMap, varPlayer, varTalknpc, varQuest, varExt )
	
	
	
	if GetGameOpenById(x300807_var_GameId)<=0 then
		x300807_ShowTips(varMap, varPlayer, "此活动未开启")
		return
	end
	...
	if IsHaveQuestNM( varMap, varPlayer, varMyQuest ) == 0 then
		if varExt~=30 then
			if x300807_GetDayCount(varMap, varPlayer) > 0 then
			x300807_ShowTips(varMap, varPlayer, "您已经完成本周军需武备任务，请下周再来接取本任务！")
			x300807_ResetQuestStep(varMap, varPlayer)
			return 0
		else
		x300807_DispatchQuestInfo( varMap, varPlayer, varTalknpc, varMyQuest, 0 )
		end
		else
			x300807_ShowTips(varMap, varPlayer, "你没有任务"..x300807_var_QuestName)
			return
		end
		...
		if varExt==7 then 
			x300807_DispatchCompletedInfo( varMap, varPlayer, varTalknpc,varMyQuest )
			
		elseif varExt==30 then
			x300807_DispatchSongxinInfo( varMap, varPlayer, varTalknpc,varMyQuest )
		else
			
			x300807_DispatchContinueInfo( varMap, varPlayer, varTalknpc,varMyQuest )
		end
	end
...
function x300807_DispatchSongxinInfo( varMap, varPlayer, varTalknpc,varQuest )
...
function x300807_DispatchQuestInfo( varMap, varPlayer, varTalknpc, varQuest, varExt )
	

	local varStep,varIndex,varExpBonus = x300807_GetData(varMap, varPlayer)

	StartTalkTask(varMap)
		TalkAppendString(varMap,"#Y"..x300807_var_QuestName)
		
			
		TalkAppendString(varMap,x300807_var_QuestInfo)
		TalkAppendString(varMap," ")
			
		
					
		TalkAppendString( varMap,x300807_var_QuestTarget)
	...
	DeliverTalkInfoNM(varMap, varPlayer, varTalknpc, x300807_var_FileId, varQuest,0);
end
...
function x300807_DispatchContinueInfo( varMap, varPlayer, varTalknpc,varQuest )
	...
	StopTalkTask()
	DeliverTalkRequire(varMap, varPlayer, varTalknpc, x300807_var_FileId, varQuest,0);
...
function x300807_DispatchCompletedInfo( varMap, varPlayer, varTalknpc,varQuest )
	local varMyQuest = x300807_GetQuestId( varMap, varPlayer )
	if IsHaveQuestNM( varMap, varPlayer, varMyQuest ) <= 0 then	
		return 0
	end
	...
	DeliverTalkContinueNM(varMap, varPlayer, varTalknpc, x300807_var_FileId, varQuest);

end


function x300807_ProcAcceptCheck(varMap, varPlayer, varTalknpc, varQuest )
...
function x300807_CheckRequest(varMap, varPlayer)
	if GetGameOpenById(x300807_var_GameId)<=0 then
		x300807_ShowTips(varMap, varPlayer, "此活动未开启")
		return 0
	end
	...
	if GetLevel(varMap, varPlayer) <x300807_var_LevelLess then
		x300807_ShowTips(varMap, varPlayer, "您的等级不足，无法接取此任务")
		return 0
	end
	...
	if IsHaveQuestNM( varMap, varPlayer, varMyQuest ) > 0 then
		x300807_ShowTips(varMap, varPlayer, "您已经接取了该任务，请先完成当前任务")
		return 0;
	end


	if x300807_var_DayCountLimited >0 then
		...
		if x300807_GetDayCount(varMap, varPlayer) > 0 then
			x300807_ShowTips(varMap, varPlayer, "很抱歉，请下周再来接取本任务！")
			x300807_ResetQuestStep(varMap, varPlayer)
			return 0
		end
	end
...
function x300807_ProcQuestAccept(varMap, varPlayer, varTalknpc, varQuest)
	
	if x300807_CheckRequest(varMap, varPlayer)<=0 then
	...
	if varFullQuest>0 then
		x300807_ShowTips(varMap, varPlayer, "任务已满，无法接受任务")
		return 0
	end
	
	local varStep,varIndex,varExpBonus = x300807_GetData(varMap, varPlayer)

	local varRet = AddQuest( varMap, varPlayer, varMyQuest, x300807_var_FileId, 1, 0, 1,1)
		
	if varRet > 0 then
		local varQuestIdx = GetQuestIndexByID( varMap, varPlayer, varMyQuest )


		local nRandom = random(1,x300807_var_QuestTable2[varIndex].ncount)
		if nRandom < 1 then
			nRandom = 1
		elseif nRandom > x300807_var_QuestTable2[varIndex].ncount then
			nRandom = x300807_var_QuestTable2[varIndex].ncount
		end
		
		local varStr = "您接受了任务："..x300807_var_QuestName;
		x300807_ShowTips(varMap, varPlayer, varStr)
		Msg2Player(varMap,varPlayer,varStr,8,2)
		
		SetQuestByIndex( varMap, varPlayer, varQuestIdx, 6, nRandom )
		

		x300807_ShowSubQuest( varMap, varPlayer, varTalknpc,varQuest,nRandom )

		x300807_CheckShoujiHaveAll(varMap, varPlayer, nRandom)

		GamePlayScriptLog(varMap, varPlayer, 1331)
		return 1
	else
		
		x300807_ShowTips(varMap, varPlayer, "接受任务错误")
...
function x300807_ShowTips(varMap, varPlayer, varStr)
	StartTalkTask(varMap);
...
function x300807_ProcQuestAbandon( varMap, varPlayer, varQuest )
	...
	if varRet>0 then
		...
		local varStr = "您放弃了任务:"..x300807_var_QuestName;
	    x300807_ShowTips(varMap, varPlayer, varStr)
		Msg2Player(varMap,varPlayer,varStr,8,2)
		...
		x300807_ShowTips(varMap, varPlayer, "放弃任务失败")
	end
end


function x300807_ProcQuestSubmit( varMap, varPlayer, varTalknpc, varRadioSelected, varQuest )
...
local nLastFlag = GetPlayerGameData(varMap, varPlayer, MD_GUILD_CIRCLE_MISSION_CURRENT_ID[1], MD_GUILD_CIRCLE_MISSION_CURRENT_ID[2], MD_GUILD_CIRCLE_MISSION_CURRENT_ID[3])
	...
	if x300807_CheckSubmit(varMap, varPlayer )>0 then
		if x300807_SubmitShouji(varMap, varPlayer,varTalknpc)<=0 then
				return 0
			end
		...
		if varRet>0 then
			...
			local varStr = "您完成了任务:"..x300807_var_QuestName;
			x300807_ShowTips(varMap, varPlayer, varStr)
			Msg2Player(varMap,varPlayer,varStr,8,2)

			
			
			x300807_GiveReward(varMap,varPlayer)

			x300807_SaveQuestStep(varMap, varPlayer)

			LuaCallNoclosure( 300807, "ProcEventEntry", varMap, varPlayer, varTalknpc, x300807_var_QuestId )	
			GamePlayScriptLog(varMap, varPlayer, 1332)
			
		else
			x300807_ShowTips(varMap, varPlayer, "交任务失败")
		end
...
function x300807_SubmitShouji(varMap, varPlayer,varTalknpc)
	...
	local itemid_t = x300807_var_QuestTable2[varIndex].needitem[nRandom].varItem
	local count_t = x300807_var_QuestTable2[varIndex].needitem[nRandom].varCount
	...
	if GetItemClass(itemid_t)~=10 then
		...
		if varRet<=0 then
			x300807_ShowTips(varMap, varPlayer, "删除物品失败")
		...
		StartGCDTask( varMap)
			GCDAppendInt( varMap, x300807_var_FileId)
...
function x300807_ProcComitSubmitItem(varMap, varPlayer,varBagIdx)
...
function x300807_GiveReward(varMap,varPlayer)
...
function x300807_CheckSubmit(varMap, varPlayer )
...
function x300807_ShowSubQuest( varMap, varPlayer, varTalknpc,varQuest, nRandom )
	...
	TalkAppendString(varMap,"#Y"..x300807_var_QuestName)
		...
		local str1 = "  收集"..x300807_var_QuestTable2[varIndex].needitem[nRandom].varCount.."个#G@itemid_"..x300807_var_QuestTable2[varIndex].needitem[nRandom].varItem.."\n"
		
		TalkAppendString(varMap,x300807_var_QuestInfo2)
		TalkAppendString( varMap,"\n#Y任务目标:")
...
function x300807_QuestLogRefresh( varMap, varPlayer, varQuest)
	...
	AddQuestLogCustomText( varMap,
							"",						
							x300807_GetQuestName(varMap,varPlayer),        
							x300807_GetQuestTask(varMap,varPlayer),		
							"@npc_"..x300807_GetReplyNpcId(varMap,varPlayer),			
							x300807_GetQuestMethod(varMap,varPlayer),               
							x300807_GetQuestText(varMap,varPlayer),	
							x300807_GetQuestHelp(varMap,varPlayer)					
							)
...
function x300807_GetQuestName(varMap,varPlayer)
	
	local varStep,varIndex,varExpBonus = x300807_GetData(varMap, varPlayer)

	return x300807_var_QuestName..format("(%d/%d)",varStep,x300807_GetMaxStep(varMap,varPlayer));
end

function x300807_GetQuestTask(varMap,varPlayer)
	local varMyQuest = x300807_GetQuestId( varMap, varPlayer )
	local varQuestIdx = GetQuestIndexByID( varMap, varPlayer, varMyQuest )
	...
	local str1=""
		...
		local varId = x300807_var_QuestTable2[varIndex].needitem[nRandom].varItem
		local count_t = x300807_var_QuestTable2[varIndex].needitem[nRandom].varCount
...
function x300807_GetReplyNpcId(varMap,varPlayer)
	return x300807_var_ReplyNpcId;
end

function x300807_GetQuestMethod(varMap,varPlayer)
...
function x300807_GetQuestText(varMap,varPlayer)
	
	

	return x300807_var_QuestInfo
end

function x300807_GetQuestHelp(varMap,varPlayer)
	local varStep,varIndex,varExpBonus = x300807_GetData(varMap, varPlayer)
	
		return x300807_var_QuestHelp2
	
end



function x300807_ProcQuestLogRefresh( varMap, varPlayer, varQuest)
	
	x300807_QuestLogRefresh( varMap, varPlayer, varQuest );
end


function x300807_ProcQuestAttach( varMap, varPlayer, varTalknpc, varTalkNpcGUID,varQuestIdx, varQuest )
...
function x300807_GetDayCount(varMap, varPlayer)
...
function x300807_SetDayCount(varMap, varPlayer)
...
function x300807_PositionUseItem( varMap, varPlayer, varBagIdx, varImpact )
	return 1
end
...
function x300807_ProcQuestItemChanged( varMap, varPlayer, varItemData, varQuest, nChangeType, varCount)
	...
	x300807_QuestLogRefresh( varMap, varPlayer, varMyQuest );

    
	
end

function x300807_CheckShoujiHaveAll(varMap, varPlayer, nRandom)
	local varStep,varIndex,varExpBonus = x300807_GetData(varMap, varPlayer)


	local itemid_t = x300807_var_QuestTable2[varIndex].needitem[nRandom].varItem
	x300807_ProcQuestItemChanged( varMap, varPlayer,itemid_t, -1, -1, -1)
	
end

function x300807_ProcSelectSubmitItem( varMap, varPlayer , varBagIdx, nGuid_Serial, nGuid_Server, nGuid_World)
	...
	if (CheckItemMatchInBag( varMap, varPlayer , varBagIdx, itemid_t,nGuid_Serial, nGuid_Server, nGuid_World))>0 then  
		x300807_ProcComitSubmitItem(varMap, varPlayer,varBagIdx)
	end
end



function x300807_ProcLeaveGuild(varMap, varPlayer, varQuest)
...
function x300807_ProcQuestObjectKilled(varMap, varPlayer, varObjData, varObj, varQuest)
end
...
-- 检测当前服务器是否开放
--------------------------------------------------------------
function x300807_CheckMissonToggle( varMap, varPlayer )
	local nToggle = 0		
		for i, item in x300807_ChangleToggleWorld do 
