...
x300857_var_TimerIndex				=	6
x300857_var_Shoubian_FileId			=	300860
x300857_var_Quest_Id				=	7569





function x300857_GetQuestId( varMap, varPlayer )
	
	return x300857_var_Quest_Id
end


function x300857_ProcIntervalOverEvent( varMap, varPlayer )
	local varCountry = GetCurCountry( varMap, varPlayer )

	local nMyQuestId = x300857_GetQuestId( varMap, varPlayer )
	...
	if varCountry==0 then
		if varMap==51 then
			if x300857_CheckPlayerInArea(varMap, varPlayer)>0 then
				x300857_AddShoubianBonus(varMap, varPlayer)
			end
		end
	end
...
function x300857_ProcActiveEvent( varMap, varPlayer )
	local nMyQuestId = x300857_GetQuestId( varMap, varPlayer )
...
function x300857_ProcFadeOutEvent( varMap, varPlayer )

	local varMyQuest = x300857_GetQuestId( varMap, varPlayer )
	...
	LuaCallNoclosure(x300857_var_Shoubian_FileId,"QuestLogRefresh",varMap, varPlayer, varMyQuest)
end
...
function x300857_CheckPlayerInArea(varMap, varPlayer)
	local varX,z =GetWorldPos(varMap,varPlayer)

	if varX>x300857_var_x1 and varX<x300857_var_x2 and z>x300857_var_z1 and z<x300857_var_z2 then
		return 1;
	end
...
function x300857_AddShoubianBonus(varMap, varPlayer)
	local varLevel = GetLevel(varMap, varPlayer)
	local varDayCount = LuaCallNoclosure(x300857_var_Shoubian_FileId,"GetDayCount",varMap, varPlayer)
...
function x300857_ProcessQuestFail(varMap, varPlayer, varQuest)
end




function x300857_GetDayCount(varMap, varPlayer)
