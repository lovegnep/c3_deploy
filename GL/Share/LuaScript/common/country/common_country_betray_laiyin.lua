x300821_var_FileId	= 300821
x300821_var_curCountryID = 1
x300821_var_GameOpenId = 1098
x300821_var_CountryTransPos = {
...
x300821_var_Interval = 30 --叛国的周期间隔
x300821_var_DynamicBetrayLevelLimit = 70 --动态叛国的等级限制
x300821_var_CountryId = 1 --国家Id

function x300821_ProcEnumEvent( varMap, varPlayer, varTalknpc, varQuest )
end


function x300821_ProcEventEntry( varMap, varPlayer, varTalknpc)		
	...
	StartTalkTask(varMap)
		...
		local Readme_5 ="#G6.您在组队状态下。#r7.您必须完成或放弃身上的所有任务。#r8.您必须解除夫妻或师徒关系。#r9.距离上次改变国籍时间小于" .. x300821_var_Interval .. "天。"
	...
	DeliverTalkInfo( varMap, varPlayer, varTalknpc, 300821, -1 );
...
function x300821_ProcAcceptCheck( varMap, varPlayer, varTalknpc )
	...
	local diffDay = x300821_GetDiffDay(varLastday,curDay)
	if diffDay < x300821_var_Interval then
		StartTalkTask(varMap)
		local varText = "很抱歉，放弃国籍" .. x300821_var_Interval .. "天内不能再次放弃国籍，请".. x300821_var_Interval - diffDay .."天后再来。"
		TalkAppendString( varMap, varText )
		StopTalkTask( varMap )
		DeliverTalkTips( varMap, varPlayer )
		return
...
function x300821_ProcAccept( varMap, varPlayer )
	...
	--玩法控制开关
	if x300821_var_GameOpenId > 0 and GetGameOpenById( x300821_var_GameOpenId ) <= 0 then
      return
  end
	
	local nCost = x300821_GetBetrayCost(varMap, varPlayer)
	if x300821_CostMoney(varMap, varPlayer, nCost ) == 0 then
		StartTalkTask(varMap)
	...
	SendCountryBetrayMsg( varMap, varPlayer, x300821_var_curCountryID, x300821_var_FileId )
...
function x300821_ProcCountryBetrayResult( varMap, varPlayer, result )
	...
	StartTalkTask(varMap)
	local varText = "恭喜，您已顺利改变国籍为"..GetCountryName(x300821_var_curCountryID)
	TalkAppendString( varMap, varText )
	StopTalkTask( varMap )
	DeliverTalkTips( varMap, varPlayer )	
	...
	local newSceneId 	= x300821_var_CountryTransPos[x300821_var_curCountryID].varMap
	local newPosX		= x300821_var_CountryTransPos[x300821_var_curCountryID].posX
	local newPosZ		= x300821_var_CountryTransPos[x300821_var_curCountryID].posZ
	SetPlayerNextSceneInfo(varMap, varPlayer, newSceneId, newPosX, newPosZ)
	NewWorld(varMap, varPlayer, newSceneId, newPosX, newPosZ, x300821_var_FileId)
...
function x300821_ProcQuestAbandon( varMap, varPlayer, varQuest )
end



function x300821_ProcContinue( varMap, varPlayer, varTalknpc )
end



function x300821_CheckSubmit( varMap, varPlayer )
end



function x300821_ProcQuestSubmit( varMap, varPlayer, varTalknpc,varRadioSelected, varQuest )
end



function x300821_ProcQuestObjectKilled( varMap, varPlayer, varObjData, varObj, varQuest )
end



function x300821_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
end



function x300821_ProcQuestItemChanged( varMap, varPlayer, varItemData, varQuest )
end

--验证动态叛国条件
function x300821_CheckDynamicBetrayCondition()
	local nWorldId = GetWorldIdEx()
	local level = GetTopListInfo_MaxLevel(nWorldId)
	WriteLog(1, format("x300821_CheckDynamicBetrayCondition --- LevelToplist max level = %d", level))
	if (level >= x300821_var_DynamicBetrayLevelLimit) then
		return 1
	end
	return 0
end

function x300821_GetBetrayCost(varMap, varPlayer)

	local nRank = GetCountryRank(x300821_var_curCountryID)
	local nCost = 0
	...
	local result = x300821_CheckDynamicBetrayCondition()
	WriteLog(1, format("x300821_GetBetrayCost --- result = %d", result))
	
	if (result == 1) then  --判断是否满足动态叛国的条件
		local total = x300821_GetStrongWeakTotal()
		...
		if (playerCountry ~= -1) then
			...
			local betrayStrongWeak = GetStrongWeakScore(x300821_var_CountryId) --判去的国家
			WriteLog(1, format("x300821_GetBetrayCost --- playerCountryStrongWeak = %d, total = %d, betrayStrongWeak = %d"
...
--获取所有国家强弱的总值
function x300821_GetStrongWeakTotal()
	local total = 0
	for i = 0, 3 do
...
function x300821_CostMoney( varMap, varPlayer, value )
...
function x300821_GetDiffDay( varLastday, curDay )
	local diffDay = 0;
