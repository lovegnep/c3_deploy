...
x300900_var_FileId = 300900
x300900_var_QuestName = "【国家】敌国除奸"

x300900_var_DayCountLimited = 1

x300900_var_SceneIdList = {51,151,251,351}
function x300900_ProcEnumEvent( varMap, varPlayer, varTalknpc, varIndex )
	local varCountry = GetCurCountry( varMap, varPlayer )
	local Level = GetLevel(varMap, varPlayer)
	if Level >= 40 then
		if varCountry<=3 and varCountry>=0 then
			if varMap == x300900_var_SceneIdList[varCountry+1] then
		
				TalkAppendButton(varMap,x300900_var_FileId, x300900_var_QuestName, 8)
			end
		end
	end
end

function x300900_ProcEventEntry( varMap , varPlayer , varTalknpc , varState , varIndex )
	local varCountry = GetCurCountry( varMap, varPlayer )
		...
		StartTalkTask(varMap)
			...
			local varScript = 300888

			local varQuestId1 = 7675
			local varQuestId2 = 7678
			local varQuestId3 = 7681
			local varQuestId4 = 7684
			...
			local questNewId1 	= 7850
			local questNewId2 	= 7851
			local questNewId3 	= 7852
			local questNewId4 	= 7853
			...
			local nDayCount1 = x300900_GetDayCount(varMap, varPlayer, DQ_CITAN_LOULAN_DATE,DQ_CITAN_LOULAN_DAYCOUNT)
			local nDayCount2 = x300900_GetDayCount(varMap, varPlayer, DQ_CITAN_LAIYIN_DATE,DQ_CITAN_LAIYIN_DAYCOUNT)
			local nDayCount3 = x300900_GetDayCount(varMap, varPlayer, DQ_CITAN_KUNLUN_DATE,DQ_CITAN_KUNLUN_DAYCOUNT)
			local nDayCount4 = x300900_GetDayCount(varMap, varPlayer, DQ_CITAN_DUNHUANG_DATE,DQ_CITAN_DUNHUANG_DAYCOUNT)

			if varCountry == 0 then
				if nDayCount2>=x300900_var_DayCountLimited and nDayCount3>=x300900_var_DayCountLimited and nDayCount4>=x300900_var_DayCountLimited then
					StartTalkTask(varMap);
			...
			elseif varCountry == 1 then
				if nDayCount1>=x300900_var_DayCountLimited and nDayCount3>=x300900_var_DayCountLimited and nDayCount4>=x300900_var_DayCountLimited then
					StartTalkTask(varMap);
						TalkAppendString(varMap, "很抱歉，您已经接取了三个国家的任务，请明日再来！");
					StopTalkTask();
					DeliverTalkTips(varMap, varPlayer);
			...
			elseif varCountry == 2 then
				if nDayCount1>=x300900_var_DayCountLimited and nDayCount2>=x300900_var_DayCountLimited and nDayCount4>=x300900_var_DayCountLimited then
					StartTalkTask(varMap);
						TalkAppendString(varMap, "很抱歉，您已经接取了三个国家的任务，请明日再来！");
					StopTalkTask();
					DeliverTalkTips(varMap, varPlayer);
			...
			elseif varCountry == 3 then
				if nDayCount1>=x300900_var_DayCountLimited and nDayCount2>=x300900_var_DayCountLimited and nDayCount3>=x300900_var_DayCountLimited then
					StartTalkTask(varMap);
						TalkAppendString(varMap, "很抱歉，您已经接取了三个国家的任务，请明日再来！");
					StopTalkTask();
					DeliverTalkTips(varMap, varPlayer);
			...
			if varCountry == 0 then
				if nDayCount2<x300900_var_DayCountLimited then
				...
				if nDayCount3<x300900_var_DayCountLimited then
				...
				if nDayCount4<x300900_var_DayCountLimited then	
			...
			elseif varCountry == 1 then
				if nDayCount1<x300900_var_DayCountLimited then
...
function x300900_GetDayCount(varMap, varPlayer, nDayIndex, nDayCountIndex)
