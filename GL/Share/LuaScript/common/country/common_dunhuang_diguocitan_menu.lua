...
x300892_var_FileId = 300892
x300892_var_QuestName = "【国家】敌国除奸"

x300892_var_DayCountLimited = 1

x300892_var_SceneIdList = {51,151,251,351}
function x300892_ProcEnumEvent( varMap, varPlayer, varTalknpc, varIndex )
	local varCountry = GetCurCountry( varMap, varPlayer )
	local Level = GetLevel(varMap, varPlayer)
	if Level >= 40 then
		if varCountry<=3 and varCountry>=0 then
			if varMap == x300892_var_SceneIdList[varCountry+1] then
		
				TalkAppendButton(varMap,x300892_var_FileId, x300892_var_QuestName, 8)
			end
		end
	end
end

function x300892_ProcEventEntry( varMap , varPlayer , varTalknpc , varState , varIndex )
	local varCountry = GetCurCountry( varMap, varPlayer )
		...
		StartTalkTask(varMap)
			...
			local varScript = 300880

			local varQuestId1 = 7651
			local varQuestId2 = 7654
			local varQuestId3 = 7657
			local varQuestId4 = 7660
			...
			local questNewId1 	= 7866
			local questNewId2 	= 7867
			local questNewId3 	= 7868
			local questNewId4 	= 7869
			...
			local nDayCount1 = x300892_GetDayCount(varMap, varPlayer, DQ_CITAN_LOULAN_DATE,DQ_CITAN_LOULAN_DAYCOUNT)
			local nDayCount2 = x300892_GetDayCount(varMap, varPlayer, DQ_CITAN_LAIYIN_DATE,DQ_CITAN_LAIYIN_DAYCOUNT)
			local nDayCount3 = x300892_GetDayCount(varMap, varPlayer, DQ_CITAN_KUNLUN_DATE,DQ_CITAN_KUNLUN_DAYCOUNT)
			local nDayCount4 = x300892_GetDayCount(varMap, varPlayer, DQ_CITAN_DUNHUANG_DATE,DQ_CITAN_DUNHUANG_DAYCOUNT)

			if varCountry == 0 then
				if nDayCount2>=x300892_var_DayCountLimited and nDayCount3>=x300892_var_DayCountLimited and nDayCount4>=x300892_var_DayCountLimited then
					StartTalkTask(varMap);
			...
			elseif varCountry == 1 then
				if nDayCount1>=x300892_var_DayCountLimited and nDayCount3>=x300892_var_DayCountLimited and nDayCount4>=x300892_var_DayCountLimited then
					StartTalkTask(varMap);
						TalkAppendString(varMap, "很抱歉，您已经接取了三个国家的任务，请明日再来！");
					StopTalkTask();
					DeliverTalkTips(varMap, varPlayer);
			...
			elseif varCountry == 2 then
				if nDayCount1>=x300892_var_DayCountLimited and nDayCount2>=x300892_var_DayCountLimited and nDayCount4>=x300892_var_DayCountLimited then
					StartTalkTask(varMap);
						TalkAppendString(varMap, "很抱歉，您已经接取了三个国家的任务，请明日再来！");
					StopTalkTask();
					DeliverTalkTips(varMap, varPlayer);
			...
			elseif varCountry == 3 then
				if nDayCount1>=x300892_var_DayCountLimited and nDayCount2>=x300892_var_DayCountLimited and nDayCount3>=x300892_var_DayCountLimited then
					StartTalkTask(varMap);
						TalkAppendString(varMap, "很抱歉，您已经接取了三个国家的任务，请明日再来！");
					StopTalkTask();
					DeliverTalkTips(varMap, varPlayer);
			...
			if varCountry == 0 then
				if nDayCount2<x300892_var_DayCountLimited then
				...
				if nDayCount3<x300892_var_DayCountLimited then
				...
				if nDayCount4<x300892_var_DayCountLimited then	
			...
			elseif varCountry == 1 then
				if nDayCount1<x300892_var_DayCountLimited then
...
function x300892_GetDayCount(varMap, varPlayer, nDayIndex, nDayCountIndex)
