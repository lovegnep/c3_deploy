...
x300678_var_FileId = 300678
x300678_var_QuestName = "【帮会】帮会跑商"
x300678_var_QuestId = 7028

x300678_var_Min_Level = 20

x300678_var_SubmitParamIndex = 0
x300678_var_SubmitMoney = 10000
...
x300678_var_MoneyBuff = 8353
x300678_gameopenid = 1029


function x300678_ProcEnumEvent( varMap, varPlayer, varTalknpc, varQuest )
	...
	if GetGuildID(varMap, varPlayer) ~= -1 then
		...
		TalkAppendButton(varMap, varQuest, x300678_var_QuestName, varState, -1)
	end
end

function x300678_DispatchQuestInfo( varMap, varPlayer, varTalknpc )
...
function x300678_ProcEventEntry( varMap, varPlayer, varTalknpc, varQuest )
	...
	if IsHaveQuestNM( varMap, varPlayer, varQuest ) == 1 then	
	 	if GetGuildID(varMap, varPlayer) == -1 then
			...
			StopTalkTask()
			DeliverTalkRequire(varMap, varPlayer, varTalknpc, x300678_var_FileId, varQuest,0);
			return
		end	
		...
		if GetQuestParam( varMap, varPlayer, varQuestIdx, 6) == 0 then	
			...
			if x300678_IsQuestCompleted( varMap, varPlayer, varQuest ) == 1 then 
				StartTalkTask(varMap)
				...
				StopTalkTask()
				DeliverTalkContinueNM(varMap, varPlayer, varTalknpc, x300678_var_FileId, varQuest);
			else
				StartTalkTask(varMap)
				TalkAppendString(varMap,"#Y【帮会】帮会跑商")
	...
	GetGuildQuestData(varMap, varPlayer, GD_GUILD_INDEX_GET_MONEY_COUNT, x300678_var_FileId, varQuest, "ProcAcceptQuest", varTalknpc );
	
end

function x300678_ProcAcceptQuest(varMap, varPlayer,QuestData,varQuest,varTalknpc)
	...
	StopTalkTask()
	DeliverTalkInfoNM(varMap, varPlayer, varTalknpc, x300678_var_FileId, varQuest);
	
end

function x300678_ProcQuestAccept( varMap, varPlayer, varTalknpc, varQuest )
	if GetGameOpenById(x300678_gameopenid)<=0 then
		return
	end
	...
	if GetLevel(varMap, varPlayer) < x300678_var_Min_Level then
		StartTalkTask(varMap)
	...
	x300678_DelQuestItems(varMap, varPlayer)
	if x300678_AddQuestItems(varMap, varPlayer) == 0 then
		StartTalkTask(varMap)
	...
	AddQuestNM(varMap, varPlayer, x300678_var_QuestId);
	
	
	local varQuestIdx = GetQuestIndexByID( varMap, varPlayer, varQuest)
	SetQuestParam(varMap, varPlayer, varQuestIdx, 0, 0)
	
	local varStr = format("您领取了任务：%s", x300678_var_QuestName)
	...
	x300678_ProcQuestLogRefresh(varMap, varPlayer, varQuest)
...
function x300678_ProcQuestSubmit( varMap, varPlayer, varTalknpc, varButtonClick, varQuest )
	...
	if HaveItem(varMap, varPlayer,x300678_Item_Money) <= 0 then
		StartTalkTask(varMap)
	...
	GetGuildQuestData(varMap, varPlayer, GD_GUILD_INDEX_GET_MONEY_COUNT, x300678_var_FileId, varQuest, "ProcQuestComplete" );
	
end


function x300678_ProcQuestComplete(varMap, varPlayer,QuestData,varQuest,varTalknpc)
	...
	if QuestData >= x300678_Max_Count-1 then
	...
	local message = format("您完成了任务：%s", x300678_var_QuestName);
	StartTalkTask(varMap);
	TalkAppendString(varMap, message);
	StopTalkTask();
	DeliverTalkTips(varMap, varPlayer);
	...
	x300678_GiveBonus(varMap,varPlayer,99 - QuestData)
...
function x300678_GiveBonus(varMap,varPlayer,varCount)
...
function x300678_ProcQuestAttach( varMap, varPlayer, varTalknpc, varTalkNpcGUID,varQuestIdx, varQuest )
	if IsHaveQuestNM( varMap, varPlayer, varQuest) == 0 then	
		return
	end
end

function x300678_ProcQuestAbandon( varMap, varPlayer, varQuest )
	
	DelQuestNM( varMap, varPlayer, varQuest)
	x300678_DelQuestItems(varMap, varPlayer)
	
	local message = format("您放弃了任务：%s", x300678_var_QuestName);
	StartTalkTask(varMap);
	TalkAppendString(varMap, message);
	StopTalkTask();
	DeliverTalkTips(varMap, varPlayer);
...
function x300678_ProcQuestLogRefresh( varMap, varPlayer, varQuest)

	local varQuestIdx = GetQuestIndexByID( varMap, varPlayer, varQuest)
	local nMoney = GetQuestParam( varMap, varPlayer, varQuestIdx, x300678_var_SubmitParamIndex)
	local strTarget = "  在@npc_129181，@npc_129182，@npc_129180处，出售帮会货物赚取足够金钱("..nMoney.."/"..x300678_var_SubmitMoney..")";
...
function x300678_IsQuestCompleted(varMap, varPlayer, varQuest)

	local varQuestIdx = GetQuestIndexByID( varMap, varPlayer, varQuest)
	local nMoney = GetQuestParam( varMap, varPlayer, varQuestIdx, x300678_var_SubmitParamIndex)
	if nMoney > x300678_var_SubmitMoney then
		return 1
	else
		return 0
	end
...
function x300678_AddQuestItems(varMap, varPlayer)
	...
	ItemAppend(varMap, x300678_Item_Money, 1)
...
function x300678_DelQuestItems(varMap, varPlayer)
	local varI = 0
	for varI, item in x300678_Item_Goods do
	...
	if HaveItem(varMap,varPlayer,x300678_Item_Money) > 0 then
		if DelItem(varMap,varPlayer,x300678_Item_Money,1) ~= 1 then return 0 end
	end
end


function x300678_ProcLeaveGuild(varMap, varPlayer, varQuest)

	DelQuestNM( varMap, varPlayer, varQuest)
	x300678_DelQuestItems(varMap, varPlayer)
	
	local message = format("您已经不在帮会中了，删除了任务：%s", x300678_var_QuestName);
	StartTalkTask(varMap);
	TalkAppendString(varMap, message);
	StopTalkTask();
	DeliverTalkTips(varMap, varPlayer);
