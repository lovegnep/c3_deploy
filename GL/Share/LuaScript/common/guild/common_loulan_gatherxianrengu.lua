...
x300827_var_FileId		=	300827
x300827_var_QuestName	=	"【帮会】丹木"
x300827_var_QuestTarget	=	"　收集#G丹木#W"
x300827_var_QuestInfo	=	"\t随着帮会的蓬勃发展，我们需要更高级的木材来满足日益增长的需求。生长在#R中立地图#W的丹木是一种神奇的木头，终年不见阳光却能生长茂盛，传说仙人们都拿它来制作酒器。在仙人谷的二层，三层，四层和五层，生长着这种神奇的丹木。那里充斥着重重危险，你愿意进入谷内带回五个丹木么？当帮主发布收集令时2小时内回复任务可获得翻倍奖励（经验，帮贡）和2点使命点！"
x300827_var_QuestHelp	=	"\t#G丹木#W只生长在地下#R中立地图#W@npc_37000，@npc_38000，@npc_39000。并且数量稀少。随着你所在帮会等级的提高，你获得的奖励也会相应增加。"
x300827_var_QuestRuse	=	"\t当你到达#R中立地图#W，进行收集#G丹木#W时，如果受到攻击，将无法收集完成。你可以进入角落，将周围的怪物杀死后，再进行收集。#R中立地图#W的怪物比较密集，您最好好组队进行收集！"
x300827_var_QuestCompleted = "我需要的丹木，你已经找到了？太感谢了，辛苦了，这是给你的酬谢。"
x300827_var_SubmitNPCGUID	=	123723
x300827_var_GroupPoint	=	{ {type=10,varId =124,target = "丹木"}}

x300827_var_NPCName		= "@npc_123723"

x300827_var_ExtTarget		=	{ {type=20,n=10,target="马车"} }
x300827_var_LevelLess		= 	40 

--DECLARE_QUEST_INFO_STOP--
x300827_var_BonusMoney8	=   50
x300827_var_GuildExpBonus =	1

x300827_var_Mis_Count		= 1
x300827_var_Leader_Index	= 5
x300827_var_PreQuestId	= 7006


function x300827_IsHavePreQuest(varMap, varPlayer)
	local QuestId1  = 7006 
...
function x300827_ProcEnumEvent( varMap, varPlayer, varTalknpc, varQuest )
	if x300827_IsHavePreQuest(varMap, varPlayer) > 0 then
		return 0
	end
	...
	if varLevel < 40  then
  	...
  	if IsHaveQuestNM( varMap, varPlayer, varQuest ) == 0 then	
		...
		TalkAppendButton( varMap, varQuest, x300827_var_QuestName ,8,varState)	
...
function x300827_ProcEventEntry( varMap, varPlayer, varTalknpc, varQuest )

	
		if IsHaveQuestNM( varMap, varPlayer, varQuest ) > 0 then	
			local varDone = x300827_CheckSubmit(varMap, varPlayer,varTalknpc, varQuest);
			if(varDone > 0) then
				x300827_DispatchCompletedInfo( varMap, varPlayer, varTalknpc,varQuest) 
			else
				x300827_DispatchContinueInfo( varMap, varPlayer, varTalknpc,varQuest ) 
			end
		else
			x300827_DispatchQuestInfo( varMap, varPlayer, varTalknpc,varQuest )		
		end
...
function x300827_CheckSubmit(varMap, varPlayer,varTalknpc,varQuest)

	local varQuestIdx = GetQuestIndexByID(varMap, varPlayer, varQuest);
	local varState = GetQuestStateNM(varMap,varPlayer,varTalknpc,varQuest)
	TalkAppendButton( varMap, varQuest,x300827_var_QuestName, 7,varState ) 
...
function x300827_DispatchCompletedInfo( varMap, varPlayer, varTalknpc,varQuest )
	
	StartTalkTask(varMap)

		
		TalkAppendString(varMap,"#Y"..x300827_var_QuestName)
		TalkAppendString(varMap,x300827_var_QuestCompleted)
		...
		if x300827_var_BonusMoney8>0 then
			AddQuestMoneyBonus8(varMap,x300827_var_BonusMoney8)
		end
	...
	StopTalkTask(varMap)
	DeliverTalkContinueNM(varMap, varPlayer, varTalknpc, x300827_var_FileId, varQuest);
...
function x300827_DispatchContinueInfo( varMap, varPlayer, varTalknpc,varQuest )
	
	StartTalkTask(varMap)

		
		TalkAppendString(varMap,"#Y"..x300827_var_QuestName)
		TalkAppendString(varMap,format("%s", x300827_var_QuestInfo))
		TalkAppendString(varMap," ")
	...
	StopTalkTask(varMap)
	DeliverTalkRequire(varMap, varPlayer, varTalknpc, x300827_var_FileId, varQuest,0);
...
function x300827_DispatchQuestInfo( varMap, varPlayer, varTalknpc,varQuest )
	...
	StartTalkTask(varMap)
		...
		TalkAppendString( varMap,format("%s", x300827_var_QuestTarget))
		TalkAppendString( varMap," ")

		
		if x300827_var_QuestHelp ~= "" then
			TalkAppendString(varMap,"#Y任务提示：")
			TalkAppendString(varMap,format("%s", x300827_var_QuestHelp))
			TalkAppendString(varMap," ")
		end
	...
	StopTalkTask(varMap)
	DeliverTalkInfoNM(varMap, varPlayer, varTalknpc, x300827_var_FileId, varQuest);
end
...
function x300827_ProcReturn1(varMap, varPlayer,QuestData,varQuest,varTalknpc)
	...
	x300827_GetBonus( varMap,varPlayer, isMultiple )
end

function x300827_GetBonus( varMap, varPlayer, isMultiple )
    ...
    if x300827_var_BonusMoney8 > 0 then
		    AddGuildUserPoint(varMap,varPlayer,x300827_var_BonusMoney8)	
			local BonusMoney8 = x300827_var_BonusMoney8
			StartTalkTask(varMap)
			TalkAppendString(varMap, "获得#R帮贡"..BonusMoney8.."点#o的奖励");
			StopTalkTask(varMap)
			DeliverTalkTips(varMap,varPlayer)	
			Msg2Player(varMap,varPlayer,format("获得#R帮贡"..BonusMoney8.."点#o的奖励", x300827_var_BonusMoney8),8,2)
			if isMultiple == 1 then
			local pos = GetGuildOfficial(varMap, varPlayer)
				if pos == 5 then
					local guildid =GetGuildID(varMap, varPlayer)
				...
				elseif pos == 4 then
					...
					StopTalkTask(varMap)
					DeliverTalkTips(varMap,varPlayer)	
					Msg2Player(varMap,varPlayer,format("帮会多倍任务额外获得#R帮贡"..BonusMoney8.."点#o的奖励", x300827_var_BonusMoney8),8,2)
				end
			end
	end
...
function x300827_ProcQuestAccept( varMap, varPlayer, varTalknpc, varQuest )
	...
	return 
 	...
 	local varLevel = GetLevel(varMap,varPlayer)
	if varLevel < x300827_var_LevelLess then
		StartTalkTask(varMap)
	...
	if x300827_GetDayCount(varMap, varPlayer) >= x300827_var_Mis_Count then
		StartTalkTask(varMap)
...
function x300827_ProcQuestSubmit( varMap, varPlayer, varTalknpc, varButtonClick, varQuest )
	...
	x300827_SetDayCount(varMap, varPlayer)
	GetGuildQuestData(varMap, varPlayer, GD_GUILD_INDEX_MULTI_MISSION_HEIMU, x300827_var_FileId,varQuest,"ProcReturn1",varTalknpc)
...
function x300827_ProcQuestAttach( varMap, varPlayer, varTalknpc, varTalkNpcGUID,varQuestIdx, varQuest )
	
	if IsHaveQuestNM( varMap, varPlayer, varQuest) == 0 then	
		return
	end

	if varTalkNpcGUID == x300827_var_SubmitNPCGUID then
		local varState = GetQuestStateNM(varMap,varPlayer,varTalknpc,varQuest)
		TalkAppendButton(varMap, varQuest, x300827_var_QuestName,7,varState);
	end
...
function x300827_ProcQuestAbandon( varMap, varPlayer,varQuest )
...
function x300827_ProcReturn(varMap, varPlayer,QuestData,varQuest,varTalknpc)
...
function x300827_ProcAccept( varMap, varPlayer )

end


function x300827_ProcQuestItemChanged( varMap, varPlayer, varItemData, varQuest, nChangeType )
	...
	if nChangeType ==1 then
		...
		if itemNum < NeedNum then
			...
			x300827_QuestLogRefresh( varMap, varPlayer, varQuest );
		end
...
function x300827_QuestLogRefresh( varMap, varPlayer, varQuest)
	...
	if varHaveQuest > 0 then
		...
		AddQuestLogCustomText( varMap,
								"",								
								x300827_var_QuestName,			
								x300827_var_QuestTarget..text,	
								x300827_var_NPCName,				
								x300827_var_QuestRuse,			
								x300827_var_QuestInfo,			
								""								
								)
		StopTalkTask(varMap)
...
function x300827_ProcQuestLogRefresh( varMap, varPlayer, varQuest)
...
function x300827_GetDayCount(varMap, varPlayer)

	if x300827_var_Mis_Count > 0 then
...
function x300827_SetDayCount(varMap, varPlayer)
	local varToday = GetDayOfYear()
