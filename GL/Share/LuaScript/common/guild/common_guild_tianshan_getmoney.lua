...
x300677_var_FileId = 300677
x300677_var_QuestName = "【帮会】帮会跑商"
x300677_var_QuestId = 7027

x300677_var_Min_Level = 20

x300677_var_SubmitParamIndex = 0
x300677_var_SubmitMoney = 10000

x300677_Item_Goods = { 13030122, 13030123, 13030124, 13030125, 13030126, 13030127 }
x300677_Item_GoodsCount = 3
x300677_Item_Money = 13030128

x300677_Max_Count = 100
x300677_var_MoneyBuff = 8353
x300677_gameopenid = 1029


function x300677_ProcEnumEvent( varMap, varPlayer, varTalknpc, varQuest )
	
	if GetGameOpenById(x300677_gameopenid)<=0 then
		return
	end
	...
	if GetGuildID(varMap, varPlayer) ~= -1 then
		...
		TalkAppendButton(varMap, varQuest, x300677_var_QuestName, varState, -1)
	end
end

function x300677_DispatchQuestInfo( varMap, varPlayer, varTalknpc )
...
function x300677_ProcEventEntry( varMap, varPlayer, varTalknpc, varQuest )
	...
	if IsHaveQuestNM( varMap, varPlayer, varQuest ) == 1 then	
	 	if GetGuildID(varMap, varPlayer) == -1 then
			...
			StopTalkTask()
			DeliverTalkRequire(varMap, varPlayer, varTalknpc, x300677_var_FileId, varQuest,0);
			return
		end	
		...
		if GetQuestParam( varMap, varPlayer, varQuestIdx, 6) == 0 then	
			...
			if x300677_IsQuestCompleted( varMap, varPlayer, varQuest ) == 1 then 
				StartTalkTask(varMap)
				...
				StopTalkTask()
				DeliverTalkContinueNM(varMap, varPlayer, varTalknpc, x300677_var_FileId, varQuest);
			else
				StartTalkTask(varMap)
				TalkAppendString(varMap,"#Y【帮会】帮会跑商")
	...
	GetGuildQuestData(varMap, varPlayer, GD_GUILD_INDEX_GET_MONEY_COUNT, x300677_var_FileId, varQuest, "ProcAcceptQuest", varTalknpc );
	
end

function x300677_ProcAcceptQuest(varMap, varPlayer,QuestData,varQuest,varTalknpc)
	...
	StartTalkTask(varMap)
	local Readme_1 = ""		
	local Readme_2 = "\t我这里有一些帮会的物资需要出售给商人@npc_126133，@npc_126134，@npc_126135为帮会赚取金钱。另外，在王城内的神秘商人@npc_126136常会以#G高价收购所有货物#W。\n\t每次跑商需要缴纳#R5#W两银卡或现银作为押金，完成任务返还5两银卡，如果任务失败，押金将被#R没收#W！#r"
	...
	StopTalkTask()
	DeliverTalkInfoNM(varMap, varPlayer, varTalknpc, x300677_var_FileId, varQuest);
	
end

function x300677_ProcQuestAccept( varMap, varPlayer, varTalknpc, varQuest )
	if GetGameOpenById(x300677_gameopenid)<=0 then
		return
	end
	...
	if GetLevel(varMap, varPlayer) < x300677_var_Min_Level then
		StartTalkTask(varMap)
	...
	x300677_DelQuestItems(varMap, varPlayer)
	if x300677_AddQuestItems(varMap, varPlayer) == 0 then
		StartTalkTask(varMap)
	...
	AddQuestNM(varMap, varPlayer, x300677_var_QuestId);
	
	
	local varQuestIdx = GetQuestIndexByID( varMap, varPlayer, varQuest)
	SetQuestParam(varMap, varPlayer, varQuestIdx, 0, 0)
	
	local varStr = format("您领取了任务：%s", x300677_var_QuestName)
	...
	x300677_ProcQuestLogRefresh(varMap, varPlayer, varQuest)
...
function x300677_ProcQuestSubmit( varMap, varPlayer, varTalknpc, varButtonClick, varQuest )
	...
	if HaveItem(varMap, varPlayer,x300677_Item_Money) <= 0 then
		StartTalkTask(varMap)
	...
	GetGuildQuestData(varMap, varPlayer, GD_GUILD_INDEX_GET_MONEY_COUNT, x300677_var_FileId, varQuest, "ProcQuestComplete" );
	
end


function x300677_ProcQuestComplete(varMap, varPlayer,QuestData,varQuest,varTalknpc)
	...
	if QuestData >= x300677_Max_Count-1 then
	...
	local message = format("您完成了任务：%s", x300677_var_QuestName);
	StartTalkTask(varMap);
	TalkAppendString(varMap, message);
	StopTalkTask();
	DeliverTalkTips(varMap, varPlayer);
	...
	x300677_GiveBonus(varMap,varPlayer,99 - QuestData)
...
function x300677_GiveBonus(varMap,varPlayer,varCount)
...
function x300677_ProcQuestAttach( varMap, varPlayer, varTalknpc, varTalkNpcGUID,varQuestIdx, varQuest )
	if IsHaveQuestNM( varMap, varPlayer, varQuest) == 0 then	
		return
	end
end

function x300677_ProcQuestAbandon( varMap, varPlayer, varQuest )
	
	DelQuestNM( varMap, varPlayer, varQuest)
	x300677_DelQuestItems(varMap, varPlayer)
	
	local message = format("您放弃了任务：%s", x300677_var_QuestName);
	StartTalkTask(varMap);
	TalkAppendString(varMap, message);
	StopTalkTask();
	DeliverTalkTips(varMap, varPlayer);
...
function x300677_ProcQuestLogRefresh( varMap, varPlayer, varQuest)

	local varQuestIdx = GetQuestIndexByID( varMap, varPlayer, varQuest)
	local nMoney = GetQuestParam( varMap, varPlayer, varQuestIdx, x300677_var_SubmitParamIndex)
	local strTarget = "  在@npc_126134，@npc_126135，@npc_126133处，出售帮会货物赚取足够金钱("..nMoney.."/"..x300677_var_SubmitMoney..")";
	...
	AddQuestLogCustomText( varMap,
							"",                             
							x300677_var_QuestName,          
							strTarget,
							"",
                             
							"\t找到城东商人，城西商人，天山猎场商人及王城游商，将身上的帮会货物出售给他们，一定要赚取足够的金钱才能完成任务。#Y\n奖励内容：\n#W经验值："..nExp.."\n#W帮会金钱：20两",
							"\t帮会的发展离不开帮会金钱。\n\t我这里有一些帮会的物资待售，听说城东商人@npc_126134，城西商人@npc_126135，天山猎场商人@npc_126133正在收购它们。\n\t你带着这些物资，卖给商人们,为帮会赚取足够的金钱吧。\n\t值得注意的是，王城内有位神秘的商人，名叫@npc_126136，他常常会以#G高价收购所有货物#W。\n\t帮会货物共有以下种类：#G波斯金鞍，大宛马铠，高丽山参，大理灵芝，南洋珍珠和琉球珊瑚。",
...
function x300677_IsQuestCompleted(varMap, varPlayer, varQuest)

	local varQuestIdx = GetQuestIndexByID( varMap, varPlayer, varQuest)
	local nMoney = GetQuestParam( varMap, varPlayer, varQuestIdx, x300677_var_SubmitParamIndex)
	if nMoney > x300677_var_SubmitMoney then
		return 1
	else
		return 0
	end
...
function x300677_AddQuestItems(varMap, varPlayer)
	local item1, item2, item3
	local nTotalNum = getn(x300677_Item_Goods)
	item1 = x300677_Item_Goods[ random(1,nTotalNum) ]
	item2 = x300677_Item_Goods[ random(1,nTotalNum) ]
	while item2 == item1 do
		item2 = x300677_Item_Goods[ random(1,nTotalNum) ]
	end
	item3 = x300677_Item_Goods[ random(1,nTotalNum) ]
	...
	ItemAppend(varMap, x300677_Item_Money, 1)
...
function x300677_DelQuestItems(varMap, varPlayer)
	local varI = 0
	for varI, item in x300677_Item_Goods do
	...
	if HaveItem(varMap,varPlayer,x300677_Item_Money) > 0 then
		if DelItem(varMap,varPlayer,x300677_Item_Money,1) ~= 1 then return 0 end
	end
end


function x300677_ProcLeaveGuild(varMap, varPlayer, varQuest)

	DelQuestNM( varMap, varPlayer, varQuest)
	x300677_DelQuestItems(varMap, varPlayer)
	
	local message = format("您已经不在帮会中了，删除了任务：%s", x300677_var_QuestName);
	StartTalkTask(varMap);
	TalkAppendString(varMap, message);
	StopTalkTask();
	DeliverTalkTips(varMap, varPlayer);
