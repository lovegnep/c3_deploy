...
x490033_var_FileId = 490033
x490033_var_TransPosloulan  ="dadumigong3todadumigong2"
x490033_var_TransPoslaiyin  ="dadumigong3todadumigong2"
x490033_var_TransPoskunlun  ="dadumigong3todadumigong2"
x490033_var_TransPosdunhuang="dadumigong3todadumigong2"
x490033_var_Notice = "您的等级还不足#R70级#Y，无法前往仙人谷二层！"


function x490033_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel <70 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490033_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490033_TransToPosition(varMap, varObj)
			end
	...
	x490033_TransToPosition(varMap, varPlayer)
end

function x490033_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490033_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490033_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490033_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490033_var_TransPosdunhuang)
	
	end
end


function x490033_ProcMapTimerTick( varMap, monsterobjid )

end


function x490033_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490033_ProcEventEntry( varMap, monsterobjid )
