...
x490026_var_FileId = 490026
x490026_var_TransPosloulan  ="dadumigong1todadumigong2"
x490026_var_TransPoslaiyin  ="dadumigong1todadumigong2"
x490026_var_TransPoskunlun  ="dadumigong1todadumigong2"
x490026_var_TransPosdunhuang="dadumigong1todadumigong2"
x490026_var_Notice = "您的等级还不足#R70级#Y，无法前往大都·后花园秘洞！"


function x490026_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel <70 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490026_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490026_TransToPosition(varMap, varObj)
			end
	...
	x490026_TransToPosition(varMap, varPlayer)
end

function x490026_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490026_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490026_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490026_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490026_var_TransPosdunhuang)
	
	end
end


function x490026_ProcMapTimerTick( varMap, monsterobjid )

end


function x490026_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490026_ProcEventEntry( varMap, monsterobjid )
