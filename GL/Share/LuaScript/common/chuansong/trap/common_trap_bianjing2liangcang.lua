...
x550106_var_FileId = 550106
x550106_var_TransPosLou="dixialiangcang"
x550106_var_TransPosLai="dixialiangcang"
x550106_var_TransPosKun="dixialiangcang"
x550106_var_TransPosDun="dixialiangcang"
x550106_var_Notice = "您的等级还不足#R20级#Y，无法前往地下粮仓！"



function x550106_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
			...
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x550106_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x550106_TransToPosition(varMap, varObj)
			end
	...
	x550106_TransToPosition(varMap, varPlayer)
end

function x550106_TransToPosition(varMap, varPlayer)

	local varCountry = GetCurCountry(varMap,varPlayer)
	
	if varMap == 51 and varCountry == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550106_var_TransPosLou)
	elseif varMap == 151 and varCountry == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550106_var_TransPosLai)
	elseif varMap == 251 and varCountry == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550106_var_TransPosKun)
	elseif varMap == 351 and varCountry == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550106_var_TransPosDun)
	else
		StartTalkTask(varMap)
...
function x550106_ProcMapTimerTick( varMap, monsterobjid )

end


function x550106_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x550106_ProcEventEntry( varMap, monsterobjid )
