...
x490470_var_FileId = 490470
x490470_var_TransPosloulan  ="shabaketominju11"
x490470_var_TransPoslaiyin  ="shabaketominju11"
x490470_var_TransPoskunlun  ="shabaketominju11"
x490470_var_TransPosdunhuang="shabaketominju11"


function x490470_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490470_TransToPosition(varMap, varObj)
			end
	...
	x490470_TransToPosition(varMap, varPlayer)
end

function x490470_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490470_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490470_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490470_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490470_var_TransPosdunhuang)
	
	end
end


function x490470_ProcMapTimerTick( varMap, monsterobjid )

end


function x490470_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490470_ProcEventEntry( varMap, monsterobjid )
