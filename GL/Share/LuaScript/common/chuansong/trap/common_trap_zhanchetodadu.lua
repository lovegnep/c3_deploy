...
x490038_var_FileId = 490038
x490038_var_TransPosloulan  ="zhanchegongchangtodadu"
x490038_var_TransPoslaiyin  ="zhanchegongchangtodadu"
x490038_var_TransPoskunlun  ="zhanchegongchangtodadu"
x490038_var_TransPosdunhuang="zhanchegongchangtodadu"


function x490038_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490038_TransToPosition(varMap, varObj)
			end
	...
	x490038_TransToPosition(varMap, varPlayer)
end

function x490038_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490038_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490038_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490038_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490038_var_TransPosdunhuang)
	
	end
end


function x490038_ProcMapTimerTick( varMap, monsterobjid )

end


function x490038_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490038_ProcEventEntry( varMap, monsterobjid )
