...
x490201_var_FileId = 490201
x490201_var_TransPosloulan  ="weihaigangtoduimadao"
x490201_var_TransPoslaiyin  ="weihaigangtoduimadao"
x490201_var_TransPoskunlun  ="weihaigangtoduimadao"
x490201_var_TransPosdunhuang="weihaigangtoduimadao"
x490201_var_Notice = "您的等级还不足#R50级#Y，无法前往对马岛！"


function x490201_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 50 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490201_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490201_TransToPosition(varMap, varObj)
			end
	...
	x490201_TransToPosition(varMap, varPlayer)
end

function x490201_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490201_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490201_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490201_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490201_var_TransPosdunhuang)
	
	end
end


function x490201_ProcMapTimerTick( varMap, monsterobjid )

end


function x490201_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490201_ProcEventEntry( varMap, monsterobjid )
