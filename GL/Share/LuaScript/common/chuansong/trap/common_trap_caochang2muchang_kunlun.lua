...
x300216_var_FileId = 300216
x300216_var_TransPosloulan  ="caochang2muchangkunlun"
x300216_var_TransPoslaiyin  ="caochang2muchangkunlun"
x300216_var_TransPoskunlun  ="caochang2muchangkunlun"
x300216_var_TransPosdunhuang="caochang2muchangkunlun"
x300216_var_Notice = "您的等级还不足#R20级#Y，无法前往昆仑外城！"


function x300216_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x300216_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300216_TransToPosition(varMap, varObj)
			end
	...
	x300216_TransToPosition(varMap, varPlayer)
end

function x300216_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300216_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300216_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300216_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300216_var_TransPosdunhuang)
	
	end
end


function x300216_ProcMapTimerTick( varMap, monsterobjid )

end


function x300216_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300216_ProcEventEntry( varMap, monsterobjid )
