...
x490418_var_FileId = 490418
x490418_var_TransPosloulan  ="minju2tohouhuayuan"
x490418_var_TransPoslaiyin  ="minju2tohouhuayuan"
x490418_var_TransPoskunlun  ="minju2tohouhuayuan"
x490418_var_TransPosdunhuang="minju2tohouhuayuan"
x490418_var_Notice = "对不起，您的等级不足10级，无法前往皇宫秘道"


function x490418_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	local nLevel = GetLevel(varMap, varPlayer)
		if nLevel < 10 then 
			Msg2Player(varMap,varPlayer,"#Y"..x490418_var_Notice.."#W",4,3)
			return
		end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490418_TransToPosition(varMap, varObj)
			end
	...
	x490418_TransToPosition(varMap, varPlayer)
end

function x490418_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490418_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490418_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490418_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490418_var_TransPosdunhuang)
	
	end
end


function x490418_ProcMapTimerTick( varMap, monsterobjid )

end


function x490418_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490418_ProcEventEntry( varMap, monsterobjid )
