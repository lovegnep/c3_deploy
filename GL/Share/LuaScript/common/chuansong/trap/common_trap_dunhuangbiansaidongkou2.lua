...
x300302_var_FileId = 300302
x300302_var_TransPosloulan  ="dunhuangbiansaidongkou2"
x300302_var_TransPoslaiyin  ="dunhuangbiansaidongkou2"
x300302_var_TransPoskunlun  ="dunhuangbiansaidongkou2"
x300302_var_TransPosdunhuang="dunhuangbiansaidongkou2"
x300302_var_Notice = "您的等级还不足#R115级#Y，无法前往干坤秘洞！"


function x300302_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 115 then 
					Msg2Player(varMap,varPlayer,"#Y"..x300302_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300302_TransToPosition(varMap, varObj)
			end
	...
	x300302_TransToPosition(varMap, varPlayer)
end

function x300302_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300302_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300302_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300302_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300302_var_TransPosdunhuang)
	
	end
end


function x300302_ProcMapTimerTick( varMap, monsterobjid )

end


function x300302_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300302_ProcEventEntry( varMap, monsterobjid )
