...
x270161_var_FileId = 270161
x270161_var_TransPosloulan  ="mishione2bianjing3"
x270161_var_TransPoslaiyin  ="mishione2bianjing3"
x270161_var_TransPoskunlun  ="mishione2bianjing3"
x270161_var_TransPosdunhuang="mishione2bianjing3"
x270161_var_Notice = "您的等级还不足#R20级#Y，无法前往边塞！"


function x270161_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x270161_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x270161_TransToPosition(varMap, varObj)
			end
	...
	x270161_TransToPosition(varMap, varPlayer)
end

function x270161_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x270161_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x270161_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x270161_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x270161_var_TransPosdunhuang)
	
	end
end


function x270161_ProcMapTimerTick( varMap, monsterobjid )

end


function x270161_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x270161_ProcEventEntry( varMap, monsterobjid )
