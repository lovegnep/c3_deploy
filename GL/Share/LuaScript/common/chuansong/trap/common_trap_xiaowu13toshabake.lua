...
x490475_var_FileId = 490475
x490475_var_TransPosloulan  ="minju13toshabake"
x490475_var_TransPoslaiyin  ="minju13toshabake"
x490475_var_TransPoskunlun  ="minju13toshabake"
x490475_var_TransPosdunhuang="minju13toshabake"


function x490475_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490475_TransToPosition(varMap, varObj)
			end
	...
	x490475_TransToPosition(varMap, varPlayer)
end

function x490475_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490475_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490475_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490475_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490475_var_TransPosdunhuang)
	
	end
end


function x490475_ProcMapTimerTick( varMap, monsterobjid )

end


function x490475_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490475_ProcEventEntry( varMap, monsterobjid )
