...
x300255_var_FileId = 300255
x300255_var_TransPosloulan  ="andao2bianjingrenwu2"
x300255_var_TransPoslaiyin  ="andao2bianjingrenwu2"
x300255_var_TransPoskunlun  ="andao2bianjingrenwu2"
x300255_var_TransPosdunhuang="andao2bianjingrenwu2"
x300255_var_Notice = "您的等级还不足#R20级#Y，无法前往边塞！"


function x300255_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x300255_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300255_TransToPosition(varMap, varObj)
			end
	...
	x300255_TransToPosition(varMap, varPlayer)
end

function x300255_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300255_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300255_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300255_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300255_var_TransPosdunhuang)
	
	end
end


function x300255_ProcMapTimerTick( varMap, monsterobjid )

end


function x300255_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300255_ProcEventEntry( varMap, monsterobjid )
