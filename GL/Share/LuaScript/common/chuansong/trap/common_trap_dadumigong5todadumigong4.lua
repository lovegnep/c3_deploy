...
x490035_var_FileId = 490035
x490035_var_TransPosloulan  ="dadumigong5todadumigong4"
x490035_var_TransPoslaiyin  ="dadumigong5todadumigong4"
x490035_var_TransPoskunlun  ="dadumigong5todadumigong4"
x490035_var_TransPosdunhuang="dadumigong5todadumigong4"
x490035_var_Notice = "您的等级还不足#R70级#Y，无法前往大都·后花园秘洞！"


function x490035_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel <70 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490035_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490035_TransToPosition(varMap, varObj)
			end
	...
	x490035_TransToPosition(varMap, varPlayer)
end

function x490035_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490035_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490035_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490035_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490035_var_TransPosdunhuang)
	
	end
end


function x490035_ProcMapTimerTick( varMap, monsterobjid )

end


function x490035_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490035_ProcEventEntry( varMap, monsterobjid )
