...
x490477_var_FileId = 490477
x490477_var_TransPosloulan  ="minju14toshabake"
x490477_var_TransPoslaiyin  ="minju14toshabake"
x490477_var_TransPoskunlun  ="minju14toshabake"
x490477_var_TransPosdunhuang="minju14toshabake"


function x490477_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490477_TransToPosition(varMap, varObj)
			end
	...
	x490477_TransToPosition(varMap, varPlayer)
end

function x490477_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490477_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490477_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490477_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490477_var_TransPosdunhuang)
	
	end
end


function x490477_ProcMapTimerTick( varMap, monsterobjid )

end


function x490477_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490477_ProcEventEntry( varMap, monsterobjid )
