...
x490482_var_FileId = 490482
x490482_var_TransPosloulan  ="shabaketoximidao2"
x490482_var_TransPoslaiyin  ="shabaketoximidao2"
x490482_var_TransPoskunlun  ="shabaketoximidao2"
x490482_var_TransPosdunhuang="shabaketoximidao2"


function x490482_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490482_TransToPosition(varMap, varObj)
			end
	...
	x490482_TransToPosition(varMap, varPlayer)
end

function x490482_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490482_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490482_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490482_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490482_var_TransPosdunhuang)
	
	end
end


function x490482_ProcMapTimerTick( varMap, monsterobjid )

end


function x490482_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490482_ProcEventEntry( varMap, monsterobjid )
