...
x490009_var_FileId = 490009
x490009_var_TransPosloulan  ="tianshanwangchengdongkou1"
x490009_var_TransPoslaiyin  ="tianshanwangchengdongkou1"
x490009_var_TransPoskunlun  ="tianshanwangchengdongkou1"
x490009_var_TransPosdunhuang="tianshanwangchengdongkou1"
x490009_var_Notice = "您的等级还不足，无法前往通天之门！"


function x490009_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 999 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490009_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490009_TransToPosition(varMap, varObj)
			end
	...
	x490009_TransToPosition(varMap, varPlayer)
end

function x490009_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490009_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490009_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490009_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490009_var_TransPosdunhuang)
	
	end
end


function x490009_ProcMapTimerTick( varMap, monsterobjid )

end


function x490009_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490009_ProcEventEntry( varMap, monsterobjid )
