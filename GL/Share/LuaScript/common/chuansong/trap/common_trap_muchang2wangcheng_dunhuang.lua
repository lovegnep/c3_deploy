...
x300208_var_FileId = 300208
x300208_var_TransPosloulan  ="muchang2wangchengdunhuang"
x300208_var_TransPoslaiyin  ="muchang2wangchengdunhuang"
x300208_var_TransPoskunlun  ="muchang2wangchengdunhuang"
x300208_var_TransPosdunhuang="muchang2wangchengdunhuang"
x300208_var_Notice = "您的等级还不足#R10级#Y，无法前往敦煌王城！"


function x300208_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 10 then 
					Msg2Player(varMap,varPlayer,"#Y"..x300208_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300208_TransToPosition(varMap, varObj)
			end
	...
	x300208_TransToPosition(varMap, varPlayer)
end

function x300208_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300208_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300208_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300208_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300208_var_TransPosdunhuang)
	
	end
end


function x300208_ProcMapTimerTick( varMap, monsterobjid )

end


function x300208_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300208_ProcEventEntry( varMap, monsterobjid )
