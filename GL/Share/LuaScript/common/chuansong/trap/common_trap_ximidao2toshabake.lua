...
x490483_var_FileId = 490483
x490483_var_TransPosloulan  ="ximidao2toshabake"
x490483_var_TransPoslaiyin  ="ximidao2toshabake"
x490483_var_TransPoskunlun  ="ximidao2toshabake"
x490483_var_TransPosdunhuang="ximidao2toshabake"


function x490483_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490483_TransToPosition(varMap, varObj)
			end
	...
	x490483_TransToPosition(varMap, varPlayer)
end

function x490483_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490483_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490483_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490483_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490483_var_TransPosdunhuang)
	
	end
end


function x490483_ProcMapTimerTick( varMap, monsterobjid )

end


function x490483_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490483_ProcEventEntry( varMap, monsterobjid )
