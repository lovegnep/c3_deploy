...
x490459_var_FileId = 490459
x490459_var_TransPosloulan  ="minju5toshabake"
x490459_var_TransPoslaiyin  ="minju5toshabake"
x490459_var_TransPoskunlun  ="minju5toshabake"
x490459_var_TransPosdunhuang="minju5toshabake"


function x490459_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490459_TransToPosition(varMap, varObj)
			end
	...
	x490459_TransToPosition(varMap, varPlayer)
end

function x490459_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490459_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490459_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490459_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490459_var_TransPosdunhuang)
	
	end
end


function x490459_ProcMapTimerTick( varMap, monsterobjid )

end


function x490459_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490459_ProcEventEntry( varMap, monsterobjid )
