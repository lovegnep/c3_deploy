...
x300219_var_FileId = 300219
x300219_var_TransPosloulan  ="gudao2wangchengkunlun"
x300219_var_TransPoslaiyin  ="gudao2wangchengkunlun"
x300219_var_TransPoskunlun  ="gudao2wangchengkunlun"
x300219_var_TransPosdunhuang="gudao2wangchengkunlun"



function x300219_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300219_TransToPosition(varMap, varObj)
			end
	...
	x300219_TransToPosition(varMap, varPlayer)
end

function x300219_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300219_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300219_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300219_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300219_var_TransPosdunhuang)
	
	end
end


function x300219_ProcMapTimerTick( varMap, monsterobjid )

end


function x300219_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300219_ProcEventEntry( varMap, monsterobjid )
