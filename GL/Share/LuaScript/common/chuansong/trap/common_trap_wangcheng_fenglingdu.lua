...
x300461_var_FileId = 300461
x300461_var_TransPos0	=	"loulanwangcheng2fenglingdu"
x300461_var_TransPos1	=	"laiyinwangcheng2fenglingdu"
x300461_var_TransPos2	=	"kunlunwangcheng2fenglingdu"
x300461_var_TransPos3	=	"dunhuangwangcheng2fenglingdu"



function x300461_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300461_TransToPosition(varMap, varObj)
			end
	...
	x300461_TransToPosition(varMap, varPlayer)

end

function x300461_TransToPosition(varMap, varPlayer)

	local varCountry = GetCurCountry(varMap,varPlayer)

	if varMap == 0 and varCountry == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300461_var_TransPos0)
	elseif varMap == 0 and varCountry == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300461_var_TransPos1)
	elseif varMap == 0 and varCountry == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300461_var_TransPos2)
	elseif varMap == 0 and varCountry == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300461_var_TransPos3)
	else
		StartTalkTask(varMap)
...
function x300461_ProcMapTimerTick( varMap, monsterobjid )

end


function x300461_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300461_ProcEventEntry( varMap, monsterobjid )
