...
x490436_var_FileId = 490436
x490436_var_TransPosloulan  ="dunhuangtosichouzhilu"
x490436_var_TransPoslaiyin  ="dunhuangtosichouzhilu"
x490436_var_TransPoskunlun  ="dunhuangtosichouzhilu"
x490436_var_TransPosdunhuang="dunhuangtosichouzhilu"
x490436_var_Notice = "您的等级还不足#R40级#Y，无法前往敦煌丝绸之路！"

function x490436_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 40 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490436_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490436_TransToPosition(varMap, varObj)
			end
	...
	x490436_TransToPosition(varMap, varPlayer)
end

function x490436_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490436_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490436_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490436_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490436_var_TransPosdunhuang)
	
	end
end


function x490436_ProcMapTimerTick( varMap, monsterobjid )

end


function x490436_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490436_ProcEventEntry( varMap, monsterobjid )
