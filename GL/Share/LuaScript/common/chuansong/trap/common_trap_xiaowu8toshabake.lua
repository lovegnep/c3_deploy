...
x490465_var_FileId = 490465
x490465_var_TransPosloulan  ="minju8toshabake"
x490465_var_TransPoslaiyin  ="minju8toshabake"
x490465_var_TransPoskunlun  ="minju8toshabake"
x490465_var_TransPosdunhuang="minju8toshabake"


function x490465_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490465_TransToPosition(varMap, varObj)
			end
	...
	x490465_TransToPosition(varMap, varPlayer)
end

function x490465_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490465_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490465_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490465_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490465_var_TransPosdunhuang)
	
	end
end


function x490465_ProcMapTimerTick( varMap, monsterobjid )

end


function x490465_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490465_ProcEventEntry( varMap, monsterobjid )
