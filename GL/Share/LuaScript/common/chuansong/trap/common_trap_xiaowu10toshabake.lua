...
x490469_var_FileId = 490469
x490469_var_TransPosloulan  ="minju10toshabake"
x490469_var_TransPoslaiyin  ="minju10toshabake"
x490469_var_TransPoskunlun  ="minju10toshabake"
x490469_var_TransPosdunhuang="minju10toshabake"


function x490469_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490469_TransToPosition(varMap, varObj)
			end
	...
	x490469_TransToPosition(varMap, varPlayer)
end

function x490469_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490469_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490469_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490469_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490469_var_TransPosdunhuang)
	
	end
end


function x490469_ProcMapTimerTick( varMap, monsterobjid )

end


function x490469_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490469_ProcEventEntry( varMap, monsterobjid )
