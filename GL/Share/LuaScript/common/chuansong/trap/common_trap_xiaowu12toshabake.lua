...
x490473_var_FileId = 490473
x490473_var_TransPosloulan  ="minju12toshabake"
x490473_var_TransPoslaiyin  ="minju12toshabake"
x490473_var_TransPoskunlun  ="minju12toshabake"
x490473_var_TransPosdunhuang="minju12toshabake"


function x490473_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490473_TransToPosition(varMap, varObj)
			end
	...
	x490473_TransToPosition(varMap, varPlayer)
end

function x490473_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490473_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490473_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490473_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490473_var_TransPosdunhuang)
	
	end
end


function x490473_ProcMapTimerTick( varMap, monsterobjid )

end


function x490473_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490473_ProcEventEntry( varMap, monsterobjid )
