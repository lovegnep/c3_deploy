...
x490013_var_FileId = 490013
x490013_var_TransPosloulan  ="dunhuangandaodongkou1"
x490013_var_TransPoslaiyin  ="dunhuangandaodongkou1"
x490013_var_TransPoskunlun  ="dunhuangandaodongkou1"
x490013_var_TransPosdunhuang="dunhuangandaodongkou1"
x490013_var_Notice = "您的等级还不足#R125级#Y，无法前往藏宝暗室！"


function x490013_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 125 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490013_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490013_TransToPosition(varMap, varObj)
			end
	...
	x490013_TransToPosition(varMap, varPlayer)
end

function x490013_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490013_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490013_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490013_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490013_var_TransPosdunhuang)
	
	end
end


function x490013_ProcMapTimerTick( varMap, monsterobjid )

end


function x490013_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490013_ProcEventEntry( varMap, monsterobjid )
