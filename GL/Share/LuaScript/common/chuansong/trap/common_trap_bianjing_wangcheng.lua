...
x300458_var_FileId = 300458
x300458_var_TransPosLou="loulanwangcheng2bianjing"
x300458_var_TransPosLai="laiyinwangcheng2bianjing"
x300458_var_TransPosKun="kunlunwangcheng2bianjing"
x300458_var_TransPosDun="dunhuangwangcheng2bianjing"
x300458_var_Notice = "您的等级还不足#R20级#Y，无法前往王国边塞！"


function x300458_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
			...
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x300458_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300458_TransToPosition(varMap, varObj)
			end
	...
	x300458_TransToPosition(varMap, varPlayer)
end

function x300458_TransToPosition(varMap, varPlayer)
	if varMap == 73 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300458_var_TransPosLou)
	elseif varMap == 150 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300458_var_TransPosLai)
	elseif varMap == 250 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300458_var_TransPosKun)
	elseif varMap == 350 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300458_var_TransPosDun)
	end
end


function x300458_ProcMapTimerTick( varMap, monsterobjid )

end


function x300458_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300458_ProcEventEntry( varMap, monsterobjid )
