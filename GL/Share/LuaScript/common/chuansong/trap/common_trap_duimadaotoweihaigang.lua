...
x490202_var_FileId = 490202
x490202_var_TransPosloulan  ="duimadaotoweihaigang"
x490202_var_TransPoslaiyin  ="duimadaotoweihaigang"
x490202_var_TransPoskunlun  ="duimadaotoweihaigang"
x490202_var_TransPosdunhuang="duimadaotoweihaigang"
x490202_var_Notice = "你的等级不足50级！"


function x490202_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 50 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490202_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490202_TransToPosition(varMap, varObj)
			end
	...
	x490202_TransToPosition(varMap, varPlayer)
end

function x490202_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490202_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490202_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490202_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490202_var_TransPosdunhuang)
	
	end
end


function x490202_ProcMapTimerTick( varMap, monsterobjid )

end


function x490202_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490202_ProcEventEntry( varMap, monsterobjid )
