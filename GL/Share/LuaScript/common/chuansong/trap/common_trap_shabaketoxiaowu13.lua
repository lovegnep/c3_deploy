...
x490474_var_FileId = 490474
x490474_var_TransPosloulan  ="shabaketominju13"
x490474_var_TransPoslaiyin  ="shabaketominju13"
x490474_var_TransPoskunlun  ="shabaketominju13"
x490474_var_TransPosdunhuang="shabaketominju13"


function x490474_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490474_TransToPosition(varMap, varObj)
			end
	...
	x490474_TransToPosition(varMap, varPlayer)
end

function x490474_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490474_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490474_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490474_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490474_var_TransPosdunhuang)
	
	end
end


function x490474_ProcMapTimerTick( varMap, monsterobjid )

end


function x490474_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490474_ProcEventEntry( varMap, monsterobjid )
