...
x300601_var_FileId = 300601
x300601_var_TransPosloulan=		"hexi2bianjingkunlun"
x300601_var_TransPoslaiyin=		"hexi2bianjingkunlun"
x300601_var_TransPoskunlun=		"hexi2bianjingkunlun"
x300601_var_TransPosdunhuang=	"hexi2bianjingkunlun"
x300601_var_TuchengzhanQuestID = 7559



function x300601_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
local minute = GetMinOfDay()	
local weekIdx = GetWeek()
         
if minute >= 900 and minute < 1020 then
	if 	x300601_IsKunlunRuoGuo(varMap, varPlayer) > 0 then
		if x300601_IsLianMeng(varMap, varPlayer) ~= 1 then
			if x300601_IsBenGuo(varMap, varPlayer) == 0 then
				if weekIdx == 6 then
					if IsHaveQuestNM(varMap, varPlayer, x300601_var_TuchengzhanQuestID ) <= 0 then
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300601_TransToPosition(varMap, varObj)
			end
	...
	x300601_TransToPosition(varMap, varPlayer)
	
end

function x300601_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300601_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300601_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300601_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300601_var_TransPosdunhuang)
	
	end
end


function x300601_ProcMapTimerTick( varMap, monsterobjid )

end


function x300601_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300601_ProcEventEntry( varMap, monsterobjid )

end
function x300601_IsKunlunRuoGuo(varMap, varPlayer)
  local maxlevel = GetTopListInfo_MaxLevel(GetWorldID(varMap, varPlayer))
	...
	if maxlevel >= 70 then
			if kunlunscore <= allscore*0.2 then
				if x300601_IsMinGuo(varMap, varPlayer) == 1 then
		 	 		return 1
		 	 	end
			end
	end
...
function x300601_IsLianMeng(varMap, varPlayer)
...
function x300601_IsBenGuo(varMap, varPlayer)
local country = GetCurCountry(varMap, varPlayer )
 ...
 function x300601_IsMinGuo(varMap, varPlayer)
	local loulanscore   = GetCountryParam(varMap, 0, CD_LASTWEEK_STRONG_WEAK_JIFEN)
	local tianshanscore = GetCountryParam(varMap, 1, CD_LASTWEEK_STRONG_WEAK_JIFEN)
	local kunlunscore   = GetCountryParam(varMap, 2, CD_LASTWEEK_STRONG_WEAK_JIFEN)
	local dunhuangscore = GetCountryParam(varMap, 3, CD_LASTWEEK_STRONG_WEAK_JIFEN)
