...
x490432_var_FileId = 490432
x490432_var_TransPosloulan  ="tianshantosichouzhilu"
x490432_var_TransPoslaiyin  ="tianshantosichouzhilu"
x490432_var_TransPoskunlun  ="tianshantosichouzhilu"
x490432_var_TransPosdunhuang="tianshantosichouzhilu"
x490432_var_Notice = "您的等级还不足#R40级#Y，无法前往天山丝绸之路！"

function x490432_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 40 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490432_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490432_TransToPosition(varMap, varObj)
			end
	...
	x490432_TransToPosition(varMap, varPlayer)
end

function x490432_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490432_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490432_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490432_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490432_var_TransPosdunhuang)
	
	end
end


function x490432_ProcMapTimerTick( varMap, monsterobjid )

end


function x490432_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490432_ProcEventEntry( varMap, monsterobjid )
