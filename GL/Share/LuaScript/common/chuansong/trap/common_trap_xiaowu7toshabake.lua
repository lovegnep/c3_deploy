...
x490463_var_FileId = 490463
x490463_var_TransPosloulan  ="minju7toshabake"
x490463_var_TransPoslaiyin  ="minju7toshabake"
x490463_var_TransPoskunlun  ="minju7toshabake"
x490463_var_TransPosdunhuang="minju7toshabake"


function x490463_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490463_TransToPosition(varMap, varObj)
			end
	...
	x490463_TransToPosition(varMap, varPlayer)
end

function x490463_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490463_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490463_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490463_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490463_var_TransPosdunhuang)
	
	end
end


function x490463_ProcMapTimerTick( varMap, monsterobjid )

end


function x490463_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490463_ProcEventEntry( varMap, monsterobjid )
