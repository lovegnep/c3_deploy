...
x490457_var_FileId = 490457
x490457_var_TransPosloulan  ="minju4toshabake"
x490457_var_TransPoslaiyin  ="minju4toshabake"
x490457_var_TransPoskunlun  ="minju4toshabake"
x490457_var_TransPosdunhuang="minju4toshabake"


function x490457_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490457_TransToPosition(varMap, varObj)
			end
	...
	x490457_TransToPosition(varMap, varPlayer)
end

function x490457_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490457_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490457_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490457_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490457_var_TransPosdunhuang)
	
	end
end


function x490457_ProcMapTimerTick( varMap, monsterobjid )

end


function x490457_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490457_ProcEventEntry( varMap, monsterobjid )
