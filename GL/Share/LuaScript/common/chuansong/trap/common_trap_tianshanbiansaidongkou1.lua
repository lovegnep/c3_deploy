...
x400001_var_FileId = 490001
x490001_var_TransPosloulan  ="tianshanbiansaidongkou1"
x490001_var_TransPoslaiyin  ="tianshanbiansaidongkou1"
x490001_var_TransPoskunlun  ="tianshanbiansaidongkou1"
x490001_var_TransPosdunhuang="tianshanbiansaidongkou1"
x490001_var_Notice = "您的等级还不足#R135级#Y，无法前往寒冰深渊！"


function x490001_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 135 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490001_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490001_TransToPosition(varMap, varObj)
			end
	...
	x490001_TransToPosition(varMap, varPlayer)
end

function x490001_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490001_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490001_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490001_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490001_var_TransPosdunhuang)
	
	end
end


function x490001_ProcMapTimerTick( varMap, monsterobjid )

end


function x490001_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490001_ProcEventEntry( varMap, monsterobjid )
