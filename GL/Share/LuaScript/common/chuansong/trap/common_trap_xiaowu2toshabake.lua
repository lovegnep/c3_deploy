...
x490453_var_FileId = 490453
x490453_var_TransPosloulan  ="minju2toshabake"
x490453_var_TransPoslaiyin  ="minju2toshabake"
x490453_var_TransPoskunlun  ="minju2toshabake"
x490453_var_TransPosdunhuang="minju2toshabake"


function x490453_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490453_TransToPosition(varMap, varObj)
			end
	...
	x490453_TransToPosition(varMap, varPlayer)
end

function x490453_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490453_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490453_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490453_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490453_var_TransPosdunhuang)
	
	end
end


function x490453_ProcMapTimerTick( varMap, monsterobjid )

end


function x490453_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490453_ProcEventEntry( varMap, monsterobjid )
