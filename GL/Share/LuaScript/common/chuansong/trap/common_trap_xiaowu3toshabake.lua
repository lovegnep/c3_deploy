...
x490455_var_FileId = 490455
x490455_var_TransPosloulan  ="minju3toshabake"
x490455_var_TransPoslaiyin  ="minju3toshabake"
x490455_var_TransPoskunlun  ="minju3toshabake"
x490455_var_TransPosdunhuang="minju3toshabake"


function x490455_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490455_TransToPosition(varMap, varObj)
			end
	...
	x490455_TransToPosition(varMap, varPlayer)
end

function x490455_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490455_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490455_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490455_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490455_var_TransPosdunhuang)
	
	end
end


function x490455_ProcMapTimerTick( varMap, monsterobjid )

end


function x490455_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490455_ProcEventEntry( varMap, monsterobjid )
