...
x300304_var_FileId = 300304
x300304_var_TransPosloulan  ="kunlunbiansaidongkou1"
x300304_var_TransPoslaiyin  ="kunlunbiansaidongkou1"
x300304_var_TransPoskunlun  ="kunlunbiansaidongkou1"
x300304_var_TransPosdunhuang="kunlunbiansaidongkou1"
x300304_var_Notice = "您的等级还不足#R135级#Y，无法前往暗夜深渊！"


function x300304_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 135 then 
					Msg2Player(varMap,varPlayer,"#Y"..x300304_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300304_TransToPosition(varMap, varObj)
			end
	...
	x300304_TransToPosition(varMap, varPlayer)
end

function x300304_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300304_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300304_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300304_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300304_var_TransPosdunhuang)
	
	end
end


function x300304_ProcMapTimerTick( varMap, monsterobjid )

end


function x300304_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300304_ProcEventEntry( varMap, monsterobjid )
