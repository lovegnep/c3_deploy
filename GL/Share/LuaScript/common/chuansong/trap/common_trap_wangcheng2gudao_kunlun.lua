...
x300201_var_FileId = 300201
x300201_var_TransPosloulan  ="wangcheng2gudaokunlun"
x300201_var_TransPoslaiyin  ="wangcheng2gudaokunlun"
x300201_var_TransPoskunlun  ="wangcheng2gudaokunlun"
x300201_var_TransPosdunhuang="wangcheng2gudaokunlun"



function x300201_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300201_TransToPosition(varMap, varObj)
			end
	...
	x300201_TransToPosition(varMap, varPlayer)
end


function x300201_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300201_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300201_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300201_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300201_var_TransPosdunhuang)
	
	end
end


function x300201_ProcMapTimerTick( varMap, monsterobjid )

end


function x300201_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300201_ProcEventEntry( varMap, monsterobjid )
