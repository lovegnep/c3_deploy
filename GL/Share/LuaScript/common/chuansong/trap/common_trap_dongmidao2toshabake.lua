...
x490489_var_FileId = 490489
x490489_var_TransPosloulan  ="dongmidao2toshabake"
x490489_var_TransPoslaiyin  ="dongmidao2toshabake"
x490489_var_TransPoskunlun  ="dongmidao2toshabake"
x490489_var_TransPosdunhuang="dongmidao2toshabake"


function x490489_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490489_TransToPosition(varMap, varObj)
			end
	...
	x490489_TransToPosition(varMap, varPlayer)
end

function x490489_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490489_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490489_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490489_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490489_var_TransPosdunhuang)
	
	end
end


function x490489_ProcMapTimerTick( varMap, monsterobjid )

end


function x490489_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490489_ProcEventEntry( varMap, monsterobjid )
