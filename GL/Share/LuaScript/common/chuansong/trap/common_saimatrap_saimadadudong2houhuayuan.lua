...
x300258_var_FileId = 300258
x300258_var_TransPosloulan  ="saimadadudong2houhuayuan"
x300258_var_TransPoslaiyin  ="saimadadudong2houhuayuan"
x300258_var_TransPoskunlun  ="saimadadudong2houhuayuan"
x300258_var_TransPosdunhuang="saimadadudong2houhuayuan"
x300258_var_Notice = "您的等级还不足#R40级！"


function x300258_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 40 then 
					Msg2Player(varMap,varPlayer,"#Y"..x300258_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300258_TransToPosition(varMap, varObj)
			end
	...
	x300258_TransToPosition(varMap, varPlayer)
end

function x300258_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300258_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300258_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300258_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300258_var_TransPosdunhuang)
	
	end
end


function x300258_ProcMapTimerTick( varMap, monsterobjid )

end


function x300258_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300258_ProcEventEntry( varMap, monsterobjid )
