...
x490031_var_FileId = 490031
x490031_var_TransPosloulan  ="dadumigong1todaduhuayuan"
x490031_var_TransPoslaiyin  ="dadumigong1todaduhuayuan"
x490031_var_TransPoskunlun  ="dadumigong1todaduhuayuan"
x490031_var_TransPosdunhuang="dadumigong1todaduhuayuan"
x490031_var_Notice = "您的等级还不足#R70级#Y，无法前往后花园！"


function x490031_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
...
--	end
	...
	local Level = GetLevel(varMap, varPlayer)
	if Level < 70 then 
			Msg2Player(varMap,varPlayer,"#Y"..x490031_var_Notice.."#W",4,3)
			return
		end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490031_TransToPosition(varMap, varObj)
			end
	...
	x490031_TransToPosition(varMap, varPlayer)
end

function x490031_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490031_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490031_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490031_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490031_var_TransPosdunhuang)
	
	end
end


function x490031_ProcMapTimerTick( varMap, monsterobjid )

end


function x490031_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490031_ProcEventEntry( varMap, monsterobjid )
