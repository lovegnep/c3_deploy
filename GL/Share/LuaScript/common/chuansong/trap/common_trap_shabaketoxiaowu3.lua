...
x490454_var_FileId = 490454
x490454_var_TransPosloulan  ="shabaketominju3"
x490454_var_TransPoslaiyin  ="shabaketominju3"
x490454_var_TransPoskunlun  ="shabaketominju3"
x490454_var_TransPosdunhuang="shabaketominju3"


function x490454_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490454_TransToPosition(varMap, varObj)
			end
	...
	x490454_TransToPosition(varMap, varPlayer)
end

function x490454_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490454_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490454_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490454_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490454_var_TransPosdunhuang)
	
	end
end


function x490454_ProcMapTimerTick( varMap, monsterobjid )

end


function x490454_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490454_ProcEventEntry( varMap, monsterobjid )
