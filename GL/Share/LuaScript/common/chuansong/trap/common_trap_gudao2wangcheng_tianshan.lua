...
x300218_var_FileId = 300218
x300218_var_TransPosloulan  ="gudao2wangchengtianshan"
x300218_var_TransPoslaiyin  ="gudao2wangchengtianshan"
x300218_var_TransPoskunlun  ="gudao2wangchengtianshan"
x300218_var_TransPosdunhuang="gudao2wangchengtianshan"



function x300218_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300218_TransToPosition(varMap, varObj)
			end
	...
	x300218_TransToPosition(varMap, varPlayer)
end

function x300218_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300218_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300218_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300218_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300218_var_TransPosdunhuang)
	
	end
end


function x300218_ProcMapTimerTick( varMap, monsterobjid )

end


function x300218_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300218_ProcEventEntry( varMap, monsterobjid )
