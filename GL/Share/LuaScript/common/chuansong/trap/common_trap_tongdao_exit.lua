...
x300480_var_FileId = 300480
x300480_var_TransPosLou="loulanbianjingexit2xiaoshuidao"
x300480_var_TransPosLai="laiyinbianjingexit2xiaoshuidao"
x300480_var_TransPosKun="kunlunbianjingexit2xiaoshuidao"
x300480_var_TransPosDun="dunhuangbianjingexit2xiaoshuidao"
x300480_var_Notice = "您的等级还不足#R20级#Y，无法前往秘道！"


function x300480_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
			...
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x300480_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300480_TransToPosition(varMap, varObj)
			end
	...
	x300480_TransToPosition(varMap, varPlayer)
end

function x300480_TransToPosition(varMap, varPlayer)
	if varMap == 51 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300480_var_TransPosLou)
	elseif varMap == 151 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300480_var_TransPosLai)
	elseif varMap == 251 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300480_var_TransPosKun)
	elseif varMap == 351 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300480_var_TransPosDun)
	end
end


function x300480_ProcMapTimerTick( varMap, monsterobjid )

end


function x300480_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300480_ProcEventEntry( varMap, monsterobjid )
