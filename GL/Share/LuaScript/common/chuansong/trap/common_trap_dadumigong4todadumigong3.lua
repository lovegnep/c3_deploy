...
x490034_var_FileId = 490034
x490034_var_TransPosloulan  ="dadumigong4todadumigong3"
x490034_var_TransPoslaiyin  ="dadumigong4todadumigong3"
x490034_var_TransPoskunlun  ="dadumigong4todadumigong3"
x490034_var_TransPosdunhuang="dadumigong4todadumigong3"
x490034_var_Notice = "您的等级还不足#R70级#Y，无法前往大都·后花园秘洞！"


function x490034_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel <70 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490034_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490034_TransToPosition(varMap, varObj)
			end
	...
	x490034_TransToPosition(varMap, varPlayer)
end

function x490034_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490034_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490034_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490034_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490034_var_TransPosdunhuang)
	
	end
end


function x490034_ProcMapTimerTick( varMap, monsterobjid )

end


function x490034_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490034_ProcEventEntry( varMap, monsterobjid )
