...
x300288_var_FileId = 300288
x300288_var_TransPosloulan  ="xiaowutwo2wangcheng1"
x300288_var_TransPoslaiyin  ="xiaowutwo2wangcheng2"
x300288_var_TransPoskunlun  ="xiaowutwo2wangcheng3"
x300288_var_TransPosdunhuang="xiaowutwo2wangcheng4"
x300288_var_Notice = "您的等级还不足#R10级#Y，无法前往王城！"


function x300288_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 10 then 
					Msg2Player(varMap,varPlayer,"#Y"..x300288_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300288_TransToPosition(varMap, varObj)
			end
	...
	x300288_TransToPosition(varMap, varPlayer)
end

function x300288_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300288_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300288_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300288_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300288_var_TransPosdunhuang)
	
	end
end


function x300288_ProcMapTimerTick( varMap, monsterobjid )

end


function x300288_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300288_ProcEventEntry( varMap, monsterobjid )
