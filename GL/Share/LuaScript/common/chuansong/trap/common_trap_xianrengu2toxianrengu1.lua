...
x490502_var_FileId = 490502
x490502_var_TransPosloulan  ="xianrengu2toxianrengu1"
x490502_var_TransPoslaiyin  ="xianrengu2toxianrengu1"
x490502_var_TransPoskunlun  ="xianrengu2toxianrengu1"
x490502_var_TransPosdunhuang="xianrengu2toxianrengu1"
x490502_var_Notice = "对不起，您的等级不足90级，无法前往仙人谷"


function x490502_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 90 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490502_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490502_TransToPosition(varMap, varObj)
			end
	...
	x490502_TransToPosition(varMap, varPlayer)
end

function x490502_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490502_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490502_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490502_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490502_var_TransPosdunhuang)
	
	end
end


function x490502_ProcMapTimerTick( varMap, monsterobjid )

end


function x490502_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490502_ProcEventEntry( varMap, monsterobjid )
