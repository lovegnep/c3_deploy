...
x300603_var_FileId = 300603
x300603_var_TransPosloulan  ="hannanheloulan2wangcheng"
x300603_var_TransPoslaiyin  ="hannanhetianshan2wangcheng"
x300603_var_TransPoskunlun  ="hannanhekunlun2wangcheng"
x300603_var_TransPosdunhuang="hannanhedunhuang2wangcheng"



function x300603_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300603_TransToPosition(varMap, varObj)
			end
	...
	x300603_TransToPosition(varMap, varPlayer)
end

function x300603_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300603_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300603_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300603_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300603_var_TransPosdunhuang)
	
	end
end


function x300603_ProcMapTimerTick( varMap, monsterobjid )

end


function x300603_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300603_ProcEventEntry( varMap, monsterobjid )
