...
x300227_var_FileId = 300227
x300227_var_TransPosloulan  ="bianjing2caochangtianshan"
x300227_var_TransPoslaiyin  ="bianjing2caochangtianshan"
x300227_var_TransPoskunlun  ="bianjing2caochangtianshan"
x300227_var_TransPosdunhuang="bianjing2caochangtianshan"



function x300227_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300227_TransToPosition(varMap, varObj)
			end
	...
	x300227_TransToPosition(varMap, varPlayer)
end

function x300227_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300227_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300227_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300227_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300227_var_TransPosdunhuang)
	
	end
end


function x300227_ProcMapTimerTick( varMap, monsterobjid )

end


function x300227_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300227_ProcEventEntry( varMap, monsterobjid )
