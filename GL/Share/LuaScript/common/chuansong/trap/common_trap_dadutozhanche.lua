...
x490037_var_FileId = 490037
x490037_var_TransPosloulan  ="dadutozhanchegongchang"
x490037_var_TransPoslaiyin  ="dadutozhanchegongchang"
x490037_var_TransPoskunlun  ="dadutozhanchegongchang"
x490037_var_TransPosdunhuang="dadutozhanchegongchang"


function x490037_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490037_TransToPosition(varMap, varObj)
			end
	...
	x490037_TransToPosition(varMap, varPlayer)
end

function x490037_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490037_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490037_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490037_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490037_var_TransPosdunhuang)
	
	end
end


function x490037_ProcMapTimerTick( varMap, monsterobjid )

end


function x490037_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490037_ProcEventEntry( varMap, monsterobjid )
