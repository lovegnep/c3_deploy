...
x300297_var_FileId = 300297
x300297_var_TransPosloulan  ="dadu2dadumingjuone1"
x300297_var_TransPoslaiyin  ="dadu2dadumingjuone2"
x300297_var_TransPoskunlun  ="dadu2dadumingjuone3"
x300297_var_TransPosdunhuang="dadu2dadumingjuone4"
x300297_var_Notice = "您的等级还不足#R20级#Y，无法前往大都密室！"


function x300297_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x300297_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300297_TransToPosition(varMap, varObj)
			end
	...
	x300297_TransToPosition(varMap, varPlayer)
end

function x300297_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300297_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300297_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300297_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300297_var_TransPosdunhuang)
	
	end
end


function x300297_ProcMapTimerTick( varMap, monsterobjid )

end


function x300297_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300297_ProcEventEntry( varMap, monsterobjid )
