...
x300206_var_FileId = 300206
x300206_var_TransPosloulan  ="muchang2wangchengtianshan"
x300206_var_TransPoslaiyin  ="muchang2wangchengtianshan"
x300206_var_TransPoskunlun  ="muchang2wangchengtianshan"
x300206_var_TransPosdunhuang="muchang2wangchengtianshan"
x300206_var_Notice = "您的等级还不足#R10级#Y，无法前往天山王城！"


function x300206_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 10 then 
					Msg2Player(varMap,varPlayer,"#Y"..x300206_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300206_TransToPosition(varMap, varObj)
			end
	...
	x300206_TransToPosition(varMap, varPlayer)
end

function x300206_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300206_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300206_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300206_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300206_var_TransPosdunhuang)
	
	end
end


function x300206_ProcMapTimerTick( varMap, monsterobjid )

end


function x300206_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300206_ProcEventEntry( varMap, monsterobjid )
