...
x490471_var_FileId = 490471
x490471_var_TransPosloulan  ="minju11toshabake"
x490471_var_TransPoslaiyin  ="minju11toshabake"
x490471_var_TransPoskunlun  ="minju11toshabake"
x490471_var_TransPosdunhuang="minju11toshabake"


function x490471_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490471_TransToPosition(varMap, varObj)
			end
	...
	x490471_TransToPosition(varMap, varPlayer)
end

function x490471_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490471_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490471_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490471_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490471_var_TransPosdunhuang)
	
	end
end


function x490471_ProcMapTimerTick( varMap, monsterobjid )

end


function x490471_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490471_ProcEventEntry( varMap, monsterobjid )
