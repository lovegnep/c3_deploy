...
x300298_var_FileId = 300298
x300298_var_TransPosloulan  ="dadumingjuone2dadu1"
x300298_var_TransPoslaiyin  ="dadumingjuone2dadu2"
x300298_var_TransPoskunlun  ="dadumingjuone2dadu3"
x300298_var_TransPosdunhuang="dadumingjuone2dadu4"
x300298_var_Notice = "您的等级还不足#R20级#Y，无法前往大都！"


function x300298_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x300298_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300298_TransToPosition(varMap, varObj)
			end
	...
	x300298_TransToPosition(varMap, varPlayer)
end

function x300298_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300298_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300298_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300298_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300298_var_TransPosdunhuang)
	
	end
end


function x300298_ProcMapTimerTick( varMap, monsterobjid )

end


function x300298_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300298_ProcEventEntry( varMap, monsterobjid )
