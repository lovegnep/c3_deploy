...
x490443_var_FileId = 490443
x490443_var_TransPosloulan  ="shabaketokunlunsichouzhilu"
x490443_var_TransPoslaiyin  ="shabaketokunlunsichouzhilu"
x490443_var_TransPoskunlun  ="shabaketokunlunsichouzhilu"
x490443_var_TransPosdunhuang="shabaketokunlunsichouzhilu"
x490443_var_TuchengzhanQuestID = 7559


function x490443_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
local minute = GetMinOfDay()	
local weekIdx = GetWeek()
         
if minute >= 900 and minute < 1020 then
	if 	x490443_IsKunlunRuoGuo(varMap, varPlayer) > 0 then
		if x490443_IsLianMeng(varMap, varPlayer) ~= 1 then
			if x490443_IsBenGuo(varMap, varPlayer) == 0 then
				if weekIdx == 6 then
					if IsHaveQuestNM(varMap, varPlayer, x490443_var_TuchengzhanQuestID ) <= 0 then
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490443_TransToPosition(varMap, varObj)
			end
	...
	x490443_TransToPosition(varMap, varPlayer)
end

function x490443_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490443_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490443_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490443_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490443_var_TransPosdunhuang)
	
	end
end


function x490443_ProcMapTimerTick( varMap, monsterobjid )

end


function x490443_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490443_ProcEventEntry( varMap, monsterobjid )

end

function x490443_IsKunlunRuoGuo(varMap, varPlayer)
  local maxlevel = GetTopListInfo_MaxLevel(GetWorldID(varMap, varPlayer))
	...
	if maxlevel >= 70 then
			if kunlunscore <= allscore*0.2 then
				if x490443_IsMinGuo(varMap, varPlayer) == 1 then
		 	 		return 1
		 	 	end
			end
	end
...
function x490443_IsLianMeng(varMap, varPlayer)
...
function x490443_IsBenGuo(varMap, varPlayer)
local country = GetCurCountry(varMap, varPlayer )
 ...
 function x490443_IsMinGuo(varMap, varPlayer)
	local loulanscore   = GetCountryParam(varMap, 0, CD_LASTWEEK_STRONG_WEAK_JIFEN)
	local tianshanscore = GetCountryParam(varMap, 1, CD_LASTWEEK_STRONG_WEAK_JIFEN)
	local kunlunscore   = GetCountryParam(varMap, 2, CD_LASTWEEK_STRONG_WEAK_JIFEN)
	local dunhuangscore = GetCountryParam(varMap, 3, CD_LASTWEEK_STRONG_WEAK_JIFEN)
