...
x270164_var_FileId = 270164
x270164_var_TransPosloulan  ="mishitwo2bianjing2"
x270164_var_TransPoslaiyin  ="mishitwo2bianjing2"
x270164_var_TransPoskunlun  ="mishitwo2bianjing2"
x270164_var_TransPosdunhuang="mishitwo2bianjing2"
x270164_var_Notice = "您的等级还不足#R20级#Y，无法前往边塞！"


function x270164_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x270164_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x270164_TransToPosition(varMap, varObj)
			end
	...
	x270164_TransToPosition(varMap, varPlayer)
end

function x270164_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x270164_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x270164_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x270164_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x270164_var_TransPosdunhuang)
	
	end
end


function x270164_ProcMapTimerTick( varMap, monsterobjid )

end


function x270164_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x270164_ProcEventEntry( varMap, monsterobjid )
