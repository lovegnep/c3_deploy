...
x300223_var_FileId = 300223
x300223_var_TransPosloulan  ="gudao2bianjingdunhuang"
x300223_var_TransPoslaiyin  ="gudao2bianjingdunhuang"
x300223_var_TransPoskunlun  ="gudao2bianjingdunhuang"
x300223_var_TransPosdunhuang="gudao2bianjingdunhuang"
x300223_var_Notice = "您的等级还不足#R20级#Y，无法前往敦煌边塞！"


function x300223_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
  	...
  	local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x300223_var_Notice.."#W",4,3)
					return
			end	
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300223_TransToPosition(varMap, varObj)
			end
	...
	x300223_TransToPosition(varMap, varPlayer)
end


function x300223_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300223_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300223_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300223_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300223_var_TransPosdunhuang)
	
	end
end


function x300223_ProcMapTimerTick( varMap, monsterobjid )

end


function x300223_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300223_ProcEventEntry( varMap, monsterobjid )
