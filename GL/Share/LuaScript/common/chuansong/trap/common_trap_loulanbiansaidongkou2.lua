...
x490004_var_FileId = 490004
x490004_var_TransPosloulan  ="loulanbiansaidongkou2"
x490004_var_TransPoslaiyin  ="loulanbiansaidongkou2"
x490004_var_TransPoskunlun  ="loulanbiansaidongkou2"
x490004_var_TransPosdunhuang="loulanbiansaidongkou2"
x490004_var_Notice = "您的等级还不足#R115级#Y，无法前往修罗秘洞！"


function x490004_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 115 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490004_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490004_TransToPosition(varMap, varObj)
			end
	...
	x490004_TransToPosition(varMap, varPlayer)
end

function x490004_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490004_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490004_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490004_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490004_var_TransPosdunhuang)
	
	end
end


function x490004_ProcMapTimerTick( varMap, monsterobjid )

end


function x490004_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490004_ProcEventEntry( varMap, monsterobjid )
