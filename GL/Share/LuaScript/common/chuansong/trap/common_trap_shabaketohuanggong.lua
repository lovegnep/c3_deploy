...
x490478_var_FileId = 490478
x490478_var_TransPosloulan  ="shabaketohuanggong"
x490478_var_TransPoslaiyin  ="shabaketohuanggong"
x490478_var_TransPoskunlun  ="shabaketohuanggong"
x490478_var_TransPosdunhuang="shabaketohuanggong"


function x490478_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490478_TransToPosition(varMap, varObj)
			end
	...
	x490478_TransToPosition(varMap, varPlayer)
end

function x490478_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490478_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490478_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490478_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490478_var_TransPosdunhuang)
	
	end
end


function x490478_ProcMapTimerTick( varMap, monsterobjid )

end


function x490478_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490478_ProcEventEntry( varMap, monsterobjid )
