...
x490440_var_FileId = 490440
x490440_var_TransPosloulan  ="tianshansichouzhilutoshabake"
x490440_var_TransPoslaiyin  ="tianshansichouzhilutoshabake"
x490440_var_TransPoskunlun  ="tianshansichouzhilutoshabake"
x490440_var_TransPosdunhuang="tianshansichouzhilutoshabake"

function x490440_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490440_TransToPosition(varMap, varObj)
			end
	...
	x490440_TransToPosition(varMap, varPlayer)
end

function x490440_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490440_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490440_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490440_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490440_var_TransPosdunhuang)
	
	end
end


function x490440_ProcMapTimerTick( varMap, monsterobjid )

end


function x490440_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490440_ProcEventEntry( varMap, monsterobjid )
