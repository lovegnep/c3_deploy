...
x300284_var_FileId = 300284
x300284_var_TransPosloulan  ="migongone2wangcheng1"
x300284_var_TransPoslaiyin  ="migongone2wangcheng2"
x300284_var_TransPoskunlun  ="migongone2wangcheng3"
x300284_var_TransPosdunhuang="migongone2wangcheng4"
x300284_var_Notice = "您的等级还不足#R1级#Y，无法前往王城！"


function x300284_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 1 then 
					Msg2Player(varMap,varPlayer,"#Y"..x300284_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300284_TransToPosition(varMap, varObj)
			end
	...
	x300284_TransToPosition(varMap, varPlayer)
end

function x300284_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300284_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300284_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300284_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300284_var_TransPosdunhuang)
	
	end
end


function x300284_ProcMapTimerTick( varMap, monsterobjid )

end


function x300284_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300284_ProcEventEntry( varMap, monsterobjid )
