...
x490491_var_FileId = 490491
x490491_var_TransPosloulan  ="dongmidao3toshabake"
x490491_var_TransPoslaiyin  ="dongmidao3toshabake"
x490491_var_TransPoskunlun  ="dongmidao3toshabake"
x490491_var_TransPosdunhuang="dongmidao3toshabake"


function x490491_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490491_TransToPosition(varMap, varObj)
			end
	...
	x490491_TransToPosition(varMap, varPlayer)
end

function x490491_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490491_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490491_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490491_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490491_var_TransPosdunhuang)
	
	end
end


function x490491_ProcMapTimerTick( varMap, monsterobjid )

end


function x490491_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490491_ProcEventEntry( varMap, monsterobjid )
