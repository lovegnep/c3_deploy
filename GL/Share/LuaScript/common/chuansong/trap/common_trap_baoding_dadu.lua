...
x300474_var_FileId = 300474
x300474_var_TransPos="baoding2dadu"
x300474_var_Notice = "您的等级还不足#R40级#Y，无法前往中原大都！"


function x300474_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varObj,x300474_var_TransPos)
			end
	...
	if nLevel < 10 then 
		Msg2Player(varMap,varPlayer,"#Y"..x300474_var_Notice.."#W",4,3)
	else	
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300474_var_TransPos)
	end
	
end


function x300474_ProcMapTimerTick( varMap, monsterobjid )

end


function x300474_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300474_ProcEventEntry( varMap, monsterobjid )
