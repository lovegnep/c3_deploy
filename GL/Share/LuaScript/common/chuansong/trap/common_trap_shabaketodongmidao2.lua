...
x490488_var_FileId = 490488
x490488_var_TransPosloulan  ="shabaketodongmidao2"
x490488_var_TransPoslaiyin  ="shabaketodongmidao2"
x490488_var_TransPoskunlun  ="shabaketodongmidao2"
x490488_var_TransPosdunhuang="shabaketodongmidao2"


function x490488_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490488_TransToPosition(varMap, varObj)
			end
	...
	x490488_TransToPosition(varMap, varPlayer)
end

function x490488_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490488_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490488_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490488_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490488_var_TransPosdunhuang)
	
	end
end


function x490488_ProcMapTimerTick( varMap, monsterobjid )

end


function x490488_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490488_ProcEventEntry( varMap, monsterobjid )
