...
x490484_var_FileId = 490484
x490484_var_TransPosloulan  ="shabaketoximidao3"
x490484_var_TransPoslaiyin  ="shabaketoximidao3"
x490484_var_TransPoskunlun  ="shabaketoximidao3"
x490484_var_TransPosdunhuang="shabaketoximidao3"


function x490484_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490484_TransToPosition(varMap, varObj)
			end
	...
	x490484_TransToPosition(varMap, varPlayer)
end

function x490484_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490484_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490484_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490484_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490484_var_TransPosdunhuang)
	
	end
end


function x490484_ProcMapTimerTick( varMap, monsterobjid )

end


function x490484_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490484_ProcEventEntry( varMap, monsterobjid )
