...
x490006_var_FileId = 490006
x490006_var_TransPosloulan  ="dunhuangwangchengdongkou2"
x490006_var_TransPoslaiyin  ="dunhuangwangchengdongkou2"
x490006_var_TransPoskunlun  ="dunhuangwangchengdongkou2"
x490006_var_TransPosdunhuang="dunhuangwangchengdongkou2"
x490006_var_Notice = "您的等级还不足#R110级#Y，无法前往无底山坳！"


function x490006_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 110 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490006_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490006_TransToPosition(varMap, varObj)
			end
	...
	x490006_TransToPosition(varMap, varPlayer)
end

function x490006_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490006_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490006_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490006_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490006_var_TransPosdunhuang)
	
	end
end


function x490006_ProcMapTimerTick( varMap, monsterobjid )

end


function x490006_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490006_ProcEventEntry( varMap, monsterobjid )
