...
x490506_var_FileId = 490506
x490506_var_TransPosloulan  ="xianrengu4toxianrengu3"
x490506_var_TransPoslaiyin  ="xianrengu4toxianrengu3"
x490506_var_TransPoskunlun  ="xianrengu4toxianrengu3"
x490506_var_TransPosdunhuang="xianrengu4toxianrengu3"
x490506_var_Notice = "对不起，您的等级不足90级，无法前往仙人谷"


function x490506_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 90 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490506_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490506_TransToPosition(varMap, varObj)
			end
	...
	x490506_TransToPosition(varMap, varPlayer)
end

function x490506_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490506_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490506_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490506_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490506_var_TransPosdunhuang)
	
	end
end


function x490506_ProcMapTimerTick( varMap, monsterobjid )

end


function x490506_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490506_ProcEventEntry( varMap, monsterobjid )
