...
x490104_var_FileId = 490104
x490104_var_TransPosloulan  ="loulanmidong110toloulanwangcheng"
x490104_var_TransPoslaiyin  ="loulanmidong110toloulanwangcheng"
x490104_var_TransPoskunlun  ="loulanmidong110toloulanwangcheng"
x490104_var_TransPosdunhuang="loulanmidong110toloulanwangcheng"
x490104_var_Notice = "你的等级不足110级！"


function x490104_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 110 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490104_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	i = 0, memberCount - 1 do
				...
				x490104_TransToPosition(varMap, varObj)
			end
	...
	x490104_TransToPosition(varMap, varPlayer)
end

function x490104_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490104_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490104_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490104_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490104_var_TransPosdunhuang)
	
	end
end


function x490104_ProcMapTimerTick( varMap, monsterobjid )

end


function x490104_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490104_ProcEventEntry( varMap, monsterobjid )
