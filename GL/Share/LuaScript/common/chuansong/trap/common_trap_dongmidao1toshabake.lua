...
x490487_var_FileId = 490487
x490487_var_TransPosloulan  ="dongmidao1toshabake"
x490487_var_TransPoslaiyin  ="dongmidao1toshabake"
x490487_var_TransPoskunlun  ="dongmidao1toshabake"
x490487_var_TransPosdunhuang="dongmidao1toshabake"


function x490487_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490487_TransToPosition(varMap, varObj)
			end
	...
	x490487_TransToPosition(varMap, varPlayer)
end

function x490487_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490487_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490487_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490487_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490487_var_TransPosdunhuang)
	
	end
end


function x490487_ProcMapTimerTick( varMap, monsterobjid )

end


function x490487_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490487_ProcEventEntry( varMap, monsterobjid )
