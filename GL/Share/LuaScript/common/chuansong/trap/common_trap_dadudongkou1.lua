...
x490021_var_FileId = 490021
x490021_var_TransPosloulan  ="dadudongkou1"
x490021_var_TransPoslaiyin  ="dadudongkou1"
x490021_var_TransPoskunlun  ="dadudongkou1"
x490021_var_TransPosdunhuang="dadudongkou1"
x490021_var_Notice = "您的等级还不足#R120级#Y，无法前往两极树洞！"


function x490021_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 120 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490021_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490021_TransToPosition(varMap, varObj)
			end
	...
	x490021_TransToPosition(varMap, varPlayer)
end

function x490021_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490021_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490021_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490021_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490021_var_TransPosdunhuang)
	
	end
end


function x490021_ProcMapTimerTick( varMap, monsterobjid )

end


function x490021_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490021_ProcEventEntry( varMap, monsterobjid )
