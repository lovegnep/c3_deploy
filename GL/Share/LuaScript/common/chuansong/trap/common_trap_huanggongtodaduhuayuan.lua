...
x490024_var_FileId = 490024
x490024_var_TransPosloulan  ="huanggongtodaduhuayuan"
x490024_var_TransPoslaiyin  ="huanggongtodaduhuayuan"
x490024_var_TransPoskunlun  ="huanggongtodaduhuayuan"
x490024_var_TransPosdunhuang="huanggongtodaduhuayuan"
x490024_var_Notice = "您的等级还不足#R90级#Y，无法前往大都·皇宫花园！"


function x490024_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 90 then 
					Msg2Player(varMap,varPlayer,"#Y"..x490024_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490024_TransToPosition(varMap, varObj)
			end
	...
	x490024_TransToPosition(varMap, varPlayer)
end

function x490024_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490024_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490024_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490024_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490024_var_TransPosdunhuang)
	
	end
end


function x490024_ProcMapTimerTick( varMap, monsterobjid )

end


function x490024_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490024_ProcEventEntry( varMap, monsterobjid )
