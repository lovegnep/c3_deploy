...
x490456_var_FileId = 490456
x490456_var_TransPosloulan  ="shabaketominju4"
x490456_var_TransPoslaiyin  ="shabaketominju4"
x490456_var_TransPoskunlun  ="shabaketominju4"
x490456_var_TransPosdunhuang="shabaketominju4"


function x490456_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x490456_TransToPosition(varMap, varObj)
			end
	...
	x490456_TransToPosition(varMap, varPlayer)
end

function x490456_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490456_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490456_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490456_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x490456_var_TransPosdunhuang)
	
	end
end


function x490456_ProcMapTimerTick( varMap, monsterobjid )

end


function x490456_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x490456_ProcEventEntry( varMap, monsterobjid )
