...
x300237_var_FileId = 300237
x300237_var_TransPosloulan  ="bianjingxia2andao0"
x300237_var_TransPoslaiyin  ="bianjingxia2andao1"
x300237_var_TransPoskunlun  ="bianjingxia2andao2"
x300237_var_TransPosdunhuang="bianjingxia2andao3"
x300237_var_Notice = "您的等级还不足#R20级#Y，无法前往边塞暗道！"


function x300237_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x300237_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300237_TransToPosition(varMap, varObj)
			end
	...
	x300237_TransToPosition(varMap, varPlayer)
end

function x300237_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if varMap == 51 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300237_var_TransPosloulan)
		
		elseif varMap == 151 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300237_var_TransPoslaiyin)
		
		elseif varMap == 251 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300237_var_TransPoskunlun)
		
		elseif varMap == 351 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300237_var_TransPosdunhuang)
	
	end
end


function x300237_ProcMapTimerTick( varMap, monsterobjid )

end


function x300237_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300237_ProcEventEntry( varMap, monsterobjid )
