...
x300526_var_FileId = 300526
x300526_var_TransPosloulan  ="gubaotwo2gubaoshree1"
x300526_var_TransPoslaiyin  ="gubaotwo2gubaoshree2"
x300526_var_TransPoskunlun  ="gubaotwo2gubaoshree3"
x300526_var_TransPosdunhuang="gubaotwo2gubaoshree4"



function x300526_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x300526_TransToPosition(varMap, varObj)
			end
	...
	x300526_TransToPosition(varMap, varPlayer)
	
end

function x300526_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if guo == 0 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300526_var_TransPosloulan)
		
		elseif guo == 1 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300526_var_TransPoslaiyin)
		
		elseif guo == 2 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300526_var_TransPoskunlun)
		
		elseif guo == 3 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x300526_var_TransPosdunhuang)
	
	end
end


function x300526_ProcMapTimerTick( varMap, monsterobjid )

end


function x300526_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x300526_ProcEventEntry( varMap, monsterobjid )
