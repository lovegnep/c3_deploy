...
x550701_var_FileId = 550701
x550701_var_TransPosloulan  ="minju3biasnai21"
x550701_var_TransPoslaiyin  ="minju3biasnai22"
x550701_var_TransPoskunlun  ="minju3biasnai23"
x550701_var_TransPosdunhuang="minju3biasnai24"
x550701_var_Notice = "您的等级还不足#R20级#Y，无法前往边塞密室！"


function x550701_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x550701_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x550701_TransToPosition(varMap, varObj)
			end
	...
	x550701_TransToPosition(varMap, varPlayer)
end

function x550701_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if varMap == 51 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550701_var_TransPosloulan)
		
		elseif varMap == 151 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550701_var_TransPoslaiyin)
		
		elseif varMap == 251 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550701_var_TransPoskunlun)
		
		elseif varMap == 351 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550701_var_TransPosdunhuang)
	
	end
end


function x550701_ProcMapTimerTick( varMap, monsterobjid )

end


function x550701_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x550701_ProcEventEntry( varMap, monsterobjid )
