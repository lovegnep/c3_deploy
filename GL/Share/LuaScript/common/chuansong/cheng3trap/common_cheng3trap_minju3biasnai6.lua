...
x550705_var_FileId = 550705
x550705_var_TransPosloulan  ="minju3biasnai61"
x550705_var_TransPoslaiyin  ="minju3biasnai62"
x550705_var_TransPoskunlun  ="minju3biasnai63"
x550705_var_TransPosdunhuang="minju3biasnai64"
x550705_var_Notice = "您的等级还不足#R20级#Y，无法前往边塞！"


function x550705_ProcAreaEntered( varMap, varPlayer, varArea, varQuest )
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel < 20 then 
					Msg2Player(varMap,varPlayer,"#Y"..x550705_var_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	
		...
		if busType == 1 then 
			...
			for	varI = 0, memberCount - 1 do
				...
				x550705_TransToPosition(varMap, varObj)
			end
	...
	x550705_TransToPosition(varMap, varPlayer)
end

function x550705_TransToPosition(varMap, varPlayer)

	local guo = GetCurCountry(varMap,varPlayer)
	
	if varMap == 59 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550705_var_TransPosloulan)
		
		elseif varMap == 159 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550705_var_TransPoslaiyin)
		
		elseif varMap == 259 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550705_var_TransPoskunlun)
		
		elseif varMap == 359 then
		LuaCallNoclosure(COMMON_CHUANSONG_NORMAL, "TransferToWorld",varMap,varPlayer,x550705_var_TransPosdunhuang)
	
	end
end


function x550705_ProcMapTimerTick( varMap, monsterobjid )

end


function x550705_ProcAreaLeaved( varMap, monsterobjid, varScript, varQuest )

end


function x550705_ProcEventEntry( varMap, monsterobjid )
