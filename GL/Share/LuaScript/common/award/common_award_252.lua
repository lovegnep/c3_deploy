...
x580252_g_ScriptId = 580252
x580252_g_MissionName = "马到成功礼包"

--一次最多显示的奖品数
x580252_g_MaxOnceAwardNum = 1
x580252_g_ItemBonus =	{	{zhiye=-1,id=12030535,num=1}	--马到成功礼包"
						}


--点击（NPC）
function x580252_ProcEnumEvent( sceneId, selfId, targetId, MissionId )
	return 0
...
--点击（按钮）点击该任务后执行此脚本
function x580252_ProcEventEntry( sceneId, selfId, targetId,state,index )	
	
	--未判断背包是否有足够空间

	x580252_DispatchMissionInfo( sceneId, selfId, targetId,index)

end

function x580252_DispatchMissionInfo( sceneId, selfId, targetId,index)
	...
	DeliverTalkInfo(sceneId, selfId, targetId, x580252_g_ScriptId, -1);

end

--领奖
function x580252_AddAward( sceneId, selfId,itemNum,index, dbIndex )
	...
	local IsHaveItem = 0
	for i,itm in x580252_g_ItemBonus do
		if itm.zhiye == -1 then
			IsHaveItem = 1
...
--接受
--**********************************
function x580252_ProcAccept( sceneId, selfId )
end

function x580252_ProcAcceptCheck( sceneId, selfId, NPCId )
	return 1
end
--点击（确定）
function x580252_ProcQuestAccept( sceneId, selfId, NPCId )
end

--**********************************
--放弃
--**********************************
function x580252_ProcQuestAbandon( sceneId, selfId, MissionId )
end
--**********************************
--继续
--**********************************
function x580252_OnContinue( sceneId, selfId, targetId )
end
--**********************************
--检测是否可以提交
--**********************************
function x580252_CheckSubmit( sceneId, selfId )
end
--**********************************
--提交
--**********************************
function x580252_ProcQuestSubmit( sceneId, selfId, targetId,selectRadioId, MissionId )
end
--**********************************
--杀死怪物或玩家
--**********************************
function x580252_ProcQuestObjectKilled( sceneId, selfId, objdataId, objId, MissionId )
end
--**********************************
--进入区域事件
