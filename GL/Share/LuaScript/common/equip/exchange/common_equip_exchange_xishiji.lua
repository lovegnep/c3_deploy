...
{targetID = 12500206, targetName = "传说级洗石剂-雪魂碎片", excItemNum = 10, questInfo = "\n\t#G传说级洗石剂-雪魂碎片#W通过开启#G洗石剂礼包（雪魂）#W获得。"},
{targetID = 12500207, targetName = "传说级洗石剂-冰魄碎片", excItemNum = 10, questInfo = "\n\t#G传说级洗石剂-冰魄碎片#W通过开启#G洗石剂礼包（冰魄）#W获得。"},
...
function x570087_ProcEventEntry( varMap, varPlayer, varTalknpc ,varScript,seleteId)	
	...
	elseif seleteId == 881210 then
		StartTalkTask(varMap)
			...
			for varI,item in x570087_var_ExcObjID1  do
				TalkAppendButton( varMap, x570087_var_FileId, "【兑换】"..item.targetName, 3, 3+varI )
	end	
		StopTalkTask()
		DeliverTalkMenu(varMap, varPlayer, varTalknpc)
	end
	...
	elseif seleteId >= 4 and seleteId <= 6 then
		...
		local varText = format("\t您现在有#R%d个#W#G%s#W！", varItemNum, x570087_var_ExcItemID[seleteId].targetName)
			StartTalkTask(varMap)
				TalkAppendString(varMap,"#Y"..format(x570087_var_QuestName,x570087_var_ExcObjID1[seleteId-3].targetName))
				TalkAppendString(varMap,format(x570087_var_QuestInfo,x570087_var_ExcItemID[seleteId].targetName,x570087_var_ExcObjID1[seleteId-3].targetName,x570087_var_ExcItemID[seleteId].excItemNum,x570087_var_ExcItemID[seleteId].targetName))
				TalkAppendString(varMap,varText)
				TalkAppendString(varMap,x570087_var_ExcItemID[seleteId].questInfo)
				AddQuestItemBonus(varMap, x570087_var_ExcObjID1[seleteId-3].targetID, 1)
			StopTalkTask()
			DeliverTalkInfo(varMap, varPlayer, varTalknpc, x570087_var_FileId, -1);		
	end
...
function x570087_ProcAccept( varMap, varPlayer )
	...
	x570087_Accept( varMap, varPlayer )
...
function x570087_Accept( varMap, varPlayer )
	...
	local bind = 0
		local SelectedInfo ={}
	if x570087_var_SelectIndex[GetGUID(varMap, varPlayer)] >=1 and x570087_var_SelectIndex[GetGUID(varMap, varPlayer)] <= 3 then
		SelectedInfo = x570087_var_ExcObjID[x570087_var_SelectIndex[GetGUID(varMap, varPlayer)]]
	elseif x570087_var_SelectIndex[GetGUID(varMap, varPlayer)] >=4 and x570087_var_SelectIndex[GetGUID(varMap, varPlayer)] <= 6 then
		SelectedInfo = x570087_var_ExcObjID1[x570087_var_SelectIndex[GetGUID(varMap, varPlayer)]-3]
	end
	...
	if Num < x570087_var_ExcItemID[x570087_var_SelectIndex[GetGUID(varMap, varPlayer)]].excItemNum then -- 道具数量不足
		...
		local Num = x570087_var_ExcItemID[x570087_var_SelectIndex[GetGUID(varMap, varPlayer)]].excItemNum
			...
			if DelItem(varMap,varPlayer,x570087_var_ExcItemID[x570087_var_SelectIndex[GetGUID(varMap, varPlayer)]].targetID,1) ~= 1 then return end
		end
