...
x310826_var_FileId = 310826 
x310826_var_lingjiangBUFF = 30577
x300826_var_ItemBonus = {
...
x310826_var_notifytable = {
12030765,
11000704,
11000703
}          
                                                           
x310826_var_countryexp = 2   	 --国家积分
x310826_var_guidexp    = 1   	 --帮会经验
x310826_var_expmulti   = 30000 --个人经验



function x310826_ProcEventEntry( varMap, varPlayer, varBagIdx )         
...
function x310826_ProcIsSpellLikeScript( varMap, varPlayer)                 
	return 1; 
end
...
function x310826_ProcCancelImpacts( varMap, varPlayer )
	return 0; 
end
...
function x310826_ProcConditionCheck( varMap, varPlayer )
...
function x310826_ProcDeplete( varMap, varPlayer )
	return 1;
end



function x310826_ProcAddItemByRandom(varMap, varPlayer)
 ...
 local varprobability =  random (1, 1000)
	for i, iter in x300826_var_ItemBonus do
	...
	if item ~= nil then 
		...
		Msg2Player( varMap, varPlayer, strTemp, 8, 2)	
			for i,j in x310826_var_notifytable do
...
function x310826_ProcActivateOnce( varMap, varPlayer, varImpact )   
	local level = GetLevel(varMap, varPlayer)
	if IsHaveSpecificImpact(varMap, varPlayer, x310826_var_lingjiangBUFF) == 1 then
	...
	if GetBagSpace(varMap, varPlayer) <= 0 then
		...
		local exp = level * x310826_var_expmulti
		...
		x310826_ProcAddItemByRandom(varMap, varPlayer)
...
function x310826_ProcActivateEachTick( varMap, varPlayer)
	return 1; 
end
