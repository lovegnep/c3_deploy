...
x310827_var_FileId = 310827 

x310827_var_ItemId = -1  

x310827_var_Bonus = {itemid = 10296074 , count = 1}  --仲夏狂欢手镯                                                                     
                                                                     



function x310827_ProcEventEntry( varMap, varPlayer, varBagIdx )         
...
function x310827_ProcIsSpellLikeScript( varMap, varPlayer)                 
	return 1; 
end
...
function x310827_ProcCancelImpacts( varMap, varPlayer )
	return 0; 
end
...
function x310827_ProcConditionCheck( varMap, varPlayer )
...
function x310827_ProcDeplete( varMap, varPlayer )
	return 1;
end



function x310827_ProcAddItemByRandom(varMap, varPlayer)
	local Min = 1
	local Max = 1
	local item = nil
	local varName = nil         
         
		local allitem = x310827_var_ItemList
	...
	x310827_ProcAddItem(varMap, varPlayer, item ,varName) 
end

function x310827_ProcAddItem(varMap, varPlayer, varItem ,iname)                               
...
function x310827_ProcActivateOnce( varMap, varPlayer, varImpact )   
	...
	if(DepletingUsedItem(varMap, varPlayer)) == 1 then
		...
		x310827_GiveBonus(varMap, varPlayer)
		local varRandom = random(1,1000)							--1/1000奖励3%的经验手镯
		if varRandom == 1 then
			StartItemTask( varMap)
				ItemAppendBind( varMap, x310827_var_Bonus.itemid, x310827_var_Bonus.count)
				if StopItemTask( varMap, varPlayer) <= 0 then
						Msg2Player( varMap, varPlayer, "你的背包已满，无法获得道具奖励。", 8, 2)
				else
						local msg  = format("打开周年庆典大礼包，你意外获得了%d个#R#{_ITEM%d}" ,x310827_var_Bonus.count, x310827_var_Bonus.itemid)
						local msg1 =	format("%s打开周年庆典大礼包意外获得#R#{_ITEM%d}", GetName( varMap, varPlayer),  x310827_var_Bonus.itemid )
						local msg2 =	format("%s打开周年庆典大礼包意外获得一个增加经验获得%s的#R#{_ITEM%d}", GetName( varMap, varPlayer),"3%",x310827_var_Bonus.itemid )
...
function x310827_ProcActivateEachTick( varMap, varPlayer)
	return 1; 
end


function x310827_GiveBonus(varMap, varPlayer)
