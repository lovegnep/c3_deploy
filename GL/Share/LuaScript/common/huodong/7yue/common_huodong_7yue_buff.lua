x302528_var_GameId = 1102  
x302528_var_Xinbing_FileId			=	302528
x302528_var_RandomItemTable0 = {
...
x302528_var_RandomCardTable ={45011051,45011041,45011046}
function x302528_ProcIntervalOverEvent( varMap, varPlayer )

	if GetGameOpenById(x302528_var_GameId) ~= 1 then
		return
	end
	...
	local hour,min1,sec = GetHourMinSec()
	    if year == 2012 and day1 > 263 and day1 <267 then
	    	if hour >=20 and hour <=22 then
					x302528_AddItem(varMap, varPlayer)
...
function x302528_ProcActiveEvent( varMap, varPlayer )

end


function x302528_ProcFadeOutEvent( varMap, varPlayer )
			local year, month, day = GetYearMonthDay();
			local day1 = GetDayOfYear()
			local hour,min1,sec = GetHourMinSec()
	    if year == 2012 and day1 > 263 and day1 <267 then
	    	if hour >=20 and hour <=22 then
	    		StartItemTask(varMap)
					ItemAppendBind( varMap, 12110217, 1)	--礼包ID
...
function x302528_ProcEventEntry()

end

function x302528_CheckPlayerInArea(varMap, varPlayer)

end

function x302528_AddItem(varMap, varPlayer)
			LuaCallNoclosure(415003,"FixedBuf",varMap,varPlayer,7767)
--获取roll到的区间			
		  local rseed = random( 1, 1000000)
		  local varIndex = 1
		  local dn = 1
		  local up = 1
		  for varI, item in x302528_var_RandomItemTable0 do
...
--加物品	  		  
		...
		if x302528_var_RandomItemTable0[ varIndex ].varNum == 1 then 	
			StartItemTask( varMap)
			if x302528_var_RandomItemTable0[ varIndex ].varItem == 11990117 then
				ItemAppendBind( varMap, x302528_var_RandomItemTable0[ varIndex ].varItem, 5)
			else
				ItemAppendBind( varMap, x302528_var_RandomItemTable0[ varIndex ].varItem, 1)
			end	
			if StopItemTask( varMap, varPlayer) == 0 then
	    	Msg2Player( varMap, varPlayer, "背包已满，无法获得奖励", 8, 3)
	    	return
	    else
	    		Msg2Player( varMap, varPlayer,format( "恭喜您获得了#G%s",x302528_var_RandomItemTable0[ varIndex ].itemName), 8, 3)
	    		DeliverItemListSendToPlayer( varMap, varPlayer)
			end
--加经验
		elseif x302528_var_RandomItemTable0[ varIndex ].varNum == 2 then 
		local playerLevel1 = GetLevel(varMap ,varPlayer)
			if x302528_var_RandomItemTable0[ varIndex ].varItem == 1 then
				if playerLevel1 < 50 then	
					local awardexp = x302528_var_levelupexp[ playerLevel1 ] 
					local a =awardexp/720
					Msg2Player( varMap, varPlayer,format( "恭喜您获得了#G%d经验",a), 8, 3)
					AddExp(varMap,varPlayer,awardexp/720)
				else
					Msg2Player( varMap, varPlayer,format( "恭喜您获得了#G%d经验",x302528_GetAllExpBonus(varMap, varPlayer )), 8, 3)
					AddExp(varMap,varPlayer,x302528_GetAllExpBonus(varMap, varPlayer ))	
				end
			elseif x302528_var_RandomItemTable0[ varIndex ].varItem == 2 then
				if playerLevel1 < 50 then	
					local awardexp1 = x302528_var_levelupexp[ playerLevel1 ] 
					Msg2Player( varMap, varPlayer,format( "恭喜您获得了#G%d经验",awardexp1), 8, 3)
					AddExp(varMap,varPlayer,awardexp1/540)
				else
					Msg2Player( varMap, varPlayer,format( "恭喜您获得了#G%d经验",x302528_GetmoreExpBonus(varMap, varPlayer )), 8, 3)
					AddExp(varMap,varPlayer,x302528_GetmoreExpBonus(varMap, varPlayer ))	
				end
			end
--加钱
		elseif x302528_var_RandomItemTable0[ varIndex ].varNum == 3 then
			Msg2Player( varMap, varPlayer, "恭喜您获得了5两#G银卡", 8, 3)
			AddMoney(varMap, varPlayer, 1, x302528_var_RandomItemTable0[ varIndex ].varItem)
--卡牌
		elseif x302528_var_RandomItemTable0[ varIndex ].varNum == 4 then 	
		local cardId = random(1,3)
			StartItemTask( varMap)	
			ItemAppendBind( varMap, x302528_var_RandomCardTable[ cardId ], 1)
				if StopItemTask( varMap, varPlayer) == 0 then
		    	Msg2Player( varMap, varPlayer, "背包已满，无法获得奖励", 8, 3)
		    	return
		    else
...
--世界通告	**	
		if x302528_var_RandomItemTable0[ varIndex ].varhorn ==1 then
			local varMsg = format( "恭喜#R%s#cffcc00在领奖活动一活动中，获得#G%s#cffcc00！", GetName( varMap, varPlayer),x302528_var_RandomItemTable0[ varIndex ].itemName )
...
function x302528_ProcessQuestFail(varMap, varPlayer, varQuest)
end


function x302528_GetAllExpBonus(varMap, varPlayer )
	local awardexp =0
...
function x302528_GetmoreExpBonus(varMap, varPlayer )
	local awardexp =0
	local playerLevel = GetLevel(varMap ,varPlayer)
			--高经验
				if playerLevel >= 50 and playerLevel < 76 then
...
function x302528_GetPerExpBonus(varMap, varPlayer )
end
--记MD
function x302528_GetDayCount(varMap, varPlayer)

end

function x302528_GetExpBonusMuil(varMap, varPlayer )
end
