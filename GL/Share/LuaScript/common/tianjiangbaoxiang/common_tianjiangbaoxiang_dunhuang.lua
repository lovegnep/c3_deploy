...
x310009_var_FileId = 310009

x310009_var_QuestList = { 8040,8041,8042,8043,8044,8045 }
x310009_var_QuestItem = { {quest=8040,item1=12030051, item2=12030052},
...
x310009_var_AcceptMessage		= "您接受了任务：%s"
x310009_var_AbandonMessage	= "您放弃了任务：%s"
x310009_var_CompleteMessage	= "您完成了任务：%s"

x310009_var_QuestCommentMessage	=	"不在接任务时间，请关注活动公告"
x310009_var_QuestAcceptFailedMessage_Accepted	=	"你在本日已经做过本任务了，请明日再来"
x310009_var_QuestSubmitFailedMessage_OverTime	=	"你已经错过了交任务的时间，请放弃任务"
x310009_var_QuestAcceptTime1 = { 450, 480 }	
x310009_var_QuestAcceptTime2 = { 750, 780 }	
x310009_var_QuestAcceptTime3 = { 1170, 1200 }	
x310009_var_AwardItem = 12030028
x310009_var_VirtualQuestId = 1368

x310009_var_RandItem = {11010100, 11010101, 11010102}

function x310009_GetQuest()
	local varIndex = random(1, 6);
	return x310009_var_QuestList[varIndex]
end

function x310009_GetMDAccepted(varMap, varPlayer, varIndex)
	if varIndex == 1 then
...
function x310009_SetMDAccepted(varMap, varPlayer, varIndex, value)
	if varIndex == 1 then
...
function x310009_GetMDDate(varMap, varPlayer)
...
function x310009_SetMDDate(varMap, varPlayer, day)
...
function x310009_NextDayReset(varMap, varPlayer)
	local varToday = GetDayOfYear()
	local varLastday = x310009_GetMDDate(varMap, varPlayer)

	if varToday ~= varLastday then
		x310009_SetMDDate(varMap, varPlayer, 0)
...
function x310009_IsNextDay(varMap, varPlayer)
...
function x310009_HaveAccepted( varMap, varPlayer )
	...
	if x310009_GetMDDate(varMap, varPlayer) > 0 then
		return 1
	end
...
function x310009_ProcAcceptCheckTime(  )

	local minspan = GetMinOfDay()
	
	
		
	if  minspan >= x310009_var_QuestAcceptTime2[1] and minspan <= x310009_var_QuestAcceptTime2[2] then
		return 2
	elseif  minspan >= x310009_var_QuestAcceptTime3[1] and minspan <= x310009_var_QuestAcceptTime3[2] then
		return 3
	end
...
function x310009_ProcEnumEvent( varMap, varPlayer, varTalknpc, varQuest )
	varQuest = x310009_GetQuest();

	for varI, item in x310009_var_QuestList do
...
function x310009_ProcEventEntry( varMap, varPlayer, varTalknpc, varQuest )
	if IsHaveQuestNM( varMap, varPlayer, varQuest ) == 0 then	
		if x310009_IsNextDay(varMap, varPlayer) > 0 then
			x310009_NextDayReset(varMap, varPlayer) 
		end
		...
		local varRet = QuestCheckAcceptNM( varMap, varPlayer, varTalknpc, varQuest )
			...
			if varRet > 0 then	
				...
				AddQuestItemBonus(varMap, x310009_var_AwardItem, 1)

				StopTalkTask(varMap);
				DeliverTalkInfoNM(varMap, varPlayer, varTalknpc, x310009_var_FileId, varQuest);
			else
...
function x310009_ProcQuestAccept( varMap, varPlayer, varTalknpc, varQuest )
	if x310009_ProcAcceptCheckTime() < 0 then
		local varQuestName = GetQuestNameNM(varMap, varPlayer, varQuest);
			StartTalkTask(varMap);
			TalkAppendString(varMap, x310009_var_QuestCommentMessage);
			StopTalkTask(varMap);
			DeliverTalkTips(varMap,varPlayer);
			Msg2Player(varMap,varPlayer,x310009_var_QuestCommentMessage,8,2)
		return 
	end

	if x310009_HaveAccepted( varMap, varPlayer ) > 0 then
		StartTalkTask(varMap);
		TalkAppendString(varMap, x310009_var_QuestAcceptFailedMessage_Accepted);
		StopTalkTask(varMap);
		DeliverTalkTips(varMap,varPlayer);
		Msg2Player(varMap,varPlayer,x310009_var_QuestAcceptFailedMessage_Accepted,8,2)
		return
	end
	...
	if varRet > 0 then 
		...
		if varRet > 0 then
			QuestCom( varMap, varPlayer, x310009_var_VirtualQuestId)
			...
			x310009_SetMDDate(varMap, varPlayer, day)
			...
			local varQuestName = GetQuestNameNM( varMap, varPlayer, varQuest )
			local message = format(x310009_var_AcceptMessage, varQuestName);
			StartTalkTask(varMap)
			...
			for varI, item in x310009_var_QuestItem do
...
function x310009_ProcQuestAbandon( varMap, varPlayer, varQuest )
	local varRet = 0
	...
	if varRet == 1 then
		...
		local varQuestName = GetQuestNameNM( varMap, varPlayer, varQuest )
		local message = format(x310009_var_AbandonMessage, varQuestName);
		StartTalkTask(varMap);
		TalkAppendString(varMap, message);
		StopTalkTask();
		DeliverTalkTips(varMap, varPlayer);
		...
		QuestUnCom( varMap, varPlayer, x310009_var_VirtualQuestId)

	end
end


function x310009_ProcQuestAttach( varMap, varPlayer, varTalknpc, varTalkNpcGUID, varQuestIdx, varQuest )
...
function x310009_ProcQuestSubmit( varMap, varPlayer, varTalknpc, varButtonClick, varQuest )
	if IsHaveQuestNM( varMap, varPlayer, varQuest ) == 0 then	
		return
	end
	...
	LuaCallNoclosure( 203785, "Finishbaoxiang", varMap, varPlayer)
	QuestUnCom( varMap, varPlayer, x310009_var_VirtualQuestId)
	...
	x310009_SetMDDate(varMap, varPlayer, day)
		...
		local varQuestName = GetQuestNameNM( varMap, varPlayer, varQuest )
		local message = format(x310009_var_CompleteMessage, varQuestName);
		StartTalkTask(varMap);
			TalkAppendString(varMap, message);
		StopTalkTask();
		DeliverTalkTips(varMap, varPlayer);
		...
		ItemAppend( varMap, x310009_var_AwardItem, 1 )
		...
		if GetLevel(varMap, varPlayer) >= 80 and random(1, 100) <= 22 then
			...
			rndItemid = x310009_var_RandItem[varIndex]
		...
		if varRet > 0 then
			...
			message = format("#Y获得物品#R#{_ITEM%d}#Y的奖励", x310009_var_AwardItem )
			StartTalkTask(varMap);
			TalkAppendString(varMap, message);
			StopTalkTask();
			DeliverTalkTips(varMap, varPlayer);
...
function x310009_ProcQuestItemChanged( varMap, varPlayer, varItemData, varQuest )
...
function x310009_TianJiangBaoXiangDisconnect( varMap, varPlayer )

end

function x310009_ProcGpOpenItemBox( varMap, varPlayer, varTalknpc, varGpType, varNeedItem )
	return LuaCallNoclosure( QUEST_SCRIPT, "ProcGpOpenItemBox", varMap, varPlayer, varTalknpc, varGpType, varNeedItem )
end

function x310009_ProcGpRecycle( varMap, varPlayer, varTalknpc, varGpType, varNeedItem )
	return LuaCallNoclosure( QUEST_SCRIPT, "ProcGpRecycle", varMap, varPlayer, varTalknpc, varGpType, varNeedItem )
end

function x310009_ProcQuestLogRefresh( varMap, varPlayer, varQuest)
...
function x310009_ProcGpProcOver( varMap, varPlayer, varTalknpc )
end


function x310009_OpenCheck( varMap, varPlayer, varTalknpc )
end
