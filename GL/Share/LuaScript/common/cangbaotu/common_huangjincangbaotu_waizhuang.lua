...
x310060_var_FileId 					= 	310060                  
x310060_var_QuestId 				= 	8101

x310060_var_QuestKind 				= 	1                       
x310060_var_LevelLess					= 	40                      

x310060_var_QuestName				= 	"【个人】外装黄金藏宝图的秘密"                                        
x310060_var_QuestTarget				= 	"  去#aB#h00CCFF{goto_%d,%d,%d}%s(%d,%d)#aE处使用铁铲挖掘宝藏"      
x310060_var_QuestInfo				= 	"通过阅读这张外装黄金藏宝图，您发现了宝藏的所在。去#aB#h00CCFF{goto_%d,%d,%d}%s(%d,%d)#aE处去挖掘宝藏吧。\n\t注意：当你使用不绑定的藏宝图且有#G江湖请柬#W状态时，能够获得不绑定的奖励道具，否则为绑定。"            
x310060_var_ContinueInfo				= 	""  
x310060_var_QuestCompleted			= 	"\t看来您已经得到@item_12030107,并初步了解外装黄金藏宝图的秘密了吧？现在我给您一条线索，传说按照这条线索追查下去有可能得到传说中的宝藏！传说中的魂魄有可能栖息在地下粮仓，粮仓里深处的五个妖孽趁传说中魂魄虚弱的时候得到了它。打败那些妖孽就有可能得到@item_12030108。"  
x310060_var_QuestHelp				=	"去杂货商人处买一个铁铲，然后去宝藏所在地使用"          

x310060_var_strQuestAbandon         = "您放弃了任务：【个人】外装黄金藏宝图的秘密"


x310060_var_ExtTarget					=	{ {type=20,n=1,target="去杂货商人处买一把铁铲，然后去宝藏所在地使用。"} }



x310060_var_ExpBonus					= 	0                    	
x310060_var_BonusItem					=	{}	

x310060_var_BonusMoney1               = 	0  
x310060_var_BonusMoney2               =   0  
x310060_var_BonusMoney3               =   0  
x310060_var_BonusMoney4               =   0  
x310060_var_BonusMoney5               =   0  
x310060_var_BonusMoney6               =   0  

x310060_var_BonusChoiceItem           =   {}


--DECLARE_QUEST_INFO_STOP--


x310060_MP_TARGET1					= 1   --地图信息
x310060_MP_TARGET2					= 2   --坐标X
x310060_MP_TARGET3					= 3   --坐标Y
x310060_MP_TARGET4					= 4   --挖宝地图ID
x310060_MP_ISCOMPLETE				= 7	  --任务是否完成

x310060_PREVIOUS_QUESTID			= -1		
x310060_NEXT_QUESTID				= 7552		
x310060_NEXT_QUESTSCRIPTID			= 300754	
x310060_QUEST_HUANSHU				= 0			

x310060_var_ItemBonus = {
...
x310060_var_CallerId      = 0
x310060_var_CountryID     = 0
x310060_var_CallerName    = ""

x310060_var_DigRange      = 2

x310060_var_MoneyBuff     = 7046

x310060_var_GemBuff       = 7047

x310060_var_MedicineBuff  = 7048

x310060_var_MonsterBuff   = 7049	




function x310060_ProcEnumEvent( varMap, varPlayer, varTalknpc, varQuest )
...
function x310060_ProcEventEntry(varMap, varPlayer, varTalknpc, varQuest,varExtIdx)	

	local varHaveQuest	= IsHaveQuestNM(varMap, varPlayer, x310060_var_QuestId);
	if(varHaveQuest > 0) then
		local varDone = x310060_CheckSubmit(varMap, varPlayer, varTalknpc);
		if(varDone > 0) then
			if varExtIdx ~= -1 then
				x310060_DispatchCompletedInfo( varMap, varPlayer, varTalknpc ) 
			end
		else
			if varExtIdx ~= -1 then
				x310060_DispatchContinueInfo( varMap, varPlayer, varTalknpc ) 
			end
		end
	else
		x310060_DeliverTalkInfo( varMap, varPlayer, varTalknpc )	
	end
end
...
function x310060_ProcAcceptCheck( varMap, varPlayer, varTalknpc)
	local varHaveQuest	= IsHaveQuestNM(varMap, varPlayer, x310060_var_QuestId);
	if( varHaveQuest > 0) then
		return 0;
	else
...
function x310060_ProcQuestAccept( varMap, varPlayer, varTalknpc, varQuest)
	return 0
end
...
function x310060_ProcQuestAbandon(varMap, varPlayer, varQuest)
	local varHaveQuest	= IsHaveQuestNM(varMap, varPlayer, x310060_var_QuestId);
	if(varHaveQuest > 0) then
		local varQuestIdx = GetQuestIndexByID(varMap, varPlayer, x310060_var_QuestId); 
		DelQuest(varMap, varPlayer, x310060_var_QuestId)

		StartTalkTask( varMap)
		TalkAppendString( varMap, x310060_var_strQuestAbandon)
		StopTalkTask( varMap)
		DeliverTalkTips( varMap, varPlayer)
		Msg2Player( varMap, varPlayer, x310060_var_strQuestAbandon, 8, 2)
	end
end
...
function x310060_CheckSubmit(varMap, varPlayer, varTalknpc)

	local varQuestIdx = GetQuestIndexByID(varMap, varPlayer, x310060_var_QuestId);
	return GetQuestParam(varMap, varPlayer, varQuestIdx, x310060_MP_ISCOMPLETE);
...
function x310060_GetBonus( varMap, varPlayer,varTalknpc, varButtonClick )

	
	if x310060_var_BonusMoney1 > 0 then
	    AddMoney( varMap, varPlayer, 1, x310060_var_BonusMoney1 )
	    Msg2Player(varMap,varPlayer,format("获得银卡#{_MONEY%d}的奖励。", x310060_var_BonusMoney1),4,2)
	end

	
	if x310060_var_BonusMoney2 > 0 then
	    AddMoney( varMap, varPlayer, 0, x310060_var_BonusMoney2 )
	    Msg2Player(varMap,varPlayer,format("获得银币#{_MONEY%d}的奖励。", x310060_var_BonusMoney2),4,2)
	end

	
	if x310060_var_BonusMoney3 > 0 then
	    AddMoney( varMap, varPlayer, 3, x310060_var_BonusMoney3 )
	    Msg2Player(varMap,varPlayer,format("获得金卡#{_MONEY%d}的奖励。", x310060_var_BonusMoney3),4,2)
	end

	
	if x310060_var_BonusMoney4 > 0 then
		local varRongYu = GetRongYu( varMap, varPlayer )
		varRongYu = varRongYu + x310060_var_BonusMoney4
		SetRongYu( varMap, varPlayer, varRongYu )
		Msg2Player(varMap,varPlayer,format("获得江湖声望%d的奖励。", x310060_var_BonusMoney4),4,2)
	end

	
	if x310060_var_BonusMoney5 > 0 then
		local varShengWang = GetShengWang( varMap, varPlayer )
		varShengWang = varShengWang + x310060_var_BonusMoney5
		SetShengWang( varMap, varPlayer, varShengWang )
		Msg2Player(varMap,varPlayer,format("获得朝廷声望%d的奖励。", x310060_var_BonusMoney5),4,2)
	end

	
	if x310060_var_BonusMoney6 > 0 then
		AddHonor(varMap,varPlayer,x310060_var_BonusMoney6)
		Msg2Player(varMap,varPlayer,format("获得荣誉值%d的奖励。", x310060_var_BonusMoney6),4,2)
	end

 	
	for varI, item in x310060_var_BonusItem do
	...
	for varI, item in x310060_var_BonusChoiceItem do
...
function x310060_ProcQuestSubmit(varMap, varPlayer, varTalknpc, varRadioSelected, varQuest)
	...
	if varHaveQuest > 0 then
		...
		local varQuestIdx = GetQuestIndexByID(varMap, varPlayer, x310060_var_QuestId)
		local completed = GetQuestParam(varMap, varPlayer, varQuestIdx, x310060_MP_ISCOMPLETE)

		if completed == 1 then
			local result = x310060_CheckPlayerBagFull(varMap,varPlayer)
			if result == 1 then
		    	x310060_GetBonus( varMap,varPlayer,varTalknpc,varRadioSelected )
		    	DelQuest(varMap, varPlayer, x310060_var_QuestId)
...
function x310060_CheckPlayerBagFull( varMap ,varPlayer )
	...
	StartItemTask(varMap)
	for j, item in x310060_var_BonusItem do
		ItemAppendBind( varMap, item.item, item.n )
    end
	...
	if bAdd == 0 then
		...
		for j, item in x310060_var_BonusChoiceItem do
	        StartItemTask(varMap)
			ItemAppendBind( varMap, item.item, item.n )
			local varRet = StopItemTask(varMap,varPlayer)
			if varRet <= 0 then
...
function x310060_DeliverTalkInfo( varMap, varPlayer, varTalknpc )

	StartTalkTask(varMap)

		
		TalkAppendString(varMap,"#Y"..x310060_var_QuestName)
		TalkAppendString(varMap,x310060_var_QuestInfo)
		TalkAppendString(varMap," ")

		
		TalkAppendString( varMap,"#Y任务目标：")
		TalkAppendString( varMap,x310060_var_QuestTarget)
		TalkAppendString( varMap," ")

		
		if x310060_var_QuestHelp ~= "" then

			TalkAppendString(varMap,"#Y任务提示：")
			TalkAppendString(varMap,x310060_var_QuestHelp )
			TalkAppendString(varMap," ")
		end
		...
		if x310060_var_ExpBonus> 0 then
			AddQuestExpBonus(varMap, x310060_var_ExpBonus )
		end

		
		if x310060_var_BonusMoney1 > 0 then
			AddQuestMoneyBonus1(varMap, x310060_var_BonusMoney1 )
		end

		
		if x310060_var_BonusMoney2 > 0 then
			AddQuestMoneyBonus2(varMap, x310060_var_BonusMoney2 )
		end
		
		if x310060_var_BonusMoney3 > 0 then
			AddQuestMoneyBonus3(varMap, x310060_var_BonusMoney3 )
		end

		
		if x310060_var_BonusMoney4 > 0 then
			AddQuestMoneyBonus4(varMap, x310060_var_BonusMoney4 )
		end

		
		if x310060_var_BonusMoney5 > 0 then
			AddQuestMoneyBonus5(varMap, x310060_var_BonusMoney5 )
		end

		
		if x310060_var_BonusMoney6 > 0 then
			AddQuestMoneyBonus6(varMap, x310060_var_BonusMoney6 )
		end
	...
	StopTalkTask()
	DeliverTalkInfoNM(varMap, varPlayer, varTalknpc, x310060_var_FileId, x310060_var_QuestId);
end




function x310060_ProcDigbyShovel( varMap, varPlayer)
    ...
    if IsHaveQuestNM( varMap, varPlayer, x310060_var_QuestId) == 0 then       
        return 0
    else
        local varQuestIdx = GetQuestIndexByID( varMap, varPlayer, x310060_var_QuestId)
        local ts = GetQuestParam( varMap, varPlayer, varQuestIdx, x310060_MP_TARGET4)
        if ts ~= varMap then
    		StartTalkTask( varMap)
        ...
        local varX = GetQuestParam( varMap, varPlayer, varQuestIdx, x310060_MP_TARGET2)
        local z = GetQuestParam( varMap, varPlayer, varQuestIdx, x310060_MP_TARGET3)
        local curX, curZ = GetWorldPos( varMap, varPlayer)
        curX = abs( varX - curX)
        curZ = abs( z - curZ)
        if curX > x310060_var_DigRange or curZ > x310060_var_DigRange then
    		StartTalkTask( varMap)
            ...
            return 0
 				...
 				local varprobability = random (1, 100000)
 					for i, iter in x310060_var_ItemBonus do
          	...
          	Msg2Player( varMap, varPlayer, strTemp, 8, 2)	
          		for i,j in x310060_var_notifytable do
...
--          	i = random( 1, getn( x310060_var_Bosstable) )
--          	local item = x310060_var_Bosstable
...
--						LuaAllScenceM2Wrold(varMap, msg1, CHAT_LEFTDOWN, 1)
 					...
 					DelQuest( varMap, varPlayer, x310060_var_QuestId)
 				 -- 提示任务完成
 				 Msg2Player( varMap, varPlayer, "您完成了："..x310060_var_QuestName, 8, 2)
 				 Msg2Player( varMap, varPlayer, "您完成了："..x310060_var_QuestName, 8, 3)
 				 -- 写日志
...
function x310060_CallBoss( varMap, varPlayer)
    local monsters = {}
    local varCount = 1
    ...
    for varI, item in x310060_var_MonsterTable do
    ...
    for varI, item in x310060_var_MonsterLive do
    ...
    local varX, z = GetWorldPos( varMap, varPlayer)
    local mapName = ""
    CreateMonster( varMap, monsters[ varI].bossId, varX + random( 1, 3), z + random( 1, 3), 1, -1, x310060_var_FileId, -1, 21, 30*60*1000)
    
    for varI, item in x310060_var_MonsterLive do
        if item.varMap == varMap then
            x310060_var_MonsterLive[ varI].live = 1
...
function x310060_ProcDie( varMap, varPlayer, varKiller)
    ...
    for varI, item in x310060_var_MonsterLive do
        if item.varMap == varMap then
            x310060_var_MonsterLive[ varI].live = 0
        end
    end
...
function x310060_DispatchContinueInfo( varMap, varPlayer, varTalknpc )
end




function x310060_DispatchCompletedInfo( varMap, varPlayer, varTalknpc )
end




function x310060_ProcQuestObjectKilled(varMap, varPlayer, varObjData ,varObj, varQuest )
end




function x310060_ProcQuestItemChanged( varMap, varPlayer, varItemData, varQuest )
end




function x310060_ProcQuestAttach( varMap, varPlayer, varTalknpc, varTalkNpcGUID, varQuestIdx, varQuest )
end




function x310060_ProcQuestLogRefresh( varMap, varPlayer, varQuest)
    local varQuestIdx = GetQuestIndexByID( varMap, varPlayer, x310060_var_QuestId)
    local mapID = GetQuestParam( varMap, varPlayer, varQuestIdx, x310060_MP_TARGET1)
    local varTalknpc = GetQuestParam( varMap, varPlayer, varQuestIdx, x310060_MP_TARGET4)
    local varX = GetQuestParam( varMap, varPlayer, varQuestIdx, x310060_MP_TARGET2)
    local z = GetQuestParam( varMap, varPlayer, varQuestIdx, x310060_MP_TARGET3)
    local strMapName = ""
	...
	StartTalkTask(varMap)
        AddQuestLogCustomText( varMap,
                                "",                             
                                x310060_var_QuestName,             
                                format( x310060_var_QuestTarget, varTalknpc, varX, z,strMapName,varX,z),
                                "无",                             
                                x310060_var_QuestHelp,
                                format( x310060_var_QuestInfo, varTalknpc, varX, z,strMapName,varX,z),
                                "" )
...
function x310060_ProcForceAddQuest( varMap, varPlayer, varTalknpc, varQuest)
	if( varQuest ~= x310060_var_QuestId) then
		return 0;
	end

	
	if(x310060_ProcAcceptCheck( varMap, varPlayer, -1 ) ~= 1) then
		return 0;
	end
	local varRet = AddQuest( varMap, varPlayer, x310060_var_QuestId, x310060_var_FileId, 0, 0, 0, 0)
	if(varRet == 0) then
		StartTalkTask(varMap)
	    ...
	    local size = getn( x310060_var_QuestTargetTable0)
	    ...
	    local varQuestIdx = GetQuestIndexByID(varMap, varPlayer, x310060_var_QuestId)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x310060_MP_TARGET1, x310060_var_QuestTargetTable0[ varI].varMap)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x310060_MP_TARGET2, x310060_var_QuestTargetTable0[ varI].posX)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x310060_MP_TARGET3, x310060_var_QuestTargetTable0[ varI].posZ)
	    local mapid = GetCurCountry( varMap, varPlayer) * 100 + x310060_var_QuestTargetTable0[ varI].varMap + 50
	    if x310060_var_QuestTargetTable0[ varI].varMap == 2 then
	        mapid = 0
	    elseif x310060_var_QuestTargetTable0[ varI].varMap == 3 then
	        mapid = 15
	    elseif x310060_var_QuestTargetTable0[ varI].varMap == 5 then
	        mapid = 5	        
	    elseif x310060_var_QuestTargetTable0[ varI].varMap == 8 then
	        mapid = 8		      
	    elseif x310060_var_QuestTargetTable0[ varI].varMap == 11 then
	        mapid = 11	
	    elseif x310060_var_QuestTargetTable0[ varI].varMap == 17 then
	        mapid = 17	     
	    elseif x310060_var_QuestTargetTable0[ varI].varMap == 20 then
	        mapid = 20	        
	    elseif x310060_var_QuestTargetTable0[ varI].varMap == 23 then
	        mapid = 23		     
	    end
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x310060_MP_TARGET4, mapid)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x310060_MP_ISCOMPLETE, 0)
	    ...
	    local size = getn( x310060_var_QuestTargetTable1)
	    local varI = random( 1, size)
	    ...
	    local varQuestIdx = GetQuestIndexByID(varMap, varPlayer, x310060_var_QuestId)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x310060_MP_TARGET1, x310060_var_QuestTargetTable1[ varI].varMap)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x310060_MP_TARGET2, x310060_var_QuestTargetTable1[ varI].posX)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x310060_MP_TARGET3, x310060_var_QuestTargetTable1[ varI].posZ)
	    local mapid= GetCurCountry( varMap, varPlayer) * 100 + x310060_var_QuestTargetTable1[ varI].varMap + 50
	    if x310060_var_QuestTargetTable1[ varI].varMap == 2 then
	        mapid = 0
	    elseif x310060_var_QuestTargetTable1[ varI].varMap == 3 then
	        mapid = 15
	    elseif x310060_var_QuestTargetTable1[ varI].varMap == 5 then
	        mapid = 5	        
	    elseif x310060_var_QuestTargetTable1[ varI].varMap == 8 then
	        mapid = 8		      
	    elseif x310060_var_QuestTargetTable1[ varI].varMap == 11 then
	        mapid = 11	
	    elseif x310060_var_QuestTargetTable1[ varI].varMap == 17 then
	        mapid = 17	     
	    elseif x310060_var_QuestTargetTable1[ varI].varMap == 20 then
	        mapid = 20	        
	    elseif x310060_var_QuestTargetTable1[ varI].varMap == 23 then
	        mapid = 23		        
	    end
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x310060_MP_TARGET4, mapid)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x310060_MP_ISCOMPLETE, 0)
	    ...
	    local size = getn( x310060_var_QuestTargetTable2)
	    local varI = random( 1, size)
	    ...
	    local varQuestIdx = GetQuestIndexByID(varMap, varPlayer, x310060_var_QuestId)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x310060_MP_TARGET1, x310060_var_QuestTargetTable2[ varI].varMap)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x310060_MP_TARGET2, x310060_var_QuestTargetTable2[ varI].posX)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x310060_MP_TARGET3, x310060_var_QuestTargetTable2[ varI].posZ)
	    local mapid = GetCurCountry( varMap, varPlayer) * 100 + x310060_var_QuestTargetTable2[ varI].varMap + 50
	    if x310060_var_QuestTargetTable2[ varI].varMap == 2 then
	        mapid = 0
	    elseif x310060_var_QuestTargetTable2[ varI].varMap == 3 then
	        mapid = 15
	    elseif x310060_var_QuestTargetTable2[ varI].varMap == 5 then
	        mapid = 5	        
	    elseif x310060_var_QuestTargetTable2[ varI].varMap == 8 then
	        mapid = 8		      
	    elseif x310060_var_QuestTargetTable2[ varI].varMap == 11 then
	        mapid = 11	
	    elseif x310060_var_QuestTargetTable2[ varI].varMap == 17 then
	        mapid = 17	     
	    elseif x310060_var_QuestTargetTable2[ varI].varMap == 20 then
	        mapid = 20	        
	    elseif x310060_var_QuestTargetTable2[ varI].varMap == 23 then
	        mapid = 23		
	    end
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x310060_MP_TARGET4, mapid)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x310060_MP_ISCOMPLETE, 0)
	    ...
	    local size = getn( x310060_var_QuestTargetTable3)
	    local varI = random( 1, size)
	    ...
	    local varQuestIdx = GetQuestIndexByID(varMap, varPlayer, x310060_var_QuestId)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x310060_MP_TARGET1, x310060_var_QuestTargetTable3[ varI].varMap)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x310060_MP_TARGET2, x310060_var_QuestTargetTable3[ varI].posX)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x310060_MP_TARGET3, x310060_var_QuestTargetTable3[ varI].posZ)
	    local mapid = GetCurCountry( varMap, varPlayer) * 100 + x310060_var_QuestTargetTable3[ varI].varMap + 50
	    if x310060_var_QuestTargetTable3[ varI].varMap == 2 then
	        mapid = 0
	    elseif x310060_var_QuestTargetTable3[ varI].varMap == 3 then
	        mapid = 15
	    elseif x310060_var_QuestTargetTable3[ varI].varMap == 5 then
	        mapid = 5	        
	    elseif x310060_var_QuestTargetTable3[ varI].varMap == 8 then
	        mapid = 8		      
	    elseif x310060_var_QuestTargetTable3[ varI].varMap == 11 then
	        mapid = 11	
	    elseif x310060_var_QuestTargetTable3[ varI].varMap == 17 then
	        mapid = 17	     
	    elseif x310060_var_QuestTargetTable3[ varI].varMap == 20 then
	        mapid = 20	        
	    elseif x310060_var_QuestTargetTable3[ varI].varMap == 23 then
	        mapid = 23		        
	    end
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x310060_MP_TARGET4, mapid)
	    SetQuestByIndex( varMap, varPlayer, varQuestIdx, x310060_MP_ISCOMPLETE, 0)
...
function x310060_ProcCreaterGetBonus(varMap,guid,questdata)
