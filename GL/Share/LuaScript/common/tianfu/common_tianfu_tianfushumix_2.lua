x570058_var_FileId = 570058
x570058_var_ScriptName = "二级平衡之书合成"
x570058_var_UICommandID = 100
x570058_var_Mixture_RESULT_LESS = -2  
x570058_var_Mixture_RESULT_FAILD = -1	
x570058_var_Mixture_RESULT_OK = 1	
x570058_var_Mixture_AddItem_UseType = 0	
...
x570058_MixConfTable_Ext      = {
									{varItem = 11010111, alwaysKeep = 1},
									{varItem = 11010120, alwaysKeep = 0},
									{varItem = 11010121, alwaysKeep = 0},
								}

function x570058_ProcEnumEvent( varMap, varPlayer, varTalknpc, varQuest )
	TalkAppendButton(varMap, x570058_var_FileId, x570058_var_ScriptName,3)
end

function x570058_ProcEventEntry( varMap, varPlayer, varTalknpc, varQuest )
    StartGCDTask(varMap);
    GCDAppendInt(varMap, x570058_var_FileId);
    GCDAppendInt(varMap, varTalknpc);
    DeliverGCDInfo(varMap,varPlayer, x570058_var_UICommandID);
    StopGCDTask(varMap);
end

function x570058_ProcMixture( varMap, varPlayer, item1, item2, item3, item4 ,AddedItemPos)
	...
	if x570058_CheckItemCanMixture(varItem[1], varItem[2], varItem[3], varItem[4]) <=0 then
		StartTalkTask(varMap);
		TalkAppendString( varMap,"放入的道具不正确，无法合成");
		StopTalkTask( varMap);
		DeliverTalkTips( varMap, varPlayer);
		return x570058_var_Mixture_RESULT_LESS;		
	end

	
	if x570058_CheckItemCount(varMap, varPlayer,varItem[1], varItem[2], varItem[3], varItem[4]) <=0 then
		StartTalkTask(varMap);
	...
	if x570058_isNeedAddedItem() > 0 then
		nAddItemId = x570058_GetAndCheckAddedItemId(varMap, varPlayer,AddedItemPos)
	...
	if x570058_IsMixtureRandOK() == 1 then
		...
		if isBind == 0 then
			ItemAppend( varMap, x570058_MixConfTable.mixitem, x570058_MixConfTable.mixitemcount)
		else
			ItemAppendBind( varMap, x570058_MixConfTable.mixitem, x570058_MixConfTable.mixitemcount)
		end
		...
		if ( varRet > 0 ) then
			...
			for varI = 1, 4 do
				if( itemPos[ varI] ~= -1) then
					local varDelCount = x570058_GetItemNeedCountByItemPos(varMap, varPlayer, itemPos[ varI])
			...
			StopTalkTask(varMap);
			DeliverTalkTips(varMap,varPlayer)
			QuestMixtureResult( varMap, varPlayer, x570058_var_Mixture_RESULT_OK, itemPos[1], itemPos[2], itemPos[3], itemPos[4] )
			return x570058_var_Mixture_RESULT_OK
		else
			StartTalkTask( varMap)
		...
		for varI = 1, 4 do
			if( itemPos[ varI] ~= -1) then
				...
				if x570058_isNeedAddedItem() <= 0  or  nAddItemId <= 0 or x570058_isAlwaysKeep(varItem[varI]) <= 0 then
					local varDelCount = x570058_GetItemNeedCountByItemPos(varMap, varPlayer, itemPos[ varI])
					local nCurPosCount = GetItemCountInBagPos(varMap, varPlayer,itemPos[ varI])
...
function x570058_CheckItemCount(varMap, varPlayer,itemId1, itemId2, itemId3, itemId4)

	local nNeedCount =0

    nNeedCount = x570058_GetItemNeedCountByItemId(varMap, varPlayer, itemId1)
    ...
    nNeedCount = x570058_GetItemNeedCountByItemId(varMap, varPlayer, itemId2)
	if  nNeedCount > 0 then
    ...
    nNeedCount = x570058_GetItemNeedCountByItemId(varMap, varPlayer, itemId3)
	if  nNeedCount > 0 then
    ...
    nNeedCount = x570058_GetItemNeedCountByItemId(varMap, varPlayer, itemId4)
	if  nNeedCount > 0 then
...
function x570058_GetItemNeedCountByItemPos(varMap, varPlayer, itemPos)
	...
	return x570058_GetItemNeedCountByItemId(varMap, varPlayer,varItem);

end



function x570058_GetItemNeedCountByItemId(varMap, varPlayer, varItem)

	if varItem < 0 then
		return 0	
	end

	if x570058_MixConfTable.itemid1 == varItem then
		return x570058_MixConfTable.itemid1count
	end

	if x570058_MixConfTable.itemid2 == varItem then
		return x570058_MixConfTable.itemid2count
	end
	
	if x570058_MixConfTable.itemid3 == varItem then
		return x570058_MixConfTable.itemid3count
	end

	if x570058_MixConfTable.itemid4 == varItem then
...
function x570058_IsNeedItem(varItem)
	if x570058_MixConfTable.itemid1 == varItem then
		return 1;
	end
...
function  x570058_GetAndCheckAddedItemId(varMap, varPlayer, nItemPos)
	...
	local bRightItem = CheckSpecialItem(nAddedItemId,x570058_var_Mixture_AddItem_UseType);
...
function  x570058_isAlwaysKeep(nItemId)
	...
	if nItemId == nil or nItemId <=0 then
	 ...
	 for varI,item in x570058_MixConfTable_Ext do
		if x570058_MixConfTable_Ext[varI].varItem  == nItemId then
			return x570058_MixConfTable_Ext[varI].alwaysKeep;
		end
	 end
