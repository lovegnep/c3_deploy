...
x310035_var_FileId = 310035


x310035_var_QuestName="圣山地宫传入1"



x310035_var_MaxHuman = 100
x310035_var_MaxHumanOther = 200 

x310035_var_ItemId = 12030027

function x310035_IsOpenedTime()
...
function x310035_HaveAccepted(varMap, varPlayer)
...
function x310035_ProcEnumEvent( varMap, varPlayer, varTalknpc, varQuest )
	if GetLevel(varMap, varPlayer) < 70 then
		return
	end
    TalkAppendButton(varMap,x310035_var_FileId,"圣山地宫(70级)",14,-1)
     
end


function x310035_ProcEventEntry( varMap, varPlayer, varTalknpc,idScript,idExt )
		...
		StartTalkTask(varMap)
		  ...
		  local varReadme = Readme_1..Readme_2..Readme_3
			...
			StopTalkTask(varMap)	
		DeliverTalkInfo(varMap,varPlayer,varTalknpc,x310035_var_FileId,-1)
	return 
...
function x310035_ProcAcceptCheck( varMap, varPlayer, varTalknpc )
		return 1
end


function x310035_IsGameTime()
	local minute = GetMinOfDay();
...
function x310035_ProcAccept( varMap, varPlayer )
	...
	--换天清零	
	if varLastday ~= GetDayOfYear() then
		x310035_SetMD( varMap, varPlayer, MD_YAOXIE_DIGONG_DAYCOUNT, 0)	
	end

	--换周清零
	if varLastweek ~= GetWeekIndex() then
		x310035_SetMD( varMap, varPlayer, MD_YAOXIE_DIGONG_WEEKCOUNT, 0)	
	end

	--获取每日和每周完成次数
	local daycount = x310035_GetMD( varMap, varPlayer, MD_YAOXIE_DIGONG_DAYCOUNT)
	local weekcount = x310035_GetMD( varMap, varPlayer, MD_YAOXIE_DIGONG_WEEKCOUNT)	
	...
	if bHave == 0 then
		if x310035_IsOpenedTime() < 0 then
...
--	if x310035_HaveAccepted(varMap, varPlayer) > 0 then
--		
...
--	end
	...
	if bHave == 0 then
		if GetItemCount(varMap, varPlayer, x310035_var_ItemId)  == 0 then
	...
	local curNum = 0
	if countryId == 0 then
		curNum = YX_LOULAN_HUMAN_NUM3 
	elseif countryId == 1 then
		curNum = YX_LAIYIN_HUMAN_NUM3 
	elseif countryId == 2 then
		curNum = YX_KUNLUN_HUMAN_NUM3 
	elseif countryId == 3 then
		curNum = YX_DUNHUANG_HUMAN_NUM3 
	end
	local sign = 0;
	...
	if daycount < 3 and weekcount < 7 then
		x310035_SetMD( varMap, varPlayer, MD_YAOXIE_DIGONG_DAYCOUNT, daycount + 1)	
		x310035_SetMD( varMap, varPlayer, MD_YAOXIE_DIGONG_WEEKCOUNT, weekcount + 1)
	end
	
	if sign == 0 then
		return
	elseif sign == 1 then
		if DelItem(varMap, varPlayer, x310035_var_ItemId, 1) ~= 1 then return 0 end
	elseif sign == 2 then
	...
	if countryId == 0 then
		NewSceneId = YX_LOULAN_SCENE_ID3
		YX_LOULAN_HUMAN_NUM3 = YX_LOULAN_HUMAN_NUM3 + 1
	elseif countryId == 1 then
		NewSceneId = YX_LAIYIN_SCENE_ID3
		YX_LAIYIN_HUMAN_NUM3 = YX_LAIYIN_HUMAN_NUM3 + 1
	elseif countryId == 2 then
		NewSceneId = YX_KUNLUN_SCENE_ID3
		YX_KUNLUN_HUMAN_NUM3 = YX_KUNLUN_HUMAN_NUM3 + 1
	elseif countryId == 3 then
		NewSceneId = YX_DUNHUANG_SCENE_ID3
		YX_DUNHUANG_HUMAN_NUM3 = YX_DUNHUANG_HUMAN_NUM3 + 1
	end
	local PosX = 67
	local PosZ = 63
	
	NewWorld(varMap, varPlayer, NewSceneId,PosX ,PosZ, 310035 )
end
...
function x310035_GetMD( varMap, varPlayer, mdName)
    return GetPlayerGameData( varMap, varPlayer, mdName[ 1], mdName[ 2], mdName[ 3] )
end

function x310035_SetMD( varMap, varPlayer, mdName, value)
