...
x310034_var_FileId = 310034


x310034_var_QuestName="圣山地宫传入1"

x310034_var_MaxHuman = 100
x310034_var_MaxHumanOther = 200

x310034_var_ItemId = 12030027
...
function x310034_HaveAccepted(varMap, varPlayer)
...
function x310034_ProcEnumEvent( varMap, varPlayer, varTalknpc, varQuest )
	if GetLevel(varMap, varPlayer) < 60 then
		return
	end
    TalkAppendButton(varMap,x310034_var_FileId,"圣山地宫(60级)",14,-1)
     
end


function x310034_ProcEventEntry( varMap, varPlayer, varTalknpc,idScript,idExt )
	...
	StartTalkTask(varMap)
	  ...
	  local varReadme = Readme_1..Readme_2..Readme_3
		...
		StopTalkTask(varMap)	
	DeliverTalkInfo(varMap,varPlayer,varTalknpc,x310034_var_FileId,-1)
...
function x310034_ProcAcceptCheck( varMap, varPlayer, varTalknpc )
		return 1
end
...
function x310034_ProcAccept( varMap, varPlayer )
	...
	--换天清零	
	if varLastday ~= GetDayOfYear() then
		x310034_SetMD( varMap, varPlayer, MD_YAOXIE_DIGONG_DAYCOUNT, 0)	
	end

	--换周清零
	if varLastweek ~= GetWeekIndex() then
		x310034_SetMD( varMap, varPlayer, MD_YAOXIE_DIGONG_WEEKCOUNT, 0)	
	end

	--获取每日和每周完成次数
	local daycount = x310034_GetMD( varMap, varPlayer, MD_YAOXIE_DIGONG_DAYCOUNT)
	local weekcount = x310034_GetMD( varMap, varPlayer, MD_YAOXIE_DIGONG_WEEKCOUNT)	
...
--	if x310034_HaveAccepted(varMap, varPlayer) > 0 then
--		
...
--	end
	...
	if bHave == 0 then
		if GetItemCount(varMap, varPlayer, x310034_var_ItemId)  == 0 then
	...
	if daycount < 3 and weekcount < 7 then
		x310034_SetMD( varMap, varPlayer, MD_YAOXIE_DIGONG_DAYCOUNT, daycount + 1)	
		x310034_SetMD( varMap, varPlayer, MD_YAOXIE_DIGONG_WEEKCOUNT, weekcount + 1)
	end
	
	if sign == 0 then
		return
	elseif sign == 1 then
		if DelItem(varMap, varPlayer, x310034_var_ItemId, 1) ~= 1 then return 0 end
	elseif sign == 2 then
	...
	NewWorld(varMap, varPlayer, NewSceneId,PosX ,PosZ, 310034 )
end
...
function x310034_GetMD( varMap, varPlayer, mdName)
    return GetPlayerGameData( varMap, varPlayer, mdName[ 1], mdName[ 2], mdName[ 3] )
end

function x310034_SetMD( varMap, varPlayer, mdName, value)
