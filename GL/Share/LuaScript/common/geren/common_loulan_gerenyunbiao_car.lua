...
x300623_var_FileId = 300623
x300623_var_QuestName = "创建镖车"


x300623_var_GodIndex = 1210

x300623_var_Carinfo = {
						{ CarType = 7,BaseAI = 3,AIScript = 3,Script = 300623 },
						{ CarType = 8,BaseAI = 3,AIScript = 3,Script = 300623 },
						{ CarType = 9,BaseAI = 3,AIScript = 3,Script = 300623 },
						{ CarType = 38,BaseAI = 3,AIScript = 3,Script = 300623 },
						{ CarType = 39,BaseAI = 3,AIScript = 3,Script = 300623 },
						{ CarType = 40,BaseAI = 3,AIScript = 3,Script = 300623 },
...
x300623_var_QuestId = {
						{ varId = 7001,varCostMoney = 1000, GameId = 51 },
						{ varId = 7009,varCostMoney = 2000, GameId = 61 },
						{ varId = 7010,varCostMoney = 3000,GameId = 71 },
						{ varId = 7543,varCostMoney = 5000,GameId = 81 },
						{ varId = 7544,varCostMoney = 6000,GameId = 741 },
						{ varId = 7545,varCostMoney = 7500,GameId = 751 },
					  }
...
function x300623_ProcEventEntry(varMap, varPlayer, varTalknpc)	
...
function x300623_ProcEnumEvent(varMap, varPlayer, varTalknpc)
	
end


function x300623_ProcCreateBus( varMap, varPlayer,varQuest ,varBusID)

    local PosX = 65
    local PosZ = 179.5
	local varObj = CreateBus(varMap, x300623_var_Carinfo[varBusID].CarType, PosX, PosZ, x300623_var_Carinfo[varBusID].BaseAI, x300623_var_Carinfo[varBusID].AIScript, x300623_var_Carinfo[varBusID].Script, varPlayer, x300623_var_GodIndex,varQuest,-1)
end

function x300623_ProcCreateBusOK(varMap,varPlayer,busId,varTalknpc,varQuest)	
...
function x300623_DeleteBus(varMap, varObj)
...
function x300623_ProcIntoBusEvent(varMap, varPlayer,varTalknpc)
...
function x300623_ProcIntoBusAfter(varMap, varPlayer,varBusID)
	...
	for varI,itm in x300623_var_QuestId do
...
function x300623_ProcLeaveBusEvent(varMap, varPlayer,BusId)
		StartTalkTask(varMap)
...
function x300623_ProcDie(varMap,BusId,idKiller)
	...
	if idKiller == -1 then
		...
		if OwnerID ~= -1 then
			x300623_Abandon( varMap, OwnerID  )
...
function x300623_ProcTime(varMap,BusId)

end

function x300623_ProcLifeTimeOut(varMap,BusId)
    local varPlayer = GetBusMemberObjId(varMap,BusId,0)
    if(IsPlayerStateNormal(varMap, varPlayer) ~= 0) then
		x300623_Abandon( varMap, varPlayer  )
	end
	DeleteBus(varMap, BusId,1)
end

function x300623_ProcWaitTimeOut(varMap,BusId)
	local varPlayer = GetBusMemberObjId(varMap,BusId,0)
	if(IsPlayerStateNormal(varMap, varPlayer) ~= 0) then
		x300623_Abandon( varMap, varPlayer  )
...
function x300623_ProcBusStopWhenOwnerFarAway(varMap, ownerId)
end
...
function x300623_CleaupQuest( varMap, varPlayer )
...
function x300623_SetDayCount(varMap, varPlayer)
