...
x302614_var_ScriptId 	 				 =	302614
x302614_var_LevelLess					= 	30

x302614_var_MissionId = {  
											10317,
											10318,
											10319,
											10320,
...
x302614_var_MissionName = "【个人】"
x302614_var_MissionNameMore = {
...
x302614_var_MissionInfo = "您可以通过王城押运官并花费#G#{_MONEY%d}银卡#W领取一辆#G%s#W，当您将%s安全护送到指定地点，系统将会给与您#G海量经验#W。"
x302614_var_MissionTarget={
												"将%s交给@npc_129174", 
												"将%s交给@npc_520113", 
												"将%s交给@npc_131514", 
												"将%s交给@npc_131514", 
...
--x302614_var_NeedPhysicalForce	= 9							   
x302614_var_MissionHelp = "\t#G护送途中如果您离开%s过远，%s会意识到危险而停止前进。\n\t#G如果%s受到袭击并死亡，您的任务将失败，同时部分押金将会掉落。\n\t#G当您离开%s一段时间或掉线，%s将消失，此时任务失败。\n\t如果您身上同时拥有#G银卡和现银，则优先扣除银卡。\n\t在#R国运令#G期间完成任务您将获得#R双倍#G的奖励。"
x302614_var_MissionCompleted		= "\t您已经将官府交给您的马车安全护送到此了？请收下官府给您的奖励！"
x302614_var_MissionUncompleted  ="\t如果不清除这些阿兰人，送往中国的贡品就不会按时完成。"
...
--扣除任务道具
x302614_var_QuestItem = 12270600	
x302614_var_QuestItemInfo = "缺少官府密令"

--记录玩家选择的任务选项
x302614_var_GetPlayerQuestNum = {} 	
-- bus
x302614_var_Mis_Count   =	3                    		--每天可做的次数
x302614_var_BusScriptId = 302611
x302614_var_GameOpenId = 3012
--15:28 2013/7/24 新加参数
x302614_var_nExpRate = 1			-- 经验比率
x302614_var_nMoneyRate = 0.18			-- 现银比率
x302614_var_nGoldsRate = 0.18			-- 金卡比率
				                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          
--DECLARE_QUEST_INFO_STOP--

function x302614_ProcEnumEvent( varMap, varPlayer, targetId, MissionId )
--	if x302614_var_GameOpenId > 0 and GetGameOpenById( x302614_var_GameOpenId ) <= 0 then
--      return
--  end
	local MissionIndex = 0
	local level = GetLevel(varMap,varPlayer)
	
	for i,lvl in x302614_var_LevelTable do
		if level >= lvl.LevelLess and level < lvl.LevelMax then
			MissionIndex = i
			break
		end
	end
	local bHaveMission = x302614_IsHaveMission(varMap, varPlayer)
	if bHaveMission <= 0 and MissionIndex > 0 then
		TalkAppendButton(varMap, x302614_var_MissionId[1], "【个人】官府押运", 8, 4)
	end

end

--点击（按钮）点击该任务后执行此脚本
function x302614_ProcEventEntry( varMap, varPlayer, targetId, MissionId, MissionNum )
--	if x302614_var_GameOpenId > 0 and GetGameOpenById( x302614_var_GameOpenId ) <= 0 then
--      return
--  end	
	local missionIndex = 0
	...
	if ( MissionNum == 4 ) then
		...
		if bHaveMission <= 0 and missionIndex > 0 then
			...
			local MissionState = GetQuestStateNM( varMap, varPlayer, targetId, x302614_var_MissionId[missionIndex] )
			...
			StartTalkTask( varMap )
				...
				TalkAppendButton(varMap, x302614_var_MissionId[missionIndex], x302614_var_MissionName..x302614_var_MissionNameMore[missionIndex][2], MissionState, 2);
				if level >= 50 then
					TalkAppendButton(varMap, x302614_var_MissionId[missionIndex], x302614_var_MissionName..x302614_var_MissionNameMore[missionIndex][1], MissionState, 1);
				end
				TalkAppendButton(varMap, x302614_var_MissionId[missionIndex], x302614_var_MissionName..x302614_var_MissionNameMore[missionIndex][3], MissionState, 3);
			StopTalkTask(varMap)
		...
		for i,mission in x302614_var_MissionId do
		...
		x302614_var_GetPlayerQuestNum[GetGUID(varMap, varPlayer)] = {}
		x302614_var_GetPlayerQuestNum[GetGUID(varMap, varPlayer)].missionIndex = missionIndex
		x302614_var_GetPlayerQuestNum[GetGUID(varMap, varPlayer)].missionNum = missionNum
		
		local bHaveMission	= IsHaveQuestNM(varMap, varPlayer, MissionId)
		if(bHaveMission > 0) then
			local bDone = x302614_CheckSubmit( varMap, varPlayer, targetId, missionIndex )
			if(bDone > 0) then
				if missionIndex ~= -1 then
					x302614_DispatchCompletedInfo( varMap, varPlayer, targetId, missionIndex, missionNum ) --完成任务显示
				end
			end
		else
			x302614_DispatchMissionInfo( varMap, varPlayer, targetId, missionIndex, missionNum )	--任务信息显示
		end
	end
...
--显示任务信息
function x302614_DispatchMissionInfo( varMap, varPlayer, NPCId, missionIndex, missionNum )

	StartTalkTask( varMap )
		--任务信息
		TalkAppendString(varMap, "#Y"..x302614_var_MissionName..x302614_var_MissionNameMore[missionIndex][missionNum])
		--local message = format( x302614_var_MissionInfo.."\n \n\t#G领取任务需要消耗#R"..x302614_var_NeedPhysicalForce.."点#G体力值", x302614_var_QuestMoney[missionIndex][missionNum], x302614_var_Mid[missionNum], x302614_var_Mid[missionNum] )
		...
		TalkAppendString( varMap, format( "\t%s", format( x302614_var_MissionTarget[missionIndex], x302614_var_Mid[missionNum] ) ) )
		TalkAppendString( varMap, " ")
		--提示信息
		if x302614_var_MissionHelp ~= "" then
			TalkAppendString(varMap, "#Y任务提示：")
			TalkAppendString(varMap, format( x302614_var_MissionHelp, x302614_var_Mid[missionNum], x302614_var_Mid[missionNum], x302614_var_Mid[missionNum], x302614_var_Mid[missionNum], x302614_var_Mid[missionNum] ) )
		end
		
		--奖励信息
		--1、经验
		local level = GetLevel( varMap, varPlayer )
		local experience = x302614_GetQuestExp(varMap, varPlayer, x302614_var_MissionId[missionIndex])
		...
		if (IsHaveSpecificImpact( varMap, varPlayer, 9011) == 1 or IsHaveSpecificImpact( varMap, varPlayer, 9013) == 1) then
			local Money = x302614_GetQuestMoney(varMap, varPlayer, x302614_var_MissionId[missionIndex])
			AddQuestMoneyBonus2( varMap, Money)
		elseif (IsHaveSpecificImpact( varMap, varPlayer, 9012) == 1 ) then
			local Golsds = x302614_GetQuestGolds(varMap, varPlayer, x302614_var_MissionId[missionIndex])
			AddQuestMoneyBonus3( varMap, Golsds)			
		end

	StopTalkTask(varMap)

	DeliverTalkInfoNM(varMap, varPlayer, NPCId, x302614_var_ScriptId,  x302614_var_MissionId[missionIndex]);
end

---------------------------------------------------------------------------------------------------
--向客户端发送任务完成信息
---------------------------------------------------------------------------------------------------
function x302614_DispatchCompletedInfo( varMap, varPlayer, targetId, missionIndex, missionNum )
	
	StartTalkTask(varMap)

		--任务完成信息
		TalkAppendString(varMap, "#Y"..x302614_var_MissionName..x302614_var_MissionNameMore[missionIndex][missionNum])
		TalkAppendString(varMap, format( x302614_var_MissionCompleted, x302614_var_Mid[missionNum] ))
		TalkAppendString(varMap, " ")
	...
	StopTalkTask(varMap)
	DeliverTalkContinueNM(varMap, varPlayer, targetId, x302614_var_ScriptId , x302614_var_MissionId[missionIndex])
...
--接受
--**********************************
function x302614_ProcQuestAccept( varMap, varPlayer, targetId, MissionId )
--	if x302614_var_GameOpenId > 0 and GetGameOpenById( x302614_var_GameOpenId ) <= 0 then
--      return
--  end
	local missionIndex  = 0
	...
	if x302614_var_GetPlayerQuestNum[GetGUID(varMap, varPlayer)].missionIndex  ~= nil and x302614_var_GetPlayerQuestNum[GetGUID(varMap, varPlayer)].missionNum ~= nil then
		missionIndex = x302614_var_GetPlayerQuestNum[GetGUID(varMap, varPlayer)].missionIndex
		missionNum = x302614_var_GetPlayerQuestNum[GetGUID(varMap, varPlayer)].missionNum
	end
--	local IshavePhysicalForce = GetPhysicalForce(varMap, varPlayer)
--	if IshavePhysicalForce < x302614_var_NeedPhysicalForce then
...
--	end
	...
	if x302614_GetDayCount(varMap, varPlayer) >= x302614_var_Mis_Count then
	  StartTalkTask(varMap)
	...
	local iCostMoney = 0
	iCostMoney = x302614_var_QuestMoney[missionIndex][missionNum]
	
	local LevelLess = x302614_var_LevelTable[missionIndex].LevelLess
	local LevelMax = x302614_var_LevelTable[missionIndex].LevelMax
	...
	if missionNum >= 3 then
		local GetItemNum = GetItemCount(varMap, varPlayer, x302614_var_QuestItem)
		if GetItemNum < 1 then
			StartTalkTask(varMap)
			TalkAppendString(varMap, x302614_var_QuestItemInfo);
			StopTalkTask(varMap)
			DeliverTalkTips(varMap,varPlayer)
			local Readme = x302614_var_QuestItemInfo
			Msg2Player(varMap,varPlayer,Readme,8,2)
			return 0
		end		
	end
	...
	for i,mission in x302614_var_MissionId do
		if MissionId == mission then
			...
			if missionNum >= 3 then
				...
				GetItemNum = GetItemCount(varMap, varPlayer, x302614_var_QuestItem)
				if( DelItem(varMap, varPlayer, x302614_var_QuestItem, 1) ) ~= 1 then 
					return 0
				end
			...
			if AddQuestNM( varMap, varPlayer, MissionId ) == 1 then
				--SetPhysicalForce(varMap, varPlayer,IshavePhysicalForce-x302614_var_NeedPhysicalForce,x302614_var_ScriptId)				
				Msg2Player(varMap, varPlayer, "您接受了任务"..x302614_var_MissionName..x302614_var_MissionNameMore[missionIndex][missionNum].."！", 0, 2)
				Msg2Player(varMap, varPlayer, "您接受了任务"..x302614_var_MissionName..x302614_var_MissionNameMore[missionIndex][missionNum].."！", 0, 3)
		 	  GamePlayScriptLog( varMap, varPlayer, 12022)			
				...
				LuaCallNoclosure( x302614_var_BusScriptId, "ProcCreateBusBus", varMap, varPlayer, MissionId, missionIndex, missionNum )
			end
			break
		end
	end
...
function x302614_ProcAccept( varMap, varPlayer )
end

function x302614_ProcQuestAttach( varMap, varPlayer, npcId, npcGuid, misIndex, MissionId )
		...
		local bHaveMission = IsHaveQuestNM( varMap, varPlayer, MissionId )
		if bHaveMission > 0 then
			if npcGuid == x302614_var_NPC[missionIndex] then
				local MissionState = GetQuestStateNM( varMap, varPlayer, npcId, MissionId )
				TalkAppendButton(varMap, x302614_var_MissionId[missionIndex], x302614_var_MissionName..x302614_var_MissionNameMore[missionIndex][missionNum], MissionState, missionNum)
			end
		end
...
--完成任务
--返回1代表成功，0代表交任务失败
function x302614_ProcQuestSubmit( varMap, varPlayer, targetId, selectId, MissionId )
--	if x302614_var_GameOpenId > 0 and GetGameOpenById( x302614_var_GameOpenId ) <= 0 then
--      return
--  end
	local misIndex = GetQuestIndexByID( varMap, varPlayer, MissionId )
	...
	if valid == 0 then
    StartTalkTask(varMap)
		TalkAppendString(varMap, format("您护送的%s离您太远了，无法提交任务！", x302614_var_Mid[missionNum]))
		StopTalkTask(varMap)
		DeliverTalkTips(varMap,varPlayer)
		local Readme = format("您护送的%s离您太远了，无法提交任务！", x302614_var_Mid[missionNum])
		Msg2Player(varMap,varPlayer,Readme,8,2)
		return 0
	end
	...
	if ret == 1 then -- 如果成功完成
		LuaCallNoclosure( 256290, "FinishYayun", varMap, varPlayer)
		x302614_SetDayCount(varMap, varPlayer)
		LuaCallNoclosure(802005,"AddActivityQuest",varMap, varPlayer, 0,0)	
		LuaCallNoclosure(888895,"EventActivity",varMap, varPlayer,6)	
		
		local Readme = "您完成了任务："..x302614_var_MissionName..x302614_var_MissionNameMore[missionIndex][missionNum]
		StartTalkTask(varMap)
		TalkAppendString(varMap, "您完成了任务："..x302614_var_MissionName..x302614_var_MissionNameMore[missionIndex][missionNum])
		StopTalkTask(varMap)
		DeliverTalkTips(varMap,varPlayer)
		local ret = CountryIsFortuneTime(varMap,varPlayer,0)
		if ret == 1  then   -- 国运令期间期间
			local level = GetLevel(varMap,varPlayer)
			local varexp = x302614_GetQuestExp(varMap, varPlayer, MissionId)
			local exp = varexp * 2 
			if (exp > 0) then				
			...
			if (IsHaveSpecificImpact( varMap, varPlayer, 9011) == 1 or IsHaveSpecificImpact( varMap, varPlayer, 9013) == 1) then
				local varMoney = x302614_GetQuestMoney(varMap, varPlayer, MissionId)
			...
			elseif (IsHaveSpecificImpact( varMap, varPlayer, 9012) == 1 ) then
				local varGolsds = x302614_GetQuestGolds(varMap, varPlayer, MissionId)
		...
		if ( missionNum < 3 ) then
			...
			if ( varRand <= 1 ) then				
				StartItemTask(varMap)
					ItemAppendBind( varMap, x302614_var_QuestItem, 1 )
				local varRet = StopItemTask( varMap, varPlayer )
				if varRet > 0 then
					DeliverItemListSendToPlayer(varMap, varPlayer)
					local message = format("您获得了物品：#R#{_ITEM%d}", x302614_var_QuestItem )
		...
		local varIndex = 1
		for varI,itm in x302614_var_MissionId do
		...
		x302614_GuoYun(varMap, varPlayer, MissionId)
	end
...
--放弃
--********************************************************************
function x302614_ProcQuestAbandon( varMap, varPlayer, MissionId )
	...
	for i,mission in x302614_var_MissionId do
		...
		if bHaveMission > 0 then
			...
			local Readme = "您放弃了任务："..x302614_var_MissionName..x302614_var_MissionNameMore[missionIndex][missionNum]
			StartTalkTask(varMap)
			TalkAppendString(varMap, Readme)
			StopTalkTask(varMap)
			DeliverTalkTips(varMap,varPlayer)
...
--任务日志刷新
---------------------------------------------------------------------------------------------------
function x302614_QuestLogRefresh( varMap, varPlayer, MissionId )

	if x302614_IsHaveMission( varMap, varPlayer ) == 0 then --如果没有任务，不刷新LOG
		return
	end	
	...
	StartTalkTask(varMap)	
		...
		local varMsg = ""
		local message = format( x302614_var_MissionInfo, x302614_var_QuestMoney[missionIndex][missionNum], x302614_var_Mid[missionNum], x302614_var_Mid[missionNum] )
		if ( missionNum < 3 ) then
		...
		AddQuestLogCustomText( 
									varMap,
									"",																																																			-- 标题
									x302614_var_MissionName..x302614_var_MissionNameMore[missionIndex][missionNum],    											-- 任务名字
									format( "  "..x302614_var_MissionTarget[missionIndex], x302614_var_Mid[missionNum] ),							      --任务目标
									x302614_var_SubmitNPCGUID[missionIndex],																													      --任务NPC
									format( x302614_var_MissionTarget[missionIndex], x302614_var_Mid[missionNum] ),     				            --任务攻略
									varMsg,																																																	--任务描述
									format( x302614_var_MissionHelp, x302614_var_Mid[missionNum], x302614_var_Mid[missionNum], x302614_var_Mid[missionNum], x302614_var_Mid[missionNum], x302614_var_Mid[missionNum] )		--任务小提示
									)
									
		local experience = x302614_GetQuestExp(varMap, varPlayer, MissionId)
		AddQuestExpBonus(varMap,experience)
		if (IsHaveSpecificImpact( varMap, varPlayer, 9011) == 1 or IsHaveSpecificImpact( varMap, varPlayer, 9013) == 1) then
			local Money = x302614_GetQuestMoney(varMap, varPlayer, MissionId)
			AddQuestMoneyBonus2( varMap, Money)
		elseif (IsHaveSpecificImpact( varMap, varPlayer, 9012) == 1 ) then
			local Golsds = x302614_GetQuestGolds(varMap, varPlayer, MissionId)
			AddQuestMoneyBonus3( varMap, Golsds)			
		end
...
function x302614_ProcQuestLogRefresh( varMap, varPlayer, MissionId)
	x302614_QuestLogRefresh( varMap, varPlayer, MissionId )
end

function x302614_IsHaveMission( varMap, varPlayer )
...
--检查任务是否可提交
---------------------------------------------------------------------------------------------------
function x302614_CheckSubmit( varMap, varPlayer, targetId, missionIndex )

	local npcGuid = GetMonsterGUID(varMap,targetId)

	if x302614_var_NPC[missionIndex] ~= npcGuid then
		return 0
	end
...
--取得此任务经验奖励
---------------------------------------------------------------------------------------------------
function x302614_GetQuestExp(varMap, varPlayer, MissionId)
	local missionIndex = 0
	local missionNum = 0
	local nExp = 0
	...
	if x302614_var_GetPlayerQuestNum[GetGUID(varMap, varPlayer)] ~= nil then
		if x302614_var_GetPlayerQuestNum[GetGUID(varMap, varPlayer)].missionIndex ~= nil and x302614_var_GetPlayerQuestNum[GetGUID(varMap, varPlayer)].missionNum ~= nil then
			missionIndex = x302614_var_GetPlayerQuestNum[GetGUID(varMap, varPlayer)].missionIndex
			missionNum = x302614_var_GetPlayerQuestNum[GetGUID(varMap, varPlayer)].missionNum
		end
	...
	if ( missionIndex ~= 0 and missionNum ~= 0 ) then
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel>= 70  then
				nExp = floor(x302614_var_QuestExpBase[missionIndex][missionNum] * ceil(nLevel*9000))
			else 
				nExp = floor(x302614_var_QuestExpBase[missionIndex][missionNum] * ceil(nLevel*100*2.5*20))
			end
	end
	nExp = nExp * x302614_var_nExpRate;
	return nExp
end
---------------------------------------------------------------------------------------------------
--取得此任务现银奖励
---------------------------------------------------------------------------------------------------
function x302614_GetQuestMoney(varMap, varPlayer, MissionId)
	local missionIndex = 0
	local missionNum = 0
	local nMoney = 0
	local misIndex = GetQuestIndexByID( varMap, varPlayer, MissionId )
	...
	if ( missionIndex ~= 0 and missionNum ~= 0 ) then
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel>= 70  then
				nMoney = floor(x302614_var_QuestExpBase[missionIndex][missionNum] * ceil(nLevel*9000))
			else 
				nMoney = floor(x302614_var_QuestExpBase[missionIndex][missionNum] * ceil(nLevel*100*2.5*20))
			end
	end
	nMoney = nMoney * x302614_var_nMoneyRate;
	return nMoney
end
---------------------------------------------------------------------------------------------------
--取得此任务金卡奖励
---------------------------------------------------------------------------------------------------
function x302614_GetQuestGolds(varMap, varPlayer, MissionId)
	local missionIndex = 0
	local missionNum = 0
	local nGolds = 0
	local misIndex = GetQuestIndexByID( varMap, varPlayer, MissionId )
	...
	if ( missionIndex ~= 0 and missionNum ~= 0 ) then
		local nLevel = GetLevel(varMap, varPlayer)
			if nLevel>= 70  then
				nGolds = floor(x302614_var_QuestExpBase[missionIndex][missionNum] * ceil(nLevel*9000))
			else 
				nGolds = floor(x302614_var_QuestExpBase[missionIndex][missionNum] * ceil(nLevel*100*2.5*20))
			end
	end
	nGolds = nGolds * x302614_var_nGoldsRate;
	return nGolds
end
...
--取得此任务当天当前已完成次数
---------------------------------------------------------------------------------------------------
function x302614_GetDayCount(varMap, varPlayer)

	if x302614_var_Mis_Count > 0 then
...
--更新当天接任务次数
---------------------------------------------------------------------------------------------------
function x302614_SetDayCount(varMap, varPlayer)
	local today = GetDayOfYear()
...
function x302614_GuoYun(varMap, varPlayer, MissionId)
