...
x300864_var_FileId = 300864
x300864_var_QuestName = "创建飞行器"


x300864_var_GodIndex = 1210
x300864_var_Carinfo = {
						{ CarType = 3,BaseAI = 3,AIScript = 3,Script = 300864 },
						{ CarType = 5,BaseAI = 3,AIScript = 3,Script = 300864 },
						{ CarType = 6,BaseAI = 3,AIScript = 3,Script = 300864 },
						{ CarType = 35,BaseAI = 3,AIScript = 3,Script = 300864 },
						{ CarType = 36,BaseAI = 3,AIScript = 3,Script = 300864 },
						{ CarType = 37,BaseAI = 3,AIScript = 3,Script = 300864 },
					}

x300864_var_QuestId = {
						{ varId = 7626,varCostMoney =5000 ,AcceptId = 281 },
						{ varId = 7627,varCostMoney = 10000, AcceptId = 291 },
						{ varId = 7628, varCostMoney = 15000, AcceptId = 301 },
						{ varId = 7629, varCostMoney = 30000, AcceptId = 761 },
						{ varId = 7630, varCostMoney = 40000, AcceptId = 771 },
						{ varId = 7631, varCostMoney = 50000, AcceptId = 781 },
					  }

x300864_var_MainScriptId = 300861
...
function x300864_ProcEventEntry(varMap, varPlayer, varTalknpc)	
...
function x300864_ProcEnumEvent(varMap, varPlayer, varTalknpc, varQuest)
	
end


function x300864_ProcCreateBus( varMap, varPlayer,varQuest,varBusID )

  local PosX,PosZ = GetWorldPos(varMap, varPlayer)

	local varObj = CreateBus(varMap, x300864_var_Carinfo[varBusID].CarType, PosX, PosZ, x300864_var_Carinfo[varBusID].BaseAI, x300864_var_Carinfo[varBusID].AIScript, x300864_var_Carinfo[varBusID].Script, varPlayer, x300864_var_GodIndex,varQuest,-1)
end

function x300864_ProcCreateBusOK(varMap, varPlayer, busId,varTalknpc,varQuest)	
...
function x300864_DeleteBus(varMap, varObj)
...
function x300864_ProcIntoBusEvent(varMap, varPlayer,varTalknpc)
...
function x300864_ProcIntoBusAfter(varMap, varPlayer,varBusID)
	...
	for varI,itm in x300864_var_QuestId do

		if varQuest == itm.varId then
			local bRet = LuaCallNoclosure( x300864_var_MainScriptId, "CostMoney", varMap, varPlayer, varQuest)
			if bRet == 1 then

				if AddQuest(varMap, varPlayer, varQuest,x300864_var_MainScriptId,0,0,0,1) == 1 then
					...
					LuaCallNoclosure( x300864_var_MainScriptId, "QuestLogRefresh", varMap, varPlayer, varQuest)	
					local CarName = GetName( varMap, varPlayer ).."的飞行器"
...
function x300864_ProcLeaveBusEvent(varMap, varPlayer,BusId)
		StartTalkTask(varMap)
...
function x300864_ProcDie(varMap,BusId,idKiller)
	...
	if idKiller == -1 then
		...
		if OwnerID ~= -1 then
			x300864_Abandon( varMap, OwnerID  )
...
function x300864_ProcTime(varMap,BusId)

end

function x300864_ProcLifeTimeOut(varMap,BusId)
    local varPlayer = GetBusMemberObjId(varMap,BusId,0)
    if(IsPlayerStateNormal(varMap, varPlayer) ~= 0) then
		x300864_Abandon( varMap, varPlayer  )
	end
	DeleteBus(varMap, BusId,1)
end

function x300864_ProcWaitTimeOut(varMap,BusId)
...
function x300864_ProcBusStopWhenOwnerFarAway(varMap, ownerId)
...
function x300864_Abandon( varMap, varPlayer )
	...
	for varI,itm in x300864_var_QuestId do
		...
		if IsHaveQuestNM( varMap, varPlayer, itm.varId) ~= 0 then	
			...
			x300864_SetDayCount(varMap, varPlayer)
			return
		end
	end
...
function x300864_CleaupQuest( varMap, varPlayer )
...
function x300864_SetDayCount(varMap, varPlayer)
