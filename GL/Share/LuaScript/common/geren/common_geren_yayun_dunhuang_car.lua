...
x302617_var_ScriptId = 302617
x302617_var_MissionName = "创建镖车"
...
x302617_var_Color={"#j","","#S"}					  
--掉落物品
x302617_var_GetDropItem = {
							{
								{
...
--获得特殊奖励
x302617_var_GetSpecialItem = 12270600	

--无敌impact在表中的索引,玩家进入BUS后获得一个无敌impact,
x302617_var_GodIndex = 1210
									  
--记录玩家选择的任务选项
x302617_var_GetPlayerQuestNum = {}     

--**********************************
--任务入口函数
--**********************************

function x302617_ProcEventEntry(varMap, varPlayer, targetId)	--点击该任务后执行此脚本
end

--**********************************
--列举事件
--**********************************
function x302617_ProcEnumEvent(varMap, varPlayer, targetId)
end

function x302617_ProcCreateBusBus( varMap, varPlayer, MissionId, missionIndex, missionNum )
	
	x302617_var_GetPlayerQuestNum[GetGUID(varMap, varPlayer)] = {}
	x302617_var_GetPlayerQuestNum[GetGUID(varMap, varPlayer)].missionIndex = missionIndex
	x302617_var_GetPlayerQuestNum[GetGUID(varMap, varPlayer)].missionNum = missionNum
	
	local PosX,PosZ = GetWorldPos(varMap, varPlayer) 
	local pos = random(1,4)
  --local PosX = PosX + pos
  --local PosZ = PosZ + pos
	local ObjID = CreateBus( varMap, x302617_var_Carinfo[missionNum][x302617_SelectBusByTopLevel(varMap)].CarType, PosX, PosZ, x302617_var_Carinfo[missionNum][x302617_SelectBusByTopLevel(varMap)].BaseAI, x302617_var_Carinfo[missionNum][x302617_SelectBusByTopLevel(varMap)].AIScript, x302617_var_ScriptId, varPlayer, x302617_var_GodIndex, MissionId, -1 )
	
end

function x302617_SelectBusByTopLevel( varMap )

	local minlevel = GetTopListInfo_MinLevel( -1 )
	
	if ( minlevel >= 1 ) then
		for i = 1, getn(x302617_var_Level) do
			if ( minlevel >= x302617_var_Level[i].min and minlevel <= x302617_var_Level[i].max ) then
				return i
			end
		end
	end
...
function x302617_ProcCreateBusOK(varMap, varPlayer, busId, targetId, MissionId)	

	local missionIndex = x302617_var_GetPlayerQuestNum[GetGUID(varMap, varPlayer)].missionIndex
	local missionNum = x302617_var_GetPlayerQuestNum[GetGUID(varMap, varPlayer)].missionNum
  local missioncount =	LuaCallNoclosure( 302616, "GetDayCount", varMap, varPlayer )
...
--	
--			 CarName ="#W"..GetName( varMap, varPlayer ).."护送的"..x302617_var_Mid[missionNum].."镖车"
--			 Name ="官府马车"
--		  end
--	
--		   if missioncount==1 then
--			 CarName ="#o"..GetName( varMap, varPlayer ).."护送的"..x302617_var_Mid[missionNum].."镖车"
--			  Name ="#o官府马车"
--		  end
--		   if missioncount==2 then
--			 CarName ="#S"..GetName( varMap, varPlayer ).."护送的"..x302617_var_Mid[missionNum].."镖车"
--			 Name ="#S官府马车"
--			missioncount=0
--		  end

	local CarName = x302617_var_Color[missionNum]..GetName( varMap, varPlayer ).."护送的"..x302617_var_Mid[missionNum].."镖车"
	local Name =x302617_var_Color[missionNum].."官府马车"
...
function x302617_DeleteBus(varMap, varPlayer)
...
--玩家请求进入BUS
function x302617_ProcIntoBusEvent(varMap, varPlayer, targetId)
...
function x302617_ProcIntoBusAfter( varMap, varPlayer, BusID )
end

--玩家离开BUS
--varPlayer：玩家ID
function x302617_ProcLeaveBusEvent(varMap, varPlayer,BusId)
		StartTalkTask(varMap)
...
function x302617_ProcDie(varMap, BusId, idKiller)
	...
	if idKiller == -1 then
		...
		if OwnerID ~= -1 then
			x302617_Abandon( varMap, OwnerID, MissionId )
			StartTalkTask(varMap)
	...
	if ObjType == 3 then		-- 被PET杀死
		...
		if IsPlayerStateNormal(varMap,idKiller) == 0 then    --主人的状态不正常
			...
			if OwnerID ~= -1 then
				...
				local Readme = "您护送/押运的镖车被摧毁"
				Msg2Player(varMap,OwnerID,Readme,8,2)
				x302617_BusDropItem(varMap, OwnerID, BusId)
			end
			return
		end
	end
	...
	if ObjType == 1 then     --被玩家杀死
		...
		if OwnerID ~= -1 then
			...
			local Readme = "您护送的镖车被"..Country..GuildTitle.."的玩家"..killerName.."摧毁，护送/押运"..x302617_var_Mid[missionNum].."镖车失败！"
			if IsHideName(varMap,idKiller) > 0 then
				Readme = "您护送的镖车被"..HIDE_NAME.."摧毁，护送/押运"..x302617_var_Mid[missionNum].."镖车失败！"
			end
			StartTalkTask(varMap)
		...
		if OwnerID ~= -1 then
			x302617_Abandon( varMap, OwnerID, MissionId )
			local Readme = "您护送的镖车被怪物摧毁，护送/押运"..x302617_var_Mid[missionNum].."镖车失败！"
			StartTalkTask(varMap)
			TalkAppendString(varMap, Readme)
			StopTalkTask(varMap)
			Msg2Player(varMap,OwnerID,Readme,8,2)
...
function x302617_ProcTime(varMap,BusId)
end

function x302617_ProcLifeTimeOut(varMap, BusId)
  ...
  if ( IsPlayerStateNormal(varMap, varPlayer) == 1 ) then 	
		x302617_Abandon( varMap, varPlayer, MissionId )
	end
	DeleteBus(varMap, BusId, 1)
	
end

function x302617_ProcWaitTimeOut(varMap,BusId)
...
--放弃
--********************************************************************
function x302617_Abandon( varMap, varPlayer, MissionId )

	if IsHaveQuestNM( varMap, varPlayer, MissionId) ~= 0 then	-- 如果有这个任务
		DelQuestNM(varMap, varPlayer, MissionId)
		x302617_SetDayCount(varMap, varPlayer)
		return
	end

end

function x302617_CleaupQuest( varMap, varPlayer )

	for i,mission in x302617_var_MissionId do
...
function x302617_BusDropItem(varMap, OwnerID, BusId)
	...
	local missionIndex = GetQuestParam(varMap, OwnerID, misIndex, 2)
	local missionNum = GetQuestParam(varMap, OwnerID, misIndex, 3)
	if x302617_var_GetDropItem[missionIndex][missionNum].num > 0 then
		StartItemTask(varMap)
		ItemAppend( varMap, x302617_var_GetDropItem[missionIndex][missionNum].item, x302617_var_GetDropItem[missionIndex][missionNum].num )
	...
	if ( varRand == 1 ) then
		StartItemTask(varMap)
			ItemAppend( varMap, x302617_var_GetSpecialItem, 1 )
			DropItemListToScene(varMap, BusId, 0)
	end
	
end

function x302617_ProcBusStopWhenOwnerFarAway(varMap, varPlayer)
	...
	local Readme = "很抱歉，由于您离开"..format("#G%s", format(x302617_var_Mid[missionNum]).."镖车").."#W#o过远，"..format(x302617_var_Mid[missionNum].."镖车").."迷失方向而停止前进！"
	StartTalkTask(varMap)
	TalkAppendString(varMap, Readme)
	StopTalkTask(varMap)
	DeliverTalkTips(varMap, varPlayer)
...
--更新当天接任务次数
---------------------------------------------------------------------------------------------------
function x302617_SetDayCount(varMap, varPlayer)
