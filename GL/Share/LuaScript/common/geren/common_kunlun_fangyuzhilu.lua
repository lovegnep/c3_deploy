...
x300982_var_FileId = 300982

x300982_var_PrintLogToggle = 0

x300982_var_QuestId_List = {
							7800,	
							7801,	
							7802,	
							7803,	
							}
x300982_var_QuestId = 7802


x300982_var_Title = "防御之路"

x300982_var_Profession_Enum = {	
								0,	
								1,	
								2, 	
								3, 	
...
x300982_var_Quest_Max_Count = 1	


x300982_var_QuestName = "【个人】防御之路"
x300982_var_QuestInfo				= 	"\t防御与攻击的关系很微妙。想要追求至高的防御，没有一定的攻击也是没有意义的。所以我要给你一个考验——击败某一个职业的6名敌国高手。通过了这个考验之后我会帮你接近那至高的防御。#G（击败的玩家不低于自己20级才有效。）" 
x300982_var_Quest_Kill_Cnt = 6				   

x300982_var_CommitNPC     = "@npc_141103"



function x300982_ProcEnumEvent(varMap, varPlayer, varTalknpc, varQuest)
	
	if GetCurCountry(varMap,varPlayer) ~=2 or GetLevel(varMap, varPlayer) < 80 then
		return -1
	end	

	
	local varState = GetQuestStateNM(varMap,varPlayer,varTalknpc,x300982_var_QuestId)
	TalkAppendButtonNM( varMap, varPlayer, varTalknpc, x300982_var_QuestId, varState, -1 )
...
function x300982_ProcEventEntry(varMap, varPlayer, varTalknpc,varScriptFileId, nFlag)
	...
	--如果有任务,而且跨天，则把次数重置为1
	--如果有任务,而且跨天，则把次数重置为1
	if IsHaveQuestNM( varMap, varPlayer, x300982_var_QuestId ) ~= 0 then
	...
	local nCnt = x300982_GetDayCount(varMap, varPlayer) + 1
	
	if x300982_var_PrintLogToggle == 1 then
		print("-----------------nCntItem_tuozhanzhishu",nCntItem_tuozhanzhishu,"nCnt",nCnt)
	end
    if IsHaveQuestNM( varMap, varPlayer, x300982_var_QuestId ) == 0 then	
				
				if nCnt == 1 then 
							x300982_DistachQuest_Info(varMap,varPlayer,varTalknpc,1)
				elseif  nCnt == 2 then
							if nCntItem_tuozhanzhishu > 0  then 
															...
															x300982_DistachQuest_Info(varMap,varPlayer,varTalknpc,2)
															
							else
															StartTalkTask(varMap)
															TalkAppendString( varMap, "#Y"..x300982_var_QuestName)
				...
				local varQuestIdx = GetQuestIndexByID( varMap, varPlayer, x300982_var_QuestId)
		        local isFinish = GetQuestParam( varMap, varPlayer, varQuestIdx, 7)
		        
				if isFinish > 0 then 
							    
							  	StartTalkTask( varMap)
								TalkAppendString( varMap, "#Y"..x300982_var_QuestName.."#W\n\t不错，不错。按照咱们的约定，我会帮助你的防御更上一层楼。" )
								TalkAppendString( varMap, "\n#Y任务奖励： ")
								TalkAppendString( varMap, "专防训练点数：13点 ")	
								StopTalkTask( varMap)
								DeliverTalkInfoNM( varMap, varPlayer, varTalknpc, x300982_var_FileId, x300982_var_QuestId)
								...
								if varLastday ~= varToday then 
									nRemain = x300982_var_Quest_Kill_Cnt
								else										
									local nKillCnt = GetQuestParam( varMap, varPlayer, varQuestIdx , 2 )
									nRemain = x300982_var_Quest_Kill_Cnt - nKillCnt
								end
								...
								if nIdx <=0 or nIdx > getn(x300982_var_Profession_Desc) then
									return -1
								end
								local strPro   = x300982_var_Profession_Desc[nIdx]								
								StartTalkTask( varMap)
								TalkAppendString( varMap, "#Y"..x300982_var_QuestName)
...
function x300982_DistachQuest_Info(varMap,varPlayer,varTalknpc,nFlag)
							...
							if nFlag == 2 then		
												StartTalkTask( varMap)
												TalkAppendString( varMap, "#Y"..x300982_var_QuestName)
												TalkAppendString( varMap, x300982_var_QuestInfo)
...
function x300982_ProcQuestAccept(varMap, varPlayer, varTalknpc, varQuest)
    ...
    if IsHaveQuestNM( varMap, varPlayer, x300982_var_QuestId) == 0 then   
						...
						if GetCurCountry(varMap, varPlayer) ~= 2 then
							
							return -1
						end
						
						local nCnt = x300982_GetDayCount(varMap, varPlayer)
						if nCnt == 1 then 
								local nCntItem_tuozhanzhishu = GetItemCountInBag( varMap, varPlayer, 11010125)
								if nCntItem_tuozhanzhishu > 0 then
						...
						local varRet	= AddQuest( varMap, varPlayer, x300982_var_QuestId, x300982_var_FileId, 1, 0, 0) 
						if varRet == 0 then
											...
											x300982_SetDayCount(varMap, varPlayer)   								
											StartTalkTask( varMap)
						...
						DelQuest(varMap, varPlayer , x300982_var_QuestId) 
...
function x300982_UpdateKillCountProfession(varMap,varPlayer,dieId)
	if varPlayer == dieId then 
		return -1
	end
	
	
	local killerId = x300982_DealKilledBy_Pet(varMap,varPlayer,dieId)
	x300982_Reset_Cnt(varMap,killerId)
	if IsHaveQuestNM( varMap, killerId, x300982_var_QuestId ) == 0 then	
							return
	else	
						    ...
						    if varTeamSize <= 1 then 
											...
											local varQuestIdx 		=  GetQuestIndexByID( varMap, killerId, x300982_var_QuestId )
											local varIndex 		=  GetQuestParam(varMap, killerId, varQuestIdx,3)
											local nProfession   =  x300982_var_Profession_Enum[varIndex]
											
											
											local varToday = GetDayOfYear()
											local varLastday = GetPlayerGameData(varMap, killerId, MD_FANGYUZHILU_DATE[1], MD_FANGYUZHILU_DATE[2], MD_FANGYUZHILU_DATE[3])
											if varLastday ~= varToday then 
														if nProfession      == GetZhiye(varMap, dieId) and GetCurCountry(varMap,dieId) ~=2  then 
																...
																x300982_QuestLogRefresh( varMap, killerId, x300982_var_QuestId)
														end
											else	
														if nProfession      == GetZhiye(varMap, dieId) and GetCurCountry(varMap,dieId) ~=2 then          
																local nKillCnt = GetQuestParam( varMap, killerId, varQuestIdx , 2 )
																nKillCnt       = nKillCnt + 1
																SetQuestByIndex(varMap, killerId, varQuestIdx,2,nKillCnt) 
																if nKillCnt >= x300982_var_Quest_Kill_Cnt then
								        ...
								        for varI = 0, varTeamSize - 1 do
								            ...
								            if memberId >= 0 and IsPlayerStateNormal( varMap, memberId) == 1 then
													if IsHaveQuestNM( varMap, memberId, x300982_var_QuestId) ~= 0 then 
																				local varQuestIdx = GetQuestIndexByID( varMap, memberId, x300982_var_QuestId)
															                    if GetQuestParam( varMap, memberId, varQuestIdx, 0) == 0 or GetQuestParam( varMap, memberId, varQuestIdx, 7) == 0 then
																								...
																								if varLastday ~= varToday then 
																											if nProfession      == GetZhiye(varMap, dieId) and GetCurCountry(varMap,dieId) ~=2  then 
																													...
																													x300982_QuestLogRefresh( varMap, memberId, x300982_var_QuestId)
																											end
																								else	
																											if nProfession      == GetZhiye(varMap, dieId) and GetCurCountry(varMap,dieId) ~=2 then          
																													local nKillCnt = GetQuestParam( varMap, memberId, varQuestIdx , 2 )
...
function x300982_ProcQuestSubmit(varMap, varPlayer, varTalknpc, varRadioSelected, varQuest)
	
	local varHaveQuest = IsHaveQuestNM(varMap, varPlayer, x300982_var_QuestId);
...
function x300982_ProcQuestAbandon( varMap, varPlayer, varQuest )
...
function x300982_GetDayCount(varMap, varPlayer)
	
	if x300982_var_Quest_Max_Count > 0 then
		local varToday = GetDayOfYear()
		local varLastday = GetPlayerGameData(varMap, varPlayer, MD_FANGYUZHILU_DATE[1], MD_FANGYUZHILU_DATE[2], MD_FANGYUZHILU_DATE[3])
		if varLastday ~= varToday then
			return 0
...
function x300982_SetDayCount(varMap, varPlayer)
...
function x300982_ProcQuestObjectKilled(varMap, varPlayer, varObjData, varObj, varQuest) 
    ...
    if varTeamSize <= 1 then 
				...
				x300982_QuestLogRefresh( varMap, varPlayer, varQuest)
		            ...
		            if memberId >= 0 and IsPlayerStateNormal( varMap, memberId) == 1 then
							if IsHaveQuestNM( varMap, memberId, x300982_var_QuestId) ~= 0 then 
									                    ...
									                    if GetQuestParam( varMap, memberId, varQuestIdx, 0) == 0 or GetQuestParam( varMap, memberId, varQuestIdx, 7) == 0 then
																		...
																		x300982_QuestLogRefresh( varMap, memberId, varQuest)
														end
							end
				  end
			end
...
function x300982_ProcQuestLogRefresh( varMap, varPlayer, varQuest)
	x300982_QuestLogRefresh( varMap, varPlayer, varQuest );
end

function x300982_QuestLogRefresh( varMap, varPlayer, varQuest)
	...
	if nKillCnt < x300982_var_Quest_Kill_Cnt then
				AddQuestLogCustomText( varMap,
										x300982_var_QuestName,		
										x300982_var_QuestName,      
										quest.."("..strTarget.."/6)",					
										x300982_var_CommitNPC,		
										quest,               		
										x300982_var_QuestInfo.."\n \n#Y任务奖励：#W\n专防训练点数：13点 ",				
	...
	StopTalkTask()
	DeliverTalkRefreshQuest(varMap, varPlayer, x300982_var_QuestId);
end

function x300982_DealKilledBy_Pet(varMap,varPlayer,dieId)
...
function x300982_Reset_Cnt(varMap,varPlayer)
	--如果有任务,而且跨天，则把次数重置为1
	if IsHaveQuestNM( varMap,varPlayer, x300982_var_QuestId ) ~= 0 then
	    local today = GetDayOfYear()
		local lastday = GetPlayerGameData(varMap,varPlayer, MD_FANGYUZHILU_DATE[1], MD_FANGYUZHILU_DATE[2], MD_FANGYUZHILU_DATE[3])
