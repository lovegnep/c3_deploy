...
x300866_var_FileId = 300866
x300866_var_QuestName = "创建飞行器"


x300866_var_GodIndex = 1210
x300866_var_Carinfo = {
						{ CarType = 3,BaseAI = 3,AIScript = 3,Script = 300866 },
						{ CarType = 5,BaseAI = 3,AIScript = 3,Script = 300866 },
						{ CarType = 6,BaseAI = 3,AIScript = 3,Script = 300866 },
						{ CarType = 35,BaseAI = 3,AIScript = 3,Script = 300866 },
						{ CarType = 36,BaseAI = 3,AIScript = 3,Script = 300866 },
						{ CarType = 37,BaseAI = 3,AIScript = 3,Script = 300866 },
					}

x300866_var_QuestId = {
						{ varId = 7638,varCostMoney =5000 ,AcceptId = 281 },
						{ varId = 7639,varCostMoney = 10000, AcceptId = 291 },
						{ varId = 7640, varCostMoney = 15000, AcceptId = 301 },
						{ varId = 7641, varCostMoney = 30000, AcceptId = 761 },
						{ varId = 7642, varCostMoney = 40000, AcceptId = 771 },
						{ varId = 7643, varCostMoney = 50000, AcceptId = 781 },
					  }

x300866_var_MainScriptId = 300863
...
function x300866_ProcEventEntry(varMap, varPlayer, varTalknpc)	
...
function x300866_ProcEnumEvent(varMap, varPlayer, varTalknpc, varQuest)
	
end


function x300866_ProcCreateBus( varMap, varPlayer,varQuest,varBusID )

  local PosX,PosZ = GetWorldPos(varMap, varPlayer)

	local varObj = CreateBus(varMap, x300866_var_Carinfo[varBusID].CarType, PosX, PosZ, x300866_var_Carinfo[varBusID].BaseAI, x300866_var_Carinfo[varBusID].AIScript, x300866_var_Carinfo[varBusID].Script, varPlayer, x300866_var_GodIndex,varQuest,-1)
end


function x300866_ProcCreateBusOK(varMap, varPlayer, busId,varTalknpc,varQuest)
...
function x300866_DeleteBus(varMap, varObj)
...
function x300866_ProcIntoBusEvent(varMap, varPlayer,varTalknpc)
...
function x300866_ProcIntoBusAfter(varMap, varPlayer,varBusID)
	...
	for varI,itm in x300866_var_QuestId do

		if varQuest == itm.varId then
			local bRet = LuaCallNoclosure( x300866_var_MainScriptId, "CostMoney", varMap, varPlayer, varQuest)
			if bRet == 1 then

				if AddQuest(varMap, varPlayer, varQuest,x300866_var_MainScriptId,0,0,0,1) == 1 then
					...
					LuaCallNoclosure( x300866_var_MainScriptId, "QuestLogRefresh", varMap, varPlayer, varQuest)	
					local CarName = GetName( varMap, varPlayer ).."的飞行器"
...
function x300866_ProcLeaveBusEvent(varMap, varPlayer,BusId)
		StartTalkTask(varMap)
...
function x300866_ProcDie(varMap,BusId,idKiller)
	...
	if idKiller == -1 then
		...
		if OwnerID ~= -1 then
			x300866_Abandon( varMap, OwnerID  )
...
function x300866_ProcTime(varMap,BusId)

end

function x300866_ProcLifeTimeOut(varMap,BusId)
    local varPlayer = GetBusMemberObjId(varMap,BusId,0)
    if(IsPlayerStateNormal(varMap, varPlayer) ~= 0) then
		x300866_Abandon( varMap, varPlayer  )
	end
	DeleteBus(varMap, BusId,1)
end

function x300866_ProcWaitTimeOut(varMap,BusId)
...
function x300866_ProcBusStopWhenOwnerFarAway(varMap, ownerId)
...
function x300866_Abandon( varMap, varPlayer )
	...
	for varI,itm in x300866_var_QuestId do
		...
		if IsHaveQuestNM( varMap, varPlayer, itm.varId) ~= 0 then	
			...
			x300866_SetDayCount(varMap, varPlayer)
			return
		end
	end
...
function x300866_CleaupQuest( varMap, varPlayer )
...
function x300866_SetDayCount(varMap, varPlayer)
