...
x310220_var_LevelMax					= 40

x310220_var_GameId					= 1023
x310220_var_NeedMoney					= 10000

--DECLARE_QUEST_INFO_START--

x310220_var_FileId 					= 	310220                  

x310220_var_LevelLess					= 	25                      


x310220_var_QuestName				= 	"新兵速成"

x310220_var_MaxKillNum				= 20

x310220_var_MaxCount					= 5


--DECLARE_QUEST_INFO_STOP--


function x310220_ProcEnumEvent( varMap, varPlayer, varTalknpc, varQuest )
...
function x310220_ProcEventEntry( varMap, varPlayer, varTalknpc, varQuest )

	if GetGameOpenById(x310220_var_GameId) ~= 1 then
	...
	if IsHaveQuestNM( varMap, varPlayer, varQuest ) == 0 then	
		...
		x310220_DispatchQuestInfo( varMap, varPlayer, varTalknpc, varQuest )
		
	
	else 
		local varQuestIdx = GetQuestIndexByID( varMap, varPlayer, varQuest )
		local killcount = GetQuestParam(varMap, varPlayer, varQuestIdx, 0)
		if killcount == x310220_var_MaxKillNum then
			x310220_DispatchCompletedInfo( varMap, varPlayer, varTalknpc,varQuest )
			
		else
			x310220_DispatchContinueInfo( varMap, varPlayer, varTalknpc,varQuest )
...
function x310220_ProcQuestAccept( varMap, varPlayer, varTalknpc, varQuest )
	...
	if GetLevel(varMap, varPlayer) < x310220_var_LevelLess then
		StartTalkTask(varMap);
	...
	local varDaycount = x310220_GetDayCount(varMap, varPlayer)
	if varDaycount >= x310220_var_MaxCount then
		StartTalkTask(varMap);
	...
	if varDaycount > 0 then
		if IsEnoughGold(varMap, varPlayer,x310220_var_NeedMoney) < 1 then
			StartTalkTask(varMap);
	...
	if varRet > 0 then 
		...
		if varRet > 0 then
			x310220_ProcQuestLogRefresh( varMap, varPlayer, varQuest)
			if varDaycount > 0 then
				...
				if goldMode == 1 then
					...
					if cash >= x310220_var_NeedMoney then
						CostMoney(varMap, varPlayer, 2, x310220_var_NeedMoney,319)
					end
				else
					local cash = GetMoney(varMap, varPlayer,2)
					local bind = GetMoney(varMap, varPlayer,3)
					if bind >= x310220_var_NeedMoney then
						CostMoney(varMap, varPlayer, 3, x310220_var_NeedMoney)
					elseif cash >= x310220_var_NeedMoney then
						CostMoney(varMap, varPlayer, 2, x310220_var_NeedMoney,319)
					end
				end
			end
...
function x310220_ProcQuestSubmit( varMap, varPlayer, varTalknpc, varButtonClick, varQuest )
	...
	if varRet == 1 then 
		x310220_SetDayCount(varMap, varPlayer)
		...
		x310220_GiveReward(varMap,varPlayer)
		
		x310220_SetFirst(varMap, varPlayer)
...
function x310220_GiveReward(varMap,varPlayer)

	local varExpBonus = x310220_GetExp( varMap, varPlayer )
...
function x310220_ProcQuestAttach( varMap, varPlayer, varTalknpc, varTalkNpcGUID,varQuestIdx, varQuest )
	if IsHaveQuestNM( varMap, varPlayer, varQuest) == 0 then	
		return
	end
...
function x310220_ProcQuestAbandon( varMap, varPlayer,varQuest )
	if IsHaveQuestNM( varMap, varPlayer, varQuest ) <= 0 then
		return -1;
	end
...
function x310220_ProcQuestObjectKilled( varMap, varPlayer, varObjData, varObj, varQuest )
					...
					if  GetLevel(varMap, humanObjId) - GetLevel(varMap, varObj) <= 5 then
						if  GetHp(varMap, humanObjId) > 0 then
							if IsHaveQuestNM( varMap, humanObjId, varQuest ) > 0 then	
							    ...
							    if KilledNum == x310220_var_MaxKillNum-1 then
								    SetQuestByIndex( varMap, humanObjId, varQuestIdx, 0, KilledNum+1 )
									SetQuestByIndex( varMap, humanObjId, varQuestIdx, 7, 1 )
									StartTalkTask(varMap)
									if ObjName == "" then
										varText = format( "#cffcf00【个人】新兵速成：您击毙了怪物(%d/%d)", KilledNum+1, x310220_var_MaxKillNum )
									else
										varText = format( "#cffcf00【个人】新兵速成：您击毙了%s(%d/%d)", ObjName, KilledNum+1, x310220_var_MaxKillNum )
									end
									TalkAppendString( varMap, varText )
									StopTalkTask( varMap )
									DeliverTalkTips( varMap, humanObjId )
									x310220_ProcQuestLogRefresh( varMap, humanObjId, varQuest)
								elseif KilledNum < x310220_var_MaxKillNum-1 then
									SetQuestByIndex( varMap, humanObjId, varQuestIdx, 0, KilledNum+1 )
									StartTalkTask(varMap)
									if ObjName == "" then
										varText = format( "#cffcf00【个人】新兵速成：您击毙了怪物(%d/%d)", KilledNum+1, x310220_var_MaxKillNum )
...
function x310220_ProcQuestItemChanged( varMap, varPlayer, varItemData, varQuest )
	return 0
end



function x310220_ProcAcceptCheck(varMap, varPlayer, varTalknpc)
	return 1
end

function x310220_GetCost(varMap, varPlayer)
	local varDaycount =x310220_GetDayCount(varMap,varPlayer)
	if varDaycount > 0 then
		return 10000
	end
...
function x310220_DispatchQuestInfo( varMap, varPlayer, varTalknpc, varQuest)
	local awardexp = x310220_GetExp( varMap, varPlayer )
	...
	TalkAppendString( varMap,"   消灭等级不比你低5级以上的20只怪物，你就可以完成新兵的训练。")
	 TalkAppendString( varMap, "\n#Y任务花费：" )
        local cost = x310220_GetCost(varMap, varPlayer)
	...
	StopTalkTask()
	DeliverTalkInfoNM(varMap, varPlayer, varTalknpc, x310220_var_FileId, varQuest,0);
end




function x310220_DispatchContinueInfo( varMap, varPlayer, varTalknpc,varQuest )
	...
	StopTalkTask()
	DeliverTalkRequire(varMap, varPlayer, varTalknpc, x310220_var_FileId, varQuest,0);
end




function x310220_DispatchCompletedInfo( varMap, varPlayer, varTalknpc,varQuest )
	local awardexp = x310220_GetExp( varMap, varPlayer )
	...
	StopTalkTask()
	DeliverTalkContinueNM(varMap, varPlayer, varTalknpc, x310220_var_FileId, varQuest);

end

function x310220_ProcQuestLogRefresh( varMap, varPlayer, varQuest)
	...
	local varStr = "  消灭20只等级适当的怪物("..killcount.."/"..tostring(x310220_var_MaxKillNum)..")";	
	StartTalkTask(varMap)	
		local awardexp = x310220_GetExp( varMap, varPlayer )

		AddQuestExpBonus(varMap,  awardexp);
	
	local count =	x310220_GetDayCount(varMap, varPlayer)
	local str1 =format("【个人】新兵速成(%d/5)",count+1)		
...
function x310220_GetDayCount(varMap, varPlayer)
	if x310220_var_MaxCount > 0 then
...
function x310220_SetDayCount(varMap, varPlayer)
	local varToday = GetDayOfYear()
	local varLastday = GetPlayerGameData(varMap, varPlayer, MD_XINBINGSUCHENG_DATE[1], MD_XINBINGSUCHENG_DATE[2], MD_XINBINGSUCHENG_DATE[3])
	if varLastday ~= varToday then
...
function x310220_SetClose( param0, varParam1, varParam2)
    SetCountryQuestData( 0, 0, CD_INDEX_MOJUN_KILL_COUNT1, 0)
    SetCountryQuestData( 0, 0, CD_INDEX_MOJUN_KILL_COUNT2, 0)
    SetCountryQuestData( 0, 0, CD_INDEX_MOJUN_KILL_COUNT3, 0)
    SetCountryQuestData( 0, 0, CD_INDEX_MOJUN_KILL_COUNT4, 0)
end
