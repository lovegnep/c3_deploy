...
function x300326_ProcEventEntry( varMap, varPlayer, varBagIdx )
...
function x300326_ProcIsSpellLikeScript( varMap, varPlayer)
	return 1; 
end
...
function x300326_ProcCancelImpacts( varMap, varPlayer )
	return 0; 
end
...
function x300326_ProcConditionCheck( varMap, varPlayer )
...
function x300326_ProcDeplete( varMap, varPlayer )
	return 1;
end
...
function x300326_ProcActivateOnce( varMap, varPlayer, varImpact )
	...
	if(DepletingUsedItem(varMap, varPlayer)) == 1 then
		...
		StartTalkTask(varMap);
		TalkAppendString(varMap, "儿童节快乐！");
		StopTalkTask();
		DeliverTalkTips(varMap, varPlayer);
		Msg2Player(varMap,varPlayer,"儿童节快乐！",CHAT_TYPE_SELF,CHAT_RIGHTDOWN)
		...
		if itemid ~=-1 then		
			...
			if varRet > 0 then
					StartTalkTask(varMap)
					TalkAppendString(varMap,"使用烟花意外获得了@item_"..itemid)
					StopTalkTask(varMap)
					DeliverTalkTips(varMap,varPlayer)
					...
					varMsg = format( "恭喜%s的%s使用儿童节烟花获得了%d个#{_ITEM%d}!", CountryName, Name, 1, itemid )
					if temp2 ==1 then
...
function x300326_ProcActivateEachTick( varMap, varPlayer)
	return 1; 
end
...
function x300326_ProcTopListYanghua(varMap,varPlayer)
