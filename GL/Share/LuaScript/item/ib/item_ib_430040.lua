--高级体力丹
x430040_var_FileId = 430040 
x430040_var_AddPhysicalForce =500		--增加体力值	
x430040_var_LimitCount =5			--限制使用个数





function x430040_ProcEventEntry( varMap, varPlayer, varBagIdx )
		return 0;
end
...
function x430040_ProcIsSpellLikeScript( varMap, varPlayer)
	return 1; 
end
...
function x430040_ProcCancelImpacts( varMap, varPlayer )
	return 0; 
end
...
function x430040_ProcConditionCheck( varMap, varPlayer )
	...
	if x430040_GetDayCount(varMap, varPlayer) >= x430040_var_LimitCount then
			local varText = format("每天只能使用#G%d#cffcf00个体力丹！",x430040_var_LimitCount)
			StartTalkTask(varMap)
			TalkAppendString(varMap, varText)
			DeliverTalkTips(varMap, varPlayer)
			StopTalkTask(varMap)
...
function x430040_ProcDeplete( varMap, varPlayer )
...
function x430040_ProcActivateEachTick( varMap, varPlayer)
	return 1; 
end

function x430040_GetDayCount(varMap, varPlayer)
...
function x430040_SetDayCount(varMap, varPlayer)
	local day = GetDayOfYear()
	local rday = GetPlayerGameData(varMap, varPlayer, MD_WUYILINGJIANG_DATE[1], MD_WUYILINGJIANG_DATE[2], MD_WUYILINGJIANG_DATE[3])
