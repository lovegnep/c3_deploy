...
x430103_var_FileId 		= 430103 


x430103_var_LevelMin		= 40
x430103_var_LevelMax		= 50

x430103_var_Exp           = 400000




function x430103_ProcEventEntry( varMap, varPlayer, varBagIdx)
...
function x430103_ProcIsSpellLikeScript( varMap, varPlayer)
	return 1; 
end
...
function x430103_ProcCancelImpacts( varMap, varPlayer)
	return 0; 
end
...
function x430103_ProcConditionCheck( varMap, varPlayer)
	
	if( 1~=VerifyUsedItem( varMap, varPlayer) ) then
		return 0
	end
    local varLevel = GetLevel( varMap, varPlayer)
    if varLevel < x430103_var_LevelMin then
        Msg2Player( varMap, varPlayer, "你的等级低于使用此物品的最低级别", 8, 3)
        return 0
    end
    if varLevel >= x430103_var_LevelMax then
...
function x430103_ProcDeplete( varMap, varPlayer)
...
function x430103_ProcActivateOnce( varMap, varPlayer, varImpact)
    ...
    AddExp( varMap, varPlayer, x430103_var_Exp)
...
function x430103_ProcActivateEachTick( varMap, varPlayer)
	return 1; 
end
