...
x418800_var_FileId = 418800 
x418800_var_ItemList = {
                         [12032031] = {      
                                      ...
                                      { varId = 12050318, varName = "坐骑降世符"          , varNum = 1  , probability = 30  , Isbroadcast = 1 },
                                      { varId = 10306091, varName = "永久时装"            , varNum = 1  , probability = 70  , Isbroadcast = 1 },    
                                    },                                            
                         [12032019] = {         
                                      { varId = 12500323, varName = "豪华将星卡礼盒" 	    , varNum = 1  , probability = 500 , Isbroadcast = 1 },
                                      { varId = 12030775, varName = "掌门牌碎片"          , varNum = 1  , probability = 70  , Isbroadcast = 1 },
...
x418800_var_ItemId =  12032031
x418800_var_ItemIdG =  12032019
                                                                     
                                                                     



function x418800_ProcEventEntry( varMap, varPlayer, varBagIdx )        
if GetBagSpace(varMap, varPlayer) <= 0 then
		Msg2Player(varMap, varPlayer, "背包已满，无法使用此道具", 0, 2)
		Msg2Player(varMap, varPlayer, "背包已满，无法使用此道具", 0, 3)
		return
	end		
		x418800_ProcAddItemByRandom(varMap, varPlayer,varBagIdx)
end                                                                  
...
function x418800_ProcIsSpellLikeScript( varMap, varPlayer)        
...
function x418800_ProcCancelImpacts( varMap, varPlayer )
	return 0; 
end
...
function x418800_ProcConditionCheck( varMap, varPlayer )
...
function x418800_ProcDeplete( varMap, varPlayer )
	return 1;
end



function x418800_ProcAddItemByRandom(varMap, varPlayer, varBagIdx)
	...
	if Useitem ~= x418800_var_ItemId and Useitem ~= x418800_var_ItemIdG then
		return
	end	
	local probability = random(1,10000)
	
	for varI,iter in x418800_var_ItemList[Useitem] do
	...
	x418800_ProcAddItem( varMap, varPlayer, item, varName, num, broadcast, varBagIdx, Useitem ) 
end

function x418800_ProcAddItem(varMap, varPlayer, varItem, iname, varNum, broadcast, varBagIdx, useItem)                               
	local Useitem = GetItemIDByIndexInBag( varMap, varPlayer, varBagIdx)
	...
	elseif ( varItem == 0 ) then
		if DelItemByIndexInBag( varMap, varPlayer, varBagIdx, 1) == 1 then
			if ( useItem == x418800_var_ItemId ) then
				AddMoney(varMap, varPlayer, 3, 500)
				Msg2Player(varMap,varPlayer,format("使用#{_ITEM%d}，你获得了#{_MONEY%d}金卡", Useitem, 500),4,3)	
			elseif ( useItem == x418800_var_ItemIdG ) then
				local perMsg = {
													"你感到一阵冰爽畅快，忍不住还想吃一根",
													"炎热的夏天来个奶油冰棍真是无上的享受",
											 }
...
function x418800_ProcActivateOnce( varMap, varPlayer, varImpact )   
...
function x418800_ProcActivateEachTick( varMap, varPlayer)
	return 1
end
