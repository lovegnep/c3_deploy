...
x418804_var_FileId = 418804
x418804_var_Item = 12032034
...
function x418804_ProcEventEntry( varMap, varPlayer, varBagIdx )
...
function x418804_ProcIsSpellLikeScript( varMap, varPlayer)
	return 1; 
end
...
function x418804_ProcCancelImpacts( varMap, varPlayer )
	return 0; 
end
...
function x418804_ProcConditionCheck( varMap, varPlayer )
...
function x418804_ProcDeplete( varMap, varPlayer )
...
function x418804_ProcActivateOnce( varMap, varPlayer, varImpact )
	...
	for k,v in ewaigailv do
		if ewairanmoney <= v then
			...
			varMsg = format( "恭喜%s的%s打开#{_ITEM%d}获得了#{_MONEY%d}金卡!", CountryName, Name, x418804_var_Item, ewainMoney )
	...
	local item ={10340012, 10340013, 10340014,10310023,11000155,11000304,11000838,11000839,11000840,11000841,11000842,11000843,11000844,11000845}
	local itemgailv = {2,4,6,14,24,74,94,104,114,124,134,144,154,164}
	...
	if itemname ~= -1 then
		...
		if varRet > 0  then
			...
			local CountryName = GetCountryName(CounID)
			local Name =  GetName(varMap, varPlayer)
			varMsg = format( "恭喜%s的%s打开#{_ITEM%d}获得了#{_ITEM%d}!", CountryName, Name, x418804_var_Item, itemname )
			LuaAllScenceM2Wrold( varMap, varMsg, 5, 1)
...
function x418804_ProcActivateEachTick( varMap, varPlayer)
	return 1; 
end
