...
x418310_var_FileId = 418310
x418310_var_QuestName = "银牌图腾礼包"
x418310_var_LevelMin = 70


function x418310_ProcEventEntry( varMap, varPlayer, varBagIdx )
...
function x418310_ProcIsSpellLikeScript( varMap, varPlayer)
	return 1; 
end
...
function x418310_ProcCancelImpacts( varMap, varPlayer )
	return 0; 
end
...
function x418310_ProcConditionCheck( varMap, varPlayer )
...
function x418310_ProcDeplete( varMap, varPlayer )
...
function x418310_ProcActivateOnce( varMap, varPlayer, varImpact )
	
	
	if GetLevel(varMap, varPlayer)< x418310_var_LevelMin then
	...
	if(DepletingUsedItem(varMap, varPlayer)) == 1 then
		...
		local varRan = random(1,1000)
			value = 2 
		if varRan <= 200 then
			exp = GetLevel(varMap, varPlayer) * 12000
...
--				Msg2Player( varMap, varPlayer, varMsg, 8, 3)
			...
			local guildvalue = GetLevel(varMap, varPlayer) * 4
		...
		elseif varRan <= 600 then
			local honor = GetLevel(varMap, varPlayer) * 4
...
--			ItemAppendBind( varMap, 11020002, 4)     -- 战车强化道具20%,后期添加
...
--			DeliverItemListSendToPlayer( varMap, varPlayer)
		elseif varRan <= 1000 then
			local bonusshengwang = GetLevel(varMap, varPlayer) * 8
...
function x418310_ProcActivateEachTick( varMap, varPlayer)
	return 1; 
end
