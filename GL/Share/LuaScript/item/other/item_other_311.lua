...
x418311_var_FileId = 418311
x418311_var_QuestName = "铜牌图腾礼包"
x418311_var_LevelMin = 70


function x418311_ProcEventEntry( varMap, varPlayer, varBagIdx )
...
function x418311_ProcIsSpellLikeScript( varMap, varPlayer)
	return 1; 
end
...
function x418311_ProcCancelImpacts( varMap, varPlayer )
	return 0; 
end
...
function x418311_ProcConditionCheck( varMap, varPlayer )
...
function x418311_ProcDeplete( varMap, varPlayer )
...
function x418311_ProcActivateOnce( varMap, varPlayer, varImpact )
	
	
	if GetLevel(varMap, varPlayer)< x418311_var_LevelMin then
	...
	if(DepletingUsedItem(varMap, varPlayer)) == 1 then
		...
		local varRan = random(1,1000)
			value = 2 
		if varRan <= 200 then
			exp = GetLevel(varMap, varPlayer) * 9000
...
--				Msg2Player( varMap, varPlayer, varMsg, 8, 3)
		...
		elseif varRan <= 600 then
			local honor = GetLevel(varMap, varPlayer) * 3
...
--			ItemAppendBind( varMap, 11020002, 3)     -- 战车强化道具20%,后期添加
...
--			DeliverItemListSendToPlayer( varMap, varPlayer)
		elseif varRan <= 1000 then
			local bonusshengwang = GetLevel(varMap, varPlayer) * 6
...
function x418311_ProcActivateEachTick( varMap, varPlayer)
	return 1; 
end
