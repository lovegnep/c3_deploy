...
x418805_var_FileId = 418805 
x418805_var_ItemList = {
                         [12032035] = {      
                                      ...
                                      { varId = 12050318, varName = "坐骑降世符"          , varNum = 1  , probability = 10  , Isbroadcast = 1 },
                                      { varId = 10306091, varName = "永久时装"            , varNum = 1  , probability = 20  , Isbroadcast = 1 },    
                                    },                                            
                         [12032019] = {         
                                      { varId = 12500323, varName = "豪华将星卡礼盒" 	    , varNum = 1  , probability = 500 , Isbroadcast = 1 },
...
x418805_var_ItemId =  12032035
x418805_var_ItemIdG =  12032019
                                                                     
                                                                     



function x418805_ProcEventEntry( varMap, varPlayer, varBagIdx )        
if GetBagSpace(varMap, varPlayer) <= 0 then
		Msg2Player(varMap, varPlayer, "背包已满，无法使用此道具", 0, 2)
		Msg2Player(varMap, varPlayer, "背包已满，无法使用此道具", 0, 3)
		return
	end		
		x418805_ProcAddItemByRandom(varMap, varPlayer,varBagIdx)
end                                                                  
...
function x418805_ProcIsSpellLikeScript( varMap, varPlayer)        
...
function x418805_ProcCancelImpacts( varMap, varPlayer )
	return 0; 
end
...
function x418805_ProcConditionCheck( varMap, varPlayer )
...
function x418805_ProcDeplete( varMap, varPlayer )
	return 1;
end



function x418805_ProcAddItemByRandom(varMap, varPlayer, varBagIdx)
	...
	if Useitem ~= x418805_var_ItemId and Useitem ~= x418805_var_ItemIdG then
		return
	end	
	local probability = random(1,10000)
	
	for varI,iter in x418805_var_ItemList[Useitem] do
	...
	x418805_ProcAddItem( varMap, varPlayer, item, varName, num, broadcast, varBagIdx, Useitem ) 
end

function x418805_ProcAddItem(varMap, varPlayer, varItem, iname, varNum, broadcast, varBagIdx, useItem)                               
	local Useitem = GetItemIDByIndexInBag( varMap, varPlayer, varBagIdx)
	...
	if ( varItem > 0 ) then
		...
		if varRet > 0 then
			if DelItemByIndexInBag( varMap, varPlayer, varBagIdx, 1) == 1 then
				...
				if ( useItem == x418805_var_ItemId ) then
	...
	elseif ( varItem == 0 ) then
		if DelItemByIndexInBag( varMap, varPlayer, varBagIdx, 1) == 1 then
			if ( useItem == x418805_var_ItemId ) then
				AddMoney(varMap, varPlayer, 3, 500)
				Msg2Player(varMap,varPlayer,format("使用#{_ITEM%d}，你获得了#{_MONEY%d}金卡", Useitem, 500),4,3)	
			elseif ( useItem == x418805_var_ItemIdG ) then
				local perMsg = {
													"你感到一阵冰爽畅快，忍不住还想吃一根",
													"炎热的夏天来个奶油冰棍真是无上的享受",
											 }
...
function x418805_ProcActivateOnce( varMap, varPlayer, varImpact )   
...
function x418805_ProcActivateEachTick( varMap, varPlayer)
	return 1
end
