...
x418354_var_FileId 		= 418354

x418354_var_LevelMin		=	1
x418354_var_SelfPos     = {}
x418354_var_BonusTable = {
[12030694] = {{varid = 11000838, varnum = 1 , varbind = 1}}, 	--主手图谱·生命
[12030695] = {{varid = 11000839, varnum = 1 , varbind = 1}}, 	--副手图谱·生命
[12030696] = {{varid = 11000840, varnum = 1 , varbind = 1}}, 	--战甲图谱·生命
[12030697] = {                                            	
...
function x418354_ProcEventEntry( varMap, varPlayer, varBagIdx )
...
function x418354_ProcIsSpellLikeScript( varMap, varPlayer)
	return 1; 
end
...
function x418354_ProcCancelImpacts( varMap, varPlayer )
	return 0; 
end
...
function x418354_ProcConditionCheck( varMap, varPlayer )
...
function x418354_ProcDeplete( varMap, varPlayer )
	return 1;
end
...
function x418354_ProcActivateOnce( varMap, varPlayer, varImpact )

	local varBagIdx = GetBagIndexOfUsedItem( varMap, varPlayer)
	if GetLevel(varMap, varPlayer)< x418354_var_LevelMin then
	...
	local size = getn( x418354_var_BonusPlace)
	local guid = GetPlayerGUID( varMap,varPlayer )
	local varIndex = 1
	
	if x418354_var_SelfPos[guid] == nil then
		x418354_var_SelfPos[guid] ={}
		x418354_var_SelfPos[guid][itemid] = {}
		varIndex = random( 1, size)
	elseif nil == x418354_var_SelfPos[guid][itemid] then
		x418354_var_SelfPos[guid][itemid] = {}
		varIndex = random( 1, size)
 	else
		varIndex = x418354_var_SelfPos[guid][itemid] 
		if varIndex < 1 or varIndex > size then
			varIndex = random( 1, size)
		end 
	end
	
	x418354_var_SelfPos[guid][itemid] = varIndex
	
	local posx =x418354_var_BonusPlace[varIndex].varX
	local posy =x418354_var_BonusPlace[varIndex].varY
	...
	if rand <= 20 then
		for i , item in x418354_var_BonusTable do 
			if i == itemid then
				local num = getn(item)
	...
	x418354_var_SelfPos[guid][itemid] = nil
...
function x418354_ProcActivateEachTick( varMap, varPlayer)
	return 1; 
end
