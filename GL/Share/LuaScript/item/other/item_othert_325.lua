x432325_var_FileId = 432325 


function x432325_ProcEventEntry( varMap, varPlayer, varBagIdx )

end

function x432325_ProcIsSpellLikeScript( varMap, varPlayer)
	return 1 
	
	
end

function x432325_ProcCancelImpacts( varMap, varPlayer )
	return 0; 
end

function x432325_ProcConditionCheck( varMap, varPlayer )
...
function x432325_ProcDeplete( varMap, varPlayer )

	return 1
end

function x432325_ProcActivateOnce( varMap, varPlayer )
	...
	if GetBagSpace(varMap, varPlayer) >= 1 then
		...
		if varRand <= 10 then
			StartItemTask( varMap )
			ItemAppendBind( varMap, 12500028, 1 ) --皇家时装礼包
			local varRet = StopItemTask( varMap, varPlayer )
			if varRet > 0 then
				DeliverItemListSendToPlayer(varMap,varPlayer)
...
function x432325_ProcActivateEachTick( varMap, varPlayer)
	return 1; 
end
