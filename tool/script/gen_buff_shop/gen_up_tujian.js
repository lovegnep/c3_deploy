...
for (const line of lines) {
    ...
    if (id <= 1060 && name.startsWith('五代')) {//10~100级普通马
        if (!nameMap[name]) {
            result.push({ id, name:name+"・黑",sort:0,level ,dai:5});
            nameMap[name] = true
            typeMap[type] = true
        }
        ...
        result.push({ id, name ,sort:1,level:120,dai:2});
        typeMap[type] = true
    }else if (id >= 2301 && id <= 2750 && name.startsWith('三代')) {//30~70 3f马
        if (!typeMap[type]) {
            result.push({ id, name ,sort:1,level,dai:3});
            typeMap[type] = true
        }
        ...
        if (!typeMap[type]) {
            result.push({ id, name ,sort:1,level:100,dai:2});
            typeMap[type] = true
        }
    }
}
...
let index = 1;
let buffID = 22710;
for (const { id, name,dai } of result) {
    let v = (index-1)*100 + 100
    res += `{index = ${index},useyuan=1, dai=${dai} ,targetID = ${id}, targetName = "${name}", buffID = ${buffID}, buffDesc = "${name}图鉴激活：全攻击+${v}全防御+${v}"},\n`
    index++;
    buffID++
}
console.log(res)
...
// 如果想保存结果到文件，请取消注释以下行：
fs.writeFileSync('gen_up_tujian.txt', res, 'utf8');
