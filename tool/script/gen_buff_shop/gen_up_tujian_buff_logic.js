...
for (const { id, name } of result) {
    ...
    res += `${buffID}\t\t物理攻击修正值\t${v}\t物理防御修正值\t${v}\t魔法攻击修正值\t${v}\t魔法防御修正值\t${v}\t冰系攻击修正值\t${v}\t冰系抗性修正值\t${v}\t火系攻击修正值\t\t火系抗性修正值\t\t电系攻击修正值\t\t电系抗性修正值\t\t毒系攻击修正值\t\t毒系抗性修正值\t\tMAX_HP修正值\t\tMAX_MP修正值\t\tMAX_RAGE修正值\t\tMAX_STRIKE_POINT修正值\t\t\t11\tSTD_IMPACT_003\t1\t${buffID}\t${buffID}\t${buffID}\t1\t1\t1\t0\t0\t0\t0\t0\t0\t1\t-1\t0\t0\t0\t-1\t0\t0\n`
    index++;
    buffID++
}
console.log(res)
...
// 如果想保存结果到文件，请取消注释以下行：
fs.writeFileSync('gen_up_tujian_buff_logic.txt', res, 'utf8');
